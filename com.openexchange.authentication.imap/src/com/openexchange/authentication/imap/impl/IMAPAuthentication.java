...
public class IMAPAuthentication extends AbstractConfigurationAwareAuthenticationService {
    ...
    private static enum PropertyNames {
        ...
        AUTO_LOWER_CASE("com.openexchange.authentication.imap.autoLowerCase");
    ...
    private static final String AUTH_IDENTIFIER = "imap";
    ...
    /**
     * Initializes a new {@link IMAPAuthentication}.
     *
     * @param services The {@link ServiceLookup}
     * @throws OXException
     */
    @SuppressWarnings("deprecation")
    public IMAPAuthentication(ServiceLookup services) throws OXException {
    ...
    public AuthenticationResult handleLoginRequest(AuthenticationRequest authenticationRequest) throws OXException {
        try {
            SplitResult splitResult = split(authenticationRequest.getLogin(), isAutoLowerCase());

            String localPart = splitResult.localPart;
            String password = authenticationRequest.getPassword();
            if (Strings.isEmpty(localPart) || Strings.isEmpty(password)) {
            ...
            // The flag indicating whether IMAP server used for authenticating is the primary one
            // Assume "true" if host is "localhost" by default.
            boolean isPrimary = "localhost".equals(host);
            ...
            if ("true".equalsIgnoreCase(props.getProperty(PropertyNames.USE_MULTIPLE.name))) {
                ...
                int userId = optUser(userInfo, authenticationRequest, ctx).get().intValue();
                ...
                if (!isPrimary) {
                    ...
                    if (optionalContext.isPresent()) {
                        ...
                        Optional<Integer> optionalUserId = optUser(userInfo, authenticationRequest, ctx).opt();
            ...
            FailureKey failureKey = new FailureKey(host, port.intValue(), imapLogin, password);
            {
                ...
                if (null != authenticationFailed) {
                    return AuthenticationResult.failed(LoginExceptionCodes.INVALID_CREDENTIALS.create(authenticationFailed));
                }
            }
            ...
            ConfigurationService configuration = services.getServiceSafe(ConfigurationService.class);
            ...
            if (secure) {
                ...
                /*
                 * Checks if STARTTLS is required for this connection
                 */
                applyRequireTls(imapprops, configuration);
                /*
            ...
            try {
                ...
                return AuthenticationResult.failed(LoginExceptionCodes.INVALID_CREDENTIALS.create(e));
            ...
            if (useFullLogin) {
                ...
                return AuthenticationResult.success(new AuthenticatedImpl(userInfo, contextInfo));
            }

            LOG.debug("Using \"defaultcontext\" as context name");
            return AuthenticationResult.success(new AuthenticatedImpl(userInfo, "defaultcontext"));
    ...
    private boolean isAutoLowerCase() {
        boolean autoLowerCase = false;
        Object value = props.get(PropertyNames.AUTO_LOWER_CASE.name);
        if (value != null) {
            autoLowerCase = Boolean.parseBoolean(value.toString().trim());
        }
    ...
    private void applyRequireTls(Properties imapprops, ConfigurationService configuration) {
        String sRequireTls = configuration.getProperty("com.openexchange.imap.requireTls", "true").trim();
        if (Strings.isNotEmpty(sRequireTls)) {
            imapprops.put("mail.imap.starttls.required", Boolean.parseBoolean(sRequireTls) ? "true" : "false");
        }
    }
    ...
    private void applySslProtocols(Properties imapprops, ConfigurationService configuration) {
        ...
        sslProtocols = configuration.getProperty("com.openexchange.imap.ssl.protocols", "").trim();
        if (Strings.isNotEmpty(sslProtocols)) {
            imapprops.put("mail.imap.ssl.protocols", sslProtocols);
        }

        SSLConfigurationService sslConfigService = services.getOptionalService(SSLConfigurationService.class);
    ...
    private Result<Context> optContext(String contextInfo) {
        try {
            ...
            if (ContextStorage.NOT_FOUND == ctxId) {
                LOG.debug("Missing context mapping for context \"{}\". Login failed.", contextInfo);
                throw INVALID_CREDENTIALS.create(contextInfo);
            }
    ...
    private Result<Integer> optUser(String userInfo, AuthenticationRequest authenticationRequest, Context ctx) {
        try {
            ...
            LOG.debug("Missing user mapping for user \"{}\". Login failed.", authenticationRequest.getLogin(), e);
            return new Result<Integer>(INVALID_CREDENTIALS.create(authenticationRequest.getLogin()));
        }
    }
    ...
    public AuthenticationResult handleAutoLoginRequest(AuthenticationRequest authenticationRequest) throws OXException {
        return AuthenticationResult.failed(LoginExceptionCodes.NOT_SUPPORTED.create(IMAPAuthentication.class.getName()));
    }
     ...
     * @param autoLowerCase Whether to convert login info to lower-case
     * @return The split login info
     * @throws LoginException if no separator is found.
     */
    private SplitResult split(String loginInfo, boolean autoLowerCase) {
        String toSplit = loginInfo;
        if (autoLowerCase) {
            toSplit = Strings.asciiLowerCase(toSplit);
        }
        int pos = toSplit.lastIndexOf('@');
        if (pos <= 0) {
            return new SplitResult(toSplit, toSplit, "defaultcontext");
        }

        // Split by '@' character
        return new SplitResult(toSplit, toSplit.substring(0, pos), toSplit.substring(pos + 1));
    }
