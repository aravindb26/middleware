...
public class TokenLoginServiceImpl implements TokenLoginService {
    ...
    public String acquireToken(Session session) throws OXException {
        ...
        if (null != token) {
            // There is already a token associated with given session. Check that token is in correct format & matches the session identifier
            String[] splitToken = Strings.splitBy(token, '-', true);
            if (null != splitToken && 2 == splitToken.length && Objects.equal(sessionId, obfuscatorService.unobfuscate(new String(Strings.unhex(splitToken[0]), StandardCharsets.UTF_8)))) {
                // Everything fine...
                return token;
            }
        ...
        String newToken = createNewToken(obfuscatorService, sessionId);
    ...
    private String lookUpSessionIdByToken(String token, ObfuscatorService obfuscatorService) throws OXException {
        ...
        try {
            ...
            String sessionId = extractSessionId(obfuscatorService, token);
