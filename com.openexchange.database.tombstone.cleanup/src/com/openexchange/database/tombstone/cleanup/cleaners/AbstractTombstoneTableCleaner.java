...
public abstract class AbstractTombstoneTableCleaner implements CleanUpExecution {
    ...
    private final AtomicLong timespan;
    ...
    /**
     * Sets the time span for cleaning tomb stone tables.
     * <p>
     * The setting of time span is mandatory for using {@link #executeFor(String, int, int, Map, CleanUpExecutionConnectionProvider)}.
     *
     ...
     */
    protected static final ResultSet logExecuteQuery(PreparedStatement stmt) throws SQLException {
     ...
     */
    protected static final int logExecuteUpdate(final PreparedStatement stmt) throws SQLException {
        try {
            if (false == LOG.isDebugEnabled()) {
    ...
    /**
     * Executes the given delete statement by setting the time stamp
     *
     * @param connection The write connection used for deletion
     * @param timestamp The time stamp to delete all entries before it
    ...
    /**
     * Reads the time span for cleaning tomb stone tables and gets the time stamp until which the entries should be kept.
     *
     * @return The time stamp.
     * @throws OXException If no time span is set.
     */
    private long getTimestamp() throws OXException {
        long timespan = this.timespan.get();
        if (timespan == -1) {
