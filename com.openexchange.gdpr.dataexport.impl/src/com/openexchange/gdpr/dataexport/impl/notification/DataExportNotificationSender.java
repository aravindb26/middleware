    ...
    private static void sendNotification(Reason reason, Date creationDate, Date expiryDate, HostInfo hostInfo, UUID taskId, int userId, int contextId, ServiceLookup services) {
        try {
            ...
            try {
                ...
                LoggerHolder.LOG.info("Sent {} notification message for data export task {} of user {} in context {}", reason, stringFor(taskId), I(userId), I(contextId));
            } finally {
                transport.close();
            }
        } catch (Exception e) {
            LoggerHolder.LOG.warn("Failed to send {} notification message for data export task {} of user {} in context {}", reason, stringFor(taskId), I(userId), I(contextId), e);
        }
    }
