...
public abstract class AbstractDataExportSql<R> {
                        ...
                        try (ResultSet rs = stmt.executeQuery()) {
                            while (rs.next()) {
                                ...
                                if (status.isAborted()) {
                                    ...
                                    LOGGER.debug("Deleting {} from storage since {}", taskInfo, status.getId());
                                    ...
                                    if (rs.getLong(3) < maxTimeToLiveThreshold) {
                                        ...
                                        if (exists) {
                                            ...
                                            tasksToDelete.add(ImmutablePair.<TaskInfo, R>newInstance(taskInfo, schemaReference));
                                            LOGGER.debug("Deleting {} from storage since user no more exists", taskInfo);
                                        }
                                    }
                                }
                            }
            ...
            try (PreparedStatement stmt = con.prepareStatement("SELECT savepoint, filestoreLocation FROM dataExportTaskWorklist WHERE taskId = ? AND id = ? AND savepoint IS NOT NULL")) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        if (!rs.wasNull()) {
                            try {
                                jSavePoint = JSONServices.parseObject(s);
    ...
    boolean deleteTask(int userId, int contextId) throws OXException {
        ...
        try {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        try {
                            ...
                            if (!com.openexchange.filestore.FileStorageCodes.NO_SUCH_FILE_STORAGE.equals(e)) {
                                throw e;
                            }
                            fileStorage = null;
                        }
            ...
            if (fileStorageLocations != null && fileStorage != null) {
                for (String fileStorageLocation : fileStorageLocations) {
                    try {
                        fileStorage.fileStorage.deleteFile(fileStorageLocation);
                    } catch (Exception e) {
    ...
    private static DataExportTask parsetTask(ResultSet rs) throws SQLException, OXException {
        ...
        try {
            ...
            if (!rs.wasNull()) {
                JSONObject jArgs = JSONServices.parseObject(args);
    ...
    private static DataExportWorkItem parseWorkItem(ResultSet rs) throws SQLException, OXException {
        ...
        try {
            ...
            item.setSavePoint(sp == null ? null : JSONServices.parseObject(sp));
            String info = rs.getString("info");
            item.setInfo(info == null ? null : JSONServices.parseObject(info));
        } catch (JSONException e) {
            throw DataExportExceptionCode.JSON_ERROR.create(e, e.getMessage());
        }
        return item;
    ...
    private static class TaskInfo implements Comparable<TaskInfo> {
        ...
        public String toString() {
            return MessageFormatter.format("data export task {} of user {} in context {}", stringFor(taskId), I(userId), I(contextId));
        }
    }
