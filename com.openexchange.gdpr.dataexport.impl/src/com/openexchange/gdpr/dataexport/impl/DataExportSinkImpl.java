...
public class DataExportSinkImpl implements DataExportSink {
     ...
     */
    public DataExportSinkImpl(DataExportTask task, String moduleId, String pathPrefix, Optional<String> fileStorageLocation, FileStorage fileStorage, boolean useZip64, DataExportStorageService storageService, Optional<DataExportDiagnosticsReport> optionalReport, ServiceLookup services) {
        super();
        this.task = task;
    ...
    private ZippedFileStorageOutputStream getZipOutputStream(Optional<String> optionalFileStorageLocation) throws OXException { // Only called when holding lock
        ...
        if (out == null) {
            ...
            try {
                out = ZippedFileStorageOutputStream.createDefaultZippedFileStorageOutputStream(fileStorage, Deflater.DEFAULT_COMPRESSION, useZip64);
                        ...
                        try {
                            ...
                            for (ZipArchiveEntry entry; (entry = zipIn.getNextZipEntry()) != null;) {
                                if (useZip64) {
                                    ...
                                    if (isTooLargeForZip32(entry)) {
                                        LOG.warn("{}'s size exceeds the limit of 4GByte. Ignoring that file for \"{}\" data export {} of user {} in context {}", entry.getName(), moduleId, UUIDs.getUnformattedString(task.getId()), I(task.getUserId()), I(task.getContextId()));
                                    } else {
                                        try {
                                            out.putArchiveEntry(entry);
                                            IOUtils.copy(zipIn, out, BUFFER_SIZE);
                                            out.closeArchiveEntry();
                                            out.flush();
                                        } catch (Zip64RequiredException x) {
                                            LOG.warn("Requires Zip64 support. Ignoring file {} for \"{}\" data export {} of user {} in context {}", entry.getName(), moduleId, UUIDs.getUnformattedString(task.getId()), I(task.getUserId()), I(task.getContextId()), x);
                                        }
                                    }
                                }
                            }
    ...
    /**
     * Checks if given entry is too large and therefore requires Zip64 support.
     *
     * @param zipArchiveEntry The entry to check
     * @return <code>true</code> if too large; otherwise <code>false</code>
     */
    private static boolean isTooLargeForZip32(ZipArchiveEntry zipArchiveEntry){
