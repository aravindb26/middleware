...
public class ProcessorServiceImpl implements ProcessorService {
    ...
    @Override
    public Processor newProcessor(String name, int numThreads, int numMaxThreads) throws OXException {
        try {
            int id = count.incrementAndGet();
            Processor processor = new StripedProcessor(null == name ? ("Processor" + id) : name, numThreads, numMaxThreads, -1);
            processors.offer(processor);
            return processor;
        } catch (RuntimeException e) {
            throw OXException.general(e.getMessage(), e);
    ...
    @Override
    public Processor newBoundedProcessor(String name, int numThreads, int numMaxThreads, int maxTasks) throws OXException {
        try {
            int id = count.incrementAndGet();
            Processor processor = new StripedProcessor(null == name ? ("Processor" + id) : name, numThreads, numMaxThreads, maxTasks);
            processors.offer(processor);
            return processor;
        } catch (RuntimeException e) {
            throw OXException.general(e.getMessage(), e);
