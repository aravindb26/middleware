...
public class IncreaseThreadsBeforeQueueingThreadPoolExecutor extends ThreadPoolExecutor {
     ...
     */
    public IncreaseThreadsBeforeQueueingThreadPoolExecutor(String name, int nThreads, int nMaxThreads, boolean supportMDC) {
        // See java.util.concurrent.Executors.newFixedThreadPool(int, ThreadFactory)
        super(nThreads, nMaxThreads, 0L, TimeUnit.MILLISECONDS, new OfferIfEmptyLinkedBlockingQueue(), new ProcessorThreadFactory(name, supportMDC), new QueuePuttingRejectedExecutionHandler());
    }
