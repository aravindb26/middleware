...
public class ProcessorThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    public ProcessorThreadPoolExecutor(String name, int nThreads, boolean supportMDC) {
        ...
        super(nThreads, nThreads, 0L, TimeUnit.MILLISECONDS, new AlwaysRejectingLinkedBlockingQueue(), new ProcessorThreadFactory(name, supportMDC));
    }
     ...
     * @param name The name prefix to use for associated threads
     * @param nThreads The number of threads in the pool
     * @param nMaxThreads The max. number of threads to be spawned by the pool
     * @param supportMDC Whether to support MDC log properties; otherwise MDC will be cleared prior to each log output
     */
    public ProcessorThreadPoolExecutor(String name, int nThreads, int nMaxThreads, boolean supportMDC) {
        // See java.util.concurrent.Executors.newFixedThreadPool(int, ThreadFactory)
        super(nThreads, nMaxThreads, 0L, TimeUnit.MILLISECONDS, new AlwaysRejectingLinkedBlockingQueue(), new ProcessorThreadFactory(name, supportMDC));
    }
    ...
    private static final class AlwaysRejectingLinkedBlockingQueue extends LinkedBlockingQueue<Runnable> {

        private static final long serialVersionUID = -3772522857604129119L;

        /**
         * Initializes a new {@link AlwaysRejectingLinkedBlockingQueue}.
         */
        public AlwaysRejectingLinkedBlockingQueue() {
            super();
        }
        ...
        public boolean offer(Runnable e) {
            /*
             * Never offer it to the queue to let ThreadPoolExecutor add another thread
             */
            return false;
        }
    }
