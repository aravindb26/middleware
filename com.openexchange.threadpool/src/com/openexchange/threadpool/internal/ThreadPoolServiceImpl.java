...
public final class ThreadPoolServiceImpl implements ThreadPoolService {
    ...
    private ThreadPoolServiceImpl(CorePoolSize corePoolSize, int maximumPoolSize, long keepAliveTime, String workQueue, int workQueueSize, boolean blocking, String refusedExecutionBehavior, long watcherMaxRunningTime, long watcherMinWaitTime) {
        ...
        if (QueueType.LINKED.equals(queueType) && this.corePoolSize < maximumPoolSize) {
            ...
            threadFactory = new CustomThreadFactory("OXWorker-");
            threadPoolExecutor =
    ...
    private static int getCorePoolSize(CorePoolSize corePoolSize) {
        ...
        if (desiredCorePoolSize < minCorePoolSize) {
            LOG.warn("{}{}\tConfigured pool size of {} through property \"com.openexchange.threadpool.corePoolSize\" does not obey the rule{}\tfor minimum core pool size: {} (number of CPUs) + 1 = {}. Using {} as core pool size.\n", Strings.getLineSeparator(), Strings.getLineSeparator(), corePoolSize, Strings.getLineSeparator(), I(Runtime.getRuntime().availableProcessors()), I(minCorePoolSize), I(minCorePoolSize));
