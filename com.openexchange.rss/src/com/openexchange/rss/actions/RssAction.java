    ...
    private void retrieveAndAddFeed(URL url, List<SyndFeed> feeds, Optional<List<OXException>> optionalWarnings) throws OXException {
        try {
            ...
            if (ExceptionUtils.isEitherOf(e, SSLHandshakeException.class)) {
                ...
                UriDeniedProtocolException opt = ExceptionUtils.extractFrom(e, UriDeniedProtocolException.class);
                oxe = opt == null ? RssExceptionCodes.IO_ERROR.create(e, e.getMessage(), url.toString()) : RssExceptionCodes.IO_ERROR.create(e, opt.getMessage(), url.toString());
            }
            if (!optionalWarnings.isPresent()) {
                throw oxe;
            }
