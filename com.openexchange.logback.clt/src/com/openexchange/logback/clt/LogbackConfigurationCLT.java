    ...
    private enum CommandLineExecutor {
        ...
        CONTEXT {
            ...
            void executeWith(CommandLine commandLine, LogbackConfigurationRMIService logbackConfigService) throws RemoteException {
                ...
                String adminUser = commandLine.getOptionValue('A');
                String adminPassword = commandLine.getOptionValue('P');
                LogResponse response = null;
                if (commandLine.hasOption('a')) {
                    response = logbackConfigService.filterContext(contextId, getLoggerMap(commandLine.getOptionValues('l')), adminUser, adminPassword);
                } else if (commandLine.hasOption('d')) {
                    response = logbackConfigService.removeContextFilter(contextId, getLoggerList(commandLine.getOptionValues('l')), adminUser, adminPassword);
                } else {
        ...
        USER {
            ...
            void executeWith(CommandLine commandLine, LogbackConfigurationRMIService logbackConfigService) throws RemoteException {
                ...
                LogResponse response = null;
                if (commandLine.hasOption('a')) {
                    response = logbackConfigService.filterUser(contextId, userId, getLoggerMap(commandLine.getOptionValues('l')), adminUser, adminPassword);
                } else if (commandLine.hasOption('d')) {
                    response = logbackConfigService.removeUserFilter(contextId, userId, getLoggerList(commandLine.getOptionValues('l')), adminUser, adminPassword);
                } else {
                    throw new IllegalArgumentException("You must specify a life cycle switch, either -a to add a filter or -d to remove a filter.");
                }
                printResponse(response);
        ...
        SESSION {
            ...
            void executeWith(CommandLine commandLine, LogbackConfigurationRMIService logbackConfigService) throws RemoteException {
                ...
                LogResponse response = null;
                if (commandLine.hasOption('a')) {
                    response = logbackConfigService.filterSession(sessionId, getLoggerMap(commandLine.getOptionValues('l')), adminUser, adminPassword);
                } else if (commandLine.hasOption('d')) {
                    response = logbackConfigService.removeSessionFilter(sessionId, getLoggerList(commandLine.getOptionValues('l')), adminUser, adminPassword);
                } else {
                    throw new IllegalArgumentException("You must specify a life cycle switch, either -a to add a filter or -d to remove a filter.");
                }
                printResponse(response);
        ...
        MODIFY {
            ...
            void executeWith(CommandLine commandLine, LogbackConfigurationRMIService logbackConfigService) throws RemoteException {
                String adminUser = commandLine.getOptionValue('A');
                String adminPassword = commandLine.getOptionValue('P');
                printResponse(logbackConfigService.modifyLogLevels(getLoggerMap(commandLine.getOptionValues('l')), adminUser, adminPassword));
            }
        },
        LIST_CATEGORIES {
        ...
        CLEAR_FILTERS {
            ...
            void executeWith(CommandLine commandLine, LogbackConfigurationRMIService logbackConfigService) throws RemoteException {
                String adminUser = commandLine.getOptionValue('A');
                String adminPassword = commandLine.getOptionValue('P');
                logbackConfigService.clearFilters(adminUser, adminPassword);
            }
        ...
        OVERRIDE_EXCEPTION_CATEGORIES {
            ...
            void executeWith(CommandLine commandLine, LogbackConfigurationRMIService logbackConfigService) throws RemoteException {
                ...
                String adminUser = commandLine.getOptionValue('A');
                String adminPassword = commandLine.getOptionValue('P');
                logbackConfigService.overrideExceptionCategories(builder.subSequence(0, builder.length() - 1).toString(), adminUser, adminPassword);
            }
        };
