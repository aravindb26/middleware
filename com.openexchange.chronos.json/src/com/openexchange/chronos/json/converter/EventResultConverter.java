...
public class EventResultConverter implements ResultConverter {
    ...
    private static final ContactField CONTACT_FIELDS_TO_LOAD[] = { ContactField.SUR_NAME, ContactField.GIVEN_NAME, ContactField.TITLE, ContactField.DISPLAY_NAME, ContactField.IMAGE1_URL, ContactField.NUMBER_OF_IMAGES, ContactField.INTERNAL_USERID };
    private static final ContactField CONTACT_FIELDS_TO_SHOW[] = { ContactField.SUR_NAME, ContactField.GIVEN_NAME, ContactField.TITLE, ContactField.DISPLAY_NAME, ContactField.IMAGE1_URL };
    ...
    /**
     * Enriches internal attendees and the organizer within the supplied event with detailed information about the underlying groupware
     * resource.
     *
    ...
    private void extendEntities(ServerSession session, String timeZoneID, JSONObject jsonEvent) {
        Map<Integer, List<JSONObject>> individualsPerId = new HashMap<Integer, List<JSONObject>>();
        Map<Integer, JSONObject> resourcesPerId = new HashMap<Integer, JSONObject>();
        Map<Integer, JSONObject> groupsPerId = new HashMap<Integer, JSONObject>();
        /*
            ...
            for (int i = 0; i < jsonAttendees.length(); i++) {
                ...
                if (0 <= entity) {
                    ...
                    if (CalendarUserType.INDIVIDUAL.matches(cuType)) {
                        com.openexchange.tools.arrays.Collections.put(individualsPerId, I(entity), jsonAttendee);
                    } else if (CalendarUserType.ROOM.matches(cuType) || CalendarUserType.RESOURCE.matches(cuType)) {
                        resourcesPerId.put(I(entity), jsonAttendee);
                    } else if (CalendarUserType.GROUP.matches(cuType)) {
                        groupsPerId.put(I(entity), jsonAttendee);
                    }
                }
            }
        }
        /*
         * also add organizer if applicable
         */
        JSONObject jsonOrganizer = jsonEvent.optJSONObject(ChronosJsonFields.ORGANIZER);
        if (null != jsonOrganizer) {
            int entity = jsonOrganizer.optInt(ChronosJsonFields.Attendee.ENTITY, -1);
            if (0 < entity) {
                com.openexchange.tools.arrays.Collections.put(individualsPerId, I(entity), jsonOrganizer);
            }
        }
        /*
         * resolve & apply extended entity information
         */
        addGroupDetailsFromId(session, groupsPerId);
        addResourceDetailsFromId(session.getContext(), session.getUser(), resourcesPerId);
        addContactDetailsFromId(session, CalendarUtils.optTimeZone(timeZoneID, TimeZones.UTC), individualsPerId);
    }

    private void addGroupDetailsFromId(Session session, Map<Integer, JSONObject> attendeesPerId) {
        if (null == attendeesPerId || attendeesPerId.isEmpty()) {
            return;
        }
        for (Entry<Integer, JSONObject> entry : attendeesPerId.entrySet()) {
            try {
                Group group = requireService(GroupService.class, services).getGroup(session, i(entry.getKey()));
    ...
    private void addResourceDetailsFromId(Context context, User user, Map<Integer, JSONObject> attendeesPerId) {
        if (null == attendeesPerId || attendeesPerId.isEmpty()) {
            return;
        }
        for (Entry<Integer, JSONObject> entry : attendeesPerId.entrySet()) {
            try {
                ...
                entry.getValue().put(ChronosJsonFields.Attendee.RESOURCE, ResourceWriter.writeResource(user, resource));
            } catch (OXException | JSONException e) {
    ...
    private void addContactDetailsFromId(Session session, TimeZone timeZone, Map<Integer, List<JSONObject>> individualsPerId) {
        if (null == individualsPerId || individualsPerId.isEmpty()) {
            return;
        }
        SearchIterator<Contact> users = null;
        try {
            users = requireService(ContactService.class, services).getUsers(session, I2i(individualsPerId.keySet()), CONTACT_FIELDS_TO_LOAD);
            while (users.hasNext()) {
                Contact contact = users.next();
                List<JSONObject> jsonObjects = individualsPerId.get(I(contact.getInternalUserId()));
                if (null == jsonObjects || jsonObjects.isEmpty()) {
                    LOG.warn("Unable to find calendar user for contact with id {}", I(contact.getInternalUserId()));
                    continue;
                }
                for (JSONObject jsonObject : jsonObjects) {
                    jsonObject.put(ChronosJsonFields.Attendee.CONTACT, serialize(contact, timeZone, session));
                }
            }
        } catch (OXException | JSONException e) {
