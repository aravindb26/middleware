...
public class FreeBusyConverter implements ResultConverter {
        ...
        if ((resultObject instanceof Map)) {
            try {
                ...
                for (Entry<Attendee, FreeBusyResult> attendee : map.entrySet()) {
                    ...
                    json.put(ChronosFreeBusyJsonFields.ATTENDEE, ((AttendeesMapping<?>) EventMapper.getInstance().get(EventField.ATTENDEES)).serialize(attendee.getKey(), timeZone));
                    FreeBusyResult freeBusyResult = attendee.getValue();
                    json.put(ChronosFreeBusyJsonFields.FREE_BUSY_TIME, serializeFreeBusyTime(freeBusyResult, session, timeZone));
     ...
     * @throws OXException
     */
    private JSONArray serializeFreeBusyTime(FreeBusyResult freeBusyResult, Session session, TimeZone timeZone) throws JSONException, OXException {
        ...
        for (FreeBusyTime time : freeBusyTimes) {
            ...
            if (event!=null){
                json.put(ChronosFreeBusyJsonFields.FreeBusyTime.EVENT, EventMapper.getInstance().serialize(event, EventMapper.getInstance().getAssignedFields(event), timeZone, session));
            }
            result.put(json);
        }
        return result;
