 ...
 * @since v7.10.6
 */
public class AnalyzeAction extends AbstractSchedulingAction {
    ...
    AJAXRequestResult process(AJAXRequestData request, IDBasedCalendarAccess access, IncomingSchedulingMessage message) throws OXException {
        ITipAnalysis analyze = access.getSchedulingAccess().analyze(message);
        if (null != analyze) {
            return new AJAXRequestResult(Collections.singletonList(analyze), new Date(), ITipAnalysisResultConverter.INPUT_FORMAT);
        }
