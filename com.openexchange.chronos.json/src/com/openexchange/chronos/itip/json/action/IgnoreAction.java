 ...
 * @since v7.10.6
 */
public class IgnoreAction extends AbstractSchedulingAction {

    /**
     * Initializes a new {@link IgnoreAction}.
     * 
     * @param services The service lookup to use
     */
    public IgnoreAction(ServiceLookup services) {
        super(services);
    }
    ...
    AJAXRequestResult process(AJAXRequestData request, IDBasedCalendarAccess access, IncomingSchedulingMessage message) throws OXException {
        MessageStatusService statusService = services.getServiceSafe(MessageStatusService.class);
        statusService.setMessageStatus(access.getSession(), message, MessageStatus.IGNORED);
