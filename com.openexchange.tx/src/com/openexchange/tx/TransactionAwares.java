...
public class TransactionAwares {
     ...
     *
     * @param transactions The transactions to finish
     */
    public static void finishSafe(final TransactionAware... transactions) {
        if (null == transactions || transactions.length <= 0) {
            return;
        }

        for (TransactionAware transaction : transactions) {
            if (null != transaction) {
                try {
                    transaction.finish();
                } catch (Exception e) {
     ...
     *
     * @param transactions The transactions to roll back
     */
    public static void rollbackSafe(final TransactionAware... transactions) {
        if (null == transactions || transactions.length <= 0) {
            return;
        }
