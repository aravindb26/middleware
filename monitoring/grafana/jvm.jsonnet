...
local templates = [
  {
    ...
    query: 'label_values(jvm_memory_used_bytes{pod="$pod", area="heap"}, id)',
    hide: 'variable',
    includeAll: true,
  },
  {
    ...
    query: 'label_values(jvm_memory_used_bytes{pod="$pod", area="nonheap"}, id)',
    hide: 'variable',
    includeAll: true,
  },
];
...
local threadThreads = graphPanel.new(
  ...
  [
    prometheus.target(
      'jvm_threads_live_threads{pod="$pod"}',
      legendFormat='Live',
    ),
    prometheus.target(
      'jvm_threads_daemon_threads{pod="$pod"}',
      legendFormat='Daemon',
    ),
    prometheus.target(
      'jvm_threads_peak_threads{pod="$pod"}',
      legendFormat='Peak',
    ),
  ]
).addSeriesOverride(
...
local threadThreadStates = graphPanel.new(
  ...
  prometheus.target(
    'jvm_threads_states_threads{pod="$pod"}',
    legendFormat='{{state}}',
  )
).addSeriesOverride(
  {
...
local overviewNonHeapUsage = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(jvm_memory_used_bytes{pod="$pod", area="nonheap"})',
      legendFormat='Used',
    ),
    prometheus.target(
      'sum(jvm_memory_committed_bytes{pod="$pod", area="nonheap"})',
      legendFormat='Committed',
    ),
    prometheus.target(
      'sum(jvm_memory_max_bytes{pod="$pod", area="nonheap"})',
      legendFormat='Max',
    ),
  ]
).addSeriesOverride(
...
local overviewUptime = singlestat.new(
  ...
  prometheus.target(
    'process_uptime_seconds{pod="$pod"}'
  )
);
...
local overviewClassesLoaded = singlestat.new(
  ...
  prometheus.target(
    'jvm_classes_loaded_classes{pod="$pod"}'
  )
);
...
local overviewClassesUnloaded = singlestat.new(
  ...
  prometheus.target(
    'jvm_classes_unloaded_classes_total{pod="$pod"}'
  )
);
...
local gcDuration = graphPanel.new(
  ...
  prometheus.target(
    'rate(jvm_gc_collection_seconds_sum{pod="$pod"}[$interval])/rate(jvm_gc_collection_seconds_count{pod="$pod"}[$interval])',
    legendFormat='{{gc}}'
  )
);
...
local gcDurationCount = graphPanel.new(
  ...
  prometheus.target(
    'rate(jvm_gc_collection_seconds_count{pod="$pod"}[$interval])',
    legendFormat='{{gc}}'
  )
);
...
local memoryPoolsHeap = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(jvm_memory_used_bytes{pod="$pod", area="heap", id=~"$mem_pools_heap"})',
      legendFormat='Used',
    ),
    prometheus.target(
      'sum(jvm_memory_committed_bytes{pod="$pod", area="heap", id=~"$mem_pools_heap"})',
      legendFormat='Committed',
    ),
    prometheus.target(
      'sum(jvm_memory_max_bytes{pod="$pod", area="heap", id=~"$mem_pools_heap"})',
      legendFormat='Max',
    ),
  ]
).addSeriesOverride(
...
local memoryPoolsNonHeap = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(jvm_memory_used_bytes{pod="$pod", area="nonheap", id=~"$mem_pools_nonheap"})',
      legendFormat='Used',
    ),
    prometheus.target(
      'sum(jvm_memory_committed_bytes{pod="$pod", area="nonheap", id=~"$mem_pools_nonheap"})',
      legendFormat='Committed',
    ),
    prometheus.target(
      'sum(jvm_memory_max_bytes{pod="$pod", area="nonheap", id=~"$mem_pools_nonheap"})',
      legendFormat='Max',
    ),
  ]
).addSeriesOverride(
