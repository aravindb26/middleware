...
local sessions = [
  {
    ...
    target: {
      expr: 'appsuite_sessions_total{client="all", pod="$pod"}',
      legendFormat: 'Total',
    },
  ...
  },
  {
    ...
    target: {
      expr: 'appsuite_sessions_active_total{client="all", pod="$pod"}',
      legendFormat: 'Active',
    },
  ...
  },
  {
    ...
    target: {
      expr: 'appsuite_sessions_short_term_total{client="all", pod="$pod"}',
  ...
  },
  {
    ...
    target: {
      expr: 'appsuite_sessions_long_term_total{client="all", pod="$pod"}',
...
local cacheRatio = [
  {
    title: 'Hit Ratio',
    expr: 'sum(rate(appsuite_jcs_cache_hits_total{pod="$pod"}[$interval])) / (sum(rate(appsuite_jcs_cache_hits_total{pod="$pod"}[$interval])) + sum(rate(appsuite_jcs_cache_misses_total{pod="$pod"}[$interval])))',
    gridPos: { h: 4, w: 3, x: 0, y: 33 },
  },
  {
    title: 'Miss Ratio',
    expr: 'sum(rate(appsuite_jcs_cache_misses_total{pod="$pod"}[$interval])) / (sum(rate(appsuite_jcs_cache_hits_total{pod="$pod"}[$interval])) + sum(rate(appsuite_jcs_cache_misses_total{pod="$pod"}[$interval])))',
    gridPos: { h: 4, w: 3, x: 0, y: 33 },
  },
];
...
local genPercentileTargets(metric, labels=[], groupBy=[]) = [
  prometheus.target(
    'histogram_quantile(0.' + p + ', sum(rate(appsuite_' + metric + '_seconds_bucket{' + std.join(',', ['pod="$pod"'] + labels) + '}[$interval])) by (' + std.join(',', ['le', 'instance'] + groupBy) + '))',
    legendFormat='p' + p,
  )
  for p in [50, 75, 90, 95, 99, 999]
] + [
  prometheus.target(
    'sum(rate(appsuite_%s_seconds_sum{%s}[$interval]))/sum(rate(appsuite_%s_seconds_count{%s}[$interval]))' % [metric, std.join(',', ['pod="$pod"'] + labels), metric, std.join(',', ['pod="$pod"'] + labels)],
    legendFormat='avg',
  ),
  prometheus.target(
    'sum(appsuite_%s_seconds_max{%s})' % [metric, std.join(',', ['pod="$pod"'] + labels)],
    legendFormat='max',
  ),
];
...
local httpClientRequestsPerSecond = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(rate(appsuite_httpclient_requests_seconds_count{pod="$pod",client="$httpclient"}[$interval])) by (client,instance)',
      legendFormat='Total',
    ),
    prometheus.target(
      'sum(rate(appsuite_httpclient_requests_seconds_count{pod="$pod",client="$httpclient",status!~"[45][0-9]{2}"}[$interval])) by (client,instance)',
      legendFormat='OK',
    ),
    prometheus.target(
      'sum(rate(appsuite_httpclient_requests_seconds_count{pod="$pod",client="$httpclient",status=~"[45][0-9]{2}"}[$interval])) by (client,instance)',
...
local httpClientConnections = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_httpclient_connections_pending{pod="$pod",client=~"$httpclient"}',
      legendFormat='Pending',
    ),
    prometheus.target(
      'appsuite_httpclient_connections_available{pod="$pod",client=~"$httpclient"}',
      legendFormat='Available',
    ),
    prometheus.target(
      'appsuite_httpclient_connections_leased{pod="$pod",client=~"$httpclient"}',
      legendFormat='Leased',
    ),
    prometheus.target(
      'appsuite_httpclient_connections_max{pod="$pod",client=~"$httpclient"}',
      legendFormat='Max',
    ),
  ]
).addSeriesOverride(
...
local threadPoolTasks = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_executor_queue_remaining_tasks{name="main",pod="$pod"}',
      legendFormat='Remaining',
    ),
    prometheus.target(
      'appsuite_executor_queued_tasks{name="main",pod="$pod"}',
      legendFormat='Queued',
    ),
    prometheus.target(
      'rate(appsuite_executor_completed_tasks_total{name="main",pod="$pod"}[$interval])',
...
local threadPool = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_executor_active_threads{name="main",pod="$pod"}',
      legendFormat='ActiveCount',
    ),
    prometheus.target(
      'appsuite_executor_pool_size_threads{name="main",pod="$pod"}',
...
local httpApiRequestsPercentilesByRequest = graphPanel.new(
  ...
  prometheus.target(
    'topk(5,histogram_quantile(0.99, sum(rate(appsuite_httpapi_requests_seconds_bucket{pod="$pod"}[${__range_s}s])) by (le, module, action, instance)))',
...
local httpApiRequestsPerSecond = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(rate(appsuite_httpapi_requests_seconds_count{pod="$pod"}[$interval]))',
      legendFormat='Total',
    ),
    prometheus.target(
      'sum(rate(appsuite_httpapi_requests_seconds_count{status=~"OK",pod="$pod"}[$interval]))',
      legendFormat='OK',
    ),
    prometheus.target(
      'sum(rate(appsuite_httpapi_requests_seconds_count{status!~"OK",pod="$pod"}[$interval]))',
...
local restApiRequestsPerSecond = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(rate(appsuite_restapi_requests_seconds_count{pod="$pod"}[$interval]))',
      legendFormat='Total',
    ),
    prometheus.target(
      'sum(rate(appsuite_restapi_requests_seconds_count{status!~"[45][0-9]{2}",pod="$pod"}[$interval]))',
      legendFormat='OK',
    ),
    prometheus.target(
      'sum(rate(appsuite_restapi_requests_seconds_count{status=~"[45][0-9]{2}",pod="$pod"}[$interval]))',
...
local imapRequestRate = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(rate(appsuite_imap_commands_seconds_count{pod="$pod"}[$interval]))',
      legendFormat='Total',
    ),
    prometheus.target(
      'sum(rate(appsuite_imap_commands_seconds_count{status=~"OK",pod="$pod"}[$interval]))',
      legendFormat='OK',
    ),
    prometheus.target(
      'sum(rate(appsuite_imap_commands_seconds_count{status!~"OK",pod="$pod"}[$interval]))',
...
local circuitBreakerDenialRate = graphPanel.new(
  ...
  prometheus.target(
    expr='rate(appsuite_circuitbreaker_denials_total{pod="$pod", name=~"$' + circuitBreakerName + '"}[$interval])',
    legendFormat='{{name}}',
  )
);
...
local circuitBreakerOpenRate = graphPanel.new(
  ...
  prometheus.target(
    expr='rate(appsuite_circuitbreaker_opens_total{pod="$pod", name=~"$' + circuitBreakerName + '"}[$interval])',
    legendFormat='{{name}}',
  )
);
...
local circuitBreakerStates = table.new(
  ...
  prometheus.target(
    expr='count(appsuite_circuitbreaker_state{pod="$pod"} > 0) by (host, name, state)',
    format='table',
    instant=true,
  )
);
...
local soapApiRequestsPerSecond = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(rate(appsuite_soapapi_requests_seconds_count{pod="$pod"}[$interval]))',
      legendFormat='Total',
    ),
    prometheus.target(
      'sum(rate(appsuite_soapapi_requests_seconds_count{status=~"OK",pod="$pod"}[$interval]))',
      legendFormat='OK',
    ),
    prometheus.target(
      'sum(rate(appsuite_soapapi_requests_seconds_count{status!~"OK",pod="$pod"}[$interval]))',
...
local webdavApiRequestsPerSecond = graphPanel.new(
  ...
  [
    prometheus.target(
      'sum(rate(appsuite_webdav_requests_seconds_count{pod="$pod"}[$interval]))',
      legendFormat='Total',
    ),
    prometheus.target(
      'sum(rate(appsuite_webdav_requests_seconds_count{status=~"OK|^0",pod="$pod"}[$interval]))',
      legendFormat='OK',
    ),
    prometheus.target(
      'sum(rate(appsuite_webdav_requests_seconds_count{status!~"OK|^0",pod="$pod"}[$interval]))',
...
local dbTimeoutRatio = singlestat.new(
  ...
  prometheus.target(
    expr='sum without (instance, type, class, pool, service) (rate(appsuite_mysql_connections_timeout_total{pod="$app"}[1m])) / sum without (instance, type, class, pool, service) (rate(appsuite_mysql_connections_total{pod="$app"}[1m]))',
  )
);
...
local configDBConnections = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_mysql_connections_total{class="configdb",pod="$pod"}',
      legendFormat='Pooled {{type}}',
    ),
    prometheus.target(
      'appsuite_mysql_connections_active{class="configdb",pod="$pod"}',
      legendFormat='Active {{type}}',
    ),
    prometheus.target(
      'appsuite_mysql_connections_idle{class="configdb",pod="$pod"}',
...
local configDBReadConnections = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_mysql_connections_total{class="configdb",pod="$pod",type="read"}',
      legendFormat='Pooled',
    ),
    prometheus.target(
      'appsuite_mysql_connections_active{class="configdb",pod="$pod",type="read"}',
      legendFormat='Active',
    ),
    prometheus.target(
      'appsuite_mysql_connections_idle{class="configdb",pod="$pod",type="read"}',
      legendFormat='Idle',
    ),
    prometheus.target(
      'sum(appsuite_mysql_connections_max{class="configdb",pod="$pod",type="read"})',
      legendFormat='Max',
    ),
  ]
...
local configDBWriteConnections = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_mysql_connections_total{class="configdb",pod="$pod",type="write"}',
      legendFormat='Pooled',
    ),
    prometheus.target(
      'appsuite_mysql_connections_active{class="configdb",pod="$pod",type="write"}',
      legendFormat='Active',
    ),
    prometheus.target(
      'appsuite_mysql_connections_idle{class="configdb",pod="$pod",type="write"}',
      legendFormat='Idle',
    ),
    prometheus.target(
      'sum(appsuite_mysql_connections_max{class="configdb",pod="$pod",type="write"})',
      legendFormat='Max',
    ),
  ]
).addSeriesOverride({ alias: 'Max', fill: 0 },);
...
local userdbConnections = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_mysql_connections_total{class=~"userdb",pod="$pod",pool=~"$' + dbPoolName + '"}',
      legendFormat='Pooled (Type: {{type}}, Pool: {{pool}})',
    ),
    prometheus.target(
      'appsuite_mysql_connections_active{class=~"userdb",pod="$pod",pool=~"$' + dbPoolName + '"}',
      legendFormat='Active (Type: {{type}}, Pool: {{pool}})',
    ),
    prometheus.target(
      'appsuite_mysql_connections_idle{class=~"userdb",pod="$pod",pool=~"$' + dbPoolName + '"}',
      legendFormat='Idle (Type: {{type}}, Pool: {{pool}})',
    ),
    prometheus.target(
      'appsuite_mysql_connections_max{class=~"userdb",pod="$pod",pool=~"$' + dbPoolName + '"}',
...
local sessionStorageBytes = graphPanel.new(
  ...
  prometheus.target(
    'appsuite_sessionstorage_memory_bytes{pod="$pod"}',
    legendFormat='{{type}}',
  )
);
...
local sessionStorageTotal = graphPanel.new(
  ...
  [
    prometheus.target(
      'appsuite_sessionstorage_sessions_total{pod="$pod"}',
      legendFormat='{{type}}',
    ),
    prometheus.target(
...
local overviewAppSuiteVersion = singlestat.new(
  ...
  prometheus.target(
    expr='appsuite_version_info{pod="$pod"}',
...
local overviewUptime = singlestat.new(
  ...
  prometheus.target(
    expr='process_uptime_seconds{pod="$pod"}',
  )
);
...
local overviewHealthStatus = singlestat.new(
  ...
  prometheus.target(
    expr='appsuite_health_status{pod="$pod"} > 0',
  )
);

grafana.newDashboard(
  title='App Suite / Core-Middleware', tags=['java', 'app-suite'], metric='jvm_info'
).addLink(
  link.dashboards(
    title='Documentation',
    tags=[],
  ...
  [
    ...
    graphPanel.new(
      ...
      [
        ...
        prometheus.target(
          expr='appsuite_sessions_max{client="all", pod="$pod"}',
          legendFormat='Max',
        ),
      ]
    ).addSeriesOverride(
    ...
    graphPanel.new(
      ...
      [
        prometheus.target(
          expr='sum(rate(appsuite_jcs_cache_puts_total{pod="$pod"}[$interval]))',
          legendFormat='Puts',
        ),
        prometheus.target(
          expr='sum(rate(appsuite_jcs_cache_removals_total{pod="$pod"}[$interval]))',
    ...
    graphPanel.new(
      ...
      prometheus.target(
        expr='topk(5,avg_over_time(appsuite_jcs_cache_elements_total{pod="$pod"}[${__range_s}s]))',
        legendFormat='{{region}}',
      )
