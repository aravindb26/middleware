...
public class PreserveOriginalTest extends ICalTest {
    ...
    public void testPreserveDirParameter() throws Exception {
        ...
        assertTrue("Missing extended properties", event.containsAttendees());

        List<ExtendedPropertyParameter> attendeeProperties = event.getAttendees().get(0).getExtendedParameters();
        assertNotNull("Missing attendee properties", attendeeProperties);
        assertFalse("Missing attendee properties", attendeeProperties.isEmpty());
        Optional<ExtendedPropertyParameter> optProperty = attendeeProperties.stream().filter(prop -> prop.getName().equals("DIR")).findAny();
        assertTrue("Missing DIR property", optProperty.isPresent());
        assertEquals("ldap://host.com:66/horst", optProperty.get().getValue());
