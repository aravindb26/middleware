...
public class AppPasswordServiceImpl implements AppPasswordService, AppAuthenticatorService, Reloadable {
    ...
    public List<AppPasswordApplication> getApplications(Session session) throws OXException {
        CapabilitySet capabilitySet = services.getServiceSafe(CapabilityService.class).getCapabilities(session.getUserId(), session.getContextId());
        return getApplications(session, capabilitySet);
    }

    /**
     * Gets the list of applications that can be configured for the session user.
     *
     * @param session The session
     * @param capabilitySet The capability set to consider
     * @return The applications, or an empty list if there are none
     */
    public List<AppPasswordApplication> getApplications(Session session, CapabilitySet capabilitySet) throws OXException {
        ServerSession serverSession = ServerSessionAdapter.valueOf(session);
        if (serverSession == null || serverSession.getUser() == null || serverSession.getUser().isGuest()) {
            return Collections.emptyList();
        }
        Collection<AppPasswordApplication> applications = getApplicationsById(session.getContextId(), session.getUserId(), capabilitySet).values();
     ...
     * @param The capability set to consider
     */
    private Map<String, AppPasswordApplication> getApplicationsById(int contextId, int userId) throws OXException {
        CapabilitySet capabilitySet = services.getServiceSafe(CapabilityService.class).getCapabilities(userId, contextId);
        return getApplicationsById(contextId, userId, capabilitySet);
    }
     ...
     */
    private Map<String, AppPasswordApplication> getApplicationsById(int contextId, int userId, CapabilitySet capabilitySet) throws OXException {
        ...
        for (String appType : appTypes) {
            ...
            if (null != capabilitySet && false == containsAll(capabilitySet, application.getRequiredCapabilities())) {
                LOG.debug("Missing capabilities \"{}\" for application {} for user {} in context {}, skipping.", application.getRequiredCapabilities(), appType, I(userId), I(contextId));
                continue;
            }
    ...
    private static boolean containsAll(CapabilitySet capabilitySet, String[] capabilities) {
        if (null != capabilities) {
            for (String capability : capabilities) {
                if (false == capabilitySet.contains(capability)) {
                    return false;
                }
            }
        }
    ...
    private Collection<String> removeSessions(int contextId, int userId, String passwordId) {
        ...
        .toString();
        try {
            return sessiondService.removeSessionsGlobally(SessionFilter.create(filterString, UserAndContext.newInstance(userId, contextId), null));
        } catch (OXException e) {
    ...
    /**
     * Gets the registered {@link AppPasswordStorage} services, in descending order of their OSGi service ranking (the one with the
     * highest ranking being first in the collection).
     *
     * @return The app password storages, or an empty list if none are available
     */
    private Collection<AppPasswordStorage> getStorages() {
        return storages.descendingSet();
    }
    ...
    private static Map<String, AppPasswordApplication> parseApps(ConfigurationService configService) throws OXException {
        ...
        for (Map.Entry<String, Object> entry : configs.entrySet()) {
            try {
                ...
                List<String> capabilities = (List<String>) values.get("requiredCapabilities");
                applicationsById.put(appType, AppPasswordApplication.builder()
                    ...
                    .setScopes(null == scopes ? Strings.getEmptyStrings() : scopes.toArray(new String[scopes.size()]))
                    .setRequiredCapabilities(null == capabilities ? Strings.getEmptyStrings() : capabilities.toArray(new String[capabilities.size()]))
                .build());
            } catch (Exception e) {
    ...
    private static List<AppPasswordApplication> getLocalizedApplications(Locale locale, Collection<AppPasswordApplication> applications) {
        ...
        for (AppPasswordApplication application : applications) {
            localizedApplications.add(AppPasswordApplication.builder()
                ...
                .setRequiredCapabilities(application.getRequiredCapabilities())
