...
public class MicrosoftGraphRESTClient extends AbstractRESTClient {
     ...
     */
    private static JSONValue tryParseAsJSON(String responseBody) {
        try {
            char c = responseBody.charAt(0);
            return switch (c) {
                case '{' -> JSONServices.parseObject(responseBody);
                case '[' -> JSONServices.parseArray(responseBody);
                default -> null;
            };
    ...
    private void assertStatusCode(RESTResponse restResponse) throws OXException {
        ...
        switch (statusCode) {
            case 400 -> throw MicrosoftGraphAPIExceptionCodes.INVALID_REQUEST.create(restResponse.getStatusLine());
            case 401 -> throw MicrosoftGraphAPIExceptionCodes.UNAUTHENTICATED.create(restResponse.getStatusLine());
            case 403 -> throw MicrosoftGraphAPIExceptionCodes.ACCESS_DENIED.create(restResponse.getStatusLine());
            case 404 -> throw MicrosoftGraphAPIExceptionCodes.ITEM_NOT_FOUND.create(restResponse.getStatusLine());
            case 409 -> throw MicrosoftGraphAPIExceptionCodes.NAME_ALREADY_EXISTS.create(restResponse.getStatusLine());
            case 416 -> throw MicrosoftGraphAPIExceptionCodes.INVALID_RANGE.create(restResponse.getStatusLine());
        }
        if (statusCode <= 499) {
        ...
        switch (statusCode) {
            case 507 -> throw MicrosoftGraphAPIExceptionCodes.QUOTA_LIMIT_REACHED.create(restResponse.getStatusLine());
            case 509 -> throw MicrosoftGraphAPIExceptionCodes.ACTIVITY_LIMIT_REACHED.create(restResponse.getStatusLine());
        }
        if (statusCode <= 599) {
