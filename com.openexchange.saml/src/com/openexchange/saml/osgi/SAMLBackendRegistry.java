...
import com.openexchange.saml.request.analyzer.SAMLRequestAnalyzer;
...
public final class SAMLBackendRegistry extends ServiceTracker<SAMLBackend, SAMLBackend> {
    ...
    public SAMLBackend addingService(ServiceReference<SAMLBackend> reference) {
        ...
        if (backends.addIfAbsent(samlBackend)) {
            try {
                ...
                registerRequestAnalyzer(samlBackend, serviceRegistrations);
                ...
                return samlBackend;
            } catch (NamespaceException | ServletException | IllegalArgumentException | OXException e) {
    ...
    private void registerRequestAnalyzer(SAMLBackend backend, Stack<ServiceRegistration<?>> serviceRegistrations) throws OXException {
        String loginActionName = SAMLLoginTools.ACTION_SAML_LOGIN + getPathString(backend.getPath());
        serviceRegistrations.push(context.registerService(RequestAnalyzer.class, new SAMLRequestAnalyzer(services, loginActionName), null));
    }
