...
import com.openexchange.chronos.common.DefaultSequenceResult;
...
public class SyncHandler extends AbstractExtensionHandler {
    ...
    public UpdatesResult getUpdatedEvents(long updatedSince) throws OXException {
        return new OSGiCalendarStorageOperation<UpdatesResult>(Services.getServiceLookup(), getSession().getContextId(), getAccount().getAccountId()) {
            ...
            protected UpdatesResult call(CalendarStorage storage) throws OXException {
                ...
                long count = Arrays.contains(ignore, "count") ? -1L : eventStorage.countEvents();
                return new DefaultUpdatesResult(newAndUpdated, tombstoneEvents, count);
            }
        }.executeQuery();
    }
    ...
    public SequenceResult getSequenceNumber() throws OXException {
        return new OSGiCalendarStorageOperation<SequenceResult>(Services.getServiceLookup(), getSession().getContextId(), getAccount().getAccountId()) {

            @Override
            protected SequenceResult call(CalendarStorage storage) throws OXException {
                long timestamp = 0L;
                ...
                String[] ignore = getParameters().get(CalendarParameters.PARAMETER_IGNORE, String[].class);
                long count = Arrays.contains(ignore, "count") ? -1L : storage.getEventStorage().countEvents();
                return new DefaultSequenceResult(timestamp, count);
            }
        }.executeQuery();
    }
