    ...
    protected void doCopy(final URI srcBaseUri, final URI dstBaseUri) throws StorageException, IOException, InterruptedException, ProgrammErrorException {
        ...
        if (useRsync) {
            ...
            if (fsDirectory.exists()) {
                ArrayOutput output = new ShellExecutor().executeprocargs(new String[] { "rsync", "-a", fsDirectory.getAbsolutePath(), ensureEndingSlash(dstBaseUri.getPath()) });
                ...
                reverter = new Reverter() {
                    ...
                    public void revertCopy() throws StorageException {
                        ...
                        try {
                            ArrayOutput output = new ShellExecutor().executeprocargs(new String[] { "rsync", "-a", dfsDirectory.getAbsolutePath(), ensureEndingSlash(srcBaseUri.getPath()) });
                            if (0 != output.exitstatus) {
                                throw new ProgrammErrorException("Wrong exit status. Exit status was: " + output.exitstatus + " Stderr was: \n" + output.errOutput.toString() + '\n' + "and stdout was: \n" + output.stdOutput.toString());
                            }
                            try {
            ...
            try {
                ...
                if (false == srcFiles.isEmpty()) {
                    // Since file list was retrieved from storage, provide an appropriate file-not-found handler
                    FileNotFoundHandler fileNotFoundHandler = (file, sourceStorage) -> {
                        // Delete file from storage to let execution of `checkconsistency` repair that
                        sourceStorage.deleteFile(file);
                        throw FileStorageCodes.FILE_NOT_FOUND.create(file);
                    };
                    final CopyResult copyResult = copyFiles(srcFiles, srcStorage, dstStorage, srcFullUri, dstFullUri, Optional.of(fileNotFoundHandler));
