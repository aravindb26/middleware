...
/**
 * {@link OXCommonImpl} - General abstraction class used by all implementation classes.
 *
 * @author d7
...
public abstract class OXCommonImpl {
    ...
    private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(OXCommonImpl.class);
    ...
    protected static final void contextcheck(final Context ctx) throws InvalidCredentialsException {
    ...
    /**
     * Logs a message with given log properties for Mapped Diagnostic Context (MDC).
     *
    ...
    public static void log(LogLevel level, org.slf4j.Logger logger, Credentials creds, String contextId, String objectId, Exception e, String message, Object... args) {
        doLog(level, logger, creds, contextId, objectId, e, message, args);
    }

    /**
     * Logs a message with given log properties in MDC
     *
     * @param level The log level
     * @param logger The logger
     * @param creds The credentials to put in MDC
    ...
    public static void log(LogLevel level, org.slf4j.Logger logger, Credentials creds, String contextId, String objectId, Exception e, String message) {
        doLog(level, logger, creds, contextId, objectId, e, message);
    }
    ...
    public static void log(LogLevel level, org.slf4j.Logger logger, Credentials creds, String contextId, Exception e, String message, Object... args) {
        doLog(level, logger, creds, contextId, null, e, message, args);
    }
    ...
    public static void log(LogLevel level, org.slf4j.Logger logger, Credentials creds, String contextId, Exception e, String message) {
        doLog(level, logger, creds, contextId, null, e, message);
    }
    ...
    public static void log(LogLevel level, org.slf4j.Logger logger, Credentials creds, Exception e, String message, Object... args) {
        doLog(level, logger, creds, null, null, e, message, args);
    }
    ...
    public static void log(LogLevel level, org.slf4j.Logger logger, Credentials creds, Exception e, String message) {
        doLog(level, logger, creds, null, null, e, message);
    }
     ...
     * @param args The arguments to log
     */
    private static void doLog(LogLevel level, org.slf4j.Logger logger, Credentials creds, String contextId, String objectId, Exception e, String message, Object... args) {
        if (null == level || null == logger) {
            LOGGER.error("LogLevel and logger instance must not be null!");
            return;
        }

        Object[] logArgs = appendExceptionToLogArguments(e, args);
        LogProperties.putProvisioningLogProperties(getAdminName(creds), null, contextId, objectId);
        try {
            switch (level) {
                case TRACE:
                    logger.trace(message, logArgs);
                    break;
                case DEBUG:
    ...
    private static Object[] appendExceptionToLogArguments(Exception e, Object... args) {
        if (null == e) {
            // No exception given
            return args;
        }
        ...
        if (null == args || 0 == args.length) {
            // No log arguments given. Just return exception as argument.
            return new Object[] { e };
        }

        // Append exception instance to tail of the array
        Object[] logArgs = new Object[args.length + 1];
        System.arraycopy(args, 0, logArgs, 0, args.length);
        logArgs[args.length] = e;
        return logArgs;
    }
