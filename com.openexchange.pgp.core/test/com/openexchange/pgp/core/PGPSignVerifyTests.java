...
public class PGPSignVerifyTests extends AbstractPGPTest {
    ...
    public void testSignVerify() throws Exception {
        ...
        List<SignatureVerificationResult> verifyResults = new PGPSignatureVerifier(keyRetrievalStrategy).verifySignatures(plainTextData, signatureStream);
        Assert.assertTrue("Verification should create at least one result", verifyResults.size() > 0);
        for (SignatureVerificationResult result : verifyResults) {
            Assert.assertTrue("The created signature should have been verified", result.isVerified());
            Assert.assertTrue("The signature should have been created with the user's key id", ((PGPSignatureVerificationResult) result).getSignature().getKeyID() == secretKey.getKeyID());
            System.out.println(((PGPSignatureVerificationResult) result).getSignature().getKeyID());
        }
    }
    ...
    public void testSignVerifyWithouthKnownPublicKeyShouldReturnFalseSignatureVerificationResult() throws Exception{
        ...
        plainTextData = new ByteArrayInputStream("test".getBytes());
        List<SignatureVerificationResult> verifyResults = new PGPSignatureVerifier(onlyFindSecretKeyStrategy).verifySignatures(plainTextData, signatureStream);
