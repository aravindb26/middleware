...
public class PacketProcessor {
    ...
    private PGPPacket parseNextPackage(BCPGInputStream in, RememberingInputStream rememberInpuStream) throws IOException {
        try {
            //Remember the first data (including the header) parsed by BC so that we can write them back to the output later on
            rememberInpuStream.resetBuffer();
            rememberInpuStream.startRemembering();
            Packet rawPacket = in.readPacket();
            if (rawPacket != null) {
                // The first data might contain more than just the packet header because
                // BC might use an internal buffer which gets filled
                final byte[] firstPacketData = rememberInpuStream.getBuffer();
                return new PGPPacket(rawPacket, firstPacketData);
            }
            return null;
        } finally {
                ...
                for (PGPPacket newPacket : newPackets) {
                    ...
                    if (rawModifiedPacket != null) {
                        if (rawModifiedPacket instanceof ContainedPacket) {
                            ...
                            //InputStreamPackets cannot be re-encoded; so we need to stream the content to the final output stream
                            @SuppressWarnings("resource")
                            BCPGInputStream inputStream = ((InputStreamPacket) rawModifiedPacket).getInputStream();
                            try {

                                /*
                                 * 1: We need to write the already parsed data to the output stream before writing the rest of the stream content
                                 */
                                byte[] firstData = newPacket.getBcFirstData();
                                if(firstData != null && firstData.length > 0) {
                                    byte[] modified = handler.handlePacketData(packet, firstData);
                                ...
                                /*
                                 * 2: Writing the rest of the InputStreamPacket's data which was not yet consumed.
                                 *
                                 *    We are writing the data to a "garbage-sink" and using the RememberingInputStream instead because:
                                 *
                                 *    - The inputStream might contain some unread/unconsumed data which was included in the "first data"
                                 *      and was already written to the output stream above. These data will be skipped here.
                                 *
