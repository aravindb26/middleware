...
public class ConfigMailFilterAction extends AbstractMailFilterAction {
    ...
    private JSONObject getOptions(int userId, int contextId, Map<String, Object> options) throws JSONException {
        ...
        result.put(OptionsProperty.allowNestedTests.jsonName(), property);
        String blockedActionsString = leanConfigurationService.getProperty(userId, contextId, OptionsProperty.blockedApplyActions);
        String[] blockedActions = Strings.splitByComma(blockedActionsString);
        if (blockedActions.length > 0) {
            JSONArray blockedActionsArray = new JSONArray(Arrays.asList(blockedActions));
            result.put(OptionsProperty.blockedApplyActions.jsonName(), blockedActionsArray);
        }
        if (options != null) {
