...
import com.openexchange.jsieve.commands.ControlOrActionCommand;
...
public class ApplyMailFilterAction extends AbstractMailFilterAction {
    ...
    public AJAXRequestResult perform(AJAXRequestData request, ServerSession session) throws OXException {
        ...
        final MailFilterService mailFilterService = services.getServiceSafe(MailFilterService.class);
        try {
            Rule rule = null;
     ...
     * @throws OXException in case the rule is not applicable
     */
    private void check(Session session, Rule rule) throws OXException {
        List<String> blockList = loadBlockList(session);
        if (blockList.isEmpty()) {
            return;
        }
        ArrayList<Command> commands = rule.getCommands();
        for (Command command : commands) {
            if (command instanceof ControlOrActionCommand com) {
                check(blockList, com);
            }
        }
    }
    ...
    /**
     *
     * Checks if the {@link ControlOrActionCommand} is applicable
     *
     * @param blockList The block list
     * @param command The {@link ControlOrActionCommand} to check
     * @throws OXException in case it is not applicable
     */
    private void check(List<String> blockList, ControlOrActionCommand command) throws OXException {
        if (command instanceof IfStructureCommand ifCommand) {
            for (ActionCommand actionCommand : ifCommand.getActionCommands()) {
                check(blockList, actionCommand);
            }
            return;
        }
    ...
    /**
     * Checks if the {@link ActionCommand} is applicable
     *
     * @param blockList The block list
     * @param command The {@link ActionCommand} to check
     * @throws OXException in case it is not applicable
     */
    private void check(List<String> blockList, ActionCommand command) throws OXException {
        if (blockList.contains(command.getCommand().getCommandName().toLowerCase())) {
            throw MailFilterExceptionCode.ILLEGAL_APPLY_ACTION.create(command.getCommand().getCommandName());
        }
    }

    /**
     * Loads the blocklist for the given user
     *
     * @param session The user session
     * @return The blocklist
     * @throws OXException
     */
    private List<String> loadBlockList(Session session) throws OXException {
        String property = services.getServiceSafe(LeanConfigurationService.class)
                                  .getProperty(session.getUserId(), session.getContextId(), OptionsProperty.blockedApplyActions);
        return Arrays.asList(Strings.splitByComma(property.toLowerCase()));
    }
