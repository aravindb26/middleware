...
public class OAuthIntrospectionAuthorizationService extends AbstractClaimSetAuthorizationService implements Reloadable {
    ...
    private static final long FALLBACK_EXPIRATION = 30;
        ...
        this.cache = Caffeine.newBuilder().expireAfter(new Expiry<String, TokenIntrospectionResponse>() {
            ...
            public long expireAfterCreate(String key, TokenIntrospectionResponse resp, long currentTime) {
                ...
                if (resp.indicatesSuccess() && ((TokenIntrospectionSuccessResponse) resp).isActive()) {
                    long expiration = getExpiration((TokenIntrospectionSuccessResponse) resp);
                ...
                return TimeUnit.SECONDS.toNanos(FALLBACK_EXPIRATION);
            }
    ...
    private URI getIntrospectionEndpoint() throws AuthorizationException {
        ...
        LOG.debug("The mandatory property {} is empty", OAuthIntrospectionProperty.ENDPOINT.getFQPropertyName());
    ...
    /**
     * Returns the expiration time from the specified token response.
     * If there is no expiration time set, a fall-back expiration time is set to
     * that of {@link #FALLBACK_EXPIRATION} in the future.
     *
     * @param response The response
     * @return The expiration time
     */
    long getExpiration(TokenIntrospectionSuccessResponse response) {
        Date expirationTime = response.getExpirationTime();
        return expirationTime == null ? System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(FALLBACK_EXPIRATION) : expirationTime.getTime();
    }

    @Override
    public Interests getInterests() {
        return Reloadables.interestsForProperties(OAuthIntrospectionProperty.ENDPOINT.getFQPropertyName(), OAuthIntrospectionProperty.BASIC_AUTH_ENABLED.getFQPropertyName(), OAuthIntrospectionProperty.CLIENT_ID.getFQPropertyName(), OAuthIntrospectionProperty.CLIENT_SECRET.getFQPropertyName());
    }
