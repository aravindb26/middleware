 ...
 */
public final class MailAccountPOP3FolderStorage implements IMailFolderStorage, IMailFolderStorageEnhanced2, IMailFolderStorageInfoSupport, IMailFolderStorageTrashAware {
    ...
    public <T> T supports(Class<T> iface) throws OXException {
        T instance = delegatee.supports(iface);
        if (instance == null) {
            // Not supported by backing folder storage
            return null;
        }
        if (IMailFolderStorageEnhanced2.class.equals(iface)) {
            return (T) this;
        }
        if (IMailFolderStorageEnhanced.class.equals(iface)) {
            return (T) this;
        }
        if (IMailFolderStorageInfoSupport.class.equals(iface)) {
            return (T) this;
        }
        if (IMailFolderStorageTrashAware.class.equals(iface)) {
            return (T) this;
        }
        return instance;
    }
    ...
    public int[] getTotalAndUnreadCounter(String fullName) throws OXException {
        IMailFolderStorageEnhanced2 folderStorage = delegatee.supports(IMailFolderStorageEnhanced2.class);
        ...
        return folderStorage.getTotalAndUnreadCounter(getRealFullname(fullName));
    }

    @Override
    public void expungeFolder(String fullName) throws OXException {
        IMailFolderStorageEnhanced folderStorage = delegatee.supports(IMailFolderStorageEnhanced.class);
        if (folderStorage == null) {
            throw MailExceptionCode.UNSUPPORTED_OPERATION.create();
        }
        ...
        folderStorage.expungeFolder(getRealFullname(fullName));
    }
    ...
    public void expungeFolder(String fullName, boolean hardDelete) throws OXException {
        ...
        folderStorage.expungeFolder(getRealFullname(fullName), hardDelete);
    }
    ...
    public int getUnreadCounter(String fullName) throws OXException {
        ...
        return folderStorage.getUnreadCounter(getRealFullname(fullName));
    }
    ...
    public int getNewCounter(String fullName) throws OXException {
        ...
        return folderStorage.getNewCounter(getRealFullname(fullName));
    }
    ...
    public int getTotalCounter(String fullName) throws OXException {
        ...
        return folderStorage.getTotalCounter(getRealFullname(fullName));
    }
    ...
    public MailFolderInfo getFolderInfo(final String fullName) throws OXException {
        ...
        if (null != infoSupport && infoSupport.isInfoSupported()) {
            String realFullname = getRealFullname(fullName);
            MailFolderInfo folderInfo = infoSupport.getFolderInfo(realFullname);
            prepareMailFolderInfo(folderInfo);
            return folderInfo;
        }
        throw MailExceptionCode.UNSUPPORTED_OPERATION.create();
    }
    ...
    public List<MailFolderInfo> getFolderInfos(final String optParentFullName, final boolean subscribedOnly) throws OXException {
        IMailFolderStorageInfoSupport infoSupport = delegatee.supports(IMailFolderStorageInfoSupport.class);
        if (null != infoSupport && infoSupport.isInfoSupported()) {
            String realParentFullname = optParentFullName == null ? null : getRealFullname(optParentFullName);
            List<MailFolderInfo> folderInfos = infoSupport.getFolderInfos(realParentFullname, subscribedOnly);
            prepareMailFolderInfos(folderInfos);
            return folderInfos;
        }
        throw MailExceptionCode.UNSUPPORTED_OPERATION.create();
    }

    private void prepareMailFolderInfos(final List<MailFolderInfo> mailFolderInfos) throws OXException {
        for (MailFolderInfo mailFolderInfo : mailFolderInfos) {
            prepareMailFolderInfo(mailFolderInfo);
        }
    }

    private void prepareMailFolderInfo(final MailFolderInfo mailFolderInfo) throws OXException {
        if (mailFolderInfo == null) {
            return;
        }
        mailFolderInfo.setFullname(stripPathFromFullname(path, mailFolderInfo.getFullname()));
        mailFolderInfo.setParentFullname(stripPathFromFullname(path, mailFolderInfo.getParentFullname()));
        mailFolderInfo.setRootFolder(false);
        mailFolderInfo.setDefaultFolder(false);
        mailFolderInfo.setDefaultFolderType(DefaultFolderType.NONE);
        if (MailFolder.ROOT_FOLDER_ID.equals(mailFolderInfo.getFullname())) {
            mailFolderInfo.setRootFolder(true);
        } else {
            setDefaultFolderInfo(mailFolderInfo);
        }
        mailFolderInfo.setShared(false);
        // POP3 does not support subscription
        mailFolderInfo.setSubscribed(true);
        mailFolderInfo.setSubscribedSubfolders(mailFolderInfo.hasSubfolders());
    }

    private void setDefaultFolderInfo(final MailFolderInfo mailFolder) throws OXException {
        final String fullname = mailFolder.getFullname();
        for (int i = 0; i < 7; i++) {
            final String stdFullname = getStandardFolder(i);
            if (null != stdFullname && stdFullname.equals(fullname)) {
        ...
        if (!isDefaultFoldersChecked(mailSessionCache)) {
            final Callable<Void> task = new Callable<Void>() {
                ...
                public Void call() throws OXException {
                    ...
                    boolean isSpamOptionEnabled = usm.isSpamOptionEnabled();
                    /*
    ...
    public void clearFolder(final String fullname, final boolean hardDelete) throws OXException {
        ...
        if (hardDelete || performHardDelete(fullname)) {
            ...
            if (fullname.equals(getTrashFolder())) {
    ...
    public String moveFolder(final String fullname, final String newFullname) throws OXException {
        if (MailFolder.ROOT_FOLDER_ID.equals(newFullname) || isDefaultFolder(fullname) || isDefaultFolder(newFullname)) {
    ...
    private static String[] getParentAndName(final String fullname, final char separator) {
