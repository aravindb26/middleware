...
public final class RdbPOP3StorageProperties implements POP3StorageProperties {
    ...
    public void addProperty(String propertyName, String propertyValue) throws OXException {
        ...
        try {
            doAddProperty(propertyName, propertyValue, con);
        } catch (SQLException e) {
            throw POP3ExceptionCode.SQL_ERROR.create(e, e.getMessage());
        } finally {
    ...
    private void doAddProperty(String propertyName, String propertyValue, Connection con) throws SQLException {
        try {
    ...
    private void addProperty(String propertyName, String propertyValue, boolean disableForeignKeyChecks, Connection con) throws SQLException {
        ...
        try {
            ...
            stmt = con.prepareStatement("INSERT INTO user_mail_account_properties (`cid`, `user`, `id`, `name`, `value`) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE `name` = ?, `value` = ?");
            int pos = 1;
            stmt.setInt(pos++, cid);
            stmt.setInt(pos++, user);
            stmt.setInt(pos++, accountId);
    ...
    public void removeProperty(String propertyName) throws OXException {
        Connection con = Database.get(cid, true);
        try {
            doRemoveProperty(propertyName, con);
        } catch (SQLException e) {
            throw POP3ExceptionCode.SQL_ERROR.create(e, e.getMessage());
        } finally {
            Database.back(cid, true, con);
    ...
    private void doRemoveProperty(String propertyName, Connection con) throws SQLException {
        try {
