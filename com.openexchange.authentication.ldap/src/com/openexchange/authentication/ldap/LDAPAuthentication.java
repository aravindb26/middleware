 ...
 */
public class LDAPAuthentication extends AbstractConfigurationAwareAuthenticationService implements Reloadable {

    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(LDAPAuthentication.class);

    private static final String AUTH_IDENTIFIER = "ldap";
    ...
    public AuthenticationResult handleLoginRequest(AuthenticationRequest authenticationRequest) throws OXException {
        ContextAndUserInfo cau = split(authenticationRequest.getLogin());
        ...
        LOG.debug("Using full login info: {}", authenticationRequest.getLogin());
        String userInfoFromLdap = bind(config.useFullLoginInfo ? authenticationRequest.getLogin() : uid, password, props, config);
        LOG.info("User {} successfully authenticated.", config.useFullLoginInfo ? authenticationRequest.getLogin() : uid);
        return AuthenticationResult.success(new AuthenticatedImpl(null == userInfoFromLdap ? cau.getUserInfo() : userInfoFromLdap, cau.getContextInfo()));
    }

    @Override
    public AuthenticationResult handleAutoLoginRequest(AuthenticationRequest authenticationRequest) throws OXException {
        return AuthenticationResult.failed(LoginExceptionCodes.NOT_SUPPORTED.create(LDAPAuthentication.class.getName()));
    }

    private static final Pattern PATTERN_USER_NAME = Pattern.compile("[a-zA-Z_0-9-\\s]*");

    private static final Pattern PATTERN_PASSWORD = Pattern.compile("[\\w]*");
    ...
    private String bind(String uid, String password, Properties props, Config config) throws OXException {
        ...
        if (false && (!PATTERN_USER_NAME.matcher(uidToUse).matches() || !PATTERN_PASSWORD.matcher(password).matches())) {
            LOG.error("Missing user mapping for user \"{}\". Login failed.", uidToUse);
            throw LoginExceptionCodes.INVALID_CREDENTIALS.create();
        }
        ...
        try {
            ...
            if (config.bindOnly) {
                ...
                if (config.adsbind) {
                    ...
                    dn = uidAttribute + '=' + escapeDN(uidToUse) + ',' + baseDN;
                }
            } else {
                ...
                final String filter = "(&" + config.searchFilter + "(" + uidAttribute + "={0}))";
                LOG.debug("Using filter=(&{}({}={}))", config.searchFilter, uidAttribute, uidToUse);
                ...
                try {
                    res = context.search(baseDN, filter, new Object[] { uidToUse }, cons);
                    if (!res.hasMoreElements()) {
                        LOG.error("No user found with {}={}", uidAttribute, uidToUse);
                        throw LoginExceptionCodes.INVALID_CREDENTIALS.create();
                    }

                    dn = res.nextElement().getNameInNamespace();
                    if (res.hasMoreElements()) {
                        LOG.error("Found more than one user with {}={}", uidAttribute, uidToUse);
                        throw LoginExceptionCodes.INVALID_CREDENTIALS.create();
                    }
                } finally {
                    close(res);
            ...
            if (config.adsbind) {
                ...
                try {
                    if (null == samAccountName) {
                        if (proxyAs != null) {
                            search = context.search(baseDN, "(displayName={0})", new Object[] { uidToUse }, searchControls);
                            searchProxy = context.search(baseDN, "(displayName={0})", new Object[] { proxyAs }, searchControls);
                        } else {
                            search = context.search(baseDN, "(displayName={0})", new Object[] { uidToUse }, searchControls);
                        }
                    } else {
                        search = context.search(baseDN, "(sAMAccountName={0})", new Object[] { samAccountName }, searchControls);
                    }
                    if (null == search || !search.hasMoreElements()) {
                        LOG.error("No user with displayname {} found.", uidToUse);
                        throw LoginExceptionCodes.INVALID_CREDENTIALS.create(uidToUse);
                    }
     ...
     *
     * @param loginInfo The combined information separated by an <code>'@'</code> character.
    ...
    public void reloadConfiguration(ConfigurationService configService) {
        try {
            @SuppressWarnings("deprecation") Properties properties = configService.getFile("ldapauth.properties");
    ...
    /**
     * Escapes data for use in an LDAP distinguished name.
     *
     * @param name The data to escape
    ...
    private static String escapeDN(String name) {
        ...
        if ((length > 0) && ((name.charAt(0) == ' ') || (name.charAt(0) == '#'))) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            switch (curChar) {
                case '\\':
                    if (sb == null) {
                        sb = initStringBuilder(i, name, length);
                    }
                    sb.append("\\\\");
                    break;
                case ',':
        ...
        if ((length > 1) && (name.charAt(length - 1) == ' ')) {
            if (sb == null) {
                sb = initStringBuilder(length - 1, name, length);
            }
    ...
    private static StringBuilder initStringBuilder(int index, String source, int length) {
        StringBuilder s = new StringBuilder(length);
        if (index > 0) {
            s.append(source, 0, index);
        }
        return s;
    }
