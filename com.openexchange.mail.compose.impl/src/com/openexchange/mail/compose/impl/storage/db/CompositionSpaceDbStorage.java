...
public class CompositionSpaceDbStorage {
    ...
    private static boolean retryInsert(DBUtils.TransactionRollbackCondition condition) throws SQLException {
    ...
    public List<UUID> deleteExpired(long maxIdleTimeMillis) throws OXException {
        ...
        try {
            ...
            modified = !deleteExpired.isEmpty();
        ...
        try (PreparedStatement stmt = connection.prepareStatement("SELECT attachments FROM compositionSpace WHERE cid=? AND user=? AND uuid=?")) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                if (rs.wasNull()) {
                    ...
                    jAttachments = JSONServices.parseArray(oldAttachments);
                }
    ...
    private static OXException handleDataTruncation(DataTruncation truncation) {
