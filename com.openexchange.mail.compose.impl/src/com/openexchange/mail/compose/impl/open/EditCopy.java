...
public class EditCopy extends AbstractOpener {
    ...
    public void doOpenForEditCopy(boolean isEditFor, OpenCompositionSpaceParameters parameters, OpenState state, Session session) throws OXException {
        ...
        Optional<Message.ContentType> optionalContentType = Optional.empty();
        {
            ...
            headerValue = HeaderUtility.decodeHeaderValue(originalMail.getFirstHeader(HeaderUtility.HEADER_X_OX_REPLY_TO));
            if (Strings.isNotEmpty(headerValue)) {
                List<Address> addresses = HeaderUtility.headerValue2Addresses(headerValue);
                if (addresses != null) {
                    state.message.setReplyTo(addresses);
                }
            }
        }
        ...
        // Set "Sender"
        {
            InternetAddress[] sender = MimeMessageUtility.getAddressHeader(MessageHeaders.HDR_SENDER, originalMail) ;
            if (null != sender && sender.length > 0) {
                state.message.setSender(toAddress(sender[0], false));
            }
        }
        ...
        // Set "Reply-To"
        {
            ...
            if (null != replyTo && replyTo.length > 0) {
                state.message.addReplyTo(toAddresses(replyTo));
            }
        }
        ...
        if (multipart) {
            ...
            if (state.message.getContentType().isImpliesHtml() && null != contentIds && !contentIds.isEmpty()) {
                ...
                if (null != inlineParts && !inlineParts.isEmpty()) {
                    ...
                    String content = CompositionSpaces.replaceCidInlineImages(state.message.getContent(), inlineAttachments, AttachmentImageDataSource.getInstance(), session);
            ...
            } else if (state.message.getContentType().isImpliesHtml()) {
                if (parameters.isDropExternalImages()) {
