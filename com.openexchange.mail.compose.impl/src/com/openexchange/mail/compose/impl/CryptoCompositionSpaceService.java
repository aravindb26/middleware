...
public class CryptoCompositionSpaceService extends AbstractCryptoAware implements CompositionSpaceService {
    ...
    public MailPath transportCompositionSpace(UUID compositionSpaceId, Optional<StreamedUploadFileIterator> optionalUploadedAttachments, UserSettingMail mailSettings, AJAXRequestData requestData, List<OXException> warnings, DeleteAfterTransportOptions deleteAfterTransportOptions, ClientToken clientToken) throws OXException {
        try {
            return delegate.transportCompositionSpace(compositionSpaceId, optionalUploadedAttachments, mailSettings, requestData, warnings, deleteAfterTransportOptions, clientToken);
        } catch (OXException e) {
    ...
    public AttachmentResult deleteAttachments(UUID compositionSpaceId, List<UUID> attachmentIds, ClientToken clientToken) throws OXException {
        try {
            return delegate.deleteAttachments(compositionSpaceId, attachmentIds, clientToken);
        } catch (OXException e) {
            if (CompositionSpaceErrorCode.MISSING_KEY.equals(e)) {
                autoDeleteSafe(compositionSpaceId, session, e);
            }
