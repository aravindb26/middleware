...
public final class MailAccountSMTPProperties extends MailAccountTransportProperties implements ISMTPProperties {
    ...
    public boolean isAllowXClient() {
        String allowXClientStr = getAccountProperty("com.openexchange.smtp.allowXCLIENT");
        if (null != allowXClientStr) {
            return Boolean.parseBoolean(allowXClientStr.trim());
        }

        if (mailAccountId == PRIMARY) {
            allowXClientStr = lookUpProperty("com.openexchange.smtp.primary.allowXCLIENT");
            if (null != allowXClientStr) {
                return Boolean.parseBoolean(allowXClientStr.trim());
            }
        }

        return lookUpBoolProperty("com.openexchange.smtp.allowXCLIENT", SMTPProperties.getInstance().isAllowXClient());
    }
    ...
    public boolean isRequireTls() {
        String tmp = getAccountProperty("com.openexchange.smtp.requireTls");
        if (Strings.isNotEmpty(tmp)) {
            return Boolean.parseBoolean(tmp);
        }
        if (mailAccountId == PRIMARY) {
            tmp = lookUpProperty("com.openexchange.smtp.primary.requireTls");
            if (Strings.isNotEmpty(tmp)) {
                return Boolean.parseBoolean(tmp);
            }
        }
        return lookUpBoolProperty("com.openexchange.smtp.requireTls", SMTPProperties.getInstance().isRequireTls());
    }
