...
abstract class AbstractSMTPTransport extends MailTransport implements MimeSupport {
    ...
    protected AbstractSMTPTransport(int contextId) throws OXException {
        ...
        this.accountId = Account.DEFAULT_ID;
        pendingInvocations = new ConcurrentLinkedQueue<Runnable>();
    }
    ...
    protected javax.mail.Session getSMTPSession(SMTPConfig smtpConfig, boolean forceSecure) throws OXException {
        if (null == smtpSession) {
            synchronized (this) {
                if (null == smtpSession) {
                    if(smtpConfig.getAccount() != null && Constants.NAME_GUARD_GUEST.equals(smtpConfig.getAccount().getName())) {
                        /* do not use any SMTP settings for guard guest because SMTP is not used anyway */
                        final Properties smtpProps = SMTPSessionProperties.getDefaultSessionProperties();
                        smtpProps.put("mail.smtp.starttls.required", "false");
                     ...
                     * Allow XCLIENT?
                     */
                    if (smtpProperties.isAllowXClient()) {
                        smtpProps.put("mail.smtp.allowxclient", "true");
                    }
                    /*
                     * Blocked hosts
                     */
                    if (smtpConfig.getAccountId() != Account.DEFAULT_ID && !MailAccounts.isSecondaryAccount(smtpConfig.getAccountId(), smtpConfig.getSession().getUserId(), smtpConfig.getSession().getContextId())) {
                        HostList blockedHosts = MailAccountUtils.getBlockedHosts();
                        if (!blockedHosts.isEmpty()) {
                            smtpProps.put("mail.smtp.blockedHosts", blockedHosts);
                        }
                    }
                    /*
                    ...
                    if (smtpConfig.isSecure()) {
                        ...
                        boolean requireTls = smtpProperties.isRequireTls();
                        if (requireTls) {
                        ...
                        try {
                            ...
                            } else if (forceSecure || requireTls) {
        ...
        if (false == forPing) {
            ...
            if (null != account) {
                ...
                if (account.isDeactivated()) {
                    throw MailExceptionCode.MAIL_TRANSPORT_DEACTIVATED.create(smtpConfig.getServer(), smtpConfig.getLogin(), I(session.getUserId()), I(session.getContextId()));
                }
        ...
        try {
            if (smtpConfig.getSMTPProperties().isSmtpAuth()) {
                ...
                if (isKerberosAuth()) {
                    ...
                    doConnectTransport(transport, server, port, null == login ? "" : login, null == password ? "" : password, smtpConfig, session, new HashMap<String, Object>(4));
                }
            } else {
                doConnectTransport(transport, server, port, null, null, smtpConfig, session, new HashMap<String, Object>(4));
            }
            ...
            if (session != null) {
                ...
                if (null != auditLogService) {
                    String eventId = forPing ? "smtp.external.login" : (Account.DEFAULT_ID == accountId ? "smtp.primary.login" : "smtp.external.login");
     ...
     * @param state A map for storing/managing state
     * @throws OXException If connect attempt fails
     * @throws MessagingException If connect attempt fails
     */
    protected static void doConnectTransport(Transport transport, String host, int port, String user, String password, final SMTPConfig smtpConfig, final Session session, Map<String, Object> state) throws OXException, MessagingException {
        boolean isPrimaryOrSecondaryAccount = smtpConfig != null && session != null && (smtpConfig.getAccountId() == Account.DEFAULT_ID || isSecondaryAccount(smtpConfig.getAccountId(), session));
        if (isPrimaryOrSecondaryAccount) {
            SMTPClientParameters.setDefaultClientParameters(transport, session);
        }
        try {
            ...
            if (false == isPrimaryOrSecondaryAccount) {
            ...
            if (isPrimaryOrSecondaryAccount) {
                ...
                if (handlerService != null) {
                    OXException oxe = MimeMailExceptionCode.TRANSPORT_INVALID_CREDENTIALS.create(e, smtpConfig.getServer(), e.getMessage());
                    AuthenticationFailureHandlerResult result = handlerService.handleAuthenticationFailed(oxe, Service.TRANSPORT, smtpConfig, session, state);
                    Type type = result.getType();
                    switch (type) {
                        case RETRY:
                            doConnectTransport(transport, host, port, user, password, smtpConfig, session, state);
                            return;
                        case EXCEPTION:
        ...
        private void logAddressesFromEvent(TransportEvent e) {
            ...
            if (null != arr) {
                for (Address address : arr) {
                    String eventId = Account.DEFAULT_ID == accountId ? "smtp.primary.invalid" : "smtp.external.invalid";
            ...
            if (null != arr) {
                for (Address address : arr) {
                    String eventId = Account.DEFAULT_ID == accountId ? "smtp.primary.unsent" : "smtp.external.unsent";
            ...
            if (null != arr) {
                for (Address address : arr) {
                    String eventId = Account.DEFAULT_ID == accountId ? "smtp.primary.sent" : "smtp.external.sent";
    ...
    private static final class SaslSmtpLoginAction implements PrivilegedExceptionAction<Object> {
        ...
        public Object run() throws Exception {
            doConnectTransport(transport, server, port, login, pw, null, null, new HashMap<String, Object>(4));
            return null;
        }
