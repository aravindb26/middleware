...
public class S3ClientConfig {
    ...
    /**
     * Gets the expected full-qualified property based on current client scope.
     *
     * @param property The {@link S3ClientConfig}
     * @return The full-qualified property based on current client scope.
     */
    public Property getExpectedFQProperty(S3ClientProperty property) {
        if (getClientScope().isShared()) {
            return DefaultProperty.valueOf(property.getFQPropertyName(getClientQualifier(getClientID())), property.getDefaultValue());
        }

        Optional<S3Property> filestoreProperty = S3Property.of(property);
        if (filestoreProperty.isPresent()) {
            return DefaultProperty.valueOf(filestoreProperty.get().getFQPropertyName(getFilestoreQualifier(filestoreID)), filestoreProperty.get().getDefaultValue());
        }
        return property;
    }
     ...
     */
    public static Map<String, String> getClientQualifier(Optional<String> clientID) {
     ...
     * @throws OXException If bucket/client cannot be returned
     */
    private static BucketAndClientInfo getBucketAndClientInfo(String filestoreID, LeanConfigurationService configService) throws OXException {
        ...
        if (bucketName != null || BucketNameUtils.isValidV2BucketName(filestoreID)) {
            ...
            Map<String, String> s3ClientProperties = configService.getProperties((k, v) -> k.startsWith(S3ClientProperty.PREFIX));
            // @formatter:off
            Map<String, String> clientIDsToBuckets = s3ClientProperties.entrySet()
                                                                       .stream()
                                                                       .filter(e -> e.getKey().endsWith('.' + S3ClientProperty.BUCKETS.getShortName()) && Strings.isNotEmpty(e.getValue()))
                                                                       .collect(Collectors.toMap(e -> getQualifierFromProperty(S3ClientProperty.BUCKETS, e.getKey()),
                                                                                                 Entry::getValue));
            Optional<Entry<String, String>> optEntry = clientIDsToBuckets.entrySet()
                                                                         .parallelStream()
                                                                         .filter(e -> PATTERN_SPLIT_BY_COMMA.splitAsStream(e.getValue())
        ...
        return new BucketAndClientInfo(clientID, bucketName);
    }

    /**
     * Checks if the given bucket name matches the bucket name pattern
     *
     ...
     */
    public static final record ConfigProperty(String key, String value) {
     ...
     */
    private static final record BucketAndClientInfo(Optional<String> clientID, String bucketName) {
