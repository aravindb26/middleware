...
public class ResumableAbortIfNotConsumedInputStream extends AbstractResumableAbortIfNotConsumedInputStream {
    ...
    protected void initNewObjectStreamAfterPrematureEof() throws IOException {
        ...
        try {
            ...
            objectContent = new RetryingS3OperationExecutor<S3ObjectInputStream>(c -> c.getObject(request).getObjectContent(), 3).execute(s3Client);
    ...
    private S3Object getObject() throws IOException {
        try {
            return new RetryingS3OperationExecutor<S3Object>(c -> c.getObject(bucketName, key), 3).execute(s3Client);
