...
public class DriveWindowsClientOnboardingProvider implements OnboardingProvider {
    ...
    private String getDownloadLink(OnboardingRequest request, Session session) throws OXException {
        DriveClientWindowsUpdateService driveUpdateService = services.getOptionalService(DriveClientWindowsUpdateService.class);
        if (null == driveUpdateService) {
            throw ServiceExceptionCode.absentService(DriveClientWindowsUpdateService.class);
        }

        return driveUpdateService.getInstallerDownloadUrl(session);
    }
