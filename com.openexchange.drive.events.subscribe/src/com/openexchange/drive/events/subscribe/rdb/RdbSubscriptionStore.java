...
public class RdbSubscriptionStore implements DriveSubscriptionStore {
    ...
    @Override
    public List<Subscription> getSubscriptions(int contextID, String[] serviceIDs, int[] userIDs) throws OXException {
        Connection connection = databaseService.getReadOnly(contextID);
        try {
            return selectSubscriptions(connection, contextID, serviceIDs, userIDs);
        } catch (SQLException e) {
            throw DriveExceptionCodes.DB_ERROR.create(e, e.getMessage());
        } finally {
            databaseService.backReadOnly(contextID, connection);
    ...
    private static List<Subscription> selectSubscriptions(Connection connection, int cid, String[] services, int[] userIDs) throws SQLException, OXException {
        List<Subscription> subscriptions = new ArrayList<Subscription>();
        PreparedStatement stmt = null;
        try {
            stmt = connection.prepareStatement(SQL.SELECT_SUBSCRIPTIONS_FOR_USERS_STMT(services.length, userIDs.length));
            int parameterIndex = 0;
            stmt.setInt(++parameterIndex, cid);
            for (String service : services) {
                stmt.setString(++parameterIndex, service);
