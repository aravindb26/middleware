...
public abstract class AbstractExtendedScribeAwareOAuthServiceMetaData extends AbstractScribeAwareOAuthServiceMetaData {
    ...
    private String trimRedirectUri(String redirectUri, Session session) throws OXException {
        ...
        String sVerifyRedirectUrl = optFromConfigViewFactory(session, null, OAuthPropertyID.verifyRedirectUrl);
        if (Strings.isEmpty(sVerifyRedirectUrl) || !Boolean.parseBoolean(sVerifyRedirectUrl.trim())) {
            return actual;
        }

        // Verify that configured redirect URL matches determined call-back URL; e.g. same host, path, ...
     ...
     */
    private static String stripProtocol(String encodedUrl) {
        String toCheck = Strings.asciiLowerCase(encodedUrl);
        if (toCheck.startsWith("https")) {
            return encodedUrl.substring(5);
        } else if (toCheck.startsWith("http")) {
     ...
     */
    private static String extractProtocol(final String url) {
