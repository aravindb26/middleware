...
public class OAuthServiceImpl implements OAuthService {
    ...
    public OAuthInteraction initOAuth(Session session, String serviceMetaData, String callbackUrl, HostInfo currentHost, Set<OAuthScope> scopes) throws OXException {
        try {
            ...
            // Something like "https://my.appsuite.com/appsuite/api/oauth/accounts?action=callback&..."
            ...
            // Apply possible modifications to call-back URL
            {
                ...
                if (modifiedUrl != null) {
                    // Something like "https://my.appsuite.com/appsuite/api/defer?redirect=https%3A%2F%2Fmy.appsuite.com%2Fappsuite%2Fapi%2Foauth%2Faccounts%3Faction%3Dcallback%26..."
                    cbUrl = modifiedUrl;
                }
            }
    ...
    private void registerTokenForDeferredAccess(String token, String cbUrl, DeferringURLService ds, int userId, int contextId) {
        // Is only applicable if call-back URL is deferred; e.g. "https://my.appsuite.com/appsuite/api/defer?redirect=https%3A%2F%2Fmy.appsuite.com%2Fappsuite%2Fapi%2Foauth%2Faccounts%3Faction%3Dcallback%26..."
        if (isDeferrerAvailable(ds, userId, contextId)) {
            if (ds.seemsDeferred(cbUrl, userId, contextId)) {
                callbackRegistry.add(token, cbUrl);
            } else {
                LOG.warn("Call-back URL cannot be registered as it is not deferred: {}", Strings.abbreviate(cbUrl, 32)); // NOSONARLINT
            }
        } else {
            // No chance to check
            callbackRegistry.add(token, cbUrl);
     ...
     */
    private static boolean isDeferrerAvailable(DeferringURLService ds, int userId, int contextId) {
     ...
     */
    private static void postOAuthDeleteEvent(int accountId, Session session) {
     ...
     * @throws OXException if the token cannot be retrieved
     */
    private static void obtainTokenByCallback(Map<String, Object> arguments, DefaultOAuthAccount account, Set<OAuthScope> scopes) throws OXException {
        obtainTokenByOutOfBand(arguments, account, scopes);
    }
     ...
     * @throws OXException if the token cannot be retrieved
     */
    private static void obtainTokenByOutOfBand(Map<String, Object> arguments, DefaultOAuthAccount account, Set<OAuthScope> scopes) throws OXException {
        try {
     ...
     */
    private static org.scribe.oauth.OAuthService getScribeService(OAuthServiceMetaData metaData, String callbackUrl, Session session, Set<OAuthScope> scopes) throws OXException {
    ...
    private static OXException handleScribeOAuthException(org.scribe.exceptions.OAuthException e) {
        ...
        if (null != message) {
            ...
            if (pos > 0) {
                String msg = toText(message.substring(pos + str.length()).trim());
            ...
            if (pos > 0) {
                String msg = toText(message.substring(pos).trim());
                return OAuthExceptionCodes.DENIED_BY_PROVIDER.create(e, msg);
            }
     ...
     */
    private static String toText(String msg) {
        // In case surrounded by (single-)quotes
        String unwrappedMsg = StringUtils.unwrap(msg, '\'');
        unwrappedMsg = StringUtils.unwrap(unwrappedMsg, '"');

        // Test against possible HTML content
        if (com.openexchange.java.HTMLDetector.containsHTMLTags(Charsets.toAsciiBytes(unwrappedMsg), 0, unwrappedMsg.length())) {
            HtmlService htmlService = Services.getService(HtmlService.class);
            return null == htmlService ? msg : htmlService.html2text(unwrappedMsg, false);
        }

        // Test against possible XML document
        if (unwrappedMsg.indexOf("<?xml ") >= 0) {
            /*-
             * Assume content to be an XML document; e.g.
             *
             ...
             * </errors>
             */
            Optional<String> errorMsg = extractErrorMessageFromXmlDocument(unwrappedMsg);
    ...
    /**
     * Consider given message as an XML document and try to extract the error information from it.
     * <pre>
     * <?xml version='1.0' encoding='UTF-8'?>
     * <errors>
     *   <error code="415">Callback URL not approved for this client application. Approved callback URLs can be adjusted in your application settings</error>
     ...
     */
    private static Optional<String> extractErrorMessageFromXmlDocument(String msg) {
        JDOMParser jdomParser = Services.getService(JDOMParser.class);
        if (jdomParser == null) {
            return Optional.empty();
        }

        try {
            Document document = jdomParser.parse(Charsets.toAsciiStream(msg));
            if (document == null) {
                return Optional.empty();
            }
            ...
            for (Element errorsElement : rootElement.getChildren("errors")) {
                for (Element errorElement : errorsElement.getChildren("error")) {
                    Attribute optCodeAttribute = errorElement.getAttribute("code");
                    String errorText = errorElement.getText();
                    if (Strings.isNotEmpty(errorText)) {
                        if (optCodeAttribute == null) {
                            return Optional.of(errorText);
                        }
                        return Optional.of(new StringBuilder(errorText.length() + 16).append(errorText).append(" (HTTP status code: ").append(optCodeAttribute.getValue()).append(')').toString());
                    }
                }
            }
        } catch (Exception e) {
            LOG.warn("Failed to parse content to an XML document. Returning content as-is.", e);
        }
        return Optional.empty();
    }
     ...
     */
    private static void isNull(Map<String, Object> arguments, String... fields) throws OXException {
