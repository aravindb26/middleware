...
abstract class AbstractICAPRequestHandler implements ICAPRequestHandler {
     ...
     * @throws IOException if an I/O error is occurred
     */
    ICAPResponse readResponse(InputStream inputStream) throws IOException {
        int position = 0;
        ...
        while (position < BUFFER_SIZE && (readBytes = inputStream.read(buffer, position, 1)) != -1) {
            ...
            switch (terminator) {
                ...
                case HTTP_HEADER:
                    terminator = Terminator.HTTP_BODY;
                    parseEncapsulatedHTTPHeaders(builder, new String(buffer, icapHeadersMark, position - icapHeadersMark, Charsets.UTF_8));
                    break;
                default:
                    continue;
            }
