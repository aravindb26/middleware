...
public class DatabaseCleanUpActivator extends HousekeepingActivator {
    ...
    protected synchronized void startBundle() throws Exception {
        ...
        String scheduleExpression = configService.getProperty(DatabaseCleanUpProperty.SCHEDULE);
        if (Strings.isEmpty(scheduleExpression)) {
            throw DatabaseCleanUpExceptionCode.NO_SCHEDULE_SPECIFIED.create();
        }

        long frequencyMillis = configService.getLongProperty(DatabaseCleanUpProperty.FREQUENCY);
        int concurrencyLevel = configService.getIntProperty(DatabaseCleanUpProperty.CONCURRENCY_LEVEL);
        boolean abortRunningGeneralJobs = configService.getBooleanProperty(DatabaseCleanUpProperty.ABORT_SCHEDULED_JOBS);

        DatabaseCleanUpServiceImpl serviceImpl = new DatabaseCleanUpServiceImpl(scheduleExpression, frequencyMillis, concurrencyLevel, abortRunningGeneralJobs, new RdbDatabaseCleanUpExecutionManagement(this), this);
        this.serviceImpl = serviceImpl;

        track(SignalStartedService.class, new DatabaseCleanUpSchedulingStarter(serviceImpl, context));
        openTrackers();
