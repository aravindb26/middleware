    ...
    protected GmailAccess connectTransport(GmailSendConfig gmailSendConfig, boolean forPing) throws OXException {
        ...
        if (false == forPing) {
            ...
            if (account.isDeactivated()) {
                throw MailExceptionCode.MAIL_TRANSPORT_DEACTIVATED.create(gmailSendConfig.getServer(), gmailSendConfig.getLogin(), I(session.getUserId()), I(session.getContextId()));
            }
    ...
    public void sendReceiptAck(MailMessage srcMail, String fromAddr) throws OXException {
        ...
        try {
            ...
            ct.setContentType(CT_READ_ACK);
            {
                ...
                final String txt = strHelper.getString(ACK_TEXT).replaceFirst("#FROM#", quoteReplacement(from.toUnicodeString())).replaceFirst("#MSG ID#", quoteReplacement(msgId));
