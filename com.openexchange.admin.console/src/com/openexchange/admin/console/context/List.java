...
public class List extends ListCore {
    ...
    protected Context[] maincall(AdminParser parser, String search_pattern, Credentials auth, final Integer length, final Integer offset) throws RemoteException, StorageException, InvalidCredentialsException, InvalidDataException, MalformedURLException, NotBoundException {
        ...
        int chunkLength = (length == null) ? 10000 : Math.min(i(length), 10000);
        int fullChunkLength = chunkLength;
        int chunkOffset = (offset == null) ? 0 : i(offset);
        int total = 0;

        boolean first = true;
        Context[] ctxs;
        while ((ctxs = oxctx.list(search_pattern, chunkOffset, chunkLength, auth)).length == fullChunkLength) {
            if (first) {
                if (csv) {
            ...
            chunkOffset = chunkOffset + chunkLength;
            if (length != null) {
                total = total + chunkLength;
                if (total + chunkLength > i(length)) {
                    chunkLength = i(length) - total;
                }
            }
        }
        if (first) {
