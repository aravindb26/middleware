...
public interface AttachmentStorage {
     ...
     * <p/>
     * Binary attachments (where {@link Attachment#getData()} is not <code>null</code>) are uploaded and stored, metadata of external
     * attachments (where {@link Attachment#getUri()} is not <code>null</code>) are saved, while existing <i>managed</i> attachments
     * (where {@link Attachment#getManagedId()} is not <code>0</code>) attached to other groupware objects are copied over.
     * <p/>
     ...
     */
    default void deleteAttachments(Session session, String folderID, String eventID) throws OXException {
        deleteAttachments(session, folderID, Collections.singletonList(eventID));
    }

    /**
     * Deletes all attachments of multiple events in a certain folder.
     * <p/>
     * Permissions may be re-checked under the hood, therefore a valid session and parent folder needs to passed.
     *
     * @param session The session of the acting user
     * @param folderID The identifier of the parent folder where the events appear for the acting user
     * @param eventIDs The identifiers of the events to delete the attachments for
     */
    void deleteAttachments(Session session, String folderID, List<String> eventIDs) throws OXException;
