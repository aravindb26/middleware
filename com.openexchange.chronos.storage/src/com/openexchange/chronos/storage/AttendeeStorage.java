...
public interface AttendeeStorage {
     ...
     *
     * @param eventIds The identifiers of the events to load the attendees for
     * @param entities The entity identifiers of the internal attendees to consider
     * @return The attendees, mapped to the identifiers of the corresponding events
     */
    Map<String, List<Attendee>> loadAttendees(String[] eventIds, int[] entities) throws OXException;
    ...
    /**
     * Loads the number of (internal) attendees for specific events, separated by their calendar user type.
     *
     * @param eventIds The identifiers of the events to load the attendee counts for
     * @param internal {@link Boolean#TRUE} to only consider internal entities, {@link Boolean#FALSE} for non-internal ones,
     *            or <code>null</code> to not filter by internal/external
     * @return The number of attendees for each {@link CalendarUserType}, mapped to the identifiers of the corresponding events
     */
    Map<String, Map<CalendarUserType, Integer>> loadAttendeeCountsPerCUType(String[] eventIds, Boolean internal) throws OXException;

    /**
     * Loads the identifiers of those events where at least one of the candidate attendees has a participation status different from the
     * supplied one.
     *
     * @param eventIds The identifiers of the events to check for a different participation status
     * @param partStat The participation status to differentiate from
     * @param internal {@link Boolean#TRUE} to only consider internal entities, {@link Boolean#FALSE} for non-internal ones,
     *            or <code>null</code> to not filter by internal/external
     ...
     * @return A set holding the identifiers of the events where at least one of the candidate attendees has a participation status
     */
    Set<String> loadEventIdsWithDifferentPartStats(String[] eventIds, ParticipationStatus partStat, Boolean internal, CalendarUserType[] cuTypes, int[] ignoredEntities) throws OXException;
    ...
    /**
     * Loads data from <i>tombstone</i> records of a specific attendee for a number of events.
     *
     * @param eventIds The identifiers of the events to load the tombstone attendee data for
     * @param attendee The attendee to load
     * @param fields The attendee fields to load, or <code>null</code> to include all properties
     * @return The loaded attendee data, mapped to the identifiers of the corresponding events
     */
    Map<String, Attendee> loadAttendeeTombstone(String[] eventIds, Attendee attendee, AttendeeField[] fields) throws OXException;

    /**
     * Loads data from <i>tombstone</i> records of specific internal attendees for a number of events
     *
     * @param eventIds The identifiers of the events to load the attendees for
     * @param entities The entity identifiers of the internal attendees to consider
     * @return The attendees, mapped to the identifiers of the corresponding events
     */
    Map<String, List<Attendee>> loadAttendeeTombstones(String[] eventIds, int[] entities) throws OXException;
    ...
    /**
     * Loads attendees of a specific event if found in given folders
     *
     * @param eventId The event identifier
     * @param folderIds The folder identifiers the event might belong to
     * @param fields The fields to load
     * @return An empty list or the attendee data for the event
     * @throws OXException In case of error
     */
    List<Attendee> loadAttendees(String eventId, List<String> folderIds, AttendeeField[] fields) throws OXException;
}
