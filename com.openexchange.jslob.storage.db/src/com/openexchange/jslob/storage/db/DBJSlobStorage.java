...
public final class DBJSlobStorage implements JSlobStorage {
    ...
    private static JSlob load(JSlobId id, int contextId, Connection con) throws OXException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            ...
            return new DefaultJSlob(JSONServices.parseObject(new AsciiReader(rs.getBinaryStream(1)))).setId(id);
    ...
    private static List<JSlob> loadThem(List<JSlobId> ids, int contextId, Connection con, List<String> failedOnes) throws OXException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            ...
            do {
                ...
                try {
                    map.put(sId, new DefaultJSlob(JSONServices.parseObject(new AsciiReader(rs.getBinaryStream(1)))).setId(new JSlobId(serviceId, sId, user, contextId)));
                } catch (JSONException e) {
    ...
    private static String getInString(final List<JSlobId> ids) {
    ...
    private static Collection<JSlob> loadAll(final JSlobId id, final int contextId, final Connection con) throws OXException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            ...
            do {
                list.add(new DefaultJSlob(JSONServices.parseObject(new AsciiReader(rs.getBinaryStream(1)))).setId(new JSlobId(id.getServiceId(), rs.getString(2), id.getUser(), contextId)));
            } while (rs.next());
            return list;
        } catch (SQLException e) {
            throw JSlobExceptionCodes.UNEXPECTED_ERROR.create(e, e.getMessage());
     ...
     * @throws OXException  f operation fails
     */
    public static Collection<String> getIDs(final JSlobId id, final int contextId, final Connection con) throws OXException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
    ...
    public JSlob remove(final JSlobId id) throws OXException {
        ...
        try {
            ...
            con.setAutoCommit(false);
            committed = false;
            stmt = con.prepareStatement("DELETE FROM `jsonStorage` WHERE `cid` = ? AND `user` = ? AND s`erviceId` = ? AND `id` = ?");
            stmt.setLong(1, contextId);
            stmt.setLong(2, id.getUser());
            stmt.setString(3, id.getServiceId());
            stmt.setString(4, id.getId());
    ...
    private static boolean exists(JSlobId id, int contextId, Connection con) throws OXException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
    ...
    private static void setBinaryStream(JSONObject jObject, PreparedStatement stmt, int... positions) throws OXException {
        try {
            JSONObject json = null != jObject ? jObject : JSONObject.EMPTY_OBJECT;
