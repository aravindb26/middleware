...
public class EmailContactHalo extends AbstractContactHalo implements HaloContactDataSource {
    ...
    private static final Comparator<MailMessage> MAIL_COMPARATOR = new Comparator<MailMessage>() {

        @Override
        public int compare(final MailMessage m1, final MailMessage m2) {
            final Date sentDate1 = m2.getSentDate();
            final Date sentDate0 = m1.getSentDate();
            if (sentDate1 == null) {
    ...
    public AJAXRequestResult investigate(final HaloContactQuery query, final AJAXRequestData req, final ServerSession session) throws OXException {
        ...
        MailAccount[] userMailAccounts;
        {
            ...
            if (searchingExternalMailboxesIsFast()) {
                ...
                userMailAccounts = mailAccountService.getUserDefaultAndSecondaryMailAccounts(session.getUserId(), session.getContextId());
            }
            userMailAccounts = MailAccounts.removeDeactivatedAccountsFrom(userMailAccounts);
        }

        List<MailMessage> inboxMessages = new LinkedList<MailMessage>();
        List<MailMessage> sentMessage = new LinkedList<MailMessage>();
        {
            ...
            for (MailAccount mailAccount : userMailAccounts) {
                ...
                try {
                    ...
                    inboxMessages.addAll(retrievees.inboxMessages);
                    sentMessage.addAll(retrievees.sentMessage);
                } finally {
                    if (mailAccess != null) {
        ...
        Collections.sort(inboxMessages, MAIL_COMPARATOR);
        Collections.sort(sentMessage, MAIL_COMPARATOR);

        List<MailMessage> messages = new ArrayList<MailMessage>(limit << 1);
        messages.addAll(inboxMessages.size() > limit ? inboxMessages.subList(0, limit) : inboxMessages);
        messages.addAll(sentMessage.size() > limit ? sentMessage.subList(0, limit) : sentMessage);
