...
public class SproxydClient {
    ...
    public InputStream get(UUID id, long rangeStart, long rangeEnd) throws OXException {
        HttpResponseStream responseStream = nonResumingGet(id, rangeStart, rangeEnd);
        if (0 >= rangeStart && 0 >= rangeEnd) {
            // No range
            return new ResumableInputStream(responseStream, id, this);
        }
        return new RangeAcceptingResumableInputStream(responseStream, new long[] { rangeStart, rangeEnd }, id, this);
    }
     ...
     * @throws OXException If stored object's input stream cannot be returned
     */
    HttpResponseStream nonResumingGet(UUID id, long rangeStart, long rangeEnd) throws OXException {
        Callable<HttpResponseStream> getDataCallable = new Callable<HttpResponseStream>() {

            @Override
            public HttpResponseStream call() throws Exception {
                HttpGet get = null;
                HttpResponse response = null;
                Endpoint endpoint = getEndpoint();
                try {
                    String url = endpoint.getObjectUrl(id);
                    get = new HttpGet(url);
                    if (0 < rangeStart || 0 < rangeEnd) {
                    ...
                    if (HttpServletResponse.SC_OK == status || HttpServletResponse.SC_PARTIAL_CONTENT == status) {
                        HttpResponseStream content = HttpClients.createContentLengthIgnoringHttpResponseStreamFor(response, Optional.of(get));
                        response = null;
                        get = null;
                        return content;
                    }
    ...
    OXException handleCommunicationError(Endpoint endpoint, IOException e) throws OXException {
        ...
        if (endpointPool.isBlacklistEndpoints() && Utils.endpointUnavailable(endpoint.getBaseUrl(), getHttpClient())) {
    ...
    private static final RetryPolicy RETRY_POLICY_LOCKED = new RetryPolicy()
        ...
        .retryOn(t -> (t instanceof OXException) && SproxydExceptionCode.LOCKED.equals((OXException) t));
