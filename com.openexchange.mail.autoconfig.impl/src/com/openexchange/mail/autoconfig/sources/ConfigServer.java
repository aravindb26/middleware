...
public class ConfigServer extends AbstractProxyAwareConfigSource {
    ...
    private DefaultAutoconfig getAutoconfig(final String emailLocalPart, final String emailDomain, int userId, int contextId, boolean forceSecure, boolean useTLS) throws OXException {
        ...
        try {
            ...
            rsp = httpclient.execute(target, req, newHttpContextFor(url, userId, contextId));
            ...
            // Check Content-Type header
            {
                ...
                if (contentType == null) {
                    LOG.info("Could not retrieve config XML from autoconfig server. Missing 'Content-Type' header.");
                    return null;
                }
                String mimeType = Strings.asciiLowerCase(contentType.getValue());
                if (mimeType == null || (!mimeType.contains("text/xml") && !mimeType.contains("application/xml"))) {
                    LOG.info("Could not retrieve config XML from autoconfig server. The response's content type is neither of type 'text/xml' nor 'application/xml'.");
                    return null;
                }
            }
    ...
    private HttpContext newHttpContextFor(URL url, int userId, int contextId) {
