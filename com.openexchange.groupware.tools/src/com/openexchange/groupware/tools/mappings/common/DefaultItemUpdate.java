...
public class DefaultItemUpdate<O, E extends Enum<E>> implements ItemUpdate<O, E> {
    ...
    private static <O, E extends Enum<E>> boolean matches(Mapping<? extends Object, O> mapping, O original, O update) {
        ...
        if (null != originalValue && (originalValue instanceof EnumeratedProperty) && ((EnumeratedProperty) originalValue).matches(updatedValue)) {
            return true;
        }
        if (null != updatedValue && (updatedValue instanceof EnumeratedProperty) && ((EnumeratedProperty) updatedValue).matches(originalValue)) {
            return true;
        }
        return false;
    }
