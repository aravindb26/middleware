...
import static com.openexchange.groupware.tools.mappings.utils.MappingUtils.quote;
...
public abstract class DefaultDbMapper<O, E extends Enum<E>> extends DefaultMapper<O, E> implements DbMapper<O, E> {
    ...
    public O fromResultSet(final ResultSet resultSet, final E[] fields, String columnLabelPrefix) throws OXException, SQLException {
        ...
        for (final E field : fields) {
            ...
            if (mapping instanceof DbMultiMapping) {
    ...
    public E getMappedField(String columnLabel) {
        ...
        for (Entry<E, ? extends DbMapping<? extends Object, O>> entry : mappings.entrySet()) {
            if ((entry.getValue() instanceof DbMultiMapping)) {
    ...
    public String getAssignments(E[] fields) throws OXException {
		return getAssignments(fields, null);
	}

	@Override
	public String getAssignments(E[] fields, String columnLabelPrefix) throws OXException {
		if (null == fields) {
			throw new IllegalArgumentException("fields");
		}
        StringBuilder stringBuilder = new StringBuilder(10 * fields.length);
		if (0 < fields.length) {
            appendAssignments(stringBuilder, get(fields[0]), columnLabelPrefix);
			for (int i = 1; i < fields.length; i++) {
                stringBuilder.append(',');
                appendAssignments(stringBuilder, get(fields[i]), columnLabelPrefix);
			}
		}
    ...
    /**
     * Appends database assignments to parameter values for the columns of a
     * specific mapping, e.g. <code>column=?</code>. Mappings with multiple columns
     ...
     * @param columnLabelPrefix A prefix to use for the column names
     * @return The passed string builder reference
     */
    private static StringBuilder appendAssignments(StringBuilder stringBuilder, DbMapping<?, ?> mapping, String columnLabelPrefix) {
        String[] labels = getColumnLabels(mapping, columnLabelPrefix);
        stringBuilder.append(quote(labels[0])).append("=?");
        for (int i = 1; i < labels.length; i++) {
            stringBuilder.append(',').append(quote(labels[i])).append("=?");
        }
    ...
    private static StringBuilder appendColumnLabels(StringBuilder stringBuilder, DbMapping<?, ?> mapping, String columnLabelPrefix) {
        String[] labels = getColumnLabels(mapping, columnLabelPrefix);
        stringBuilder.append(quote(labels[0]));
        for (int i = 1; i < labels.length; i++) {
            stringBuilder.append(',');
            stringBuilder.append(quote(labels[i]));
        }
        return stringBuilder;
    }
