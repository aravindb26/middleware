...
import com.openexchange.chronos.alarm.mail.notification.AlarmNotificationMail;
...
public class MailAlarmMailGenerator {
    ...
    public static MailAlarmMailGenerator init(Event event, User user, int contextId, int accountId, long trigger, ServiceLookup services) throws OXException {
        ...
        AlarmNotificationMail mail = new MailAlarmNotificationGenerator(services, event, user, context, accountId).create();
        ...
        try {
            Builder mailData = MailData.newBuilder()
                ...
                .addMailHeader(MessageHeaders.HDR_MESSAGE_ID, generateHeaderValue(event.getUid(), true))
                .addMailHeader(MessageHeaders.HDR_IN_REPLY_TO, generateHeaderValue(event.getUid(), false))
                .addMailHeader(MessageHeaders.HDR_REFERENCES, generateHeaderValue(event.getUid(), false))
