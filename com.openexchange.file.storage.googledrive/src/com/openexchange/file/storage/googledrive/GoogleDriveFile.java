...
public final class GoogleDriveFile extends DefaultFile {
    ...
    private static final FastDateFormat EXIF_FORMAT = FastDateFormat.getInstance("yyyy-MM-dd HH:mm:ss", TimeZone.getTimeZone("UTC"), Locale.US);
                    ...
                    if (set.contains(Field.CAPTURE_DATE)) {
                        if (null != imageMediaMetadata.getTime()) {
                            try {
                                 ...
                                 */
                                Date date = EXIF_FORMAT.parse(imageMediaMetadata.getTime());
                                setCaptureDate(date);
