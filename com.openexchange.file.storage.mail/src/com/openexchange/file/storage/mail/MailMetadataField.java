...
public class MailMetadataField implements AdditionalFileField {
    ...
    public Object renderJSON(AJAXRequestData requestData, Object value) {
        if ((value instanceof MailMetadataImpl)) {
            return ((MailMetadataImpl) value).renderJSON();
        }
        return value;
    }

    private static MailMetadataImpl getMailMetadata(File file) {
        MailDriveFileImpl mailDriveFileImpl = FileStorageUtility.launderDelegate(file, MailDriveFileImpl.class);
        return null != mailDriveFileImpl ? mailDriveFileImpl.getMetadata() : null;
    }
