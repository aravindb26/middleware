...
public class JSlobEntryRegistry {
    ...
    private final ConcurrentMap<JSlobKey, ConcurrentMap<String, JSlobEntryWrapper>> registry;
    ...
    public synchronized boolean addJSlobEntry(JSlobEntry jSlobEntry) throws OXException {
        ...
        ConcurrentMap<String, JSlobEntryWrapper> entries = registry.get(jSlobEntry.getKey());
        if (null == entries) {
            ConcurrentMap<String, JSlobEntryWrapper> newMap = new ConcurrentHashMap<>(4, 0.9F, 1);
            entries = registry.putIfAbsent(jSlobEntry.getKey(), newMap);
            if (null == entries) {
                entries = newMap;
            }
        }
     ...
     */
    public Map<JSlobKey, ConcurrentMap<String, JSlobEntryWrapper>> getAvailableJSlobEntries() {
        return registry;
    }
