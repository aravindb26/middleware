...
public final class MailFilterServiceImpl implements MailFilterService, Reloadable {
    ...
    private static final ConcurrentMap<UserAndContext, Object> LOCKS = new ConcurrentHashMap<>(256, 0.9f, 1);
    ...
    public void reinitBreaker(ConfigurationService config) {
        ...
        try {
            ...
            int failureExecutions = getFailureExecutions(config, failureThreshold);
            ...
            int successExecutions = getSuccessExecutions(config, successThreshold);
            ...
            circuitBreaker = new CircuitBreaker().withFailureThreshold(failureThreshold, failureExecutions).withSuccessThreshold(successThreshold, successExecutions).withDelay(delayMillis, TimeUnit.MILLISECONDS).onOpen(() -> {
            ...
            }).onHalfOpen(() -> LOGGER.info("Mail filter circuit breaker half-opened")).onClose(() -> LOGGER.info("Mail filter circuit breaker closed"));
    ...
    /**
     * Returns the value of the success executions property
     * 
     * @param config The config service
     * @param successThreshold The success threshold as fallback
     * @return The value of the success executions property
     */
    private int getSuccessExecutions(ConfigurationService config, int successThreshold) {
        ...
        int successExecutions = Integer.parseInt(tmp);
    ...
    /**
     * Retrieves the value of the failure executions property
     * 
     * @param config The config service
     * @param failureThreshold The failure threshold for fallback
     * @return The value of the failure executions
     */
    private int getFailureExecutions(ConfigurationService config, int failureThreshold) {
        ...
        int failureExecutions = Integer.parseInt(tmp);
    ...
    private void checkConfigfile(ConfigurationService config) throws OXException {
        try {
            ...
            for (final MailFilterProperty property : MailFilterProperty.values()) {
                if (!property.isOptional() && null == file.getProperty(property.getFQPropertyName()) && hasNoDefaultValue(property)) {
                    throw MailFilterExceptionCode.PROPERTY_NOT_FOUND.create(property.getFQPropertyName());
                }
            }
            parseConnectionTimeoutProperty(file);
        } catch (OXException e) {
            throw e;
        } catch (Exception e) {
            throw MailFilterExceptionCode.PROBLEM.create(e, e.getMessage());
        }
    ...
    /**
     * Parses the connection timeout property from the specified properties file
     * 
     * @param file The properties file
     * @throws OXException if the property cannot be parsed
     */
    private void parseConnectionTimeoutProperty(Properties file) throws OXException {
        try {
            Integer.parseInt(file.getProperty(MailFilterProperty.connectionTimeout.getFQPropertyName()));
        } catch (NumberFormatException e) {
            throw MailFilterExceptionCode.PROPERTY_ERROR.create(e, "Property " + MailFilterProperty.connectionTimeout.getFQPropertyName() + " is not an integer value");
        }
    }

    /**
     * Checks whether the specified property has no default value
     * 
     * @param property the property to check
     * @return <code>true</code> if the property has no default value; <code>false</code> otherwise
     */
    private boolean hasNoDefaultValue(MailFilterProperty property) {
        Object o = property.getDefaultValue();
        return o == null || (o instanceof String s && Strings.isEmpty(s));
    }
     ...
     */
    private String getScript(SieveHandler sieveHandler, String expectedScriptName) throws OXSieveHandlerException, IOException {
    ...
    public final int createFilterRule(Credentials credentials, Rule rule) throws OXException {
        ...
        synchronized (lock) {
            ...
            try {
                ...
                String writeback = sieveTextFilter.writeback(clientrulesandrequire, new HashSet<>(sieveHandler.getCapabilities().getSieve()));
    ...
    public final void updateFilterRule(Credentials credentials, Rule rule, int uid) throws OXException {
        ...
        synchronized (lock) {
            ...
            try {
                ...
                String writeback = sieveTextFilter.writeback(clientRulesAndReq, new HashSet<>(sieveHandler.getCapabilities().getSieve()));
                writeback = sieveTextFilter.rewriteRequire(writeback, script);
                LOGGER.debug("The following sieve script will be written:\n{}", writeback);
    ...
    public final Rule getFilterRule(Credentials credentials, int uid) throws OXException {
        ...
        synchronized (lock) {
            ...
            try {
                ...
                return applyOptionalCopyRequired(rightRule.rule);
            } catch (UnsupportedEncodingException e) {
                throw MailFilterExceptionCode.UNSUPPORTED_ENCODING.create(e);
            } catch (OXSieveHandlerException e) {
                throw MailFilterExceptionCode.handleParsingException(e, credentials, useSIEVEResponseCodes(credentials.getUserid(), credentials.getContextid()));
    ...
    public Set<String> getCapabilities(Credentials credentials) throws OXException {
        ...
        synchronized (lock) {
            ...
            try {
                ...
                return new HashSet<>(capabilities.getSieve());
            } catch (OXSieveHandlerException e) {
                throw MailFilterExceptionCode.handleParsingException(e, credentials, useSIEVEResponseCodes(credentials.getUserid(), credentials.getContextid()));
            } finally {
                closeSieveHandler(sieveHandler);
    ...
    public Set<String> getStaticCapabilities(final Credentials credentials) throws OXException {
        ...
        synchronized (lock) {
            ...
            try {
                Capabilities capabilities = staticCapabilities.get(key, () -> {
                    try {
                        handlerConnect(sieveHandler, credentials.getSubject());
    ...
    /**
     * Applies the optional <code>copy</code> required for the {@link ActionCommand.Commands#FILEINTO} and {@link ActionCommand.Commands#REDIRECT}
     * action commands.
     *
     * @param rule The rule
     * @return The rule
     */
    private Rule applyOptionalCopyRequired(Rule rule) {
        IfCommand ifCommand = rule.getIfCommand();
        if (ifCommand == null) {
            return rule;
        }
        for (ActionCommand actionCommand : ifCommand.getActionCommands()) {
            IActionCommand command = actionCommand.getCommand();
            if (!command.equals(ActionCommand.Commands.FILEINTO) && false == command.equals(ActionCommand.Commands.REDIRECT)) {
                continue;
            }
            Hashtable<String, List<String>> tagArguments = actionCommand.getTagArguments();
            if (tagArguments.containsKey(":copy")) {
                ifCommand.addOptionalRequired("copy");
                break;
            }
        }
        return rule;
    ...
    private void checkRedirects(Rule rule, Credentials credentials) throws OXException {
        ...
        if (!(maxRedirectObject instanceof Integer)) {
            return;
        }
    ...
    private void changeIncomingVacationRule(int userId, int contextId, Rule rule) throws SieveException {
        ...
        boolean isVacationRule = isVacationRule(rule);
        RuleComment ruleComment = rule.getRuleComment();
        if (isVacationRule && ruleComment != null && Strings.isEmpty(ruleComment.getRulename())) {
            // ruleComment.setRulename("vacation notice");
        }
        ...
        TestCommand newTestCommand = new TestCommand(Commands.ADDRESS, argList, new ArrayList<>());
        ...
        // Found other tests
        List<TestCommand> arrayList = new ArrayList<>();
        arrayList.add(newTestCommand);
        arrayList.add(testcommand);
        ifCommand.setTestcommand(new TestCommand(Commands.ALLOF, new ArrayList<>(), arrayList));
    }

    /**
     * Change the outgoing vacation rule
     *
     * @param clientrules the client rules
    ...
    /**
     * Processes the specified rule to check if a vacation rule is already present
     * 
     * @param rule The rule
     * @throws SieveException if a parsing error is occurred
     */
    private void processRule(Rule rule) throws SieveException {
        IfCommand ifCommand = rule.getIfCommand();
        if (!isVacationRule(rule)) {
            return;
        }
        ...
        if (Commands.ADDRESS.equals(testCommand.getCommand())) {
            processAddressCommand(ifCommand, testCommand);
        } else if (Commands.ALLOF.equals(testCommand.getCommand())) {
            // In this case we find "our" rule at the first place
            processAllOfCommand(ifCommand, testCommand.getTestCommands());
        }
    }

    /**
     * Processes the address command
     * 
     * @param ifCommand The if command to set the test command
     * @param testCommand The test command to check
     * @throws SieveException if a parsing error is occurred
     */
    private void processAddressCommand(IfCommand ifCommand, TestCommand testCommand) throws SieveException {
        if (!checkOwnVacation(testCommand.getArguments())) {
            return;
        }
        ifCommand.setTestcommand(new TestCommand(TestCommand.Commands.TRUE, new ArrayList<>(), new ArrayList<>()));
    }

    /**
     * Processes the all-of command
     * 
     * @param ifCommand The if command
     * @param testCommands test commands
     */
    private void processAllOfCommand(IfCommand ifCommand, List<TestCommand> testCommands) {
        if (null == testCommands || testCommands.size() <= 1) {
            return;
        }
        TestCommand testCommand = testCommands.get(0);
        if (!checkOwnVacation(testCommand.getArguments())) {
            return;
        }
        // now remove...
    ...
    private boolean checkOwnVacation(List<Object> arguments) {
        ...
        return null != arguments && 
            null != arguments.get(0) && 
            arguments.get(0) instanceof TagArgument t && 
            ":is".equals(t.getTag()) && 
            null != arguments.get(1) && 
            arguments.get(1) instanceof TagArgument t2 && 
            ":domain".equals(t2.getTag()) && 
            null != arguments.get(2) && 
            arguments.get(2) instanceof List<?> && 
            "From".equals(((List<?>) arguments.get(2)).get(0));
        //@formatter:on
    }
     ...
     */
    private void writeScript(SieveHandler sieveHandler, String activeScript, String writeback, String scriptName) throws OXSieveHandlerException, IOException {
    ...
    protected void handlerConnect(final SieveHandler sieveHandler, Subject kerberosSubject) throws OXException, OXSieveHandlerException {
        try {
            ...
            synchronized (kerberosSubject) {
                Subject.doAs(kerberosSubject, (PrivilegedExceptionAction<Object>) () -> {
                    sieveHandler.initializeConnection();
                    return null;
                });
            }
