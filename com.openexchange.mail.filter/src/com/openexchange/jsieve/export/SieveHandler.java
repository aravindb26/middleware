...
public class SieveHandler {
    ...
    /**
     * The byte array constant for CRLF (carriage-return line-feed).
     */
    ...
    /**
     * The assignment operator for XCLIENT attributes
     */
    protected final static String XCLIENT_ASSIGN = "=";
    ...
    private final static String SIEVE_XCLIENT = "XCLIENT ";
    ...
    private static enum WelcomeKeyword {
        STARTTLS, XCLIENT, IMPLEMENTATION, SIEVE, SASL, MAXREDIRECTS
    }

    /**
     * {@link XClientAttributes} - XClient attributes
     */
    private static enum XClientAttributes {
        ADDR, SESSION
    }
     ...
     * @param sessionIp The session users IP address
     * @param sessionInfo The session information
     */
    public SieveHandler(String userName, String authUserName, String authUserPasswd, String host, int port, String authEnc, String oauthToken, Optional<CircuitBreakerInfo> optionalCircuitBreaker, int userId, int contextId, String sessionIp, String sessionInfo) {
        super();
    ...
    public void initializeConnection() throws IOException, OXSieveHandlerException, UnsupportedEncodingException, OXSieveHandlerInvalidCredentialsException {
        ...
        if (false == onlyWelcome) {
            ...
            boolean requireTls = mailFilterConfig.getBooleanProperty(userId, contextId, MailFilterProperty.requireTls);
            boolean tlsAvailable = capa.getStarttls().booleanValue();
            if (requireTls) {
                if (false == tlsAvailable) {
                    throw new OXSieveHandlerException("Communication to SIEVE server aborted, STARTTLS required but not available. ", sieve_host, sieve_host_port, null);
                }
            }

            final boolean issueTLS = requireTls || (tlsenabled && tlsAvailable);
            ...
            if (issueTLS) {
                ...
                String[] protocols;
                {
                    ...
                    if (Strings.isNotEmpty(sProtocols)) {
                        ...
                        SSLConfigurationService sslConfigService = Services.optService(SSLConfigurationService.class);
                        protocols = sslConfigService == null ? NetUtils.getProtocolsArray() : sslConfigService.getSupportedProtocols();
                    }
                }
             ...
             * XCLIENT extension
             */
            boolean allowXClient = mailFilterConfig.getBooleanProperty(userId, contextId, MailFilterProperty.allowXCLIENT);
            if (allowXClient && capa.getXClient()) {
                measureStart();
                commandBuilder = new StringBuilder(32);
                String addr = Strings.isEmpty(sessionIp) ? ClientParameterUtility.getLocalHost() : sessionIp;
                commandBuilder.append(SIEVE_XCLIENT)
                              .append(XClientAttributes.ADDR)
                              .append(XCLIENT_ASSIGN)
                              .append(addr)
                              .append(StringUtils.SPACE)
                              .append(XClientAttributes.SESSION)
                              .append(XCLIENT_ASSIGN)
                              .append(sessionInfo)
                              .append(CRLF);
                bos_sieve.write(commandBuilder.toString().getBytes(UTF_8));
                bos_sieve.flush();
                String response = bis_sieve.readLine();
                if (response.startsWith(SIEVE_NO)) {
                    OXSieveHandlerException e = new OXSieveHandlerException(response, sieve_host, sieve_host_port, null);
                    log.warn("Failed to override client-related session attributes", e);
                }
                measureEnd("sendXCLIENT");
                commandBuilder.setLength(0);
            }
            /*
    ...
    private static final List<Class<? extends Throwable>> NETWORK_COMMUNICATION_ERRORS = List.of(
    ...
    public String readResponseLine(MetricHelper helper) throws IOException {
        ...
        try {
            ...
            helper.updateRequestTimer(status);
            throw e;
        }
    }
    ...
    public void setScript(final String script_name, final byte[] script, final StringBuilder commandBuilder) throws OXSieveHandlerException, IOException, UnsupportedEncodingException {
        ...
        if (null != currentLine && currentLine.startsWith(SIEVE_OK)) {
            ...
            final String errorMessage = parseError(currentLine, helper).replace(CRLF, "\n");
            throw new OXSieveHandlerException(errorMessage, sieve_host, sieve_host_port, parseSIEVEResponse(currentLine, errorMessage), true);
        } else {
    ...
    private boolean selectAuth(PreferredSASLMech auth_mech, StringBuilder commandBuilder, int timeout) throws IOException, UnsupportedEncodingException, OXSieveHandlerException {
        ...
        synchronized (s_sieve) {
            ...
            try {
                ...
                throw new OXSieveHandlerException(message, sieve_host, sieve_host_port, null, e, true);
            } finally {
    ...
    public static class MetricHelper {
        ...
        MetricHelper(SieveHandler sieveHandler, boolean groupByEndpoints) {
            ...
            if (groupByEndpoints) {
                String hostAddress;
                try {
                    hostAddress = com.openexchange.java.InetAddresses.forString(sieveHandler.sieve_host).getHostAddress();
         ...
         */
        public void updateRequestTimer(String status) {
