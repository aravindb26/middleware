            ...
            for (final Contact contact : contacts.values()) {
                ...
                if (distributionList != null && distributionList.length > 0) {
                    for (final DistributionListEntryObject entry : distributionList) {
                        ...
                        setIntOrNull(i++, stmt, null == entry.getEntryID() ? 0 : Strings.parseUnsignedInt(entry.getEntryID()));
                        setIntOrNull(i++, stmt, entry.getEmailfield());
                        setIntOrNull(i++, stmt, null == entry.getFolderID() ? 0 : Strings.parseUnsignedInt(entry.getFolderID()));
        ...
        for (Contact contact : contacts.values()) {
            if (contact.containsDistributionLists()) {
                ...
                for (final DistributionListEntryObject entry : distributionList) {
                    ...
                    final int integerEntryId = Strings.parseUnsignedInt(entryId);
                    if (integerEntryId > 0) {
                        /*
                         ...
                         */
                        final Contact dlistContact = contacts.get(I(integerEntryId));
                        if (dlistContact == null) {
                            entry.setEntryID(null);
                            entry.setEmailfield(-1);
                            entry.setFolderID(null);
                        } else {
                            entry.setEntryID(dlistContact.getId(true));
                        }
                    }
                }
            }
    ...
    void loadDistributionListsFromDB(final Map<Integer, Contact> contacts, final Connection con, final int cid) throws OXException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int entryId = getIntOrNegative(i++, rs);
                entry.setEntryID(entryId < 1 ? null : I(entryId).toString());
                entry.setEmailfield(getIntOrNegative(i++, rs));
                int folderID = getIntOrNegative(i++, rs);
                entry.setFolderID(0 < folderID ? Integer.toString(folderID) : null);
                entry.setDisplayname(rs.getString(i++));
                entry.setLastname(rs.getString(i++));
                entry.setFirstname(rs.getString(i++));
                entry.setEmailaddress(rs.getString(i++), false);
