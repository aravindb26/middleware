...
import com.openexchange.http.requestwatcher.internal.NoopRequestWatcherService;
...
public class RequestWatcherActivator extends HousekeepingActivator {
    ...
    protected synchronized void startBundle() throws OXException {
        ...
        try {
            ...
            if (isWatcherEnabled) {
                RequestWatcherServiceImpl requestWatcher = new RequestWatcherServiceImpl(configService, getService(TimerService.class));
                this.requestWatcher = requestWatcher;
                registerService(RequestWatcherService.class, requestWatcher);
            } else {
                registerService(RequestWatcherService.class, new NoopRequestWatcherService());
            }
        } catch (Exception e) {
            log.warn("Failed to start request watcher", e);
        }
    }
