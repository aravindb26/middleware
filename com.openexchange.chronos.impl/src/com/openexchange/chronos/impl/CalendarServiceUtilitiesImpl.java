...
import com.openexchange.chronos.impl.performer.GetRecurrenceInfoPerformer;
...
public class CalendarServiceUtilitiesImpl implements CalendarServiceUtilities {
    ...
    @Override
    public List<Event> lookupByField(CalendarSession session, EventField field, Object value, int calendarUserId, boolean tombstones) throws OXException {
        return new InternalCalendarStorageOperation<List<Event>>(session) {

            @Override
            protected List<Event> execute(CalendarSession session, CalendarStorage storage) throws OXException {
                return new ResolvePerformer(session, storage).lookupByField(field, value, calendarUserId, tombstones);
            }
        }.executeQuery();
    }

    @Override
    public List<Event> resolveDeletedEventsByUID(CalendarSession session, String uid, int calendarUserId) throws OXException {
        return new InternalCalendarStorageOperation<List<Event>>(session) {

            @Override
            protected List<Event> execute(CalendarSession session, CalendarStorage storage) throws OXException {
                return new ResolvePerformer(session, storage).resolveDeletedEventsByUID(uid, calendarUserId);
            }
        }.executeQuery();
    }
    ...
    @Override
    public RecurrenceInfo getRecurrenceInfo(CalendarSession session, EventID eventID) throws OXException {
        return new InternalCalendarStorageOperation<RecurrenceInfo>(session) {

            @Override
            protected RecurrenceInfo execute(CalendarSession session, CalendarStorage storage) throws OXException {
                return new GetRecurrenceInfoPerformer(session, storage).perform(eventID.getFolderID(), eventID.getObjectID(), eventID.getRecurrenceID());
            }
        }.executeQuery();
    }
