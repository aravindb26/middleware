...
import static com.openexchange.chronos.ParticipationStatus.NEEDS_ACTION;
...
public class InternalAttendeeUpdates implements CollectionUpdate<Attendee, AttendeeField> {
     ...
     * 
     * @param calendarUser The calendar user to send a reply to
    ...
    /**
     * Gets a list of updated attendees for whom the participant status was reseted (participant status has been set {@link ParticipationStatus#NEEDS_ACTION} again)
     *
     * @return The list with reseted attendees, or empty list if there are none
     */
    public List<ItemUpdate<Attendee, AttendeeField>> getResetedItems() {
        List<ItemUpdate<Attendee, AttendeeField>> resetedItems = new ArrayList<>();
        for (ItemUpdate<Attendee, AttendeeField> itemUpdate : attendeesToUpdate) {
            if (itemUpdate.getUpdatedFields().contains(AttendeeField.PARTSTAT) && //
                false == NEEDS_ACTION.matches(itemUpdate.getOriginal().getPartStat()) && //
                NEEDS_ACTION.matches(itemUpdate.getUpdate().getPartStat())) {
                resetedItems.add(itemUpdate);
            }
        }
        return resetedItems;
    }
    ...
    private List<Attendee> prepareNewAttendees(List<Attendee> existingAttendees, List<Attendee> newAttendees) throws OXException {
        ...
        /*
         * resolve & add any internal resource attendees, check scheduling privileges & set initial participation status
         */
        for (Attendee resourceAttendee : filter(newAttendees, Boolean.TRUE, CalendarUserType.RESOURCE)) {
            ...
            Attendee attendee = session.getEntityResolver().applyEntityData(resourceAttendee);
            SchedulingPrivilege privilege = Check.hasSchedulingPrivilege(session, attendee.getEntity(), SchedulingPrivilege.ASK_TO_BOOK);
            attendee.setPartStat(privilege.implies(SchedulingPrivilege.BOOK_DIRECTLY) ? ParticipationStatus.ACCEPTED : ParticipationStatus.NEEDS_ACTION);
    ...
    private final static AttendeeField[] CHANGEABLE_ATTENDEE_FIELDS = { AttendeeField.RSVP, AttendeeField.COMMENT, AttendeeField.PARTSTAT, AttendeeField.ROLE, //
        AttendeeField.PARTSTAT, AttendeeField.CN, AttendeeField.URI, AttendeeField.TIMESTAMP, AttendeeField.SEQUENCE, AttendeeField.TRANSP };
    ...
    public static Attendee getDefaultAttendee(CalendarSession session, CalendarFolder folder, List<Attendee> requestedAttendees, Date timestamp) throws OXException {
        ...
        if (null != requestedAttendee) {
            AttendeeMapper.getInstance().copy(requestedAttendee, defaultAttendee, CHANGEABLE_ATTENDEE_FIELDS);
        }
     ...
     * <p/>
     * If a default attendee is enforced, this method ensures that at least one internal calendar user attendee is always present for
     * events, leading to pseudo-group-scheduled events where necessary. Otherwise, if the actual calendar user would be the last one in
     * the resulting attendee list, this attendee is removed.
     *
    ...
    private void handleDefaultAttendee(boolean enforceDefaultAttendee) throws OXException {
        ...
        } else if (filter(attendees, Boolean.TRUE, CalendarUserType.INDIVIDUAL).isEmpty()) {
            /*
             * enforce 'pseudo' group scheduled event by preserving calendar user attendee if necessary
             */
            Attendee removedAttendee = find(attendeesToDelete, calendarUserId);
            if (null != removedAttendee) {
                LOG.info("Implicitly preserving default calendar user {} in {}.", I(calendarUserId), folder);
                attendeesToDelete.remove(removedAttendee);
            } else {
                LOG.info("Implicitly adding default calendar user {} in {}.", I(calendarUserId), folder);
    ...
    private static final AttendeeField[] RELEVANT_FIELDS = { AttendeeField.RSVP, AttendeeField.HIDDEN, AttendeeField.COMMENT, AttendeeField.PARTSTAT, AttendeeField.ROLE, //
        AttendeeField.EXTENDED_PARAMETERS, AttendeeField.TIMESTAMP, AttendeeField.SEQUENCE, AttendeeField.TRANSP };

    /**
     * Instantiates a new attendee, copies over all data from the original attendee, then takes over all <i>set</i>, non-static properties
     * from the updated item in the supplied attendee update, then - so that the resulting attendee represents the attendee after the
     * update.
     * 
    ...
    private static Attendee apply(ItemUpdate<Attendee, AttendeeField> attendeeUpdate) throws OXException {
        Attendee changedAttendee = AttendeeMapper.getInstance().copy(attendeeUpdate.getOriginal(), null, (AttendeeField[]) null);
        return AttendeeMapper.getInstance()
                             .copy(attendeeUpdate.getUpdate(), changedAttendee, RELEVANT_FIELDS);
    }
