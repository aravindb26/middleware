...
public class Consistency extends com.openexchange.chronos.common.Consistency {
    ...
    /**
     * Sets the event's <i>modified</i> properties to the supplied date and calendar user entity. This always includes the
     * {@link EventField#TIMESTAMP} and {@link EventField#MODIFIED_BY} fields, and optionally also the {@link EventField#DTSTAMP} (for
     * <i>internally</i> organized events only).
     * 
     * @param session The calendar session
     * @param lastModified The last modified date to apply
     * @param event The event to set the <i>modified</i> properties in
     * @param modifiedBy The user entity identifier to use for the <i>modified by</i> field
     * @param hasExternalOrganizer <code>true</code> if the event in question is organized <i>externally</i>, <code>false</code>, otherwise
     */
    public static void setModified(CalendarSession session, Date lastModified, Event event, int modifiedBy, boolean hasExternalOrganizer) throws OXException {
        setModified(lastModified, event, session.getEntityResolver().applyEntityData(new CalendarUser(), modifiedBy), hasExternalOrganizer);
    }

    /**
     * Sets the event's <i>modified</i> properties to the supplied date and calendar user. This always includes the
     * {@link EventField#TIMESTAMP} and {@link EventField#MODIFIED_BY} fields, and optionally also the {@link EventField#DTSTAMP} (for
     * <i>internally</i> organized events only).
     * 
     * @param lastModified The last modified date to apply
     * @param event The event to set the <i>modified</i> properties in
     * @param modifiedBy The calendar user to use for the <i>modified by</i> field
     * @param hasExternalOrganizer <code>true</code> if the event in question is organized <i>externally</i>, <code>false</code>, otherwise
     */
    public static void setModified(Date lastModified, Event event, CalendarUser modifiedBy, boolean hasExternalOrganizer) {
        event.setLastModified(lastModified);
        event.setModifiedBy(modifiedBy);
        event.setTimestamp(lastModified.getTime());
        if (false == hasExternalOrganizer) {
            event.setDtStamp(lastModified.getTime());
        }
    }
