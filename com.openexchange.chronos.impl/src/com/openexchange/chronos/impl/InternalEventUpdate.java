...
import static com.openexchange.chronos.impl.Utils.canBookDirectly;
...
public class InternalEventUpdate implements EventUpdate {
    ...
    public InternalEventUpdate(CalendarSession session, CalendarFolder folder, Event originalEvent, List<Event> originalChangeExceptions, Event originalSeriesMasterEvent, Event updatedEvent, Date timestamp, boolean assumeExternalOrganizerUpdate, EventField... ignoredFields) throws OXException {
        ...
        this.eventUpdate = new DefaultItemUpdate<>(originalEvent, changedEvent, differentFields);
    ...
    public List<Event> getDeletedExceptions() {
        ...
        List<Event> deletedExceptions = new ArrayList<>();
    ...
    private void ensureConsistency(Event originalEvent, Event updatedEvent, Event originalSeriesMaster) throws OXException {
        ...
        if (null != originalEvent.getOrganizer() && isInternal(originalEvent.getOrganizer(), CalendarUserType.INDIVIDUAL)) {
            /*
             * reset attendee's partstats or transparency if required & increment sequence number as needed
             */
            if (needsParticipationStatusReset(originalEvent, updatedEvent)) {
                resetParticipationStatus(updatedEvent.getAttendees());
                resetAttendeeTransparency(updatedEvent);
            }
            ...
            if (isOpaqueTransparency(originalEvent) != isOpaqueTransparency(updatedEvent)) {
                resetAttendeeTransparency(updatedEvent);
            }
        } else if (hasExternalOrganizer(originalEvent)) {
            /*
             * un-hide calendar user's attendee explicitly if required
             */
            Attendee originalAttendee = find(originalEvent.getAttendees(), calendarUser);
            Attendee updatedAttendee = find(updatedEvent.getAttendees(), calendarUser);
            if (null != originalAttendee && originalAttendee.isHidden() && null != updatedAttendee && ParticipationStatus.NEEDS_ACTION.equals(updatedAttendee.getPartStat())) {
                updatedAttendee.setHidden(false);
            }
        }
        /*
         * apply timestamp/last-modified info
         */
        Consistency.setModified(session, timestamp, updatedEvent, session.getUserId(), hasExternalOrganizer(originalEvent));
        /*
         * check if it is an internal organizer and if there is a recurrence rule,
         * if yes, check if the date matches the rule and adjust it if necessary
         */
        if (!hasExternalOrganizer(updatedEvent)) {
            Consistency.adjustRecurrenceRuleDeviation(updatedEvent, session);
        }
    }
    ...
    private Event apply(Event originalEvent, Event updatedEvent, boolean assumeExternalOrganizerUpdate, EventField... ignoredFields) throws OXException {
         ...
         */
        Set<EventField> updatedFields = new HashSet<>(java.util.Arrays.asList(EventMapper.getInstance().getAssignedFields(updatedEvent)));
        ...
        if (isAttendeeSchedulingResource && ignoreForbiddenAttendeenChanges && !assumeExternalOrganizerUpdate) {
            /*
             * Translate TRANSP change to user, before we ignore the change
             */
            setTranspToCalendarUser(updatedEvent, updatedFields);
            updatedFields.retainAll(EnumSet.of(
                EventField.ALARMS, EventField.ATTENDEES, EventField.DELETE_EXCEPTION_DATES, EventField.CREATED, EventField.TIMESTAMP, EventField.LAST_MODIFIED, EventField.DTSTAMP));
        }
        /*
        ...
        if (updatedFields.contains(EventField.ATTENDEES)) {
            ...
            if (isAttendeeSchedulingResource && ignoreForbiddenAttendeenChanges) {
                ...
                List<Attendee> updatedAttendees = new ArrayList<>(originalEvent.getAttendees().size());
    ...
    /**
     * Copies the transparency set in the event to the calendar user, if feasible
     *
     * @param updatedEvent The updated event
     * @param updatedFields The updated fields
     */
    private void setTranspToCalendarUser(Event updatedEvent, Set<EventField> updatedFields) {
        if (false == updatedFields.contains(EventField.TRANSP)) {
            return;
        }
        Attendee updatedAttendee = find(updatedEvent.getAttendees(), calendarUser.getEntity());
        if (null == updatedAttendee) {
            LOG.debug("Unable to set per-attendee transparency for calendar user {} in event {}.", calendarUser, updatedEvent);
            return;
        }
        updatedAttendee.setTransp(updatedEvent.getTransp());
    }
    ...
    private void adjustForGroupScheduled(Event originalEvent, Event updatedEvent) throws OXException {
        /*
         * group-scheduled event, ensure to take over an appropriate organizer
         */
        if (null == originalEvent.getOrganizer()) {
            ...
            if (null == updatedEvent.getOrganizer() || matches(originalEvent.getOrganizer(), organizer)) {
                /*
                 * organizer not set or matches the original one, ignore in update
                 */
                updatedEvent.setOrganizer(originalEvent.getOrganizer());
            } else {
                /*
                 * organizer changed, let through in special iCloud scenario or yell
                 */
                if (false == isSimilarICloudIMipMeCom(originalEvent.getOrganizer(), organizer)) {
                    LOG.debug("Applying change for iCloud organizer [original: {}, updated: {}]", originalEvent.getOrganizer(), organizer);
        ...
        /*
         * reset common calendar folder (unless public)
         */
    ...
    private List<Event> propagateToChangeExceptions(Event originalMaster, Event updatedMaster, List<Event> originalChangeExceptions, List<Event> updatedChangeExceptions) throws OXException {
        ...
        /*
         * reset attendee transparency if event transparency has changed
         */
        Mapping<? extends Object, Event> mapping = EventMapper.getInstance().get(EventField.TRANSP);
        if (false == mapping.equals(originalMaster, updatedMaster)) {
            for (Event changeException : propagateFieldUpdate(originalMaster, updatedMaster, EventField.TRANSP, changedChangeExceptions, false)) {
                changeException.getAttendees().forEach(a -> a.setTransp(null));
            }
        }
        /*
        ...
        changedChangeExceptions = propagateAttendeeUpdates(attendeeUpdates, changedChangeExceptions, timestamp);
        /*
     ...
     * @return The (possibly adjusted) list of change exception events, might be empty
     */
    private static List<Event> propagateFieldUpdate(Event originalMaster, Event updatedMaster, EventField field, List<Event> changeExceptions, boolean overwrite) throws OXException {
        ...
        List<Event> changedChangeExceptions = new ArrayList<>(changeExceptions.size());
        for (Event changeException : changeExceptions) {
            if (overwrite || mapping.equals(originalMaster, changeException)) {
                ...
                changedChangeExceptions.add(changeException);
            }
        }
        return changedChangeExceptions;
    }
     ...
     * from each attendee list if contained. Furthermore, participant status resets of certain attendees are propagated to one or more event,
     * unless they don't attend a specific event
     *
     ...
     */
    private static List<Event> propagateAttendeeUpdates(InternalAttendeeUpdates attendeeUpdates, List<Event> changeExceptions, Date timestamp) {
        ...
        for (Event changeException : changeExceptions) {
            ...
            for (ItemUpdate<Attendee, AttendeeField> itemUpdate : attendeeUpdates.getResetedItems()) {
                /*
                 * Reset status of the attendee in the exception, too, if participant status status within the 
                 * exceptions matches the status of the series master, for which the status was already reseted
                 */
                Attendee attendee = CalendarUtils.find(changeException.getAttendees(), itemUpdate.getUpdate());
                if (null != attendee && itemUpdate.getOriginal().getPartStat().matches(attendee.getPartStat())) {
                    resetParticipationStatus(attendee, timestamp);
                }
            }
        }
        return changeExceptions;
    ...
    protected List<Event> removeInvalidRecurrenceIds(Event seriesMaster, List<Event> changeExceptions) throws OXException {
         ...
         */
        SortedSet<RecurrenceId> exceptionDates = combine(combine(seriesMaster.getDeleteExceptionDates(), seriesMaster.getChangeExceptionDates()), getRecurrenceIds(changeExceptions));
        ...
        if (false == isNullOrEmpty(seriesMaster.getDeleteExceptionDates())) {
            SortedSet<RecurrenceId> newDeleteExceptionDates = new TreeSet<>(seriesMaster.getDeleteExceptionDates());
        ...
        if (false == isNullOrEmpty(seriesMaster.getChangeExceptionDates())) {
            SortedSet<RecurrenceId> newChangeExceptionDates = new TreeSet<>(seriesMaster.getChangeExceptionDates());
        ...
        if (false == isNullOrEmpty(changeExceptions)) {
            List<Event> newChangeExceptions = new ArrayList<>(changeExceptions);
    ...
    private List<Event> ensureChangeExceptionConsistency(Event seriesMaster, List<Event> changeExceptions) {
         ...
         */
        List<Event> checkedChangeExceptions = new ArrayList<>(changeExceptions.size());
        SortedSet<RecurrenceId> checkedChangeExceptionDates = new TreeSet<>();
        for (Event changeException : changeExceptions) {
        ...
        if (false == originalSeriesStart.equals(updatedSeriesStart)) {
            ...
            if (false == isNullOrEmpty(originalChangeExceptions)) {
                ...
                for (Event changeException : changedChangeExceptions) {
                    ...
                    changeException.setChangeExceptionDates(new TreeSet<>(Collections.singleton(newRecurrenceId)));
                }
                return changedChangeExceptions;
            }
        }
     ...
     * {@link ParticipationStatus#NEEDS_ACTION} for a specific event. Same goes for resource attendees that cannot be booked directly.
     * This includes setting the attendee timestamp as well.
     *
    ...
    private void resetParticipationStatus(List<Attendee> attendees) throws OXException {
        for (Attendee attendee : attendees) {
            if (isInternal(attendee)) {
                if (CalendarUserType.INDIVIDUAL.matches(attendee.getCuType())) {
                    /*
                     * reset for all user attendees despite the calendar user
                     */
                    ...
                    /*
                     * reset for all 'managed' resources
                     */
                    if (false == canBookDirectly(session, attendee.getEntity())) {
                        resetParticipationStatus(attendee, timestamp);
                    }
                }
            } else {
                /*
                 * always reset for external attendees
                 */
                resetParticipationStatus(attendee, timestamp);
            }
        }
    ...
    /**
     * Resets the participation status of an individual attendees to
     * {@link ParticipationStatus#NEEDS_ACTION} for a specific event.
     *
     * @param attendees The attendee to reset the status for
     */
    private static void resetParticipationStatus(Attendee attendee, Date timestamp) {
    ...
    /**
     * Reset the attendee transparency for all attendees in the given event
     *
     * @param event The event to reset attendees on
     */
    private static void resetAttendeeTransparency(Event event) {
        event.getAttendees().forEach(a -> a.setTransp(null));
    }
