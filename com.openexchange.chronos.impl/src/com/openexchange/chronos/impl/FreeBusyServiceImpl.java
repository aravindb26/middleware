...
public class FreeBusyServiceImpl implements FreeBusyService, AdministrativeFreeBusyService {
    ...
    @Override
    public Map<Attendee, FreeBusyResult> getFreeBusy(int ctxId, List<Attendee> attendees, Date from, Date until, boolean merge, boolean includeDetails) throws OXException {
        ...
        return new OSGiCalendarStorageOperation<Map<Attendee, FreeBusyResult>>(Services.getServiceLookup(), ctxId, Utils.ACCOUNT_ID) {
            ...
            protected Map<Attendee, FreeBusyResult> call(CalendarStorage storage) throws OXException {
                return new AdministrativeFreeBusyPerformer(storage, optEntityResolver(), Optional.empty()).perform(attendees, from, until, merge, includeDetails);
            }
        }.executeQuery();
    }
