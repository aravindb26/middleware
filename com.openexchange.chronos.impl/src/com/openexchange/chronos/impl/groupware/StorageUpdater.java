...
public class StorageUpdater {
    ...
    private static final EventField[] SEARCH_FIELDS = { EventField.ID, EventField.SERIES_ID, EventField.RECURRENCE_ID, EventField.FOLDER_ID, EventField.CREATED_BY, EventField.MODIFIED_BY, EventField.CALENDAR_USER, EventField.ORGANIZER, EventField.ATTENDEES };
    ...
    public StorageUpdater(ServiceLookup services, CalendarStorage storage, EntityResolver entityResolver, CalendarEventNotificationService notificationService, int attendeeId, int destinationUserId) throws OXException {
        ...
        this.replacement = entityResolver.applyEntityData(new CalendarUser(), destinationUserId);
        this.replacement.setEntity(destinationUserId); // re-set entity in case of missing calendar access
    ...
    void removeAttendeeFrom(Event event) throws OXException {
        ...
        if (null != attendee) {
            ...
            Consistency.setModified(date, eventUpdate, replacement, hasExternalOrganizer(event));
    ...
    private void replaceOrganizer(Event event, Event eventUpdate, int contextId) throws OXException {
        Organizer organizer = entityResolver.applyEntityData(new Organizer(), replacement.getEntity());
        organizer.setEntity(replacement.getEntity()); // re-set entity in case of missing calendar access
        eventUpdate.setOrganizer(organizer);
        /*
        ...
        if (null == attendee) {
            ...
            attendee.setEntity(replacement.getEntity()); // re-set entity in case of missing calendar access
    ...
    private static void deleteEvents(CalendarStorage storage, CalendarFolder folder, List<Event> eventsToDelete) throws OXException {
        ...
        List<String> eventIdsWithAttachment = new ArrayList<String>();
        for (Event originalEvent : eventsToDelete) {
            ...
            if (null != originalEvent.getAttachments() && 0 < originalEvent.getAttachments().size()) {
                eventIdsWithAttachment.add(originalEvent.getId());
            }
        }
        /*
         * perform deletions
        ...
        storage.getAttendeeStorage().deleteAttendees(eventIds);
        storage.getEventStorage().deleteEvents(eventIds);
        if (0 < eventIdsWithAttachment.size()) {
            storage.getAttachmentStorage().deleteAttachments(folder.getSession(), folder.getId(), eventIdsWithAttachment);
        }
    }
