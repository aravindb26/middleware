...
public class CalendarConfigImpl implements CalendarConfig {
    ...
    @Override
    public AutoProcessIMip getAutoProcessIMip(int userId) throws OXException {
        return getUserSettings(userId).getAutoProcessIMip();
    }

    @Override
    public FreeBusyVisibility getFreeBusyVisibility(int userId) throws OXException {
        FreeBusyVisibility freeBusyVisibility = getUserSettings(userId).getFreeBusyVisibility();
        if (null == freeBusyVisibility || getBooleanPropertyValue(CalendarProperty.FREE_BUSY_VISIBILITY_PROTECTED)) {
            FreeBusyVisibility defaultValue = Enums.parse(FreeBusyVisibility.class, CalendarProperty.FREE_BUSY_VISIBILITY_DEFAULT.getDefaultValue(String.class));
            return Enums.parse(FreeBusyVisibility.class, getPropertyValue(CalendarProperty.FREE_BUSY_VISIBILITY_DEFAULT), defaultValue);
        }
        return freeBusyVisibility;
    }
    ...
    @Override
    public boolean isCrossContextFreeBusy() {
        return getBooleanPropertyValue(CalendarProperty.ENABLE_CROSS_CONTEXT_FREE_BUSY);
    }

    @Override
    public boolean isCrossContextConflictCheck() {
        return getBooleanPropertyValue(CalendarProperty.ENABLE_CROSS_CONTEXT_CONFLICTS);
    }
    ...
    private String getPropertyValue(Property property) {
        ...
        try {
            return Services.getService(LeanConfigurationService.class, true).getProperty(userId, contextId, property);
        } catch (OXException e) {
            LOG.warn("Error getting \"{}\", falling back to \"{}\"", property.getFQPropertyName(), property.getDefaultValue(), e);
            return property.getDefaultValue(String.class);
        }
    }

    private boolean getBooleanPropertyValue(Property property) {
        int userId = null == optSession ? -1 : optSession.getUserId();
        try {
            return Services.getService(LeanConfigurationService.class, true).getBooleanProperty(userId, contextId, property);
        } catch (OXException e) {
            LOG.warn("Error getting \"{}\", falling back to \"{}\"", property.getFQPropertyName(), property.getDefaultValue(), e);
