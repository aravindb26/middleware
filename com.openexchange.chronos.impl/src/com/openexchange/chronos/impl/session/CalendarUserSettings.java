...
public class CalendarUserSettings {
    ...
    /**
     * Gets the defined value for the auto-processing of iMIP mails from the underlying user configuration.
     *
     * @return The defined value or <code>null</code> if not defined
     * @throws OXException In case of error
     */
    @Nullable
    public AutoProcessIMip getAutoProcessIMip() throws OXException {
        UserConfigWrapper userConfig = getUserConfig(userId);
        if (null == userConfig) {
            LOG.debug("No user configuration available for user {} in context {}, unable to get autoProcessIMip.", I(userId), I(contextId));
            return null;
        }
        return userConfig.getAutoProcessIMip();
    }
    ...
    public FreeBusyVisibility getFreeBusyVisibility() throws OXException {
        UserConfigWrapper userConfig = getUserConfig(userId);
        if (null == userConfig) {
            LOG.debug("No user configuration available for user {} in context {}, unable to get freeBusyVisibility.", I(userId), I(contextId));
            return null;
        }
        return userConfig.getFreeBusyVisibility();
    }
    ...
    private CalendarAccount getAccount(int userId) throws OXException {
        ...
        return accountService.getAccount(optSession == null ? contextId : optSession.getContextId(), userId, PROVIDER_ID);
    }
