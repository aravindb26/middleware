...
public class SearchPerformer extends AbstractSearchPerformer {
    ...
    private Map<String, EventsResult> search(List<String> folderIds, SearchTerm<?> term, List<String> queries) throws OXException {
        /*
         * check search input, then get folders and/or directly handle virtual ids, storing a possible exception in the results
         */
        ...
        Map<String, EventsResult> resultsPerFolderId = new HashMap<String, EventsResult>();
        List<CalendarFolder> folders = initializeSearchedFolders(folderIds, term, resultsPerFolderId);
        /*
         * perform combined search in remaining folders, then associate found events with possible folder views
         */
        if (0 < folders.size()) {
            SearchTerm<?> searchTerm = buildSearchTerm(folders, term, queries, null == folderIds);
            EventField[] fields = getFields(session, EventField.ORGANIZER, EventField.ATTENDEES);
            List<Event> foundEvents = storage.getEventStorage().searchEvents(searchTerm, new SearchOptions(session), fields);
            foundEvents = storage.getUtilities().loadAdditionalEventData(-1, foundEvents, fields);
            for (Entry<String, List<Event>> entry : getEventsPerFolderId(foundEvents, folders).entrySet()) {
    ...
    private Map<String, List<Event>> getEventsPerFolderId(List<Event> events, List<CalendarFolder> folders) throws OXException {
        ...
        Map<String, List<Event>> eventsPerFolderId = new HashMap<String, List<Event>>(folders.size());
        for (Event event : events) {
            List<CalendarFolder> foldersForEvent = getFoldersForEvent(folders, event);
            if (foldersForEvent.isEmpty()) {
                if (null != event.getFolderId()) {
                    ...
                    if (Utils.isVisible(invisibleFolder, event) && false == isClassifiedFor(event, session.getUserId())) {
                        List<Event> processedEvents = postProcessor.process(event, invisibleFolder).getEvents(true);
                        com.openexchange.tools.arrays.Collections.put(eventsPerFolderId, invisibleFolder.getId(), processedEvents);
                    }
                }
            } else {
                for (int i = 1; i < foldersForEvent.size(); i++) {
                    Event copiedEvent = EventMapper.getInstance().copy(event, new Event(), (EventField[]) null);
                    List<Event> processedEvents = postProcessor.process(copiedEvent, foldersForEvent.get(i)).getEvents(true);
                    com.openexchange.tools.arrays.Collections.put(eventsPerFolderId, foldersForEvent.get(i).getId(), processedEvents);
                }
                List<Event> processedEvents = postProcessor.process(event, foldersForEvent.get(0)).getEvents(true);
    ...
    private SearchTerm<?> buildSearchTerm(List<CalendarFolder> folders, SearchTerm<?> term, List<String> queries, boolean includeUserAttendee) throws OXException {
        /*
        ...
        /*
         * combine term with additional term & queries as needed
         */
        if (null != term) {
            searchTerm = new CompositeSearchTerm(CompositeOperation.AND).addSearchTerm(searchTerm).addSearchTerm(term);
        }
