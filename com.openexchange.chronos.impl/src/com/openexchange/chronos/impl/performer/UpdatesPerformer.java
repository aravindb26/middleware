...
public class UpdatesPerformer extends AbstractQueryPerformer {
    ...
    public UpdatesResult perform(long timestamp) throws OXException {
        ...
        /*
         * count current events of user
         */
        long count = -1L;
        SearchTerm<?> calendarUserTerm = getSearchTerm(AttendeeField.ENTITY, SingleOperation.EQUALS, I(session.getUserId()));
        if (false == isEnforceDefaultAttendee(session)) {
            /*
             * also include not group-scheduled events associated with the calendar user
             */
            calendarUserTerm = new CompositeSearchTerm(CompositeOperation.OR)
                .addSearchTerm(getSearchTerm(EventField.CALENDAR_USER, SingleOperation.EQUALS, I(session.getUserId())))
                .addSearchTerm(calendarUserTerm);
        }
        if (false == com.openexchange.tools.arrays.Arrays.contains(ignore, "count")) {
            count = storage.getEventStorage().countEvents(calendarUserTerm);
        }
        /*
         * construct search term to determine changes
         */
        /*
        ...
        return new DefaultUpdatesResult(newAndModifiedEvents, deletedEvents, count);
    }
    ...
    public UpdatesResult perform(String folderId, long since) throws OXException {
        ...
        EventField[] fields = getFields(session, EventField.ATTENDEES, EventField.ORGANIZER);
        /*
         * count current events in folder
         */
        long count = -1L;
        SearchTerm<?> folderIdTerm = getFolderIdTerm(session, folder);
        if (false == com.openexchange.tools.arrays.Arrays.contains(ignore, "count")) {
            count = storage.getEventStorage().countEvents(folderIdTerm);
        }
        /*
         * construct search term to determine changes
         */
        CompositeSearchTerm searchTerm = new CompositeSearchTerm(CompositeOperation.AND)
            .addSearchTerm(folderIdTerm)
        ...
        if (isNullOrEmpty(deletedEvents) && isNullOrEmpty(newAndModifiedEvents)) {
             ...
             */
            return new DefaultUpdatesResult(newAndModifiedEvents, deletedEvents, since, false, count);
        }
        /*
         * limit returned results as requested
         */
        return getResult(newAndModifiedEvents, deletedEvents, count, 0 < searchOptions.getLimit() ? searchOptions.getLimit() - 1 : searchOptions.getLimit());
    }
    ...
    protected static DefaultUpdatesResult getResult(List<Event> newAndModifiedEvents, List<Event> deletedEvents, long count, int limit) {
        ...
        if (newAndModifiedTruncated) {
            ...
            return new DefaultUpdatesResult(newAndModifiedEvents, deletedEvents, timestamp, true, count);
        }
        if (deletedTruncated) {
            /*
