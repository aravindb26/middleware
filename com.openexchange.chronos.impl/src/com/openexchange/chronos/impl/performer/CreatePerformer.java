...
public class CreatePerformer extends AbstractUpdatePerformer {
    ...
    protected Event prepareNewEvent(Event eventData, List<Event> existingEvents) throws OXException {
         ...
         */
        EventMapper.getInstance().copy(eventData, event, EventField.DTSTAMP);
        Consistency.setCreated(session, timestamp, event, session.getUserId());
        Consistency.setModified(session, timestamp, event, session.getUserId(), hasExternalOrganizer(eventData));
         ...
         */
        if (!hasExternalOrganizer(event)) {
            Consistency.adjustRecurrenceRuleDeviation(event, session);
        }
        return event;
    }
