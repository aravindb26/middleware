...
public class ImportPerformer {
    ...
    public List<InternalImportResult> perform(List<Event> events) {
        ...
        Map<String, List<Event>> eventsByUID = getEventsByUID(events, true);
        eventsByUID = patch(eventsByUID);
    ...
    private List<InternalImportResult> importEventGroup(List<Event> eventGroup) {
        ...
        try {
            ...
            return Collections.singletonList(new InternalImportResult(new InternalCalendarResult(session, calendarUserId, folder),
                                                                      extractIndex(eventGroup.get(0)),
                                                                      extractWarnings(eventGroup.get(0)),
                                                                      e));
        }
    }
    ...
    private static List<OXException> extractWarnings(Event importedEvent) {
        if ((importedEvent instanceof ImportedComponent)) {
    ...
    /**
     * Patches the events as needed
     *
     * @param eventsByUID Event to import grouped by event UID
     * @return Events to import
     */
    private Map<String, List<Event>> patch(Map<String, List<Event>> eventsByUID) {
        Map<String, List<Event>> retVal = new LinkedHashMap<>(eventsByUID.size());
        for (List<Event> eventGroup : eventsByUID.values()) {
            if (isNullOrEmpty(eventGroup)) {
                continue;
            }
            boolean containsSeriesMaster = CalendarUtils.looksLikeSeriesMaster(eventGroup.get(0));
            for (int i = 0; i < eventGroup.size(); i++) {
                Event event = eventGroup.get(i);
                /*
                 * Workaround for bug 57282
                 * Remove attendees and organizer to avoid e.g. scheduling
                 */
                ...
                /*
                 * Remove recurrence IDs for orphan event instances
                 */
                if (false == containsSeriesMaster) {
                    event.removeRecurrenceId();
                    event.removeRecurrenceRule();
                    if (i > 0) {
                        /*
                         * Reassign UID automatically, to avoid conflicts between instances
                         * Afterwards, put into own UID group
                         */
                        event.setUid(UUID.randomUUID().toString());
                        retVal.put(event.getUid(), Collections.singletonList(event));
                    }
                }
            }
            retVal.put(eventGroup.get(0).getUid(), containsSeriesMaster ? eventGroup : Collections.singletonList(eventGroup.get(0)));
        }
        return retVal;
    }
