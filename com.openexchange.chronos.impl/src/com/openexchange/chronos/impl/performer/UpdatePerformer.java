...
import static com.openexchange.chronos.impl.performer.SplitPerformer.PROPERTY_X_OX_SPLIT_FROM;
...
public class UpdatePerformer extends AbstractUpdatePerformer {
    ...
    /** <i>Meta</i>-fields of events that are always allowed to be changed, independently of the calendar user's role */
    private static final Set<EventField> META_FIELDS = Collections.unmodifiableSet(EnumSet.of(EventField.DTSTAMP, SKIPPED_FIELDS));
    ...
    private void handleScheduling(InternalEventUpdate eventUpdate, Event updatedEvent, List<Event> updatedChangeExceptions) throws OXException {
        ...
        if (eventUpdate.isReschedule() && false == hasExternalOrganizer(eventUpdate.getOriginal())) {
            if (isSeriesMaster(eventUpdate.getOriginal())) {
                ...
                CalendarObjectResource updatedResource = new DefaultCalendarObjectResource(updatedEvent, loadExceptionData(updatedEvent));
        ...
        } else if (false == hasExternalOrganizer(eventUpdate.getOriginal()) && false == isNullOrEmpty(eventUpdate.getAttendeeUpdates().getResetedItems())) {
            /*
             * Re-invite attendees
             */
            CalendarObjectResource updatedResource = new DefaultCalendarObjectResource(updatedEvent, updatedChangeExceptions);
            List<Attendee> attendees = eventUpdate.getAttendeeUpdates().getResetedItems().stream().map(i -> i.getUpdate()).collect(Collectors.toList());
            schedulingHelper.trackUpdate(updatedResource, null, eventUpdate, attendees);
        } else {
            /*
    ...
    protected InternalUpdateResult updateRecurrence(Event originalSeriesMaster, RecurrenceId recurrenceId, Event updatedEventData, EventField... ignoredFields) throws OXException {
        ...
        if (contains(originalSeriesMaster.getDeleteExceptionDates(), recurrenceId) && false == hasExternalOrganizer(originalSeriesMaster)) {
            /*
             * cannot update a delete exception (for internally organized events)
             */
        ...
        if (null != recurrenceId.getRange()) {
            ...
            /*
             * track scheduling messages and notifications for the newly created, detached series (externals, only - masked as update for comprehensible annotations)
             */
            CalendarObjectResource detachedSeries = splitResult.getKey();
            if (null != detachedSeries && null != detachedSeries.getSeriesMaster()) {
                EventUpdate detachedSeriesEventUpdate = getDetachedSeriesUpdate(
                    originalSeriesMaster.getRecurrenceRule(), optExtendedProperty(originalSeriesMaster, PROPERTY_X_OX_SPLIT_FROM), detachedSeries.getSeriesMaster());
                schedulingHelper.trackUpdate(detachedSeries, null, detachedSeriesEventUpdate, collectAttendees(detachedSeries, Boolean.FALSE, (CalendarUserType[]) null));
            }
            /*
             ...
             */
            EventField[] additionallyIgnoredFields = hasExternalOrganizer(originalSeriesMaster) ? new EventField[] { EventField.ID, EventField.RECURRENCE_RULE } : 
                new EventField[] { EventField.ID, EventField.RECURRENCE_RULE, EventField.SEQUENCE };
            InternalUpdateResult result = updateEvent(newExceptionEvent, updatedEventData, Arrays.add(additionallyIgnoredFields, ignoredFields));
    ...
    protected InternalUpdateResult updateEvent(Event originalEvent, Event eventData, EventField... ignoredFields) throws OXException {
        ...
        /*
         * prepare event update, check UID conflicts and conflicting events for newly added attendees as needed
         */
        ...
        Check.uidIsUnique(session, storage, originalEvent, eventUpdate.getAttendeeUpdates().getAddedItems());
     ...
     * <li>the organizer matches in the original and in the updated event (including some relaxed check for Apple iCloud)</li>
    ...
    private boolean assumeExternalOrganizerUpdate(Event originalEvent, Event updatedEvent) {
        if (hasExternalOrganizer(originalEvent) && Objects.equals(originalEvent.getUid(), updatedEvent.getUid()) && updatedEvent.getSequence() >= originalEvent.getSequence() && 
            (matches(originalEvent.getOrganizer(), updatedEvent.getOrganizer()) || isSimilarICloudIMipMeCom(originalEvent.getOrganizer(), updatedEvent.getOrganizer()))) {
            return true;
        }
        return false;
    }
    ...
    private boolean needsConflictCheck(EventUpdate eventUpdate) throws OXException {
        ...
        if (needsConflictCheck(eventUpdate.getAttendeeUpdates().getUpdatedItems())) {
            /*
             * (re-)check conflicts if at least one attendee's participation status changes from DECLINED
             */
            return true;
        }
        return false;
    ...
    private boolean updateDeleteExceptions(Event originalEvent, Event updatedEvent) throws OXException {
        if (isSeriesMaster(originalEvent) && false == isNullOrEmpty(updatedEvent.getDeleteExceptionDates())) {
            ...
            SortedSet<RecurrenceId> originalDeleteExceptionDates = Utils.applyExceptionDates(storage, folder, originalEvent).getDeleteExceptionDates();
    ...
    protected boolean storeEventUpdate(Event originalEvent, Event deltaEvent, Set<EventField> updatedFields, boolean assumeExternalOrganizerUpdate) throws OXException {
        Set<EventField> updatedEventFields = new HashSet<EventField>(updatedFields);
        updatedEventFields.removeAll(java.util.Arrays.asList(EventField.ATTACHMENTS, EventField.ALARMS, EventField.ATTENDEES, EventField.CONFERENCES));
        if (updatedEventFields.isEmpty()) {
            return false;
        }
        if (updatedEventFields.stream().allMatch((updatedField) -> META_FIELDS.contains(updatedField))) {
            /*
             * only metadata fields changed, require attendee-centric write permissions based on current folder view
             */
            requireWritePermissions(originalEvent, calendarUser, assumeExternalOrganizerUpdate);
        } else {
            /*
             * change beyond metadata fields, require organzizer write permissions
             */
            requireWritePermissions(originalEvent, assumeExternalOrganizerUpdate);
        }
    ...
    private final static AttendeeField[] CHANGEABLE_ATTENDEE_FIELDS = { AttendeeField.RSVP, AttendeeField.HIDDEN, AttendeeField.COMMENT, AttendeeField.PARTSTAT, AttendeeField.ROLE,//
        AttendeeField.EXTENDED_PARAMETERS, AttendeeField.TIMESTAMP, AttendeeField.SEQUENCE, AttendeeField.TRANSP };
    ...
    protected boolean storeAttendeeUpdates(Event originalEvent, CollectionUpdate<Attendee, AttendeeField> attendeeUpdates, boolean assumeExternalOrganizerUpdate) throws OXException {
        ...
        if (0 < updatedItems.size()) {
            ...
            for (ItemUpdate<Attendee, AttendeeField> attendeeToUpdate : updatedItems) {
                ...
                AttendeeMapper.getInstance().copy(attendeeToUpdate.getUpdate(), newAttendee, CHANGEABLE_ATTENDEE_FIELDS);
         ...
         */
        List<Attendee> addedItems = attendeeUpdates.getAddedItems();
        if (0 < addedItems.size()) {
            requireWritePermissions(originalEvent, assumeExternalOrganizerUpdate);
            Check.uidIsUnique(session, storage, originalEvent, addedItems);
            for (Attendee attendee : addedItems) {
                attendee.setTimestamp(timestamp.getTime());
                attendee.setSequence(originalEvent.getSequence());
            }
            storage.getAttendeeStorage().insertAttendees(originalEvent.getId(), addedItems);
        }
        return true;
    }
    ...
    protected static class InternalUpdateResult {
        ...
        /**
         * Gets the event update as it was prepared prior saving it to the storage.
         * 
        ...
        /**
         * Gets the updated event after it was persisted in the storage.
         * 
