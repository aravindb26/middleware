...
public class WebSocketServiceImpl implements WebSocketService {
    ...
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(WebSocketServiceImpl.class);
    ...
    public boolean exists(String pathFilter, int userId, int contextId) throws OXException {
        ...
        if (localApp.existsAny(pathFilter, userId, contextId)) {
            LOG.debug("Found node-local Web Socket for path-filter '{}' for user {} in context {}", pathFilter, I(userId), I(contextId));
            return true;
        }
        LOG.debug("No node-local Web Socket for path-filter '{}' for user {} in context {}. Checking for remote ones...", pathFilter, I(userId), I(contextId));

        boolean foundRemoteOne = remoteDistributor.existsAnyRemote(pathFilter, userId, contextId);
        if (foundRemoteOne) {
            LOG.debug("Found remote Web Socket for path-filter '{}' for user {} in context {}", pathFilter, I(userId), I(contextId));
        } else {
            LOG.debug("No remote Web Socket for path-filter '{}' for user {} in context {}", pathFilter, I(userId), I(contextId));
        }
        return foundRemoteOne;
    }
