...
public class DefaultGrizzlyWebSocketApplication extends AbstractGrizzlyWebSocketApplication<DefaultSessionBoundWebSocket> {
    ...
    private static final AtomicReference<DefaultGrizzlyWebSocketApplication> APPLICATION_REFERENCE = new AtomicReference<>();
     ...
     * @param ipUtils The ip utils
     * @param services The service look-up
     * @return The newly created (or existing) instance
     */
    public static DefaultGrizzlyWebSocketApplication initializeGrizzlyWebSocketApplication(WebSocketListenerRegistry listenerRegistry, // @formatter:off
                                                                                           RemoteWebSocketDistributor remoteDistributor,
                                                                                           GrizzlyConfig config,
                                                                                           ClientIPUtil ipUtils,
                                                                                           ServiceLookup services) { // @formatter:on
        ...
        do {
            ...
            newApp = new DefaultGrizzlyWebSocketApplication(listenerRegistry, remoteDistributor, config, ipUtils, services);
     ...
     * @param remoteDistributor
     * @param config
     * @param ipUtils
     * @param services
     */
    private DefaultGrizzlyWebSocketApplication(WebSocketListenerRegistry listenerRegistry,
                                               RemoteWebSocketDistributor remoteDistributor,
                                               GrizzlyConfig config,
                                               ClientIPUtil ipUtils,
    ...
    protected DefaultSessionBoundWebSocket doCreateSocket(ConnectionId connectionId, Parameters parameters, ProtocolHandler handler, HttpRequestPacket requestPacket, WebSocketListener[] listeners) {
        ...
        return new DefaultSessionBoundWebSocket(connectionId, path, parameters, handler, requestPacket, config, ipUtils, effectiveListeners);
    }
