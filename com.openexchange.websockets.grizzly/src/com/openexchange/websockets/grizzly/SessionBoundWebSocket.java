...
public class SessionBoundWebSocket extends DefaultWebSocket {
     ...
     * @param request The http header
     * @param config The grizzly config
     * @param ipUtil The ip utils
     * @param listeners An array of listeners
     */
    public SessionBoundWebSocket(ConnectionId connectionId, String path, ProtocolHandler protocolHandler, HttpRequestPacket request, GrizzlyConfig config, ClientIPUtil ipUtil, WebSocketListener... listeners) {
        super(protocolHandler, request, listeners);
        this.ipUtil = ipUtil;
        this.sessionInfoReference = new AtomicReference<>(null);
        this.connectionId = connectionId;
        this.path = path;
        this.config = config;
        this.wrappedRequest = buildHttpServletRequestWrapper(this.servletRequest);
        individualWebSocketListeners = initIndividualListenerState(this.listeners);
        LOG.debug("Initialized SessionBoundWebSocket '{}' with listeners: {}", connectionId, this.listeners);
    }
    ...
    private static Map<Class<? extends IndividualWebSocketListener>, WebSocketListenerAdapter> initIndividualListenerState(Collection<WebSocketListener> listeners) {
        ...
        for (WebSocketListener l : listeners) {
            if (l instanceof IndividualWebSocketListenerAdapter il) {
                map.put(il.getListenerClass(), il);
            } else if (l instanceof WebSocketListenerAdapter la) {
    ...
    public synchronized void addListenersIfAbsent(Collection<WebSocketListener> listenersToAdd) {
        ...
        for (WebSocketListener grizzlyWebSocketListener : listenersToAdd) {
            if (grizzlyWebSocketListener instanceof IndividualWebSocketListenerAdapter individualWebSocketListener) {
    ...
    public synchronized boolean removeListener(WebSocketListener listenerToRemove) {
        ...
        if (!(listenerToRemove instanceof IndividualWebSocketListenerAdapter individualWebSocketListener)) {
    ...
    private WebsocketServletRequestWrapper buildHttpServletRequestWrapper(HttpServletRequest httpRequest) {
        ...
        String remoteAddress = ipUtil.getIP(httpRequest);
