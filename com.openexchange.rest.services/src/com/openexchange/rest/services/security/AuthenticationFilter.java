...
public class AuthenticationFilter implements ContainerRequestFilter {
    ...
    /**
     * Initializes a new {@link AuthenticationFilter}.
     *
    ...
    /**
     * Initializes the master admin authenticator
     *
        ...
        try (BufferedReader bf = new BufferedReader(new InputStreamReader(new FileInputStream(file), Charsets.UTF_8), 2048)) {
            for (String line; (line = bf.readLine()) != null;) {
                if (!line.startsWith("#") && line.indexOf(':') >= 0) {
                    ...
                    String[] tokens = Strings.splitByColon(line);
                    if (tokens.length != 3) {
    ...
    /**
     * Initializes the default {@link EndpointAuthenticator}
     *
     * @param configurationService The {@link ConfigurationService}
    ...
    private EndpointAuthenticator acquireAuthenticator() {
        ...
        if (resourceInstance instanceof EndpointAuthenticator) {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        // ... again using generic 'javax.annotation.security.RolesAllowed' annotation class
        {
            ...
            if (null != rolesAllowed) {
                ...
                // Other roles unknown...
                LOG.warn("Encountered unknown roles '{}' in class {}", LogUtility.toStringObjectFor(roles), resourceInfo.getResourceClass().getName());
                deny(requestContext);
                return;
            }
        }
     ...
     */
    private static void authenticatorAuth(EndpointAuthenticator authenticator, ContainerRequestContext requestContext, Method invokedMethod) {
        ...
        boolean authenticated = authenticator.authenticate(credentials.getLogin(), credentials.getPassword(), invokedMethod, requestContext);
     ...
     * @param requestContext The {@link RequestContext}
     */
    private static void reflectAuthenticated(boolean authenticated, String realm, ContainerRequestContext requestContext) {
        if (authenticated) {
     ...
     * @param requestContext The {@link RequestContext}
     */
    private static void deny(ContainerRequestContext requestContext) {
     ...
     */
    private static boolean hasRole(String roleToCheck, String[] roles) {
        if (null == roles) {
            return false;
        }
     ...
     */
    private static Credentials acquireCredentialsFromAuthHeader(String authHeader) {
        if (null == authHeader) {
