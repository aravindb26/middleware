...
public class ExportFileNameCreator {
    ...
    public static String createFolderExportFileName(ServerSession session, String folder, String extension) {
        ...
        try {
            ...
            prefix = folderObj.getName(session.getUser().getLocale());
        } catch (OXException e) {
            LOG.debug("", e);
    ...
    private static String createBatchExportFileName(ServerSession session, String folder, String batchId) {
        ...
        try {
            ...
            if (null == batchId || batchId.equals("")) {
                sb.append(getLocalizedFileName(session, folderObj.getName(session.getUser().getLocale())));
            } else {
                if (ContactsContentType.getInstance().equals(folderObj.getContentType()) || AddressDataContentType.getInstance().equals(folderObj.getContentType())) {
    ...
    private static String createSingleContactName(ServerSession session, String folder, String batchId) throws OXException {
        ...
        Contact contactObj = ImportExportServices.getContactsAccess().getContact(session, folder, batchId, null);
