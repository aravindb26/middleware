 ...
 */
public class CSVContactImporter extends ContactImporter {
    ...
    public boolean canImport(final ServerSession session, final Format format, final List<String> folders, final Map<String, String[]> optionalParams) throws OXException {
        ...
        try {
            return ImportExportServices.getContactsAccess().canCreateObjectsInFolder(session, folders.get(0));
    ...
    public ImportResults importData(final ServerSession sessObj, final Format format, final InputStream is, final List<String> folders, final Map<String, String[]> optionalParams) throws OXException {
        ...
        for (Contact contact : contacts) {
            try {
                createContact(sessObj, contact, folder);
            } catch (OXException e) {
                errors.add(e);
            }
        }
        ...
        for (final ImportIntention intention : intentions) {
            ...
            if (intention.exceedsLimit) {
                ...
                result.setObjectId(intention.contact.getId());
    ...
    protected ImportIntention createIntention(final List<String> fields, final List<String> entry, final String folder, final ContactSwitcher conSet, final int lineNumber, final ServerSession session) {
        ...
        try {
            ...
            contactObj.setFolderId(folder.trim());
            ...
            if (e.getCategory() != Category.CATEGORY_TRUNCATED || (e.getCategory() == Category.CATEGORY_TRUNCATED && !canOverrideInCaseOfTruncation)) {
                ...
                addErrorInformation(result, lineNumber);
            }
        }
        return new ImportIntention(result);
    }
     ...
     */
    protected void addErrorInformation(final ImportResult result, final int lineNumber) {
