        ...
        try {
            ...
            if (CometEvent.Type.NOTIFY == event.getType()) {
                ...
                if (null != driveEvent && driveEvent.getContextID() == session.getServerSession().getContextId()) {
                    Set<String> folderIDs = driveEvent.getFolderIDs(session.getServerSession().getUserId());
                    if (null != folderIDs && null != rootFolderIDs) {
                        write(driveEvent.getActions(session.getServerSession().getUserId(), rootFolderIDs, SubscriptionMode.SEPARATE.equals(mode)));
                    }
                }
            }
        } finally {
