...
public class TimeRange implements Comparable<TimeRange> {
    ...
    /**
     * Folds the given sorted(!) time ranges that is to combine two consecutive time ranges; e.g. <code>"0-6,6-12"</code> becomes <code>"0-12"</code>.
     *
     * @param timeRanges The time ranges to fold
     * @return The folded time ranges
     */
    public static List<TimeRange> foldTimeRanges(List<TimeRange> timeRanges) {
        List<TimeRange> toFold = new ArrayList<>(timeRanges);

        boolean anythingFolded = false;
        boolean fold = true;
        while (fold && toFold.size() > 1) {
            fold = false;
            int i = 0;
            TimeRange nu = null;
            while (nu == null && i < toFold.size()) {
                TimeRange timeRange = toFold.get(i);
                if (i < toFold.size() - 1) {
                    TimeRange next = toFold.get(i + 1);
                    if (timeRange.getEnd().equals(next.getStart())) {
                        nu = new TimeRange(timeRange.getStart(), next.getEnd());
                    }
                }
                if (nu == null) {
                    i++;
        ...
        return anythingFolded ? toFold : timeRanges;
    }
    ...
    public boolean overlapsWith(TimeRange range) {
        return range == null ? false : (start.isEarlierThan(range.end) && end.isLaterThan(range.start));
    }
