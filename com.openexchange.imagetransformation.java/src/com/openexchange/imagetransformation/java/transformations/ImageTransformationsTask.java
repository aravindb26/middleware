...
public final class ImageTransformationsTask extends ImageTransformationsImpl {
    ...
    protected BufferedImage getImage(String formatName, ImageTransformationSignaler signaler) throws IOException {
        ...
        return waitTimeoutSeconds <= 0 ? getImageWithoutTimeout(formatName, signaler) : getImageWithTimeout(formatName, signaler, waitTimeoutSeconds);
    }

    private BufferedImage getImageWithoutTimeout(String formatName, ImageTransformationSignaler signaler) throws IOException {
        ...
        boolean success = submitTask(ft);
        if (!success) {
            throw new IOException("Image transformation rejected: Too many queued image transformation tasks");
        }
    ...
    private BufferedImage getImageWithTimeout(String formatName, ImageTransformationSignaler signaler, int waitTimeoutSeconds) throws IOException, InterruptedIOException {
    ...
    private boolean submitTask(Runnable task) throws IOException {
        try {
            return Scheduler.getInstance().execute(optSource, ProcessorTasks.processorTaskFor(task));
        } catch (OXException e) {
