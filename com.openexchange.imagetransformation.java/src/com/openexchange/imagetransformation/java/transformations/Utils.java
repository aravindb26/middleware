...
public class Utils {
        ...
        try {
            ...
            if (null == reader) {
                ...
                if (false == readers.hasNext()) {
                    throw new NoSuchImageReader("No image reader available for format " + optContentType);
                }
                reader = readers.next();
                LoggerHolder.LOG.trace("Using {} for indicated format \"{}\".", reader.getClass().getName(), optContentType);
            }
     ...
     */
    public static BufferedImage removeTransparencyIfNeeded(BufferedImage image, String formatName, TransformationContext transformationContext) {
    ...
    public static ImageInputStream getImageInputStream(IFileHolder imageFile) throws IOException {
        try {
             ...
             */
            if ((imageFile instanceof ThresholdFileHolder)) {
