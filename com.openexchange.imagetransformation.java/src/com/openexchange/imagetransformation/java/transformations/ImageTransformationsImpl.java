...
public class ImageTransformationsImpl implements ImageTransformations {
    ...
    static int waitTimeoutSeconds() {
        return ImageTransformationConfig.getConfig().getTransformationWaitTimeoutSeconds();
    }

    static long maxSize() {
        return ImageTransformationConfig.getConfig().getTransformationMaxSize();
    }

    static long maxResolution() {
        return ImageTransformationConfig.getConfig().getTransformationMaxResolution();
    }

    static float preferThumbnailThreshold() {
        return ImageTransformationConfig.getConfig().getTransformationPreferThumbnailThreshold();
    }
    ...
    public ImageTransformations scale(int maxWidth, int maxHeight, ScaleType scaleType, boolean shrinkOnly) {
        int maxWidthLimit = ImageTransformationConfig.getConfig().getMaxWidth();
        if (maxWidth > maxWidthLimit) {
            throw new IllegalArgumentException("Width " + maxWidth + " exceeds max. supported width " + maxWidthLimit);
        }
        int maxHeightLimit = ImageTransformationConfig.getConfig().getMaxHeight();
        if (maxHeight > maxHeightLimit) {
            throw new IllegalArgumentException("Height " + maxHeight + " exceeds max. supported height " + maxHeightLimit);
        }
    ...
    public Optional<BasicTransformedImage> optTransformedImage(String formatName) throws IOException {
        return Optional.ofNullable(optFullTransformedImage(formatName).orElse(null));
    }
    ...
    public Optional<TransformedImage> optFullTransformedImage(String formatName) throws IOException {
        String imageFormat = getImageFormat(formatName);
        BufferedImage bufferedImage = getImage(imageFormat, null);
        return transformationContext.isTransformed() ? Optional.of(writeTransformedImage(bufferedImage, imageFormat)) : Optional.empty();
    }
    ...
    protected BufferedImage getImage(String formatName, ImageTransformationSignaler signaler) throws IOException {
        ...
        image = removeTransparencyIfNeeded(image, formatName, transformationContext);
        return image;
    }
