...
public class DataExportCancelCLT extends AbstractRmiCLI<Void> {
    ...
    protected Void invoke(Options options, CommandLine cmd, String optRmiHostName) throws Exception {
        ...
        if (userId > 0) {
            rmiService.cancelDataExportTask(userId, contextId, new Credentials(adminUser, adminPassword));
            System.out.println("Requested to cancel data export for user " + userId + " in context " + contextId);
        } else {
            rmiService.cancelDataExportTasks(contextId, new Credentials(adminUser, adminPassword));
