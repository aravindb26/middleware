...
public class DefaultShareService implements ShareService {
        ...
        do {
            try {
                ...
                if (++count < MAX_RETRIES) {
                    ...
                    /*
                     * try again in case of invalid/duplicate permissions during concurrent updates
                     */
                    if ("FLD-1039".equals(e.getErrorCode())) {
                        LOG.info("Detected invalid permissions during link creation: \"{}\" - trying again ({}/{})...", e.getMessage(), I(count), I(MAX_RETRIES));
                        continue;
                    }
                }
                throw e;
    ...
    public ShareLink updateLink(Session session, ShareTarget target, LinkUpdate linkUpdate, Date clientTimestamp) throws OXException {
        return updateLink(session, target, linkUpdate, clientTimestamp, true);
    }

    @Override
    public ShareLink updateLink(Session session, ShareTarget target, LinkUpdate linkUpdate, Date clientTimestamp, boolean touchTarget) throws OXException {
        /*
        ...
        try {
            ...
            if (guestUserUpdated || folderPermissionUpdate) {
                ...
                shareInfo = optLinkShare(session, context, targetProxy, connectionHelper);
    ...
    private Pair<Boolean, ShareInfo> prepareShare(ConnectionHelper connectionHelper, Session session, ShareRecipient recipient, ShareTarget shareTarget) throws OXException {
         ...
         */
        if ((recipient instanceof AnonymousRecipient) && false == dstTarget.getFolderToLoad().equals(dstTarget.getFolder())) {
    ...
    private Pair<Boolean, User> getGuestUser(Connection connection, Context context, User sharingUser, int permissionBits, ShareRecipient recipient, ShareTarget target) throws OXException {
        ...
        } else if ((recipient instanceof InternalRecipient)) {
    ...
    private ShareTarget getLinkTarget(int contextId, User guestUser) throws OXException {
        if (guestUser.isGuest() && ShareTool.isAnonymousGuest(guestUser)) {
            try {
                ...
                return ShareTool.jsonToTarget(JSONServices.parseObject(targetAttr));
    ...
    private DefaultShareLink getOrCreateLink(Session session, ShareTarget target) throws OXException {
        ...
        try {
            ...
            AnonymousRecipient recipient = new AnonymousRecipient(LINK_PERMISSION_BITS, null, null, true);
    ...
    private void removeGuestReferences(List<GuestInfo> createdGuests) {
        ...
        try {
            ...
            if (null == guestService) {
                LOG.error("{} is missing. Can't remove orphaned entries in globalDB for guest users: [{}]", GuestService.class.getSimpleName(), createdGuests);
                return;
            }
