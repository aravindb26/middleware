    ...
    public boolean checkPassword(String candidate, String encoded, byte[] salt) throws OXException {
        try {
            ...
            if (CryptoErrorMessage.LegacyEncryption.equals(e)) {
                @SuppressWarnings("deprecation") String decodedLegacy = cryptoService.decrypt(new EncryptedData(encoded, null), cryptKey, false);
                if (candidate.equals(decodedLegacy)) {
                    // TODO Can this still happen? UpdateTask should have done this
                    LOG.info("Legacy encryption found for anonymous share. Will be recrypted when sharing user logs in again.");
                    return true;
                }
            }
            throw e;
