...
public class PortableFolderContentChange implements CustomPortable, VersionedPortable {
    ...
    private static final String PARAMETER_MYFILES_USER_ID = "u";
    ...
    static ClassDefinition CLASS_DEFINITION = new ClassDefinitionBuilder(FACTORY_ID, CLASS_ID, CLASS_VERSION)
        .addStringField(PARAMETER_FOLDER_ID)
        .addIntField(PARAMETER_MYFILES_USER_ID)
        .addStringArrayField(PARAMETER_PATH_TO_ROOT)
    .build();

    private String folderId;
    private List<IdAndName> pathToRoot;
    private int myFilesUserId;
    ...
    public void writePortable(PortableWriter writer) throws IOException {
        writer.writeString(PARAMETER_FOLDER_ID, folderId);
        writer.writeInt(PARAMETER_MYFILES_USER_ID, myFilesUserId);
        writer.writeStringArray(PARAMETER_PATH_TO_ROOT, encode(pathToRoot));
    }

    @Override
    public void readPortable(PortableReader reader) throws IOException {
        folderId = reader.readString(PARAMETER_FOLDER_ID);
        myFilesUserId = reader.readInt(PARAMETER_MYFILES_USER_ID);
        pathToRoot = decode(reader.readStringArray(PARAMETER_PATH_TO_ROOT));
    }
    ...
    public static Portable[] wrap(List<DriveContentChange> contentChanges) {
        ...
        for (DriveContentChange contentChange : contentChanges) {
            ...
            portableContentChange.myFilesUserId = contentChange.getMyFilesUserId();
    ...
    public static List<DriveContentChange> unwrap(Portable[] portableContentChanges) {
        ...
        for (Portable portable : portableContentChanges) {
            ...
            folderContentChanges.add(new DriveContentChangeImpl(portableContentChange.folderId, portableContentChange.pathToRoot, portableContentChange.myFilesUserId));
        }
