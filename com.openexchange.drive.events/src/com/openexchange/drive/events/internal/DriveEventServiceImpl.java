...
public class DriveEventServiceImpl implements org.osgi.service.event.EventHandler, DriveEventService {
    ...
    /** The identifier drive root folder, which is the system infostore folder */
    static final String DRIVE_ROOT_ID = Integer.toString(FolderObject.SYSTEM_INFOSTORE_FOLDER_ID);

    /** The identifier shared files folder, which is the userstore folder */
    static final String SHARED_FILES_ID = Integer.toString(FolderObject.SYSTEM_USER_INFOSTORE_FOLDER_ID);

    /** The static id and name of the shared files folder, which is the userstore folder */
    static final IdAndName SHARED_FILES_ID_AND_NAME = new IdAndName(SHARED_FILES_ID, FolderStrings.SYSTEM_USER_FILES_FOLDER_NAME);

    /** The name of the my files folder */
    static final String MY_FILES_NAME = FolderStrings.DEFAULT_FILES_FOLDER_NAME;
    ...
    void notifyPublishers(FolderBuffer buffer) {
        if (null != buffer) {
            DriveEventImpl driveEvent = buffer.drain();
            if (null != driveEvent) {
                notifyPublishers(driveEvent);
            }
        }
    }
