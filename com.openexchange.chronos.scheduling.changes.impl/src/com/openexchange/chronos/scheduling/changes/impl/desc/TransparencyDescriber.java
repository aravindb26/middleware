...
public class TransparencyDescriber extends AbstractChangeDescriber<Transp> {
    ...
    public List<SentenceImpl> describe(Transp original, Transp updated) {
        if ((null == original || Transp.OPAQUE.equals(original.getValue())) && (null == updated || Transp.OPAQUE.equals(updated.getValue()))) {
            return Collections.emptyList(); // OPAQUE by default, so no effective change
        }
