...
public class ChangesUtils {
        ...
        public int compare(Attendee a1, Attendee a2) {
            if (CalendarUtils.isInternal(a1, cuType)) {
                if (CalendarUtils.isInternal(a2, cuType)) {
                    // Both internal users
                    return Strings.compare(a1.getCn(), a2.getCn());
                }
                return -1;
            }
            if (CalendarUtils.isInternal(a2, cuType)) {
    ...
    /**
     * Constructs a {@link FormattableArgument} for a date-time in a certain formatting style.
     * 
     * @param dateTime The date-time to format
     * @param style The formatting style. @see {@link DateFormat#getDateInstance(int, Locale)}
     * @return A formattable argument for the date-time
     */
    public static FormattableArgument getDateArgument(DateTime dateTime, int style) {
        return (MessageContext context) -> {
            ...
            dateFormat.setTimeZone(dateTime.isAllDay() ? TimeZone.getTimeZone("UTC") : context.getTimeZone());
            return dateFormat.format(new Date(dateTime.getTimestamp()));
        };
    }
