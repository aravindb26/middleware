...
public class DriveServiceImpl implements DriveService {
    ...
    private SyncResult<DirectoryVersion> syncFolders(DriveSession session, List<DirectoryVersion> originalVersions, List<DirectoryVersion> clientVersions, VersionsProvider<ServerDirectoryVersion> serverVersionsProvider) throws OXException {
        ...
        while (true) {
            ...
            if (driveSession.getStorage().isInTrash(rootFolder, DriveConstants.ROOT_PATH)) {
                return getErrorResult(driveSession, DriveExceptionCodes.NOT_SYNCHRONIZABLE_DIRECTORY_IN_TRASH.create(session.getRootFolderID()), true);
            }
            ...
            String pathToRoot = driveSession.getStorage().getInternalPath(session.getRootFolderID(), null);
            String localizedPathToRoot = driveSession.getStorage().getInternalPath(session.getRootFolderID(), session.getLocale());
            return new DefaultSyncResult<DirectoryVersion>(actionsForClient, driveSession.getDiagnosticsLog(), quota, pathToRoot, localizedPathToRoot);
        }
    }
    ...
    public SyncResult<FileVersion> syncFiles(DriveSession session, final String path, List<FileVersion> originalVersions, List<FileVersion> clientVersions) throws OXException {
        ...
        while (true) {
            ...
            if (driveSession.getStorage().isInTrash(rootFolder, DriveConstants.ROOT_PATH)) {
                return getErrorResult(driveSession, path, DriveExceptionCodes.NOT_SYNCHRONIZABLE_DIRECTORY_IN_TRASH.create(session.getRootFolderID()), true);
            }
            if (driveSession.getStorage().isExcludedSubfolder(rootFolder, DriveConstants.ROOT_PATH)) {
            ...
            String pathToRoot = driveSession.getStorage().getInternalPath(session.getRootFolderID(), null);
            String localizedPathToRoot = driveSession.getStorage().getInternalPath(session.getRootFolderID(), session.getLocale());
            return new DefaultSyncResult<FileVersion>(actionsForClient, driveSession.getDiagnosticsLog(), quota, pathToRoot, localizedPathToRoot);
        }
    }
    ...
    public SyncResult<FileVersion> upload(DriveSession session, String path, InputStream uploadStream, FileVersion originalVersion,
        ...
        if (syncSession.isTraceEnabled()) {
            syncSession.trace("Handling upload: original version: " + originalVersion + ", new version: " + newVersion +
                ...
                ", created: " + (null != created ? DriveConstants.LOG_FAST_DATE_FORMAT.format(created) : "") +
                ", modified: " + (null != modified ? DriveConstants.LOG_FAST_DATE_FORMAT.format(modified) : ""));
        }
    ...
    public DriveSettings getSettings(DriveSession session) throws OXException {
        ...
        settings.setServerVersion(DriveServiceLookup.getService(VersionService.class, true).getVersion().toString());
        ...
        settings.setPathToRoot(storage.getInternalPath(session.getRootFolderID(), null));
        settings.setLocalizedPathToRoot(storage.getInternalPath(session.getRootFolderID(), session.getLocale()));
