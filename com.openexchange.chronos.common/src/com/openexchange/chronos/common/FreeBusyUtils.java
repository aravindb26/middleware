...
public class FreeBusyUtils {
    ...
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(FreeBusyUtils.class);
    ...
    public static FreeBusyResult combine(List<FreeBusyResult> freeBusyResults) {
        ...
        List<FreeBusyTime> freeBusyTimes = null;
        for (FreeBusyResult result : freeBusyResults) {
            ...
            if (null != result.getFreeBusyTimes()) {
                if (null == freeBusyTimes) {
                    freeBusyTimes = new ArrayList<FreeBusyTime>();
                }
                freeBusyTimes.addAll(result.getFreeBusyTimes());
            }
        }
        if (null != freeBusyTimes && null != warnings) {
            for (OXException warning : warnings) {
                LOG.debug("Omitting warning from combined free/busy result: {}", warning.getMessage(), warning);
            }
            warnings = null;
        }
        if (null != freeBusyTimes && 1 < freeBusyTimes.size()) {
