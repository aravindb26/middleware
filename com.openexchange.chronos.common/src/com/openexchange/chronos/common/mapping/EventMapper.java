...
public class EventMapper extends DefaultMapper<Event, EventField> {
     ...
     */
    private final EnumMap<EventField, Mapping<? extends Object, Event>> mappings;
    ...
    /**
     * Retains only specific properties in the supplied event reference by removing all others.
     * 
     * @param event The event to adjust
     * @param fields The fields to retain
     */
    public void retain(Event event, Set<EventField> fields) {
        for (Entry<EventField, Mapping<? extends Object, Event>> entry : getMappings().entrySet()) {
            if (false == fields.contains(entry.getKey())) {
    ...
    @Override
    protected EnumMap<EventField, Mapping<? extends Object, Event>> getMappings() {
        return mappings;
    }

    private EnumMap<EventField, Mapping<? extends Object, Event>> createMappings() {
        ...
        mappings.put(EventField.DTSTAMP, new DefaultMapping<Long, Event>() {

            @Override
            public boolean isSet(Event object) {
                return object.containsDtStamp();
            }

            @Override
            public void set(Event object, Long value) throws OXException {
                object.setDtStamp(null == value ? 0 : l(value));
            }

            @Override
            public Long get(Event object) {
                return L(object.getDtStamp());
            }

            @Override
            public void remove(Event object) {
                object.removeDtStamp();
            }
        });
