...
public class SelfProtectionFactory {
    ...
    public static class SelfProtection {
        ...
        public void checkEventCollection(Collection<?> collection) throws OXException {
            if (isEventLimitExceeded(collection)) {
                throw CalendarExceptionCodes.TOO_MANY_EVENT_RESULTS.create();
            }
        }

        /**
         * Checks if a {@link Collection} of events exceeded the configured limit
         *
         * @param collection A collections of {@link Event} objects or similar (e.g. EventIds)
         * @return <code>true</code> if the collection contains enough elements to exceed the configured limit, <code>false</code> otherwise
         */
        public boolean isEventLimitExceeded(Collection<?> collection) {
            return collection.size() > eventLimit;
        }
        ...
        /**
         * Checks of the overall amount of events exceeded the defined limits
         *
         * @param map The map containing the events
         * @throws OXException In case there are too many events found in the values
         */
        public void checkMap(Map<?, ? extends Collection<Event>> map) throws OXException {
            if (isEventLimitExceeded(map)) {
                throw CalendarExceptionCodes.TOO_MANY_EVENT_RESULTS.create();
            }
        }

        /**
         * Checks if a {@link Map} holding events contains enough elements to exceed the configured limit
         *
         * @param map A map of holding event
         * @return <code>true</code> if the overall amount of elements exceeded the configured limit, <code>false</code> otherwise
         */
        public boolean isEventLimitExceeded(Map<?, ? extends Collection<?>> map) {
            int sum = 0;
