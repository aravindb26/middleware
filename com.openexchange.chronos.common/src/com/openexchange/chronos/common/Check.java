...
public class Check {
    ...
    /**
     * Checks that the length of each of the patterns within the term's constant operands is equal to or greater than a configured minimum.
     *
     * @param searchTerm The search term to check
     * @param minimumPatternLength The minimum search pattern length, or <code>0</code> for no limitation
     * @return The passed search term, after its operand's length was checked
     * @throws OXException {@link CalendarExceptionCodes#QUERY_TOO_SHORT}
     */
    public static SearchTerm<?> minimumSearchPatternLength(SearchTerm<?> searchTerm, int minimumPatternLength) throws OXException {
        if (null != searchTerm && 0 < minimumPatternLength) {
            if ((searchTerm instanceof SingleSearchTerm)) {
                for (Object operand : ((SingleSearchTerm) searchTerm).getOperands()) {
                    if ((operand instanceof ConstantOperand)) {
                        Object value = ((ConstantOperand<?>) operand).getValue();
                        if ((value instanceof String)) {
                            Check.minimumSearchPatternLength((String) value, minimumPatternLength);
                        }
                    }
                }
            } else if ((searchTerm instanceof CompositeSearchTerm)) {
                for (SearchTerm<?> operand : ((CompositeSearchTerm) searchTerm).getOperands()) {
                    Check.minimumSearchPatternLength(operand, minimumPatternLength);
                }
            }
        }
        return searchTerm;
