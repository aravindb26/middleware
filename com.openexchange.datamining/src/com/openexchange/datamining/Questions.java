...
public class Questions {
    ...
    protected static void reportMaximumNumberOfContactsForOneUser() {
        try {
            ...
            String sql = "SELECT MAX(count) FROM (SELECT count(*) AS count FROM prg_contacts WHERE userid IS NULL GROUP BY cid, created_from) AS x;";
    ...
    protected static void reportMaximumNumberOfCreatedDocumentsForOneUser() {
        try {
            ...
            String sql = "SELECT MAX(count) FROM (SELECT count(version_number) AS count FROM infostore_document GROUP BY cid, created_by) AS x;";
            BigInteger numberOfInfostoreObjects = Datamining.maximumForAllSchemata(sql);
    ...
    protected static void reportMaximumNumberOfCreatedTasksForOneUser() {
        try {
            ...
            String sql = "SELECT MAX(count) FROM (SELECT count(id) AS count FROM task GROUP BY cid, created_from) AS x;";
            BigInteger numberOfInfostoreObjects = Datamining.maximumForAllSchemata(sql);
    ...
    protected static void reportMaximumNumberOfCreatedContactsForOneUser() {
        try {
            Datamining.allTheQuestions.add(MAXIMUM_NUMBER_OF_CREATED_CONTACTS_FOR_ONE_USER);
            // GROUP BY CLAUSE: ensure ONLY_FULL_GROUP_BY compatibility
            String sql = "SELECT MAX(count) FROM (SELECT count(*) AS count FROM prg_contacts WHERE userid IS NULL AND field02 IS NOT NULL AND field03 IS NOT NULL GROUP BY cid, created_from) AS x;";
            BigInteger numberOfInfostoreObjects = Datamining.maximumForAllSchemata(sql);
            Datamining.report(MAXIMUM_NUMBER_OF_CREATED_CONTACTS_FOR_ONE_USER, numberOfInfostoreObjects.toString());
        } catch (Exception e) {
            handleError(e);
