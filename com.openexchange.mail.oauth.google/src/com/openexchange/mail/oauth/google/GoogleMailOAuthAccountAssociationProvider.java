...
public class GoogleMailOAuthAccountAssociationProvider implements OAuthAccountAssociationProvider {
    ...
    @Override
    public Collection<OAuthAccountAssociation> getAssociationsFor(int oauthAccountId, Session session) throws OXException {
        ...
        for (MailAccount mailAccount : storage.getUserMailAccounts(session.getUserId(), session.getContextId())) {
            if (oauthAccountId == mailAccount.getMailOAuthId()) {
                if (null == associations) {
                    associations = new LinkedList<>();
                }
                associations.add(new GoogleMailOAuthAccountAssociation(oauthAccountId, session.getUserId(), session.getContextId(), mailAccount));
            }
        }
