...
public class HazelcastCheck extends AbstractCachingMWHealthCheck {
    ...
    protected MWHealthCheckResponse doCall() {
        ...
        if (null == hzInstance) {
            return new DefaultMWHealthCheckResponse(NAME, null, true);
        }

        boolean status = true;
        Cluster cluster = hzInstance.getCluster();
        if (null == cluster) {
            return new DefaultMWHealthCheckResponse(NAME, Collections.emptyMap(), status);
        }
        ...
        return new DefaultMWHealthCheckResponse(NAME, data, status);
    }
