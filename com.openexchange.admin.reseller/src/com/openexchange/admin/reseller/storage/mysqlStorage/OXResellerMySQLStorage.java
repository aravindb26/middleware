...
public final class OXResellerMySQLStorage extends OXResellerSQLStorage {
    ...
    @Override
    public boolean ownsContextOrIsPidOfOwner(final List<Context> ctxList, final int admid) throws StorageException {
        Connection oxcon = null;
        PreparedStatement prep = null;
        ResultSet rs = null;
        try {
            ...
            String selectSids = DBUtils.getIN("SELECT sid FROM context2subadmin WHERE cid IN (", ctxList.size());
            prep = oxcon.prepareStatement(selectSids);
            int pos = 1;
            for (int x = 0; x < ctxList.size(); x++) {
                prep.setInt(pos++, i(ctxList.get(x).getId()));
            }
            rs = prep.executeQuery();
            Map<Integer, Integer> cid2sid = new HashMap<>(ctxList.size());
            int x = 0;
            while (rs.next()) {
                cid2sid.put(ctxList.get(x++).getId(), I(rs.getInt("sid")));
            }

            List<Integer> notMatchingSids = new ArrayList<Integer>();
            cid2sid.forEach((cid, sid) -> {
                if (sid.intValue() != admid) {
                    notMatchingSids.add(sid);
                }
            });
            if (notMatchingSids.isEmpty()) {
                // Alle ctxs are owned by the given subadmin
                return true;
            }
            rs.close();
            prep.close();

            // Check if ctx not owned by subadmin are owned by subsubadmins
            String countSubadminsSQL = DBUtils.getIN("SELECT COUNT(*) FROM subadmin WHERE pid=? AND sid IN (", notMatchingSids.size());
            prep = oxcon.prepareStatement(countSubadminsSQL);
            pos = 1;
            prep.setInt(pos++, admid);
            for (x = 0; x < notMatchingSids.size(); x++) {
                prep.setInt(pos++, i(notMatchingSids.get(x)));
            }

            rs = prep.executeQuery();
            if (!rs.next() || rs.getInt(1) != notMatchingSids.size()) {
                // One or more ctxs are not owned by subsubadmins
                return false;
            }
    ...
    @Override
    public void addSid(Context ctx) throws StorageException {
        Objects.requireNonNull(ctx);
        Connection oxcon = null;
        LOGGER.debug("addSid ctx={}", ctx.getId());
        try {
            oxcon = cache.getReadConnectionForConfigDB();
            ResellerAdmin admin = getResellerAdminForContext(ctx, oxcon);
            OXContextExtensionImpl oxContextExtension = (OXContextExtensionImpl) ctx.getFirstExtensionByName(OXContextExtensionImpl.class.getName());
            if (null == oxContextExtension) {
                try {
                    ctx.addExtension(new OXContextExtensionImpl(i(admin.getId())));
                } catch (DuplicateExtensionException e) {
                    throw new StorageException(e);
                }
            } else {
                oxContextExtension.setSid(i(admin.getId()));
            }
        } catch (SQLException | PoolException e) {
            LOGGER.error("", e);
            throw new StorageException(e.getMessage());
        } finally {
            cache.closeReadConfigDBSqlStuff(oxcon, null);
        }
    }
    ...
    /**
     * Gets the {@link ResellerAdmin} for the given context
     *
     * @param ctx The context
     * @param con The db connection
     * @return The {@link ResellerAdmin}
     * @throws SQLException
     * @throws StorageException
     */
    ...
    private void changeCapabilities(ResellerAdmin resellerAdmin, Connection connection) throws StorageException {
        ...
        try {
            ...
            if (!capsToInsert.isEmpty()) {
                // If capsToAdd is not null remove the -[capability] ones first.
                if (capsToAdd != null) {
