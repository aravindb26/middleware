...
public class ResellerAuth extends OXCommonImpl implements BasicAuthenticatorPluginInterface {
    ...
    @Override
    public boolean isOwnerOfContext(Credentials creds, Context ctx) throws InvalidCredentialsException {
        try {
            doNullCheck(creds);
        } catch (InvalidDataException e) {
            LoggerHolder.LOG.error("authdata is null", e);
        ...
        try {
            ...
            ResellerAdmin adm = oxresell.getData(new ResellerAdmin[] { new ResellerAdmin(creds.getLogin()) })[0];
            return oxresell.ownsContext(ctx, adm.getId().intValue());
        } catch (StorageException e) {
            LoggerHolder.LOG.error("", e);
            throw new InvalidCredentialsException("authentication failed");
        }
    ...
    @Override
    public boolean isMasterOfContext(Credentials creds, List<Context> ctx) throws InvalidCredentialsException {
        try {
            doNullCheck(creds);
        } catch (InvalidDataException e) {
            LoggerHolder.LOG.error("authdata is null", e);
