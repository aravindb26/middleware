...
public class RMITrackerCustomizer implements ServiceTrackerCustomizer<Remote, Remote> {
    ...
    public Remote addingService(ServiceReference<Remote> reference) {
        ...
        Optional<Integer> port = RMIUtility.findRMIPort(reference, r);
        try {
            if (port.isPresent()) {
                logger.info("Exposing {} RMI interface under port {}", name, port.get());
            }
            Remote exportObject = UnicastRemoteObject.exportObject(r, port.orElse(Integer.valueOf(0)).intValue());
