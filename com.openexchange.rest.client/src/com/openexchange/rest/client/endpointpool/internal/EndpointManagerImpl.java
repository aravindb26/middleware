...
/**
 * {@link EndpointManagerImpl} - The default implementation for end-point manager.
 *
...
public class EndpointManagerImpl implements EndpointManager {
    ...
    private final AtomicReference<ScheduledTimerTask> heartbeatReference;
    ...
    public EndpointManagerImpl(List<URI> endpointUris, String httpClientId, EndpointAvailableStrategy availableStrategy, long heartbeatIntervalMillis, TimerService timerService, HttpClientService httpClientService) {
        ...
        Heartbeat heartbeat = new Heartbeat(httpClientService.getHttpClient(httpClientId), availableStrategy);
        heartbeatReference = new AtomicReference<>(timerService.scheduleWithFixedDelay(heartbeat, heartbeatIntervalMillis, heartbeatIntervalMillis));
    }
    ...
    @Override
    public void close() {
        ScheduledTimerTask heartbeat = heartbeatReference.getAndSet(null);
        if (heartbeat != null) {
            heartbeat.cancel();
        }
    }
    ...
    private final class Heartbeat implements Runnable {
        ...
        public void run() {
            try {
                List<Endpoint> blacklistedEndpoints = getBlacklist();
                if (blacklistedEndpoints.isEmpty()) {
                    return;
                }

                for (Endpoint endpoint : blacklistedEndpoints) {
