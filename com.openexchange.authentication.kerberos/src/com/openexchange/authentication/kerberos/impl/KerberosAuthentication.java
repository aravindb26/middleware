...
public class KerberosAuthentication implements AuthenticationService {
    ...
    private AuthenticationResult handleLoginRequest(AuthenticationRequest request) throws OXException {
        LoginInfo loginInfo = DefaultLoginInfo.of(request);
        ...
        if (Authorization.checkForKerberosAuthorization(authHeader)) {
            ...
            if (null != proxyUser && null != proxyDelimiter && uid.contains(proxyDelimiter)) {
                ...
                for (final String pu : Strings.splitByComma(proxyUser)) {
        ...
        if (ContextStorage.NOT_FOUND == ctxId) {
            LOG.debug("Missing context mapping for context \"{}\". Login failed.", splitted[0]);
            return AuthenticationResult.failed(LoginExceptionCodes.INVALID_CREDENTIALS.create());
        }
        ...
        try {
            ...
            if (LdapExceptionCode.USER_NOT_FOUND.equals(e)) {
                LOG.debug("Missing user mapping for user \"{}\". Login failed.", splitted[1]);
                return AuthenticationResult.failed(LoginExceptionCodes.INVALID_CREDENTIALS.create());
            }
            throw e;
        }
        ...
        if (null != proxyAs) {
            return AuthenticationResult.success(new Authed(splitted[0], uid + proxyDelimiter + proxyAs, principal));
        }
        return AuthenticationResult.success(new Authed(splitted[0], splitted[1], principal));
    }
    ...
    public AuthenticationResult doLogin(AuthenticationRequest authenticationRequest, boolean autologin) throws OXException {
        if (autologin) {
            return AuthenticationResult.failed(LoginExceptionCodes.NOT_SUPPORTED.create());
        }
        return handleLoginRequest(authenticationRequest);
    }
}
