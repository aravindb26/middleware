...
public class MailMappingServiceTest {
    ...
    private MailResolverService resolver = null;
    ...
    private void mockContext() throws OXException {
        ...
        when(users.getUser(12, 42)).thenReturn(charlie);
        when(users.getUser(13, 42)).thenReturn(linus);

        resolver = services.mock(MailResolverService.class);
    }

    @Test
    @SuppressWarnings("unchecked")
    public void testResolveMail() throws OXException, JSONException {
        when(resolver.resolveMultiple("charlie@test.invalid")).thenReturn(new ResolvedMail[] { new ResolvedMail(12, 42) });
    ...
    public void testResolveMultipleMails() throws OXException, JSONException {
        when(resolver.resolveMultiple("charlie@test.invalid", "linus@test.invalid")).thenReturn(new ResolvedMail[] { new ResolvedMail(12, 42), new ResolvedMail(13, 42) });
    ...
    public void testResolveUnknownMail() throws OXException {
        when(resolver.resolveMultiple("charlie@test.invalid")).thenReturn(new ResolvedMail[] { null });
