...
class SingleMessageDeliveryTask implements Runnable {
    ...
    public static class Builder {
        ...
        private LockedAlarmTrigger trigger;
        ...
        public Builder setTrigger(LockedAlarmTrigger trigger) {
    ...
    private final LockedAlarmTrigger trigger;
     ...
     * @param claim The claim for the locked trigger
    ...
    public void run() {
        ...
        LockedAlarmTrigger unlockMe = trigger;
            ...
            if (writeCon != null) {
                ...
                if (processFinished == false) {
                    try {
                        ...
                        if (processFinished == false) {
                            try {
                                storage.dropProcessingStatus(writeCon, account, ctx.getContextId(), trigger);
                                isReadOnly = false;
    ...
    private Event prepareEvent(Connection writeCon) throws OXException {
        ...
        if (loadedTrigger == null || !trigger.getClaim().equals(loadedTrigger.getClaim())) {
            // Abort since the triggers is either gone or picked up by another node (e.g. because of an update)
            LOG.trace("Skipped message alarm task for {}. Its trigger claim is not up to date!", new Key(ctx.getContextId(), account, trigger.getEventId(), alarm.getId()));
            return null;
        }
        Event event = null;
    ...
    private boolean checkEvent(Connection writeCon, Event event) throws OXException {
        ...
        List<LockedAlarmTrigger> triggers = callback.checkEvents(writeCon, Collections.singletonList(event), cid, account, true);
        if (triggers.isEmpty() == false) {
            ...
            for (LockedAlarmTrigger tmpTrigger : triggers) {
                callback.scheduleTaskForEvent(calStorage, new Key(cid, account, tmpTrigger.getEventId(), tmpTrigger.getAlarm().intValue()), tmpTrigger);
            }
            return false;
        }
        return true;
