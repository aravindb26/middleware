...
public final class IMAPStoreCache {
    ...
    /**
     * Signals that previously obtained IMAP store instance had been closed.
     *
     * @param accountId The account identifier
     * @param server The host name of the IMAP server
     * @param port The port
     ...
     * @param session The associated session
     */
    public void signalClosed(int accountId, String server, int port, String login, Session session) {
        IMAPStoreContainer container = map.get(newKey(accountId, server, port, login, session.getUserId(), session.getContextId()));
        if (null == container) {
            return;
        }
        if (container.hadAcquired()) {
            container.backStore(null);
        }
    }
    ...
    static final class Key implements Comparable<Key> {
        ...
        public int compareTo(Key o) {
            int c = Integer.compare(port, o.port);
            if (c != 0) {
                return c;
            }
            ...
            c = Integer.compare(contextId, o.contextId);
            if (c != 0) {
                return c;
            }
            c = host.compareTo(o.host);
            if (c != 0) {
                return c;
            }
