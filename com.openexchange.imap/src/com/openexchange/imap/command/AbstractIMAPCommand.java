...
public abstract class AbstractIMAPCommand<T> {
    ...
    private static final class IMAPCommandResponseInterceptor implements ResponseInterceptor {
        ...
        public boolean intercept(Response response) {
            if (exception == null) {
                try {
                    return abstractIMAPCommand.addLoopCondition() && abstractIMAPCommand.handleResponse(response);
    ...
    private static final class CallbackIMAPProtocolCommand implements IMAPFolder.ProtocolCommand {
        ...
        public Object doCommand(IMAPProtocol protocol) throws ProtocolException {
            ...
            for (int argsIndex = 0; argsIndex < args.length; argsIndex++) {
                ...
                IMAPCommandResponseInterceptor nullableInterceptor = null;
                if (abstractIMAPCommand.isFetchCommand() || imapCmd.indexOf("FETCH ") >= 0) {
                    nullableInterceptor = new IMAPCommandResponseInterceptor(abstractIMAPCommand);
    ...
    /**
     * Checks if this IMAP command issues an IMAP <code>FETCH</code> command.
     *
     * @return <code>true</code> if an IMAP <code>FETCH</code> command is issued; otherwise <code>false</code>
     */
