...
public final class SeqNumIMAPCommand extends AbstractIMAPCommand<int[]> {
    ...
    public SeqNumIMAPCommand(IMAPFolder imapFolder, long[] uids, boolean isSequential) {
        ...
        if (length == 0) {
            ...
            if (isSequential) {
                args = new String[] { new StringBuilder(64).append(this.uids[0]).append(':').append(this.uids[this.uids.length - 1]).toString() };
            } else {
                args = IMAPNumArgSplitter.splitUIDArg(this.uids, true, -1);
            }
        }
