...
public final class IMAPSort {
    ...
    private static final Set<SortTerm> SORTED_BY_LOCAL_PART_TERMS = Set.of(SortTerm.FROM, SortTerm.TO, SortTerm.CC);
    ...
    public static ImapSortResult sortMessages(IMAPFolder imapFolder, javax.mail.search.SearchTerm jmsSearchTerm, MailSortField sortField, OrderDirection order, IndexRange indexRange, boolean allowESORT, boolean allowSORTDISPLAY, boolean fallbackOnCommandFailed, IMAPConfig imapConfig) throws MessagingException, OXException {
        ...
        for (SortTerm sortTerm : sortTerms) {
            if (SORTED_BY_LOCAL_PART_TERMS.contains(sortTerm)) {
                    ...
                    case FOLDER_CLOSED:
                        {
                            ...
                            try {    imapFolder.open(Folder.READ_ONLY);    } catch (@SuppressWarnings("unused") Exception x) { /*Ignore*/ }
                        }
                        break;
                    default:
                        break;
    ...
    public static SortPartialResult sortReturnPartial(SortTerm[] sortTerms, javax.mail.search.SearchTerm jmsSearchTerm, IndexRange indexRange, IMAPFolder imapFolder) throws MessagingException {
        try {
            ...
            return (SortPartialResult) ImapUtility.doCommandAndLog(LOG, imapFolder, new ProtocolCommand() {
            ...
            if (cause instanceof com.sun.mail.iap.ConnectionException) {
                if (cause.getCause() instanceof com.sun.mail.iap.ByeIOException) { // NOSONARLINT
    ...
    private static int[] sort(SortTerm[] sortTerms, javax.mail.search.SearchTerm jmsSearchTerm, IMAPFolder imapFolder, boolean fallbackOnCommandFailed) throws MessagingException, OXException {
        try {
            return (int[]) ImapUtility.doCommandAndLog(LOG, imapFolder, new SearchAndSortProtocolCommand(sortTerms, jmsSearchTerm));
        } catch (FolderClosedException | StoreClosedException e) {
            throw e;
        }  catch (MessagingException e) {
    ...
    public static long[] allUIDs(IMAPFolder imapFolder, boolean descending, IMAPConfig imapConfig) throws MessagingException {
         ...
         * Cast & return
         */
        return ((long[]) ImapUtility.doCommandAndLog(LOG, imapFolder, new SORTProtocolCommand(descending, imapFolder)));
    }
    ...
    private static final class SORTProtocolCommand implements IMAPFolder.ProtocolCommand {
        ...
        SORTProtocolCommand(boolean descending, IMAPFolder imapFolder) {
            super();
    ...
    private static final class SearchAndSortProtocolCommand implements IMAPFolder.ProtocolCommand {

        private final SortTerm[] sortTerms;
        private final SearchTerm jmsSearchTerm;

        SearchAndSortProtocolCommand(SortTerm[] sortTerms, SearchTerm jmsSearchTerm) {
            super();
            this.sortTerms = sortTerms;
            this.jmsSearchTerm = jmsSearchTerm;
        }
    ...
    } // End of SearchAndSortProtocolCommand
