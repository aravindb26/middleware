...
public abstract class IMAPFolderWorker extends MailMessageStorageLong {
    ...
    protected IMAPFolderWorker(IMAPStore imapStore, IMAPAccess imapAccess, Session session) throws OXException {
        ...
        previewMode = IMAPProperties.previewModeFor(imapConfig);
    }
    ...
    protected boolean holdsMessages() throws OXException, MessagingException {
        if (holdsMessages == -1) {
            ListLsubEntry listEntry = ListLsubCache.tryCachedLISTEntry(imapFolder.getFullName(), accountId, session);
            if (null != listEntry && listEntry.exists()) {
                holdsMessages = listEntry.canOpen() ? 1 : 0;
            } else {
                ListInfo listInfo = IMAPCommandsCollection.getListInfo(imapFolder.getFullName(), imapFolder);
                if (null != listInfo) {
                    ListLsubCache.addSingleByFolder(accountId, listInfo, imapFolder, session, imapConfig.getIMAPProperties());
                    holdsMessages = listInfo.canOpen ? 1 : 0;
                } else {
                    // Apparently no such folder...
                    holdsMessages = 0;
                }
            }
        }
    ...
    protected final IMAPFolder setAndOpenFolder(IMAPFolder imapFolder, String fullName, int desiredMode) throws MessagingException, OXException {
        ...
        if (imapFolder != null) {
            ...
            synchronized (imapFolder) { // NOSONARLINT
        ...
        synchronized (retval) {
            ...
            ListLsubEntry listEntry = ListLsubCache.getCachedLISTEntry(fullName, accountId, retval, session, imapConfig.getIMAPProperties());
            if (!isRootFolder && !"INBOX".equals(fullName) && (!listEntry.exists())) {
                ...
                openIt = false;
                listEntry = ListLsubCache.addSingleByFolder(accountId, retval, session, imapConfig.getIMAPProperties());
            }
    ...
    private static int getMode(IMAPFolder imapFolder) {
        try {
    ...
    private static boolean tryOpen(IMAPFolder imapFolder, int desiredMode) throws MessagingException {
        boolean close = true;
        try {
            imapFolder.open(desiredMode);
            close = false;
