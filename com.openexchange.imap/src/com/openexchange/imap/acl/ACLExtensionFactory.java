...
public final class ACLExtensionFactory {
    ...
    private static final AtomicReference<ACLExtensionFactory> INSTANCE_REF = new AtomicReference<>();

    static void createInstance() {
        INSTANCE_REF.set(new ACLExtensionFactory());
    }
    ...
    public static ACLExtensionFactory getInstance() {
        ACLExtensionFactory tmp = INSTANCE_REF.get();
        if (null == tmp) {
    ...
    public ACLExtension getACLExtension(IMAPConfig imapConfig) {
        if (instantiated.get() && Account.DEFAULT_ID == imapConfig.getAccountId()) {
            return configured;
        }
