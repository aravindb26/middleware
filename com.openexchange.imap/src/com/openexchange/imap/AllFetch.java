...
public final class AllFetch {
    ...
    public static enum LowCostItem {
         ...
         * INTERNALDATE
         */
        INTERNALDATE("INTERNALDATE", INTERNALDATE.class, (item, m, config, logger) -> m.setReceivedDate(((INTERNALDATE) item).getDate())),
        /**
         * UID
         */
        UID("UID", UID.class, (item, m, config, logger) -> m.setMailId(Long.toString(((UID) item).uid))),
        /**
         * FLAGS
         */
         ...
         * SIZE
         */
        SIZE("RFC822.SIZE", RFC822SIZE.class, (item, m, config, logger) -> m.setSize(((RFC822SIZE) item).size)),
        /**
         * ENVELOPE
         * <p>
         * The fields of the envelope: date, subject, from, sender, reply-to, to, cc, bcc, in-reply-to, and message-id.
         */
        ENVELOPE("ENVELOPE", ENVELOPE.class, (item, m, config, logger) -> {
        ...
        try {
            ...
            if (tracerState.isTrace()) {
                ...
                try {
                     ...
                     */
                    OutputStreamWriter writer = new OutputStreamWriter(tracerState.getOut(), StandardCharsets.UTF_8);
     ...
     */
    protected static final Comparator<MailMessage> ASC_COMP = (m1, m2) -> {
     ...
     */
    protected static final Comparator<MailMessage> DESC_COMP = (m1, m2) -> {
