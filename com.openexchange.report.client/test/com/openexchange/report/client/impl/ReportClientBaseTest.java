...
public class ReportClientBaseTest {
    ...
    private void setUpAppsuiteReport() throws Exception {
        ...
        Mockito.when(report.get("tasks")).thenReturn(Integer.valueOf(1111));
        Mockito.when(report.get("pendingTasks")).thenReturn(Integer.valueOf(2));
        ...
        protected static List<List<Object>> createDetailList(final List<ContextDetail> contextDetails) {
            ...
            for (final ContextDetail tmp : contextDetails) {
                for (final ContextModuleAccessCombination moduleAccessCombination : tmp.getModuleAccessCombinations()) {
                    ...
                    sorted.add(Integer.valueOf(tmp.getId()));
                    ...
                    tmpList.add(Arrays.asList((Object) Integer.valueOf(tmp.getId()), tmp.getAge(), tmp.getCreated(), tmp.getAdminmac(), moduleAccessCombination.getUserAccessCombination(), moduleAccessCombination.getUserCount(), moduleAccessCombination.getInactiveCount()));
