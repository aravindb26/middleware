...
public abstract class DefaultLongPollingListener implements LongPollingListener {
    ...
    protected AJAXRequestResult createResult(DriveEvent event) throws OXException {
         ...
         */
        List<DriveAction<? extends DriveVersion>> actions = null != event ? event.getActions(driveSession.getServerSession().getUserId(), rootFolderIDs, SubscriptionMode.SEPARATE.equals(mode)) :
    ...
    protected boolean isInteresting(DriveEvent event) {
        if (null != event) {
            Set<String> folderIDs = event.getFolderIDs(driveSession.getServerSession().getUserId());
