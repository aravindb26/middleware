...
/**
 * {@link ServerConfigServiceImpl} - The server configuration service implementation.
 *
...
public class ServerConfigServiceImpl implements ServerConfigService {
    ...
    List<Map<String, Object>> calculateCustomHostConfigurations(String hostName, int userID, int contextID) throws OXException {
        // Host name is needed for the host and hostRegex check
        ...
        for (Map.Entry<String, Object> configEntry : configurations.entrySet()) {
            ...
            if (null == value) {
                ...
                if (value instanceof Map) {
                    ...
                    HostNameApplicableResult hostNameApplicableResult = looksApplicable(possibleConfiguration, hostName, false);
                    if (hostNameApplicableResult == HostNameApplicableResult.NOT_APPLICABLE) {
                        ...
                        if (hostNameApplicableResult == HostNameApplicableResult.APPLICABLE_ALL) {
     ...
     * @param hostName The host name that should match
    ...
    protected boolean looksApplicable(Map<String, Object> possibleConfiguration, String hostName) {
        return looksApplicable(possibleConfiguration, hostName, true) != HostNameApplicableResult.NOT_APPLICABLE;
    }
     ...
     * @param possibleConfiguration A possible configuration Object that should be checked
     * @param hostName The host name that should match
     * @param checkHostName Whether provided host name shall be checked
     * @return true if the configuration object should be applied, false otherwise
     */
    private HostNameApplicableResult looksApplicable(Map<String, Object> possibleConfiguration, String hostName, boolean checkHostName) {
        if (possibleConfiguration == null) {
            return HostNameApplicableResult.NOT_APPLICABLE;
        }

        // We need a host name for the host and hostRegex check
        if (checkHostName && Strings.isEmpty(hostName)) {
            return HostNameApplicableResult.NOT_APPLICABLE;
        }
        ...
        // Check "host"
        {
            ...
            if (host != null) {
                if ("all".equals(host)) {
                    return HostNameApplicableResult.APPLICABLE_ALL;
                }

                if (host.equals(hostName)) {
                    return HostNameApplicableResult.APPLICABLE_MATCH;
                }
    ...
    /** Possible results for host name applicability check */
    private static enum HostNameApplicableResult {
        /** Not applicable */
        NOT_APPLICABLE,
        /** Applicable since host name does match */
        APPLICABLE_MATCH,
        /** Applicable since <code>"all"</code> is accepted */
        APPLICABLE_ALL
    }
