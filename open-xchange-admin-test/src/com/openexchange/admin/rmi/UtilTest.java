...
public class UtilTest extends AbstractRMITest {
    ...
    /**
     * Initializes a new {@link UtilTest}.
     */
    public UtilTest() {
        super();
    }
    ...
    public static Database getTestDatabaseObject(String hostname, String name) {
        String rmiTestDBHost = System.getProperty("rmi_test_dbhost");
        String tmpHostName = rmiTestDBHost != null ? rmiTestDBHost : hostname;

        Database client_db = new Database();
        client_db.setName(name);
        client_db.setDriver(DatabaseConstants.DRIVER);
        client_db.setLogin("openexchange");
        client_db.setMaster(Boolean.TRUE);
        client_db.setMaxUnits(I(1000));
        String dbpw = "secret";
        if (System.getProperty("rmi_test_dbpw") != null) {
            dbpw = System.getProperty("rmi_test_dbpw");
        }
        client_db.setPassword(dbpw);
        client_db.setPoolHardLimit(I(20));
        client_db.setPoolInitial(I(2));
        client_db.setPoolMax(I(100));
        client_db.setUrl("jdbc:mysql://" + tmpHostName);
        client_db.setMasterId(I(0));
        return client_db;
    }
}
