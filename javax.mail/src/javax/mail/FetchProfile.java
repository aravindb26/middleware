...
public class FetchProfile {
    ...
    /**
     * Create a FetchProfile initialized with given items.
     *
     * @param items The items to set
     */
    public FetchProfile(Item... items) {
        this();
        if (items != null) {
            for (Item item : items) {
                if (item != null) {
    ...
    /**
     * Create a FetchProfile initialized with given items and header names.
     *
     * @param itemsAndHeaderNames The itemd and header names to set
     */
    public FetchProfile(Object... itemsAndHeaderNames) {
        this();
        if (itemsAndHeaderNames != null) {
            for (Object itemsOrHeaderName : itemsAndHeaderNames) {
                if (itemsOrHeaderName instanceof Item item) {
                    specials.add(item);
                } else if (itemsOrHeaderName instanceof String headerName) {
                    headers.put(com.openexchange.java.Strings.asciiLowerCase(headerName), headerName);
                }
            }
        }
    }
    ...
    public boolean contains(String headerName) {
        return null == headerName ? false : headers.containsKey(com.openexchange.java.Strings.asciiLowerCase(headerName));
    }
    ...
    public boolean contains(String... headerNames) {
        if (null != headerNames) {
            for (int i = headerNames.length; i-- > 0;) {
                if (headers.containsKey(com.openexchange.java.Strings.asciiLowerCase(headerNames[i]))) {
                    return true;
                }
            }
        }
    ...
    public String[] getHeaderNames() {
        ...
        return size == 0 ? com.openexchange.java.Strings.getEmptyStrings() : headers.values().toArray(new String[size]);
    }
