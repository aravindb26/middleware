...
import javax.mail.util.LineOutputStream;
...
public class MimeMultipart extends Multipart {
    ...
    /**
     * The optional sanitizer for broken Content-Type strings.
     */
    protected ContentTypeSanitizer contentTypeSanitizer;
	...
	// ContentTypeSanitizer
	Object tmp = getProp(System.getProperties(), "mail.mime.contentTypeSanitizer");
	if (tmp instanceof ContentTypeSanitizer) {
	    contentTypeSanitizer = (ContentTypeSanitizer) tmp;
    }
    }
    ...
    private ContentType parseContentType() throws ParseException {
        try {
            return new ContentType(contentType);
        } catch (ParseException e) {
            ContentTypeSanitizer sanitizer = this.contentTypeSanitizer;
            if (sanitizer == null) {
                throw e;
            }
            contentType = sanitizer.sanitizeContentType(contentType, e);
            return new ContentType(contentType);
        }
    }
    ...
    public synchronized void setSubType(String subtype)
			throws MessagingException {
	ContentType cType = parseContentType();
     ...
     *
     * @param  contentId 	the <code>Content-ID</code> of the desired part
     * @return          the Part
     * @exception	MessagingException for failures
     */
    public synchronized BodyPart getBodyPart(String contentId)
			throws MessagingException {
	parse();
	...
	String boundary = "--" +
		parseContentType().getParameter("boundary");
	...
	if (preamble != null) {
	    ...
	    byte[] pb = MimeUtility.getBytes(p);
        los.write(pb);
	}
