...
public class InternetHeaders {
    ...
    for (int i = 0; i < headers.size(); i++) {
        ...
        if (h.line != null && (colonPos = h.line.indexOf(':')) >= 0) {
            ...
            if (length > 76) {
                ...
                if (null == optNames || (matching ? optNames.contains(com.openexchange.java.Strings.asciiLowerCase(name)) : false == optNames.contains(com.openexchange.java.Strings.asciiLowerCase(name)))) {                    
    ...
    private static java.util.Set<String> lowerCaseSetFor(String[] names) {
        ...
        for (int i = names.length; i-- > 0;) {
            String name = names[i];
            if (null != name)
                set.add(com.openexchange.java.Strings.asciiLowerCase(name));
        }
        return set;
    }
