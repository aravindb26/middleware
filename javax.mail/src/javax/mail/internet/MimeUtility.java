...
import javax.mail.util.StreamProvider;
import javax.mail.util.StreamProvider.EncoderTypes;
...
public class MimeUtility {
    ...
    private static final boolean ignoreUnknownEncoding = getBooleanSystemProperty("mail.mime.ignoreunknownencoding", false);
    private static final boolean allowUtf8 = getBooleanSystemProperty("mail.mime.allowutf8", false);
    private static final boolean monitorInputStream = getBooleanSystemProperty("mail.mime.monitorinputstream", false);
    /*
    ...
    private static final java.util.concurrent.atomic.AtomicReference<RateTracker> TRACKER_REF = new java.util.concurrent.atomic.AtomicReference<RateTracker>(null);
    ...
    public static InputStream decode(InputStream is, String encoding) throws MessagingException {
        if (!monitorInputStream) {
            return doDecode(is, encoding);
        }
        
        RateTracker rateTracker = TRACKER_REF.get();
        if (rateTracker == null) {
            RateTracker nuTracker = new RateTracker();
            if (TRACKER_REF.compareAndSet(null, nuTracker)) {
                rateTracker = nuTracker;
            } else {
                rateTracker = TRACKER_REF.get();
            }
        }
        
        if (rateTracker.isFull()) {
            if (rateTracker.averageRate() < 2800) {
                throw new MessagingException("Bandwidth too slow");
            }
        }
        
        return new com.openexchange.java.bandwidth.SelfMonitoringInputStream(doDecode(is, encoding), rateTracker);
    }
     ...
     * @exception MessagingException	if the encoding is unknown
     */
    private static InputStream doDecode(InputStream is, String encoding)
    ...
    private static class RateTracker implements com.openexchange.java.bandwidth.ClosedHandler {
        
        private final java.util.concurrent.atomic.AtomicReferenceArray<Float> trackedAverageRates;
        private final java.util.concurrent.atomic.AtomicInteger index;
        private final int numberOfRates;
        private final java.util.concurrent.atomic.AtomicBoolean full;
        
        RateTracker() {
            super();
            numberOfRates = 100;
            Float[] arr = new Float[numberOfRates];
            Arrays.fill(arr, null);
            trackedAverageRates = new java.util.concurrent.atomic.AtomicReferenceArray<Float>(arr);
            index = new java.util.concurrent.atomic.AtomicInteger(0);
            full = new java.util.concurrent.atomic.AtomicBoolean(false);
        }
        ...
        float averageRate() {
            float retval = 0;
            for (int i = numberOfRates; i-- > 0;) {
                Float f = trackedAverageRates.get(i);
                retval += f.floatValue();
            }
            return retval / numberOfRates;
        }

        @Override
        public void onClosed(com.openexchange.java.bandwidth.SelfMonitoringInputStream in) {
            float averageRate = in.averageRate();
            if (averageRate <= 0) {
                return;
            }
            ...
            do {
                i = index.get();
                if (i == numberOfRates - 1) {
                    next = 0;
                    full.set(true);
                } else {
                    next = i + 1;
                }
            } while (!index.compareAndSet(i, next));
            
            trackedAverageRates.set(i, Float.valueOf(averageRate));
        }
    }
}
