...
public final class EmailAddressParser {
     ...
     */
    @SuppressWarnings("unused")
    public static InternetAddress[] extractHeaderAddresses(CharSequence header_txt, EnumSet<EmailAddressCriteria> criteria, boolean extractCfwsPersonalNames) {
        ...
        while (true) {
            if (group_end) {
                next_comma_index = header_txt.toString().indexOf(',', just_after_group_end);
            ...
            if (m.lookingAt()) {
                ...
                next_semicolon_index = header_txt.toString().indexOf(';', m.regionStart());
        ...
        return size > 0 ? result.toArray(new InternetAddress[size]) : new InternetAddress[0];
    }
