...
public class LineInputStream extends FilterInputStream implements javax.mail.util.LineInputStream {

    private final boolean allowutf8;
    private byte[] lineBuffer = null; // reusable byte buffer
    private final CharsetDecoder decoder;
    private org.mozilla.universalchardet.UniversalDetector detector; // reused charset detector

    private static final boolean defaultutf8 =
    PropUtil.getBooleanSystemProperty("mail.mime.allowutf8", false);
    private static final int MAX_INCR = 1024*1024;	// 1MB
    ...
    String ascii = stringIfAscii(buf, 0, offset);
    if (ascii != null) {
        // Purely ASCII
        return ascii;
    }
    initOrResetDetector();
    detector.handleData(buf, 0, offset);
    detector.dataEnd();
    String charsetName = com.openexchange.java.CharsetDetector.getResultingCharset(detector);
    return new String(buf, 0, offset, charsetName);

    }

    private void initOrResetDetector() {
        if (detector == null) {
    ...
    private static String stringIfAscii(byte[] buf, int off, int len) {
