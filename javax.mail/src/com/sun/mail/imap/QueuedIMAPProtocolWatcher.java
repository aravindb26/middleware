...
public final class QueuedIMAPProtocolWatcher {
    ...
    public void initWatcher(final ConcurrentMap<URLName, CountingQueue> queues, final ScheduledThreadPoolExecutor executor) {
        if (!isEnabled()) {
            LOG.info("{} not initialized since configured log level does not imply DEBUG log level.", QueuedIMAPProtocolWatcher.class.getSimpleName());
            return;
        }
        ...
        if (null == tmp) {
            synchronized (this) {
                ...
                if (null == tmp) {
                    ...
                    watcherFuture = tmp;
                    LOG.info("{} successfully initialized.", QueuedIMAPProtocolWatcher.class.getSimpleName());
                }
            }
        }
    }
