...
import javax.mail.search.TextTerm;
...
public class SearchSequence {
    ...
    else if (term instanceof TextTerm)  // TEXT
        return text((TextTerm)term, charset);
	else
	...
	for (int i = 0; i < uf.length; i++) {
	    ...
	    result.writeAtom(protocol.isUseUTF7ForUserFlags() ? BASE64MailboxEncoder.encode(uf[i]) : uf[i]);
	}
    ...
    protected Argument text(TextTerm term, String charset) 
        throws SearchException, IOException {
    Argument result = new Argument();
	...
	if (protocol != null && !protocol.hasCapability("WITHIN"))
	    throw new javax.mail.search.UnsupportedSearchTermException("Server doesn't support OLDER searches");
	Argument result = new Argument();
	...
	if (protocol != null && !protocol.hasCapability("WITHIN"))
	    throw new javax.mail.search.UnsupportedSearchTermException("Server doesn't support YOUNGER searches");
	Argument result = new Argument();
	...
	if (protocol != null && !protocol.hasCapability("CONDSTORE"))
	    throw new javax.mail.search.UnsupportedSearchTermException("Server doesn't support MODSEQ searches");
	Argument result = new Argument();
    ...
    protected Argument fileName(javax.mail.search.FileNameTerm term, String charset) 
        throws SearchException, IOException {
    if (term.isFileNameSearchSupported() == false && protocol != null && !protocol.hasCapability("SEARCH=X-MIMEPART"))
        throw new javax.mail.search.UnsupportedSearchTermException("Server doesn't support file name searches");
    Argument result = new Argument();
