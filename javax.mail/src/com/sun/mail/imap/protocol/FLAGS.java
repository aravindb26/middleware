...
public class FLAGS extends Flags implements Item {
     ...
     */
    public FLAGS(IMAPResponse r, boolean decodeUserFlagsWithUTF7) throws ParsingException {
	msgno = r.getNumber();
	...
	if (flags != null) { // if not empty flaglist
		...
		if (s.length() >= 2 && s.charAt(0) == '\\') {
			...
			if (s.length() >= 3) {
			    ...
			    add(decodeUserFlagsWithUTF7 ? com.sun.mail.imap.protocol.BASE64MailboxDecoder.decode(s) : s);	// unknown, treat it as possibly encoded user flag
			break;
		    case 'A': // \Answered
		    ...
		    add(decodeUserFlagsWithUTF7 ? com.sun.mail.imap.protocol.BASE64MailboxDecoder.decode(s) : s);     // treat it as possibly encoded user flag
	    }
	}
    }
}
