...
class IMAPAddress extends InternetAddress {
	...
	if (host == null) {
		...
		if (a.isEndOfGroup())	// reached end of group
		    break;
		if (!v.isEmpty())	// if not first element, need a comma
		    sb.append(',');
	    ...
	    if (mb == null || mb.length() == 0)
	        ...
	        address = quoteIfNeeded(mb).append('@').append(host).toString();
	}

    }

    /**
     * Quotes specified mailbox name if needed & returns an instance of <code>StringBuilder</code> containing proper content.
     * 
     * @param mb The mailbox name
     * @return An instance of <code>StringBuilder</code> containing proper content
     */
    private static StringBuilder quoteIfNeeded(String mb) {
        if (mb.charAt(0) == '"' && mb.startsWith("\"", mb.length() - 1)) {
            // Already quoted
            return new StringBuilder(mb);
        }
        
        if (mb.charAt(0) == '.' || mb.indexOf("..") >= 0) {
            StringBuilder retval = new StringBuilder(mb.length() + 2);
            if (mb.charAt(0) != '"') {
                retval.append('"');
            }
            retval.append(mb);
            if (!mb.startsWith("\"", mb.length() - 1)) {
                retval.append('"');
            }
            return retval;
        }
