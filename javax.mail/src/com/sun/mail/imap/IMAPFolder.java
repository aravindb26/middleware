...
public class IMAPFolder extends Folder implements UIDFolder, ResponseHandler {
	...
	*/
    ...
    /**
     * Create this folder, with the specified type and given flag whether
     * failed <tt>CREATE</tt> command should be handled (<code>true</code>) or ignored (<code>false</code>).
     */
    public synchronized boolean create(final int type, final boolean handleCommandFailure)
				throws MessagingException {
	...
	final char sep = c;
	Object ret;
	if (handleCommandFailure) {
	    ret = doCommand(new ProtocolCommand() {
            @Override
            public Object doCommand(IMAPProtocol p)
                throws ProtocolException {
                if ((type & HOLDS_MESSAGES) == 0)   // only holds folders
        ...
        ret = doCommandIgnoreFailure(new ProtocolCommand() {
            @Override
            public Object doCommand(IMAPProtocol p)
                throws ProtocolException {
                if ((type & HOLDS_MESSAGES) == 0)	// only holds folders
    ...
    for (FetchProfile.Item item : fp.getItems()) {
        if ("ORIGINAL-MAILBOX".equals(item.name())) {
            if (protocol.getCapabilities().containsKey("XDOVECOT")) {
            ...
            if (protocol.getCapabilities().containsKey("SNIPPET=FUZZY")) {
            ...
            if (protocol.getCapabilities().containsKey("PREVIEW=FUZZY")) {
            ...
            if (protocol.getCapabilities().containsKey("PREVIEW")) {
	...
	return attributes == null ? com.openexchange.java.Strings.getEmptyStrings() : attributes.clone();
    }
	...
	try {
        ...
        throw handleCommandFailedException(cxf);
    } catch (com.sun.mail.iap.WrappingProtocolException wex) {
        throw wex.getMessagingException();
    } catch (ProtocolException pex) {
	    throw new MessagingException(pex.getMessage(), pex);
	}
	return null;
