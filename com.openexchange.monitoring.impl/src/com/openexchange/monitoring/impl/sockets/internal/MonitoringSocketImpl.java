...
public class MonitoringSocketImpl extends SocketImpl {
    ...
    private static class MonitoringSocket extends Socket {

        private final Delegator<SocketImpl> delegator;

        public MonitoringSocket(Delegator<SocketImpl> socketImplDelegator) throws SocketException {
            super((SocketImpl)socketImplDelegator.getDelegate());
            this.delegator = socketImplDelegator;
        }

        @Override
        public InetAddress getInetAddress() {
            return this.delegator.invoke(options().withMethodName("getInetAddress"));
        }

        @Override
        public int getPort() {
            return this.delegator.invoke(options().withMethodName("getPort"));
        }
    }
     ...
     */
    public MonitoringSocketImpl() throws SecurityException, SocketException {
        var parameterTypes = new Class<?>[] {Boolean.TYPE};
        var parameters = new Object[] {Boolean.FALSE}; //passing "false" to the cstor of PlainSocketImpl, because it is a client socket
        this.delegator = new Delegator<SocketImpl>(this, SocketImpl.class, "java.net.PlainSocketImpl", parameterTypes, parameters);
        this.socket = new MonitoringSocket(delegator);
    }

    private Socket getSocket0() {
        return socket;
    }
