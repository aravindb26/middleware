...
public class SubscriptionSQLStorage implements AdministrativeSubscriptionStorage {
    ...
    public Subscription getSubscription(final Context ctx, final int id) throws OXException {
        ...
        try {
            ...
            resultSet = builder.executeQuery(readConnection, select, List.of(I(id), I(ctx.getContextId())));
    ...
    public List<Subscription> getSubscriptions(final Context ctx, final String folderId) throws OXException {
        ...
        try {
            ...
            builder = new StatementBuilder();
            resultSet = builder.executeQuery(readConnection, select, List.of(I(ctx.getContextId()), folderId));
    ...
    public List<Subscription> getSubscriptionsOfUser(Context ctx, int userId) throws OXException {
        ...
        try {
            ...
            builder = new StatementBuilder();
            resultSet = builder.executeQuery(readConnection, select, List.of(I(ctx.getContextId()), I(userId)));
    ...
    private int getConfigurationId(final Subscription subscription) throws OXException {
        ...
        try {
            ...
            builder = new StatementBuilder();
            resultSet = builder.executeQuery(readConection, select, List.of(I(subscription.getContext().getContextId()), I(subscription.getId())));
    ...
    private List<Subscription> parseResultSet(final ResultSet resultSet, final Context ctx, final Connection readConnection) throws OXException, SQLException {
        ...
        List<Subscription> retval = new LinkedList<>();
        do {
    ...
    private boolean exist(final int id, final Context ctx) throws OXException {
        ...
        try {
            ...
            builder = new StatementBuilder();
            resultSet = builder.executeQuery(readConnection, select, List.of(I(ctx.getContextId()), I(id)));
    ...
    public Map<String, Boolean> hasSubscriptions(final Context ctx, final List<String> folderIds) throws OXException {
        ...
        try {
            readConnection = dbProvider.getReadConnection(ctx);
            final ArrayList<Expression> placeholders = new ArrayList<>(folderIds.size());
            for (int i = 0; i < folderIds.size(); i++) {
                placeholders.add(PLACEHOLDER);
            }

            SELECT select = new SELECT("folder_id").FROM(subscriptions).WHERE(new EQUALS("cid", PLACEHOLDER).AND(new IN("folder_id", new LIST(placeholders))));
            builder = new StatementBuilder();
            List<Object> values = new ArrayList<>(1 + folderIds.size());
            values.add(I(ctx.getContextId()));
            values.addAll(folderIds);
            resultSet = builder.executeQuery(readConnection, select, values);
            if (!resultSet.next()) {
                return allFalse(folderIds);
            }

            Map<String, Boolean> retval = allFalse(folderIds);
            do {
                retval.put(resultSet.getString(1), Boolean.TRUE);
            } while (resultSet.next());
            return retval;
        } catch (SQLException e) {
            throw SQLException.create(e);
    ...
    private static Map<String, Boolean> allFalse(List<String> folderIds) {
        Map<String, Boolean> retval = new HashMap<>(folderIds.size());
    ...
    public boolean hasSubscriptions(final Context ctx, final User user) throws OXException {
        ...
        try {
            readConnection = dbProvider.getReadConnection(ctx);
            SELECT select = new SELECT("1").FROM(subscriptions).WHERE(new EQUALS("cid", PLACEHOLDER).AND(new EQUALS("user_id", PLACEHOLDER)));
            builder = new StatementBuilder();
            resultSet = builder.executeQuery(readConnection, select, List.of(I(ctx.getContextId()), I(user.getId())));
