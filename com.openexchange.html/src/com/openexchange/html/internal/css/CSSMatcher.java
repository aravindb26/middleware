...
public final class CSSMatcher {
            ...
            if (null != elementName) {
                ...
                if (null != allowedValuesSet) {
                    ...
                    if (matches(elementValues, allowedValuesSet)) {
                        ...
                        boolean first = true;
                        for (Token token : splitToTokens(elementValues)) {
                            if (token.isFunction()) {
                                if (HtmlServices.containsEventHandler(token.getToken()) || false == HtmlServices.isSafe(token.getToken(), token.getToken())) {
                                    modified = true;
                                } else {
                                    if (first) {
                                        elemBuilder.append(elementName).append(':').append(' ');
                            ...
                            } else if (matches(token.getToken(), allowedValuesSet)) {
                                if (HtmlServices.containsEventHandler(token.getToken()) || false == HtmlServices.isSafe(elementValues, elementValues)) {
                                    modified = true;
                                } else {
                                    if (first) {
                                        elemBuilder.append(elementName).append(':').append(' ');
    ...
    private static List<Token> splitToTokens(final String elementValues) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            case ' ':
            {
                if (open > 0) {
                    ...
                    if (token.length() > 0) {
                        l.add(new PlainToken(token.toString()));
                        token.setLength(0);
                    }
                }
            }
            ...
            case ')':
                ...
                if (open == 0) {
                    if (token.length() > 0) {
                        Function f = new Function(token.toString());
                        token.setLength(0);
                        if ("expression".equals(f.getName()) || "url".equals(f.getName())) {
                            l.add(new PlainToken(f.toString()));
                            f = null; // discard
                        } else {
                            l.add(f);
                        }
    ...
    private static class Function implements Token {
        ...
        Function(String function) {
            ...
            this.name = index > 0 ? Strings.asciiLowerCase(function.substring(0, index)) : null;
        }
    ...
    private static class PlainToken implements Token {
