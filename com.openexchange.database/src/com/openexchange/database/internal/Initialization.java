...
import com.openexchange.database.preupgrade.CrossContextDBPreUpdateTask;
...
public final class Initialization {
     ...
     * @param leanConfigService A reference to the lean configuration service
     ...
     */
    public synchronized DatabaseServiceImpl start(ConfigurationService configurationService, LeanConfigurationService leanConfigService, ConfigViewFactory configViewFactory, DBMigrationExecutorService migrationService, ServiceListing<GeneralDatabaseConnectionListener> connectionListeners, Configuration configuration, ConnectionReloaderImpl reloader, BundleContext context) throws OXException {
        ...
        // Await availability of database
        ReachabilityCheck.checkReachability(0, configDatabaseService, configuration.getWriteUrl());
        // Init server ID
        ...
        // Schedule pending migrations only if upgrade preparation is disabled
        boolean upgradePreparation = leanConfigService.getBooleanProperty(PreUpgradeProperty.ENABLED);
        if (null != migrationService && !upgradePreparation) {
            configDatabaseService.scheduleMigrations(migrationService);
            globalDatabaseService.scheduleMigrations(migrationService);
        }
        ConfigDBMigrationServiceImpl configDBMigrationService = new ConfigDBMigrationServiceImpl(configDatabaseService, migrationService);
        context.registerService(ConfigDBMigrationService.class, configDBMigrationService, null);
        if (upgradePreparation) {
            context.registerService(PreUpgradeTask.class, configDatabaseService.createPreUpgradeTask(configDBMigrationService), Tools.withRanking(Integer.MAX_VALUE));
            context.registerService(PreUpgradeTask.class, new CrossContextDBPreUpdateTask(globalDatabaseService, migrationService), Tools.withRanking(Integer.MAX_VALUE - 10)); // Give headroom for other configdb pre-upgrade tasks
        }
