...
public final class ConfigDatabaseServiceImpl implements ConfigDatabaseService {
    ...
    public DBMigrationState scheduleMigrations(DBMigrationExecutorService migrationService) {
        DBMigration migration = getMigration();
        migrationService.register(migration);
        return migrationService.scheduleDBMigration(migration);
    }

    /**
     * Creates a pre-upgrade task for the config database
     *
     * @return The config db pre-upgrade task
     */
    public PreUpgradeTask createPreUpgradeTask(ConfigDBMigrationService configDBMigrationService) {
        return new ConfigDBPreUpgradeTask(configDBMigrationService, CONFIGDB_CHANGE_LOG, getResourceAccessor());
    }
     ...
     */
    private DBMigration getMigration() {
        // Use appropriate connection provider from config database & a local resource accessor for the changeset file
        ResourceAccessor localResourceAccessor = getResourceAccessor();
        ...
        return new DBMigration(connectionProvider, CONFIGDB_CHANGE_LOG, localResourceAccessor, schema);
    }

    /**
     * Creates a {@link ResourceAccessor}
     *
     * @return The {@link ResourceAccessor}
     */
    private ResourceAccessor getResourceAccessor() {
        return new BundleResourceAccessor(FrameworkUtil.getBundle(ConfigDatabaseServiceImpl.class));
    }
    ...
    @Override
    public String getServerUid() throws OXException {
        return Server.getServerUid();
    }
