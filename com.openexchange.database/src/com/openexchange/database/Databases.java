...
public final class Databases {
    ...
    /** The default limit when paginating over big result sets: <code>100,000</code */
    public static final int PAGINATION_LIMIT = 100000;
    ...
    private static final Pattern PAT_TRUNCATED_IDS = Pattern.compile("[^']*'(\\S+)'[^']*"); // NOSONARLINT
     ...
     */
    public static String[] parseTruncatedFields(DataTruncation trunc) { // NOSONARLINT
    ...
    /**
     * Appends a <code>"CONCAT ... IN"</code> expression to given string builder; e.g.<br>
     * <div>
     * "<code>UPDATE `mytable` SET `col3`='foo' WHERE CONCAT(`col1`, `col2`) IN (CONCAT(?, ?), CONCAT(?, ?))"</code>
     * </div>
     * <p>
     * <b>Note</b>: SQL statement is expected to end with a space (<code>" "</code>)

     *
     * @param sqlBuilder The string builder containing SQL statement; e.g. <code>"UPDATE `mytable` SET `col3`='foo' WHERE "</code>
     * @param count The number of <code>CONCAT</code> tuples
     * @param columnNames The column names; e.g. <code>"CONCAT(`col1`, `col2`)"</code>
     */
    public static void appendConcatIN(StringBuilder sqlBuilder, int count, String... columnNames) {
        if (count <= 0) {
            throw new IllegalArgumentException("The specified number of CONCAT tuples must not be less than/equal to 0 (zero)");
        }
        if (columnNames == null || columnNames.length <= 0) {
            throw new IllegalArgumentException("The specified column names must not be null or empty");
        }

        if (columnNames.length == 1) {
            // Eh...? No "CONCAT ... IN" needed then...
            sqlBuilder.append('`').append(columnNames[0]).append('`');
        ...
        for (int i = 1; i < columnNames.length; i++) {
            sqlBuilder.append(", ").append('`').append(columnNames[i]).append('`');
        }
        sqlBuilder.append(") IN (");

        appendConcatIN(sqlBuilder, columnNames.length, count);
    }
     ...
     *
     * @param sqlBuilder The string builder containing SQL statement; e.g. <code>"UPDATE `mytable` SET `col3`='foo' WHERE CONCAT(`col1`, `col2`) IN ("</code>
     * @param columnCount The number of columns to consider for a single <code>"CONCAT"</code>; e.g. <code>"CONCAT(?, ?)"</code> or <code>"CONCAT(?, ?, ?)"</code>
     * @param count The number of <code>CONCAT</code> tuples
     */
    public static void appendConcatIN(StringBuilder sqlBuilder, int columnCount, int count) {
        if (count <= 0) {
            throw new IllegalArgumentException("The specified number of CONCAT tuples must not be less than/equal to 0 (zero)");
        }
        if (columnCount <= 0) {
            throw new IllegalArgumentException("The specified number of columns must not be less than/equal to 0 (zero)");
        }

        appendConcatWithPlaceholders(sqlBuilder, columnCount);
        for (int i = count - 1; i-- > 0;) {
            sqlBuilder.append(", ");
            appendConcatWithPlaceholders(sqlBuilder, columnCount);
        }
    ...
    private static void appendConcatWithPlaceholders(StringBuilder sqlBuilder, int columnCount) {
        sqlBuilder.append("CONCAT(?");
        for (int i = columnCount - 1; i-- > 0;) {
            sqlBuilder.append(", ?");
        }
        sqlBuilder.append(')');
    }
    ...
    public static int getColumnSize(Connection con, String table, String column) throws SQLException {
        ...
        int retval = -1;
        try (ResultSet result = metas.getColumns(con.getCatalog(), null, table, column)) {
            if (result.next()) {
        ...
        try {
            ...
            if (table.equalsIgnoreCase(foundTable)) {
                ...
                try (PreparedStatement stmt = con.prepareStatement("SHOW variables LIKE 'lower_case_table_names';"); ResultSet result = stmt.executeQuery()) {
                    if (result.next()) {
                        return switch (result.getInt("Value")) {
                            case 1, // table names are stored in lowercase, name comparisons are case-insensitive ("windows")
                                 2 // table names are stored in specified lettercase, but lowercased on lookup, name comparisons are case-insensitive ("mac os")
                                 -> true;
    ...
    /**
     * Checks if specified SQL function exists by probing.
     *
     * @param con The connection to use
     * @param functionName The name of the function to call
     * @param optArgument The argument to pass or <code>null</code>
     * @param literal <code>true</code> to append argument (if any) as raw literal; otherwise <code>false</code> to treat as string argument
     * @return <code>true</code> if such a function exists; otherwise <code>false</code>
     * @throws SQLException If something goes wrong
     */
    public static boolean functionExists(Connection con, String functionName, String optArgument, boolean literal) throws SQLException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            if (optArgument == null) {
                stmt = con.prepareStatement("SELECT " + functionName + "() AS test");
            } else {
                if (literal) {
                    stmt = con.prepareStatement("SELECT " + functionName + "(" + optArgument + ") AS test");
                } else {
                    stmt = con.prepareStatement("SELECT " + functionName + "(?) AS test");
                    stmt.setString(1, optArgument);
                }
            }
            try {
                ...
                // ERROR 1305 (42000): FUNCTION XYZ does not exist
                // ERROR 1370 (42000): execute command denied to user 'XYZ' for routine XYZ
                if ("42000".equals(e.getSQLState())) {
                    // Check error code, too
                    int errorCode = e.getErrorCode();
                    if (1305 == errorCode || 1370 == errorCode) {
                        return false;
                    }
                }
                throw e;
    ...
    public static boolean columnsExist(final Connection con, final String table, final String... columns) throws SQLException {
        ...
        try {
            rs = metaData.getColumns(con.getCatalog(), null, table, null);
            while (rs.next()) {
     ...
     */
    public static boolean isNullable(Connection con, String table, String column) throws SQLException {
        DatabaseMetaData meta = con.getMetaData();
        ResultSet result = null;
        try {
            result = meta.getColumns(con.getCatalog(), null, table, column);
            if (!result.next()) {
    ...
    public static String getCharacterSet(Connection connection) throws SQLException {
        ...
        try {
            String characterSet = CHARSETS_BY_SCHEMA.getIfPresent(schemaName);
            return characterSet != null ? characterSet : CHARSETS_BY_SCHEMA.get(schemaName, () -> readCharacterSet(connection));
        } catch (ExecutionException e) {
            Throwable cause = e.getCause();
            throw cause instanceof SQLException ? (SQLException) cause : new SQLException(cause);
        }
    }
    ...
    public static boolean isTransactionRollbackException(final SQLException sqlException) {
        ...
        if (null == cause || !(cause instanceof Exception)) { // NOSONARLINT
            return false;
        }
    ...
    public static boolean isPacketTooBigException(SQLException e) {
        return e instanceof com.mysql.cj.jdbc.exceptions.PacketTooBigException;
    }

    private static final RetryPolicy RETRY_POLICY_TRANSACTION_ROLLBACK = new RetryPolicy()
        .withMaxRetries(3).withBackoff(1, 10, TimeUnit.SECONDS).withJitter(0.25F)
        .retryOn(t -> t instanceof Exception && isTransactionRollbackException((Exception) t));

    private static final CheckedConsumer<Throwable> ON_RETRY_LISTENER_TRANSACTION_ROLLBACK = new CheckedConsumer<Throwable>() {

        @Override
        public void accept(Throwable t) throws Exception {
            LOG.debug("Retrying transaction", t);
        }
    };

    /**
     * Executes given callable with retry behavior on transaction roll-back error.
     *
     ...
     */
    public static <V> V executeWithRetryOnTransactionRollbackException(Callable<V> callable) throws OXException {
        try {
            return Failsafe.with(RETRY_POLICY_TRANSACTION_ROLLBACK).onRetry(ON_RETRY_LISTENER_TRANSACTION_ROLLBACK).get(callable);
            ...
            throw OXException.general(t.getMessage(), t);
