...
public class PooledData<T> {
    ...
    public long getLastPacketDiffFallbackToTimeDiff() {
        Long idleTime = MysqlUtils.getIdleTime(pooled);
        return idleTime == null ? getTimeDiff() : l(idleTime);
    }

    /**
     * Gets the idle time from pooled object (if available); otherwise fall-back to {@link #getTimeDiff()}.
     *
     * @return The idle time or {@link #getTimeDiff()}
     */
    public long getIdleTimeFallbackToTimeDiff() {
        Long idleTime = MysqlUtils.getIdleTime(pooled);
        return idleTime == null ? getTimeDiff() : l(idleTime);
    }
