...
public class HazelcastSessionStorageActivator extends HousekeepingActivator implements Unregisterer {
    ...
    /** The logger */
    static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(HazelcastSessionStorageActivator.class);

    private ServiceTracker<HazelcastInstance, HazelcastInstance> mHzSessionStorageRegistrationTracker;
    ...
    protected synchronized void startBundle() throws Exception {
        ...
        if (!enabled) {
            LOG.warn("Hazelcast session storage service will be disabled due to disabled Hazelcast services");
        } else {
            /*
            ...
            boolean storageEnabled = null == configService || configService.getBoolProperty("com.openexchange.sessionstorage.hazelcast.enabled", true);
            if (storageEnabled && configService != null && configService.getBoolProperty("com.openexchange.sessiond.redis.enabled", false)) {
                LOG.info("Redis sessiond service enabled via configuration. Aborting start-up of Hazelcast session storage service...");
            ...
            if (storageEnabled) {
                ...
                ServiceTracker<HazelcastInstance, HazelcastInstance> hzSessionStorageRegistrationTracker = new ServiceTracker<>(context, HazelcastInstance.class, new ServiceTrackerCustomizer<HazelcastInstance, HazelcastInstance>() {

                    private ServiceRegistration<SessionStorageService> mSessionStorageRegistration;
                    private ScheduledTimerTask mSessionToucherTask;
                    ...
                    public synchronized HazelcastInstance addingService(final ServiceReference<HazelcastInstance> reference) {
                        ...
                        mSessionStorageRegistration = context.registerService(SessionStorageService.class, sessionStorage, null);
                        /*
                        ...
                        mSessionToucherTask = getService(TimerService.class).scheduleAtFixedRate(new SessionToucher(sessionStorage), period, period);
                    ...
                    public synchronized void removedService(final ServiceReference<HazelcastInstance> reference, final HazelcastInstance service) {
                         ...
                         */
                        ScheduledTimerTask sessionToucherTask = this.mSessionToucherTask;
                        if (null != sessionToucherTask) {
                            sessionToucherTask.cancel();
                            this.mSessionToucherTask = null;
                        }
                        /*
                         * remove session storage registration
                         */
                        ServiceRegistration<SessionStorageService> sessionStorageRegistration = this.mSessionStorageRegistration;
                        if (null != sessionStorageRegistration) {
                            sessionStorageRegistration.unregister();
                            this.mSessionStorageRegistration = null;
                        }
                ...
                this.mHzSessionStorageRegistrationTracker = hzSessionStorageRegistrationTracker;
                // Open others
                openTrackers();
            }
        }
    ...
    @Override
    public synchronized void unregisterSessionStorage() {
        ServiceTracker<HazelcastInstance, HazelcastInstance> hzSessionStorageRegistrationTracker = this.mHzSessionStorageRegistrationTracker;
        if (null != hzSessionStorageRegistrationTracker) {
            hzSessionStorageRegistrationTracker.close();
            this.mHzSessionStorageRegistrationTracker = null;
        }
    }

    @Override
    public synchronized void propagateNotActive(HazelcastInstanceNotActiveException notActiveException) {
        final BundleContext context = this.context;
        if (null != context) {
