 ...
 */
public class JerichoRenderer extends Renderer {
    ...
    /**
     * Constructs a new <code>JerichoRenderer</code> based on the specified {@link Segment}.
     * @param segment  the segment containing the HTML to be rendered.
     * @see Segment#getRenderer()
     */
    public JerichoRenderer(final Segment segment) {
        super(segment);
        rootSegment=segment;
    }
    ...
    private static final class Processor {
        private final JerichoRenderer renderer;
        private final Segment rootSegment;
        ...
        private static ElementHandler getElementHandler(final Element element) {
            if (element.getStartTag().getStartTagType().isServerTag())
             {
                return RemoveElementHandler.INSTANCE; // hard-coded configuration does not include server tags in child element hierarchy, so this is normally not executed.
            }
    ...
    abstract private static class AbstractBlockElementHandler implements ElementHandler {
        ...
        public void process(Processor x, Element element) throws IOException {
            ...
            if (leftMargin>x.maxLineLength-x.indentSize) {
                leftMargin=x.maxLineLength-x.indentSize;
            }
    ...
    private static final class BR_ElementHandler implements ElementHandler {
        ...
        public void process(Processor x, Element element) throws IOException {
            if (x.isBlockBoundary() && !x.atStartOfLine && !x.skipInitialNewLines)
             {
                x.newLine(); // add an extra new line if we're at a block boundary and aren't already at the start of the next line and it's not the first element after <li>
            }
            x.newLine();
            x.blockBoundary(0);
        }
