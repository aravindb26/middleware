...
import com.openexchange.processing.ProcessorTasks;
...
public final class SessionHandler {
    ...
    /** The processor to perform session-storage related tasks */
    private static volatile Processor sessionStorageProcessor;
    ...
    private static void logAbsentSessionDataInstance() {
    ...
    public static synchronized void close() {
        ...
        if (null != sd) {
            ...
            sd.clear();
            LOG.info("Closed SessionD. All sessions dropped...");
            SESSION_DATA_REF.set(null);
        } else {
            logAbsentSessionDataInstance();
        }
        ...
        Processor p = sessionStorageProcessor;
        if (null != p) {
            sessionStorageProcessor = null;
            p.stop();
        }
    ...
    /**
     * Gets the processor to perform session-storage related tasks.
     *
     * @return The processor
     */
    public static Processor getSessionStorageProcessor() {
        Processor p = sessionStorageProcessor;
        if (null == p) {
            synchronized (SessionHandler.class) {
                p = sessionStorageProcessor;
                if (null == p) {
                    ProcessorService processorService = Services.optService(ProcessorService.class);
                    if (processorService == null) {
                        LOG.warn("Processor service is absent. Using dummy caller-runs processor instead");
                        return ProcessorTasks.getCallerRunsProcessor();
                    }

                    ConfigurationService configService = Services.optService(ConfigurationService.class);
                    int defaultNumThreads = 10;
                    int numThreads = null == configService ? defaultNumThreads : configService.getIntProperty("com.openexchange.sessiond.storage.processor.numThreads", defaultNumThreads);
                    int defaultMaxNumTasks = 2048;
                    int maxNumTasks = null == configService ? defaultMaxNumTasks : configService.getIntProperty("com.openexchange.sessiond.storage.processor.maxNumTasks", defaultMaxNumTasks);

                    try {
                        int numMinThreads = 4;
                        if (numThreads > numMinThreads) {
                            p = processorService.newBoundedProcessor("SessiondStorageProcessor", numMinThreads, numThreads, maxNumTasks);
                        } else {
                            p = processorService.newBoundedProcessor("SessiondStorageProcessor", numThreads, maxNumTasks);
                        }
                    } catch (Exception e) {
                        LOG.warn("Failed creation of processor instance. Using dummy caller-runs processor instead", e);
                        return ProcessorTasks.getCallerRunsProcessor();
                    }
                    sessionStorageProcessor = p;
                }
            }
        }
        return p;
    ...
    private static void removeRemoteContextSessions(final Set<Integer> contextIds) throws OXException {
        ...
        if (hazelcastInstance != null) {
            LOG.debug("Trying to remove sessions for context ids {} from remote nodes", LogUtility.toStringObjectFor(contextIds, ", "));
                ...
                for (Entry<Member, Future<Set<Integer>>> memberEntry : submitToMembers.entrySet()) {
                    ...
                    try {
                        ...
                        if ((contextIdsSessionsHaveBeenRemovedFor != null) && (future.isDone())) {
                            LOG.info("Removed sessions for context ids {} on remote node {}", LogUtility.toStringObjectFor(contextIdsSessionsHaveBeenRemovedFor, ", "), member.getSocketAddress());
                        } else {
                            LOG.warn("No sessions for context ids {} removed on node {}.", LogUtility.toStringObjectFor(contextIds, ", "), member.getSocketAddress());
                        }
                    } catch (TimeoutException e) {
                        // Wait time elapsed; enforce cancelation
                        future.cancel(true);
                        LOG.error("Removing sessions for context ids {} on remote node {} took to longer than {} seconds and was aborted!", LogUtility.toStringObjectFor(contextIds, ", "), member.getSocketAddress(), I(hzExecutionTimeout), e);
                        ...
                        future.cancel(true);
                        LOG.error("Removing sessions for context ids {} on remote node {} took to longer than {} seconds and was aborted!", LogUtility.toStringObjectFor(contextIds, ", "), member.getSocketAddress(), I(hzExecutionTimeout), e.getCause());
                    } catch (Exception e) {
                        ...
                        throw SessionExceptionCodes.REMOTE_SESSION_REMOVAL_FAILED.create(Strings.concat(", ", contextIds), member.getSocketAddress(), e);
                    }
                }
            } else {
            ...
            LOG.warn("Cannot find HazelcastInstance for remote execution of session removing for context ids {}. Only local sessions will be removed.", LogUtility.toStringObjectFor(contextIds, ", "));
        }
    }
    ...
    private static void removeRemoteUserSessions(int userId, int contextId) {
        ...
        for (Entry<Member, Integer> memberEntry : results.entrySet()) {
            ...
            if (numOfRemovedSessions == null) {
                LOG.warn("No sessions removed for user {} in context {} on remote node {}.", I(userId), I(contextId), member.getSocketAddress());
            } else {
                LOG.info("Removed {} sessions for user {} in context {} on remote node {}", numOfRemovedSessions, I(userId), I(contextId), member.getSocketAddress());
            }
        }
    }
    ...
    public static List<String> removeRemoteSessions(SessionFilter filter) {
        ...
        for (Entry<Member, Collection<String>> memberEntry : results.entrySet()) {
            ...
            if (memberSessionIds != null) {
                LOG.debug("Removed {} sessions on node {} for filter '{}'", I(memberSessionIds.size()), memberEntry.getKey().getSocketAddress(), filter);
    ...
    public static List<String> findRemoteSessions(SessionFilter filter) {
        ...
        for (Entry<Member, Collection<String>> memberEntry : results.entrySet()) {
            Collection<String> memberSessionIds = memberEntry.getValue();
            if (memberSessionIds != null) {
                LOG.debug("Found {} sessions on node {} for filter '{}'", I(memberSessionIds.size()), memberEntry.getKey().getSocketAddress(), filter);
                sessionIds.addAll(memberSessionIds);
            }
        }
    ...
    private static <T> Map<Member, T> executeGlobalTask(Callable<T> callable) {
        ...
        for (Entry<Member, Future<T>> memberEntry : submitToMembers.entrySet()) {
            ...
            try {
                ...
                future.cancel(true);
                LOG.error("Executing callable {} on remote node {} took to longer than {} seconds and was aborted!", callable, member.getSocketAddress(), I(hzExecutionTimeout), e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                future.cancel(true);
                LOG.error("Executing callable {} on remote node {} took to longer than {} seconds and was aborted!", callable, member.getSocketAddress(), I(hzExecutionTimeout), e);
                ...
                future.cancel(true);
                LOG.error("Executing callable {} on remote node {} failed!", callable, member.getSocketAddress(), e.getCause());
            } finally {
    ...
    private static SessionImpl createNewSession(int userId, String loginName, String password, int contextId, String clientHost, String login, String authId, String hash, String client, boolean tranzient, boolean staySignedIn, Origin origin) throws OXException {
        ...
        return new SessionImpl(userId, loginName, password, contextId, sessionId, secret, randomToken, clientHost, login, authId, hash, client, tranzient, staySignedIn, origin);
    }
    ...
    public static void storeSession(SessionImpl session, final SessionStorageService sessionStorageService, final boolean addIfAbsent, final boolean async) {
        ...
        if (async) {
            boolean submitted = getSessionStorageProcessor().execute(UserAndContext.newInstance(session), ProcessorTasks.processorTaskFor(new StoreSessionTask(session, sessionStorageService, addIfAbsent)));
            if (submitted == false) {
                LOG.warn("Too many pending session storage tasks currently executed by session storage processor. Could not add session {} to session storage", session.getSessionID());
            }
        } else {
    ...
    protected static void changeSessionPassword(final String sessionid, final String newPassword) throws OXException {
        ...
        if (null != sessionStorage && useSessionStorage(currentSession)) {
            ...
            boolean submitted = getSessionStorageProcessor().execute(UserAndContext.newInstance(currentSession), ProcessorTasks.processorTaskFor(c));
            if (submitted == false) {
                LOG.warn("Too many pending session storage tasks currently executed by session storage processor. Could not propagate changed password of session {} to session storage", currentSession.getSessionID());
            }
        }
        /*
    ...
    protected static void cleanUp() {
        ...
        LOG.info("Going to rotate short-term containers");
    ...
    protected static void cleanUpLongTerm() {
        ...
        LOG.info("Going to rotate long-term containers");
    ...
    private static void postSessionRemoval(final SessionControl sessionControl) {
        ...
        if (useSessionStorage(session)) {
            ...
            if (sessionStorageService != null) {
                ...
                boolean submitted = getSessionStorageProcessor().execute(UserAndContext.newInstance(session), ProcessorTasks.processorTaskFor(ft));
                if (submitted) {
                    dropSessionFromHz = ft;
                } else {
                    LOG.warn("Too many pending session storage tasks currently executed by session storage processor. Could not remove session {} from session storage", session.getSessionID());
                }
            }
        }
    ...
    private static void postLastSessionGone(int userId, int contextId, EventAdmin eventAdmin) {
        ...
        SessionData sessionData = SESSION_DATA_REF.get();
        if (null != sessionData && false == sessionData.hasForContext(contextId)) {
    ...
    private static List<String> determineSessionsToRemove(List<? extends SessionControl> sessionControls) {
    ...
    protected static void postContainerRemoval(List<? extends SessionControl> sessionControls, boolean removeFromSessionStorage) {
        ...
        if (removeFromSessionStorage) {
            ...
            SessionStorageService sessionStorageService = Services.optService(SessionStorageService.class);
            if (sessionStorageService != null) {
                List<String> sessionsToRemove = determineSessionsToRemove(sessionControls);
                if (sessionsToRemove != null) {
                    Callable<Void> task = new AbstractTask<Void>() {
                        ...
                        public Void call() {
                            try {
                                ...
                                LOG.error("Failed to remove multiple sessions from session storage: {}", sessionsToRemove, e);
                            }
                            return null;
                        }
                    };
                    boolean submitted = getSessionStorageProcessor().execute(UserAndContext.newInstance(0, 0), ProcessorTasks.processorTaskFor(task));
                    if (submitted == false) {
                        LOG.warn("Too many pending session storage tasks currently executed by session storage processor. Could not remove sessions {} from session storage", sessionsToRemove);
                    }
                }
            }
        }
    ...
    public static Reloadable getReloadable() {
        return new Reloadable() {
            ...
            public Interests getInterests() {
                return Reloadables.interestsForProperties("com.openexchange.sessiond.sessionstorage.timeout");
            }
