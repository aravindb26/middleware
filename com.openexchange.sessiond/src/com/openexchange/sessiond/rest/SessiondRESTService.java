...
public class SessiondRESTService extends JAXRSService {
    ...
    private void log(Collection<String> sessions, SessionFilter filter, boolean global) {
        ...
        if (sessions.isEmpty()) {
            LOGGER.debug("No sessions were cleared {} via REST invocation with filter '{}'.", global ? "globally" : "locally", filter);
            return;
        }
        ...
        LOGGER.debug("Cleared sessions {} via REST invocation with filter '{}', {}", global ? "globally" : "locally", filter, sessions);
    }
