        ...
        mappings.put(ContactField.DISTRIBUTIONLIST, new ArrayMapping<DistributionListEntryObject, Contact>(ContactFields.DISTRIBUTIONLIST, I(Contact.DISTRIBUTIONLIST)) {
            ...
            protected DistributionListEntryObject deserialize(JSONArray array, int index) throws JSONException, OXException {
                ...
                if (entry.hasAndNotNull(DataFields.ID) && 0 < entry.getString(DataFields.ID).length()) {
                    member.setEntryID(entry.getString(DataFields.ID));
                }
            ...
            public void copy(Contact from, Contact to) throws OXException {
                DistributionListEntryObject[] members = get(from);
                DistributionListEntryObject[] copiedMembers;
                if (null == members || 0 == members.length) {
                    copiedMembers = members;
                } else {
                    copiedMembers = new DistributionListEntryObject[members.length];
                    for (int i = 0; i < members.length; i++) {
                        copiedMembers[i] = new DistributionListEntryObject(members[i]);
                    }
                }
                set(to, copiedMembers);
            }
