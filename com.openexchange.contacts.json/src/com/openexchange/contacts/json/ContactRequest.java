...
public class ContactRequest {
    ...
    /** The previously used special "use count, global first" sort order */
    private static final int DEPRACATED_USE_COUNT_GLOBAL_FIRST = 609;
    ...
    public boolean isInternalSort() throws OXException {
        ...
        return 0 == sort || Contact.SPECIAL_SORTING == sort;
    }
    ...
    public boolean sortInternalIfNeeded(List<Contact> contacts) throws OXException {
        if (this.isInternalSort() && null != contacts && 1 < contacts.size()) {
            Collections.sort(contacts, new SpecialAlphanumSortContactComparator(user.getLocale(), Order.DESCENDING != getOrder()));
            return true;
        }
        return false;
    }
    ...
    public JSONObject getJSONData() throws OXException {
        ...
        if (!(data instanceof JSONObject)) {
            throw data == null ? OXJSONExceptionCodes.MISSING_FIELD.create("data") : OXJSONExceptionCodes.INVALID_VALUE.create("data", data.toString());
        }
    ...
    public int getSort() throws OXException {
        int sort = RequestTools.getNullableIntParameter(request, "sort");
        return DEPRACATED_USE_COUNT_GLOBAL_FIRST == sort ? Contact.USE_COUNT : sort;
    }

    public Order getOrder() throws OXException {
        Order retval = this.order;
        if (retval == null) {
            String order = request.getParameter("order");
            retval = null == order && Contact.USE_COUNT == getSort() ? Order.DESCENDING : OrderFields.parse(order);
            this.order = retval;
        }
        return retval;
    }
    ...
    public JSONObject getContactJSON(final boolean isUpload) throws OXException {
        ...
        try {
            return JSONServices.parseObject(jsonField);
        } catch (JSONException e) {
    ...
    public int[] getUserIds() throws OXException {
        JSONArray jArray = (JSONArray) request.requireData();
        int[] userIdArray = new int[jArray.length()];
        for (int i = userIdArray.length; i-- > 0;) {
            try {
                userIdArray[i] = jArray.getInt(i);
            } catch (JSONException e) {
                throw OXJSONExceptionCodes.JSON_READ_ERROR.create(e, jArray);
            }
        }
