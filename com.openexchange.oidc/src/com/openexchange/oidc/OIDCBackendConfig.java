...
public interface OIDCBackendConfig {
    ...
    /**
     * Gets a value indicating whether backchannel logout is enabled or not. If enabled, a separate servlet is
     * registered under <code>/<dispatcherPrefix>/oidc/<backendPath>/backchannel_logout</code>, which will accept
     * backchannel logout requests from the OP (the "sid" claim is required in the logout token to associate the
     * OP session with the corresponding OX session properly).
     * 
     * @return <code>true</code> if backchannel logout is enabled, <code>false</code>, otherwise
     */
    boolean isBackchannelLogoutEnabled();

    /**
     * Gets a value indicating whether frontchannel logout is enabled or not. If enabled, a separate servlet is
     * registered under <code>/<dispatcherPrefix>/oidc/<backendPath>/frontchannel_logout</code>, which will accept
     * frontchannel logout requests from the OP (the "sid" parameter is required in the request to associate the
     * OP session with the corresponding OX session properly).
     * 
     * @return <code>true</code> if frontchannel logout is enabled, <code>false</code>, otherwise
     */
    boolean isFrontchannelLogoutEnabled();
