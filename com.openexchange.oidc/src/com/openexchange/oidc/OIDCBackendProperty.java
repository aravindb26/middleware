...
public enum OIDCBackendProperty implements Property {
    ...
    /**
     * backchannelLogoutEnabled - Configures whether backchannel logout is enabled or not. If enabled, a
     * separate servlet is registered under <code>/<dispatcherPrefix>/oidc/<backendPath>/backchannel_logout</code>,
     * which will accept backchannel logout requests from the OP (the "sid" claim is required in the logout token
     * to associate the OP session with the corresponding OX session properly).
     */
    backchannelLogoutEnabled(OIDCProperty.PREFIX, Boolean.FALSE),
    /**
     * frontchannelLogoutEnabled - Configures whether frontchannel logout is enabled or not. If enabled, a
     * separate servlet is registered under <code>/<dispatcherPrefix>/oidc/<backendPath>/frontchannel_logout</code>,
     * which will accept frontchannel logout requests from the OP (the "sid" parameter is required in the request
     * to associate the OP session with the corresponding OX session properly).
     */
    frontchannelLogoutEnabled(OIDCProperty.PREFIX, Boolean.FALSE),
