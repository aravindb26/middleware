...
public class LoggingActivator implements BundleActivator, Reloadable {
    ...
    /** The logger */
    protected static final Logger LOGGER = LoggerFactory.getLogger(LoggingActivator.class);
    ...
    public synchronized void start(BundleContext context) throws Exception {
        ...
        AtomicReference<Authenticator> authenticatorReference = new AtomicReference<Authenticator>(null);
        final ServiceTracker<Authenticator, Authenticator> tracker = new ServiceTracker<>(context, Authenticator.class, null) {

            @Override
            public Authenticator addingService(ServiceReference<Authenticator> reference) {
                Authenticator authenticator = super.addingService(reference);
            ...
            public void removedService(ServiceReference<Authenticator> reference, Authenticator service) {
                authenticatorReference.set(null);
                super.removedService(reference, service);
            }

        };
        tracker.open();
        this.authenticatorTracker = tracker;

        // Register RMI logback config service
        logbackConfigurationRMIService = new LogbackConfigurationRMIServiceImpl(logbackConfigService, authenticatorReference);
    ...
    public synchronized void stop(BundleContext context) throws Exception {
        ...
        final ServiceTracker<Authenticator, Authenticator> authenticatorTracker = this.authenticatorTracker;
        if (null != authenticatorTracker) {
            this.authenticatorTracker = null;
    ...
    private RankingAwareTurboFilterList initialiseRankingAwareTurboFilterList(LoggerContext loggerContext) {
        final RankingAwareTurboFilterList tmp = new RankingAwareTurboFilterList();
        this.rankingAwareTurboFilterList = tmp;
        loggerContext.addTurboFilter(tmp);
        return tmp;
    }
    ...
    protected synchronized void registerDeprecatedLogstashAppenderMBeanTracker(BundleContext context) {
        try {
            ...
            ServiceTracker<ManagementService, ManagementService> tracker = new ServiceTracker<>(context, ManagementService.class, logstashAppenderRegistration);
    ...
    private synchronized ServiceTracker<ManagementService, ManagementService> registerRemoteAppenderMBeanTracker(BundleContext context, AbstractRemoteAppenderMBeanRegisterer registerer) {
        ServiceTracker<ManagementService, ManagementService> tracker = new ServiceTracker<>(context, ManagementService.class, registerer);
        tracker.open();
        return tracker;
    }
    ...
    protected synchronized void registerExceptionCategoryFilter(final BundleContext context, final RankingAwareTurboFilterList turboFilterList, IncludeStackTraceServiceImpl serviceImpl) {
        ...
        final ServiceTracker<ConfigurationService, ConfigurationService> tracker = new ServiceTracker<>(context, ConfigurationService.class, exceptionCategoryFilterRegisterer);
