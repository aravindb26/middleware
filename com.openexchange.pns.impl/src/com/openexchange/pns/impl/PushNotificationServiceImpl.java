...
public class PushNotificationServiceImpl implements PushNotificationService {
    ...
    /** The logger constant */
    static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(PushNotificationServiceImpl.class);
    ...
    private static volatile Integer numMaxProcessorThreads;

    private static int numMaxProcessorThreads(ConfigurationService configService) {
        Integer tmp = numMaxProcessorThreads;
        if (null == tmp) {
            synchronized (PushNotificationServiceImpl.class) {
                tmp = numMaxProcessorThreads;
                if (null == tmp) {
                    int defaultValue = 100; // 100 threads
                    if (null == configService) {
                        return defaultValue;
                    }
                    tmp = Integer.valueOf(configService.getIntProperty("com.openexchange.pns.numMaxProcessorThreads", defaultValue));
                    numMaxProcessorThreads = tmp;
                }
            }
        }
        return tmp.intValue();
    ...
    public PushNotificationServiceImpl(PushSubscriptionRegistry subscriptionRegistry, ConfigurationService configService, TimerService timerService, ProcessorService processorService, PushNotificationTransportRegistry transportRegistry) throws OXException {
        ...
        processor = processorService.newBoundedProcessor(getClass().getSimpleName(), numProcessorThreads(configService), numMaxProcessorThreads(configService), maxProcessorTasks(configService));
    ...
    public void handle(Collection<PushNotification> notifications, PushPriority priority) throws OXException {
        ...
        try {
            ...
            if (null == scheduledTimerTask) {
                ...
                scheduledTimerTask = timerService.scheduleWithFixedDelay(() -> checkNotifications(), initialDelay, delay);
    ...
    private boolean tryExecuteTask(Runnable task, int userId, int contextId) {
        return processor.execute(UserAndContext.newInstance(userId, contextId), ProcessorTasks.processorTaskFor(task));
    }

    private static void put(PushNotification notification, Map<ContextAndTopicKey, Map<Integer, List<PushNotification>>> polledNotificationsPerUser) {
        Map<Integer, List<PushNotification>> m = polledNotificationsPerUser.computeIfAbsent(new ContextAndTopicKey(notification), Functions.getNewHashMapFuntion());
        m.computeIfAbsent(I(notification.getUserId()), Functions.getNewArrayListFuntion()).add(notification);
    }
     ...
     * @param notificationsPerUser The notifications grouped by user identifier
    ...
    protected void doHandle(int contextId, String topic, Map<Integer, List<PushNotification>> notificationsPerUser) throws OXException {
        ...
        for (Map.Entry<PushNotificationTransport, List<Hit>> entry : getHitsPerTransport(hits).entrySet()) {
            ...
            for (Hit hit : entry.getValue()) {
                addMatchesFromHit(hit, notifications, contextId, topic, notificationsPerUser, transport);
            }

            // Verify... for safety's sake
            for (Map.Entry<PushNotification, List<PushMatch>> notification2matches : notifications.entrySet()) {
                PushNotification notification = notification2matches.getKey();
                for (PushMatch match : notification2matches.getValue()) {
                    if (notification.getContextId() != match.getContextId()) {
                        throw OXException.general("Determined wrong push match for notification with topic \"" + notification.getTopic() + "\" for user " + notification.getUserId() + " in context " + notification.getContextId() + ": Wrong context identifier in " + match);
                    }
                    if (notification.getUserId() != match.getUserId()) {
                        throw OXException.general("Determined wrong push match for notification with topic \"" + notification.getTopic() + "\" for user " + notification.getUserId() + " in context " + notification.getContextId() + ": Wrong user identifier in " + match);
                    }
                    LOGGER.debug("Verified push match for notification with topic \"{}\" to user {} in context {}: {}", notification.getTopic(), I(notification.getUserId()), I(notification.getContextId()), match);
                }
            }
    ...
    private static void addMatchesFromHit(Hit hit, Map<PushNotification, List<PushMatch>> notifications, int contextId, String topic, Map<Integer, List<PushNotification>> notificationsPerUser, PushNotificationTransport transport) {
        Iterator<PushMatch> it = hit.getMatches().iterator();
        if (!it.hasNext()) {
            LOGGER.info("No match for topic \"{}\" with transport '{}' for client '{}'", topic, hit.getTransportId(), hit.getClass());
            return;
        }

        // Handle hit's matches
        do {
            PushMatch match = it.next();
            LOGGER.debug("Handling match: {}", match);
            int userId = match.getUserId();
            if (isTransportAllowed(transport, topic, hit.getClient(), userId, contextId)) {
                LOGGER.debug("Transport '{}' enabled for client '{}' to publish notification \"{}\" to user {} in context {}",
                    transport.getId(), hit.getClient(), topic, I(userId), I(contextId));
                List<PushNotification> notificationsForUser = notificationsPerUser.get(I(userId));
                if (notificationsForUser.isEmpty()) {
                    LOGGER.debug("No notifications for topic '{}' user {} in context {}", match.getTopic(), I(userId), I(contextId));
                } else {
                    for (PushNotification notification : notificationsForUser) {
                        if (matchesSourceToken(match, notification.getSourceToken())) {
                            LOGGER.debug("Skipping push match for source token: {}", match);
                        } else {
                            LOGGER.debug("Collecting push match: {}", match);
                            notifications.computeIfAbsent(notification, Functions.getNewArrayListFuntion()).add(match);
                        }
                    }
                }
            } else {
    ...
    private Map<PushNotificationTransport, List<Hit>> getHitsPerTransport(Hits hits) throws OXException {
        ...
        for (Hit hit : hits) {
            ...
            LOGGER.debug("Look-up of transport '{}' for client '{}'.", transportId, client);
            PushNotificationTransport transport = transportRegistry.getTransportFor(client, transportId);
            if (null == transport) {
                LOGGER.info("No such transport '{}' for client '{}' available, skipping notificaton.", transportId, client);
            } else {
                LOGGER.debug("Found transport '{}' for client '{}'.", transportId, client);
                hitsPerTransport.computeIfAbsent(transport, Functions.getNewArrayListFuntion()).add(hit);
            }
        }
        return hitsPerTransport;
    }

    private static boolean isTransportAllowed(PushNotificationTransport transport, String topic, String client, int userId, int contextId) {
        try {
    ...
    private static boolean matchesSourceToken(PushMatch match, String sourceToken) {
        return null != sourceToken && null != match && sourceToken.equals(match.getToken().getValue());
    }

    private static int getNumberOfNotificationsFrom(Map<Integer, List<PushNotification>> notificationsPerUser) {
        int count = 0;
    ...
    private final class NotificationsHandler implements Runnable {
        ...
        public void run() {
            try {
                ...
                Object sUserIds = userIds.length <= 10 && LOGGER.isDebugEnabled() ? toStringObjectFor(userIds) : null;
                if (sUserIds == null) {
                    LOGGER.error("Failed to handle notification(s) with topic \"{}\" for users in context {}", topic, I(contextId), e);
                } else {
                    LOGGER.error("Failed to handle notification(s) with topic \"{}\" for users {} in context {}", topic, sUserIds, I(contextId), e);
                }
            }
        }
    }
