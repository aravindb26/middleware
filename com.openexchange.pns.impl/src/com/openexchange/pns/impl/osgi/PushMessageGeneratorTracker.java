...
public final class PushMessageGeneratorTracker implements ServiceTrackerCustomizer<PushMessageGenerator, PushMessageGenerator>, PushMessageGeneratorRegistry {
    ...
    @Override
    public PushMessageGenerator getGenerator(String client, String transportId) throws OXException {
        if (null == client && null == transportId) {
            return null;
        }

        // First, try by direct look-up
        PushMessageGenerator generator = generators.get(client);
        if (generator != null && generator.servesClient(client, transportId)) {
            // Direct hit
            return generator;
        }

        // Check for each generator
        for (PushMessageGenerator msgGen : generators.values()) {
            if (msgGen.servesClient(client, transportId)) {
                return msgGen;
            }
        }

        // No suitable generator available
        return null;
    }
