...
public class ICalHandler extends HttpAuthShareHandler {
     ...
     */
    private final static EventField[] EVENT_TOMBSTONE_FIELDS = { EventField.UID, EventField.TIMESTAMP, EventField.SEQUENCE, EventField.RECURRENCE_ID, EventField.DTSTAMP };
    ...
    protected void handleResolvedShare(ResolvedShare resolvedShare) throws OXException, IOException {
        try {
            ...
            LOG.error("Unexpected error during ICal retrieval", e);
    ...
    private void writeEvents(HttpServletResponse response, ICalParameters parameters, String name, Session session, UserizedFolder folder, String ifNoneMatch, String prefer) throws OXException, IOException {
        ...
        if (null == folder.getAccountID()) {
            ...
            SequenceResult sequenceNumber = calendarSession.getCalendarService().getSequenceNumber(calendarSession, folder.getID());
            ...
            SequenceResult sequenceNumber = calendarAccess.getSequenceNumbers(Collections.singletonList(folder.getID())).get(folder.getID());
            eTag = encodeETag(folder.getLastModifiedUTC(), sequenceNumber);
    ...
    private static String encodeETag(Date folderLastModified, SequenceResult sequenceNumber) {
        byte[] bytes = ByteBuffer.allocate(24)
            .putLong(null == folderLastModified ? 0 : folderLastModified.getTime())
            .putLong(null == sequenceNumber ? 0 : sequenceNumber.getTimestamp())
            .putLong(null == sequenceNumber ? 0 : sequenceNumber.getTotalCount())
        .array();
