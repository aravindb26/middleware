...
public class SendUserFeedback extends AbstractUserFeedback {
    ...
    protected WebTarget getEndpoint(String endpointHost, CommandLine cmd) {
        String endpoint;
        if (cmd.hasOption(ENDPOINT_LONG)) {
            endpoint = cmd.getOptionValue(ENDPOINT_LONG, ENDPOINT_DEFAULT);
        } else {
            endpoint = endpointHost + "/userfeedback/v1/send/";
        }
    ...
    protected Void invoke(Options option, CommandLine cmd, Builder context) throws Exception {
        ...
        try {
            JSONObject json = JSONServices.parseObject(new AsciiReader(response));
