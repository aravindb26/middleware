...
public class IPRange {
    ...
    private static String getAddressSafelyFor(String address) {
        try {
            return address == null ? null : com.google.common.net.InetAddresses.forString(address).getHostAddress();
        } catch (Exception e) {
            LOGGER.warn("Address is not a valid IP string literal", e);
            return address;
        }
    }

    private static IPRange handleSingleAddress(String address) {
        String addr = com.google.common.net.InetAddresses.forString(address).getHostAddress();
        if (InetAddressValidator.getInstance().isValidInet4Address(addr)) {
            byte[] octets = IPAddressUtil.textToNumericFormatV4(addr);
            return new IPRange(new LongRange(ipToLong(octets), ipToLong(octets)), null);
        }
        final IPv6Address iPv6Address = IPv6Address.fromString(addr);
    ...
    private static IPRange handleWithSlash(String range) {
        String[] addresses = com.openexchange.java.Strings.splitBy(range, '-', true);
        for (int i = addresses.length; i-- > 0;) {
            addresses[i] = com.google.common.net.InetAddresses.forString(addresses[i]).getHostAddress();
        }
    ...
    public boolean contains(String ipAddress) {
        ...
        // Calculate...
        String addr = getAddressSafelyFor(ipAddress);
        byte[] octets = IPAddressUtil.textToNumericFormatV4(addr);
        if (null != octets) {
            /*
             * IPv4
        ...
        return containsIPv6(ipAddress, addr);
    }
    ...
    public boolean containsIPv6(String ipAddress) {
        return containsIPv6(ipAddress, getAddressSafelyFor(ipAddress));
    }

    private boolean containsIPv6(String ipAddress, String addr) {
        if (null != ipAddress) {
            Boolean cached = this.cache.getIfPresent(ipAddress);
            if (null != cached) {
                return cached.booleanValue();
        ...
        IPv6Address v6Address = IPv6Address.fromString(addr);
        boolean ret = null != this.ipv6Range && this.ipv6Range.contains(v6Address);
        if (null != ipAddress) {
            this.cache.put(ipAddress, Boolean.valueOf(ret));
        }
