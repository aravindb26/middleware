...
public class DelegatingSSLSocket extends SSLSocket {
    ...
    private static final MessageMatcher MATCHER_UNRECOGNIZED_SSL_MESSAGE = new EqualsMatcher("unsupported or unrecognized ssl message", true);
    ...
    public void startHandshake() throws IOException {
        try {
            ...
            if (null != optHostnameVerifier) {
                ...
                String hostname = (endpoint instanceof InetSocketAddress) ? ((InetSocketAddress) endpoint).getHostName() : session.getPeerHost();
            ...
            if (matchesException(e, java.io.EOFException.class, MATCHER_INCORRECT_SHUTDOWN_ERROR)) {
                ...
                //@formatter:off
                throw new javax.net.ssl.SSLException("The remote end-point" + endpointInfo + " closed connection during handshake unexpectedly. Most likely because the end-point expects a protocol,"
            ...
            if (MATCHER_HANDSHAKE_FAILURE_ERROR.matches(e.getMessage())) {
                ...
                //@formatter:off
                throw new javax.net.ssl.SSLException("The SSL hand-shake with remote end-point" + endpointInfo + " failed. Most likely because the end-point expects a protocol or cipher suite,"
                    + " that is not enabled/available for the JVM. Enabled protocols are: " + protocols + ". You may want to let supported protocols/cipher suite be determined by using https://www.ssllabs.com/ssltest/index.html", e);
                //@formatter:on
            }

            if (MATCHER_UNRECOGNIZED_SSL_MESSAGE.matches(e.getMessage())) {
                // E.g. the remote end-point most likely only accepts plain connections
                SocketAddress endpoint = this.endpoint;
                String endpointInfo = null == endpoint ? "" : (" \"" + endpoint + "\"");
                //@formatter:off
                throw new javax.net.ssl.SSLException("Establishing an SSL connection against remote end-point" + endpointInfo + " failed."
                    + " Most likely because the end-point expects a plain connection being established and does not accept SSL connections", e);
                //@formatter:on
            }
