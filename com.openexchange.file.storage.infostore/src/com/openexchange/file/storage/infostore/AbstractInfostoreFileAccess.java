    ...
    public File getFileMetadata(String folderId, String id, String version) throws OXException {
        try {
            ...
            if (null == folderId) {
                ...
                if(false == Integer.toString(FolderObject.SYSTEM_USER_INFOSTORE_FOLDER_ID).equals(folderId) && metadata.getFolderId() == FolderObject.SYSTEM_USER_INFOSTORE_FOLDER_ID) {
                    // If the requested folderId is not "10", but the infostore returned metadata with folder "10": compare the original folder instead
                    if (0 < metadata.getOriginalFolderId() && false == folderId.equals(Long.toString(metadata.getOriginalFolderId()))) {
