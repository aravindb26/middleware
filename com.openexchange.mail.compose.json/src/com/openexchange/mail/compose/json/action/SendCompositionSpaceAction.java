...
public class SendCompositionSpaceAction extends AbstractMailComposeAction {
    ...
    protected AJAXRequestResult doPerform(AJAXRequestData requestData, ServerSession session) throws OXException, JSONException {
        ...
        final boolean deleteDraftAfterTransport = AJAXRequestDataTools.parseBoolParameter("deleteDraftAfterTransport", requestData);
            ...
            if (null != upload) {
                ...
                JSONObject jMessage = null;
                {
                    ...
                    if (Strings.isNotEmpty(expectedJsonContent)) {
                        jMessage = JSONServices.parseObject(expectedJsonContent);
                    }
                }

                if (null != jMessage) {
                    MessageDescription md = new MessageDescription();
                    parseJSONMessage(jMessage, md, compositionSpaceId, true);
        ...
        DeleteAfterTransportOptions options = DeleteAfterTransportOptions.builder().withDeleteAfterTransport(true).withDeleteDraftAfterTransport(deleteDraftAfterTransport).build();

        List<OXException> warnings = new ArrayList<OXException>(4);
        MailPath mailPath = compositionSpaceService.transportCompositionSpace(compositionSpaceId.getId(), optionalUploadedAttachments, session.getUserSettingMail(), requestData, warnings, options, clientToken);
