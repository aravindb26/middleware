 ...
 */
public class RankingAwareNearRegistryServiceTracker<S> extends ServiceTracker<S, S> implements ServiceListing<S>, RankingAwareServiceListing<S> {
    ...
    public RankingAwareNearRegistryServiceTracker(final BundleContext context, final Class<S> clazz, final int defaultRanking) {
        ...
        services = new SortableConcurrentList<>();
     ...
     */
    protected boolean onServiceAppeared(@SuppressWarnings("unused") S service) {
        return true;
    }
     ...
     */
    protected void onServiceAdded(@SuppressWarnings("unused") S service) {
        // Nothing
    }
     ...
     */
    protected void onServiceDisappeared(@SuppressWarnings("unused") S service) {
        // Nothing
    }
     ...
     */
    protected void onServiceRemoved(@SuppressWarnings("unused") S service) {
        // Nothing
    }
    ...
    public Iterator<S> iterator() {
        return empty ? Collections.<S> emptyIterator() : new Iter<>(services.getSnapshot());
    }
    ...
    private List<S> asList() {
        ...
        List<S> ret = new ArrayList<>(snapshot.size());
    ...
    public synchronized S addingService(final ServiceReference<S> reference) {
        ...
        if (!onServiceAppeared(service)) {
            context.ungetService(reference);
            return null;
        }

        final RankedService<S> rankedService = new RankedService<>(service, getRanking(service, reference, defaultRanking));
    ...
    public synchronized void removedService(final ServiceReference<S> reference, final S service) {
        ...
        if (services.remove(new RankedService<>(service, getRanking(service, reference, defaultRanking)))) {
