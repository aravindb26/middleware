...
public class IDMangling extends com.openexchange.chronos.provider.composition.IDMangling {
    ...
    public static AccountAwareCalendarFolder withUniqueID(CalendarFolder folder, CalendarAccount account) {
        if ((folder instanceof GroupwareCalendarFolder)) {
     ...
     * @return The events result representations with unique identifiers
     */
    public static Map<Attendee, EventsResult> withUniqueIDsFromResultsPerAttendee(Map<Attendee, EventsResult> relativeResults, int accountId) {
        if (null == relativeResults || relativeResults.isEmpty()) {
            return relativeResults;
        }
        Map<Attendee, EventsResult> results = new HashMap<Attendee, EventsResult>(relativeResults.size());
        for (Map.Entry<Attendee, EventsResult> entry : relativeResults.entrySet()) {
            results.put(entry.getKey(), new IDManglingEventsResult(entry.getValue(), accountId));
        }
        return results;
    }
    ...
    /**
     * Gets a recurrence info result equipped with unique composite identifiers representing recurrence information from a specific 
     * calendar account.
     *
     * @param relativeRecurrenceInfo The recurrence info from the account
     * @param accountId The identifier of the account
     * @return The recurrence info representation with unique identifiers
     */
    public static RecurrenceInfo withUniqueID(RecurrenceInfo relativeRecurrenceInfo, int accountId) {
        return new DefaultRecurrenceInfo(relativeRecurrenceInfo.isOverridden(), relativeRecurrenceInfo.isRescheduled(), 
            withUniqueID(relativeRecurrenceInfo.getSeriesMaster(), accountId), withUniqueID(relativeRecurrenceInfo.getRecurrence(), accountId));
    }
