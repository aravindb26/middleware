...
public class SessionReservationServiceImpl implements SessionReservationService {
    ...
    private Reservation peekReservation(String token) {
        blocker.acquire();
        try {
            return useHzMap ? peekFromHzMap(hzMapName, token) : reservations.asMap().get(token);
        } finally {
            blocker.release();
        }
    }
    ...
    private Reservation pollFromHzMap(String mapIdentifier, String token) {
        ...
        return convertReservation(token, hzMap.remove(token));
    }
    ...
    private Reservation peekFromHzMap(String mapIdentifier, String token) {
        ...
        return convertReservation(token, hzMap.get(token));
    }

    private static Reservation convertReservation(String token, PortableReservation portableReservation) {
