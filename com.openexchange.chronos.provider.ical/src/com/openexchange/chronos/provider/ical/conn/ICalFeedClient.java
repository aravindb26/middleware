...
public class ICalFeedClient {
    ...
    private ImportedCalendar importCalendar(HttpEntity httpEntity, long maxFeedSize) throws OXException {
        if (null == httpEntity) {
            return null;
        }
        ...
        try (CountingInputStream inputStream = new CountingInputStream(Streams.bufferedInputStreamFor(httpEntity.getContent()), maxFeedSize, (total, max) -> wrapExceededError(max))) {
            return iCalService.importICal(inputStream, parameters);
        } catch (UnsupportedOperationException | IOException e) {
            // @formatter:off
            if(e instanceof IOException ioe &&
               ioe.getCause() instanceof OXException oxe &&
               ICalProviderExceptionCodes.FEED_SIZE_EXCEEDED.equals(oxe)) {
    ...
    /**
     * Wraps a new {@link ICalProviderExceptionCodes#FEED_SIZE_EXCEEDED} error inside an {@link IOException}
     *
     * @param max The maximal amount of bytes allowed
     * @return The {@link IOException}
     */
    private IOException wrapExceededError(long max) {
        String msg = new StringBuilder(32).append("Max. byte count of ").append(max).append(" exceeded.").toString();
        OXException oxe = ICalProviderExceptionCodes.FEED_SIZE_EXCEEDED.create(iCalFeedConfig.getFeedUrl(), L(max), L(-1));
        return new IOException(msg, oxe);
    }
    ...
    private HttpClient getHttpClient() throws OXException {
        ...
        ManagedHttpClient httpClient = httpClientService.getHttpClient("icalfeed");
        return httpClient;
    }
    ...
    private GetResponse prepareResponse(URI uri, HttpResponse httpResponse) throws OXException {
        ...
        response.setCalendar(importCalendar(entity, allowedFeedSize));
        return response;
    }
