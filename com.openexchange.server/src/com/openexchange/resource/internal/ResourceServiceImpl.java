...
public final class ResourceServiceImpl implements ResourceService {
    ...
    @Override
    public Resource[] searchResourcesWithPrivilege(User user, Context ctx, SchedulingPrivilege privilege) throws OXException {
        int[] entities = new int[1 + user.getGroups().length];
        entities[0] = user.getId();
        System.arraycopy(user.getGroups(), 0, entities, 1, user.getGroups().length);
        return ServerServiceRegistry.getServize(ResourceStorage.class, true).searchResourcesByPrivilege(entities, privilege, ctx);
    }
    ...
    @Override
    public Resource getResource(Connection connection, int resourceId, Context context) throws OXException {
        return ServerServiceRegistry.getServize(ResourceStorage.class, true).getResource(resourceId, context, connection);
    }
