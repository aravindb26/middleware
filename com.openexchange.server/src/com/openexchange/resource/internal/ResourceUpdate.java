...
public final class ResourceUpdate extends AbstractResourcePerformer {
    ...
    private void check() throws OXException {
        ...
        if (resource.isSimpleNameSet()) {
            ...
            final Resource tmp = storage.getByIdentifier(resource.getSimpleName(), ctx);
            if (null != tmp && tmp.getIdentifier() != resource.getIdentifier()) {
        ...
        if (resource.isMailSet()) {
            ...
            final Resource tmp = storage.getByMail(resource.getMail(), ctx);
            if (null != tmp && tmp.getIdentifier() != resource.getIdentifier()) {
                throw ResourceExceptionCode.RESOURCE_CONFLICT_MAIL.create(resource.getMail());
            }
        }
        if (resource.isPermissionsSet()) {
            ResourceTools.validatePermissions(ctx, resource.getPermissions());
        }
    }
