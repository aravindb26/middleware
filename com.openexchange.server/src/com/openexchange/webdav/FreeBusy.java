...
public class FreeBusy extends HttpServlet {
    ...
    /** The path under which the free/busy servlet is registered */
    public static final String SERVLET_PATH = "/servlet/webdav.freebusy";

    /** The name of the URI parameter for the context identifier targeted by the query */
    public static final String PARAMETER_CONTEXTID = "contextId";
    ...
    protected void doGet(final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        try {
            ...
            if (indicatesNotFound(freeBusyResult)) {
                response.sendError(HttpServletResponse.SC_NOT_FOUND);
                return;
            }
            ...
            if (FreeBusyExceptionCode.MISSING_PARAMETER.equals(e) || FreeBusyExceptionCode.USER_NOT_FOUND.equals(e) ||
                CalendarExceptionCodes.FREEBUSY_NOT_AVAILABLE.equals(e) || com.openexchange.groupware.contexts.impl.ContextExceptionCodes.NOT_FOUND.equals(e)) {
                response.sendError(HttpServletResponse.SC_NOT_FOUND);
                return;
            }
     ...
     * response is not null, response contains exactly one entry.
    ...
    private void writeICalendar(HttpServletResponse response, FreeBusyResult freeBusyResult, Attendee attendee, Date from, Date until, boolean simple) throws OXException, IOException {
        ...
        List<FreeBusyTime> fbTimes = new ArrayList<>();
        if (simple) {
    ...
    /**
     * Gets a value indicating whether the warnings in the supplied free/busy result would justify sending a HTTP 404 "not found" response.
     *
     * @param freeBusyResult The free/busy result
     * @return <code>true</code> if the warnings indicate "not found", <code>false</code>, otherwise
     */
    private static boolean indicatesNotFound(FreeBusyResult freeBusyResult) {
        if (null != freeBusyResult.getWarnings()) {
            for (OXException warning : freeBusyResult.getWarnings()) {
                if (CalendarExceptionCodes.FREEBUSY_NOT_AVAILABLE_PER_CONFIGURATION.equals(warning)) {
                    LOGGER.debug("Free busy visibility restricted: {}", warning.getMessage(), warning);
                    return true;
                }
            }
        }
