...
final class CachingMailAccountStorage implements MailAccountStorageService {
    ...
    public MailAccount getDefaultMailAccount(int userId, int contextId) throws OXException {
        ...
        Cache cache = cacheService.getCache(REGION_NAME);
        Object object = cache.get(newCacheKey(cacheService, Account.DEFAULT_ID, userId, contextId));
        ...
        Lock lock = null == lockService ? LockService.EMPTY_LOCK : lockService.getSelfCleaningLockFor(new StringBuilder(32).append("mailaccount-").append(contextId).append('-').append(userId).append('-').append(Account.DEFAULT_ID).toString());
        lock.lock();
        try {
            object = cache.get(newCacheKey(cacheService, Account.DEFAULT_ID, userId, contextId));
            if (object instanceof MailAccount) {
                return (MailAccount) object;
            }

            MailAccount defaultMailAccount = delegate.getDefaultMailAccount(userId, contextId);
            cache.put(newCacheKey(cacheService, Account.DEFAULT_ID, userId, contextId), defaultMailAccount, false);
    ...
    public boolean isSecondaryMailAccount(int id, int userId, int contextId) throws OXException {
        ...
        Object object = cache.get(key);
        if (object instanceof MailAccount) {
            return ((MailAccount) object).isSecondaryAccount();
        }
        ...
        try {
            ...
            return mailAccount.isSecondaryAccount();
        } finally {
            lock.unlock();
        }
    }
    ...
    public boolean isDeactivatedMailAccount(int id, int userId, int contextId) throws OXException {
        ...
        Object object = cache.get(key);
        if (object instanceof MailAccount) {
            return ((MailAccount) object).isDeactivated();
        }
        ...
        try {
            ...
            MailAccount mailAccount = delegate.getMailAccount(id, userId, contextId);
            cache.put(key, mailAccount, false);
            return mailAccount.isDeactivated();
        } finally {
            lock.unlock();
        }
    }
    ...
    /**
     * Gets the mail account with the given id
     *
     * @param id The account id
     * @param userId The user id
     * @param contextId The context id
     * @param con The {@link Connection} to use
     * @return The {@link MailAccount}
     * @throws OXException
     */
    ...
    public MailAccount[] getUserDefaultAndSecondaryMailAccounts(int userId, int contextId) throws OXException {
        ...
        int[] ids;
        {
            CacheService cacheService = ServerServiceRegistry.getInstance().getService(CacheService.class);
            if (cacheService == null) {
                ids = delegate.getUserDefaultAndSecondaryMailAccountIDs(userId, contextId);
                filter = false;
            } else {
                Cache cache = cacheService.getCache(REGION_NAME);
                CacheKey key = accountsCacheKey(cacheService, userId, contextId);
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            if (!filter || account.isDefaultAccount() || account.isSecondaryAccount()) {
    ...
    public MailAccount[] resolveLogin(String login, int contextId) throws OXException {
        int[][] idsAndUsers = resolveFromCache(login, contextId, (pattern, contextId1) -> getDelegate().resolveLogin2IDs(pattern, contextId1), CachedResolveType.LOGIN);
    ...
    public int insertMailAccount(MailAccountDescription mailAccount, int userId, Context ctx, Session session) throws OXException {
        return insertMailAccount(mailAccount, userId, ctx, session, null);
    }
    ...
    public int insertMailAccount(MailAccountDescription mailAccount, int userId, Context ctx, Session session, Connection con) throws OXException {
        int id = con == null ? delegate.insertMailAccount(mailAccount, userId, ctx, session) : delegate.insertMailAccount(mailAccount, userId, ctx, session, con);
    ...
    public MailAccount[] resolvePrimaryAddr(String primaryAddress, int contextId) throws OXException {
        int[][] idsAndUsers = resolveFromCache(primaryAddress, contextId, (pattern, contextId1) -> getDelegate().resolvePrimaryAddr2IDs(pattern, contextId1), CachedResolveType.PRIMARY_ADDRESS);
        List<MailAccount> l = new ArrayList<MailAccount>(idsAndUsers.length);
        for (int[] idAndUser : idsAndUsers) {
