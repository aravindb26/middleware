...
public abstract class AbstractMailAccountAction implements AJAXActionService {
     ...
     */
    protected static boolean isDefaultOrSecondaryMailAccount(final MailAccount mailAccount) {
        return mailAccount.isDefaultAccount() || Account.DEFAULT_ID == mailAccount.getId() || mailAccount.isSecondaryAccount();
    }
     ...
     * @throws OXException If check fails
     */
    protected static boolean isDefaultOrSecondaryMailAccount(final MailAccountDescription mailAccount, Session session) throws OXException {
        return Account.DEFAULT_ID == mailAccount.getId() || MailAccounts.isSecondaryAccount(mailAccount.getId(), session);
    }
     ...
     * @throws OXException If check fails
     */
    protected static boolean isDefaultOrSecondaryMailAccount(int mailAccountId, Session session) throws OXException {
        return Account.DEFAULT_ID ==mailAccountId || MailAccounts.isSecondaryAccount(mailAccountId, session);
    }
    ...
    protected static MailAccount[] checkSpamInfo(MailAccount[] mailAccounts, ServerSession session) throws OXException {
        ...
        for (int i = revisedMailAccounts.length; i-- > 0;) {
            ...
            if (null == mailAccount) {
                ...
                revisedMailAccounts[i] = ((Account.DEFAULT_ID == mailAccount.getId()) ? checkSpamInfo0(mailAccount, session) : mailAccount);
            }
        }
    ...
    protected static MailAccount checkSpamInfo(MailAccount mailAccount, ServerSession session) throws OXException {
        ...
        return (Account.DEFAULT_ID == mailAccount.getId()) ? checkSpamInfo0(mailAccount, session) : mailAccount;
    }
    ...
    private static MailAccount checkSpamInfo0(MailAccount mailAccount, ServerSession session) throws OXException {
        ...
        SpamHandler spamHandler = SpamHandlerRegistry.getSpamHandlerBySession(session, Account.DEFAULT_ID, provider);
        boolean suppressSpam = (!spamHandler.isCreateConfirmedSpam(session));
        if (suppressSpam) {
            return new SpamSuppressingMailAccount(mailAccount, suppressSpam, (!spamHandler.isCreateConfirmedHam(session)));
        }
        boolean suppressHam = (!spamHandler.isCreateConfirmedHam(session));
    ...
    protected static MailAccess<? extends IMailFolderStorage, ? extends IMailMessageStorage> getMailAccess(MailAccountDescription accountDescription, ServerSession session, List<OXException> warnings) throws OXException {
        ...
        boolean isSecondary;
        {
            isDefault = accountDescription.getId() == Account.DEFAULT_ID;
            isSecondary = isDefault ? false : (accountDescription.getId() < 0 ? false : MailAccounts.isSecondaryAccount(accountDescription.getId(), session));
            if (isDefault) {
                if (ServerSource.GLOBAL == MailProperties.getInstance().getMailServerSource(session.getUserId(), session.getContextId(), isGuest(session))) {
                ...
                if (ServerSource.GLOBAL == MailProperties.getInstance().getSecondaryMailServerSource(session.getUserId(), session.getContextId(), isGuest(session))) {
                    ConfiguredServer mailServer = MailProperties.getInstance().getSecondaryMailServer(session.getUserId(), session.getContextId());
                    if (mailServer == null) {
                        throw MailConfigException.create("Property \"com.openexchange.mail.secondary.mailServer\" not set in mail properties for user " + session.getUserId() + " in context " + session.getContextId());
                    }
                    mailProvider = getMailProviderByURL(mailServer.getUrlString(false));
                } else {
                    mailProvider = getMailProviderByURL(mailServerURL);
        ...
        try {
            ...
            if (isDefault || isSecondary) {
                mailConfig.setAuthType(MailConfig.getConfiguredAuthTypeForMailAccess(AccountNature.accountNatureFor(isDefault, isSecondary), session));
            } else {
            ...
            if (!isDefault || PasswordSource.GLOBAL != MailProperties.getInstance().getPasswordSource(session.getUserId(), session.getContextId())) {
                mailConfig.setPassword(accountDescription.getPassword());
            }

            // Set server and port
            if (!isDefault || ServerSource.GLOBAL != MailProperties.getInstance().getMailServerSource(session.getUserId(), session.getContextId(), isGuest(session))) {
            ...
            if (null != mailConfig) {
                ...
                while (cause instanceof OXException) {
