...
public final class NewAction extends AbstractMailAccountAction implements MailAccountFields {
    ...
    protected AJAXRequestResult innerPerform(AJAXRequestData requestData, ServerSession session, JSONValue jData) throws OXException, JSONException {
        if (!session.getUserPermissionBits().isMultipleMailAccounts()) {
            throw MailAccountExceptionCodes.NOT_ENABLED.create(I(session.getUserId()), I(session.getContextId()));
        }
        if (null == jData) {
        ...
        if (availableAttributes.contains(Attribute.SECONDARY_ACCOUNT_LITERAL) && accountDescription.isSecondaryAccount()) {
        ...
        if (MailAccountUtils.isUnifiedINBOXAccount(accountDescription.getMailProtocol())) {
            ...
            throw MailAccountExceptionCodes.UNIFIED_INBOX_ACCOUNT_CREATION_FAILED.create(I(accountDescription.getId()));
        }
        ...
        if (!pop3) {
            ...
            try {
                if (false == ValidateAction.actionValidateBoolean(accountDescription, session, true, warnings, true).booleanValue()) {
                    valid = false;
                    final OXException warning = MimeMailExceptionCode.CONNECT_ERROR_SIMPLE.create(accountDescription.getMailServer(), accountDescription.getLogin());
                    ...
                    OXException warning = MimeMailExceptionCode.CONNECT_ERROR_SIMPLE.create(accountDescription.getTransportServer(), login);
                    warning.setCategory(Category.CATEGORY_WARNING);
                    warnings.add(0, warning);
                }
            } finally {
        ...
        if (!pop3 && valid) {
            ...
            try {
                ...
                // Check names
                Tools.checkNames(accountDescription, availableAttributes, C(separator));
            } finally {
                if (null != mailAccess) {
                    mailAccess.close(false);
                }
        ...
        MailAccount newAccount = null;
        {
            ...
            try {
                ...
                if (null == newAccount) {
                    throw MailAccountExceptionCodes.NOT_FOUND.create(I(id), I(session.getUserId()), I(session.getContextId()));
                }
        ...
        boolean reload = false;
        {
            ...
            try {
                ...
                LOGGER.warn("Failed to live-connect against mail server {} on port {}. Aborting to check default folders consistency.", accountDescription.getMailServer(), I(accountDescription.getMailPort()), e);
            } finally {
                if (null != mailAccess) {
                    mailAccess.close();
                }
