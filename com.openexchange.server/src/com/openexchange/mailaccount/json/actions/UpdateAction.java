...
public final class UpdateAction extends AbstractMailAccountAction implements MailAccountFields {
    ...
    protected AJAXRequestResult innerPerform(final AJAXRequestData requestData, final ServerSession session, final JSONValue jData) throws OXException, JSONException {
        ...
        if (fieldsToUpdate.contains(Attribute.SECONDARY_ACCOUNT_LITERAL) && accountDescription.isSecondaryAccount()) {
            throw MailAccountExceptionCodes.NO_SECONDARY_CHANGE.create();
        }
        ...
        if (fieldsToUpdate.contains(Attribute.PASSWORD_LITERAL)) {
            ...
            if (Account.DEFAULT_ID != id && isEmpty(pw)) {
        ...
        if (fieldsToUpdate.contains(Attribute.MAIL_URL_LITERAL)) {
            ...
            if (Account.DEFAULT_ID != id && isEmpty(server)) {
        ...
        // Check attributes to update
        {
            ...
            if (!session.getUserPermissionBits().isMultipleMailAccounts() && (!isDefaultOrSecondaryMailAccount(accountDescription, session) || (!notAllowed.isEmpty()))) {
        ...
        if (MailAccountUtils.isUnifiedINBOXAccount(toUpdate)) {
            // Treat as no hit
        ...
        if (isDefaultOrSecondaryMailAccount(accountDescription, session) == false) {
                ...
                if (!mailServerURL.equals(accountDescription.generateMailServerURL())) {
                    if (pop3) {
                        if (MailAccountUtils.isDenied(accountDescription.getMailServer(), accountDescription.getMailPort())) {
                            throw MailAccountExceptionCodes.VALIDATE_FAILED_MAIL.create(accountDescription.getMailServer(), accountDescription.getLogin());
                        }
                    } else {
                        if (!ValidateAction.checkMailServerURL(accountDescription, session, warnings, true)) {
                            OXException warning = MimeMailExceptionCode.CONNECT_ERROR_SIMPLE.create(accountDescription.getMailServer(), accountDescription.getLogin());
                ...
                if (transportServerURL == null || !transportServerURL.equals(accountDescription.generateTransportServerURL())) {
                    if (pop3) {
                        if (MailAccountUtils.isDenied(accountDescription.getTransportServer(), accountDescription.getTransportPort())) {
                            ...
                            throw MailAccountExceptionCodes.VALIDATE_FAILED_TRANSPORT.create(accountDescription.getTransportServer(), login);
                        }
                    } else {
                        if (!ValidateAction.checkTransportServerURL(accountDescription, session, warnings, true)) {
                            String transportLogin = accountDescription.getTransportLogin();
                            OXException warning = MimeMailExceptionCode.CONNECT_ERROR_SIMPLE.create(accountDescription.getTransportServer(), transportLogin == null ? accountDescription.getLogin() : transportLogin);
                            warning.setCategory(Category.CATEGORY_WARNING);
                            warnings.add(0, warning);
                        }
                    }
