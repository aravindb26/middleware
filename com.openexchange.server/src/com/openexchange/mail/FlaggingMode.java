...
public enum FlaggingMode {
    ...
    public static final int getFlaggingColor(Session session, ConfigViewFactory factory) {
        int def = MailMessage.COLOR_LABEL_RED;
        if (factory != null) {
            try {
    ...
    public static FlaggingMode getFlaggingMode(Session session, ConfigViewFactory factory) {
        if (factory != null) {
            try {
                ...
                return null == modeName ? FlaggingMode.FLAGGED_IMPLICIT : FlaggingMode.getModeByName(modeName);
            } catch (OXException e) {
                // fall back to default
            }
        }
        return FlaggingMode.FLAGGED_IMPLICIT;
    }
