...
public final class MessageUtility {
    ...
    /**
     * Checks if part seems to be an inline image.
     *
     * @param part The part to examine
     * @param knownImage <code>true</code> if caller is confident that passed part is an image; otherwise <code>false</code> to have this method check part's MIME type
     * @return <code>true</code> for inline image; otherwise <code>false</code>
     */
    public static boolean seemsInlineImage(MailPart part, boolean knownImage) {
        if (part == null) {
            return false;
        }

        if (!knownImage) {
            ContentType contentType = part.containsContentType() ? part.getContentType() : ContentType.APPLICATION_OCTETSTREAM_CONTENT_TYPE;
            if (!contentType.startsWith(PRIMARY_IMAGE)) {
                // Not of type "image/*"
                return false;
            }
        }

        return null == part.getFileName() && Part.INLINE.equalsIgnoreCase(part.containsContentDisposition() ? part.getContentDisposition().getDisposition() : null);
    }
