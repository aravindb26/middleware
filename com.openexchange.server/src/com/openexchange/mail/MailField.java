...
public enum MailField {
     ...
     * <b>[low cost]</b>
     */
    SENDER(MailListField.SENDER),
    /**
     * Reply-To<br>
     * <b>[low cost]</b>
     */
    REPLY_TO(MailListField.REPLY_TO),
    /**
     * Subject<br>
     * <b>[low cost]</b>
     */
    ...
    private static final Map<MailListField,MailField> LIST_FIELDS_MAP;

    private static final TIntObjectMap<MailField> FIELDS_MAP;

    static {
        EnumMap<MailListField, MailField> listField2MailField = new EnumMap<MailListField, MailField>(MailListField.class);
        TIntObjectMap<MailField> field2MailField = new TIntObjectHashMap<MailField>(25);

        for (MailField mailField : MailField.values()) {
            final MailListField listField = mailField.getListField();
            if (listField != null) {
                listField2MailField.put(listField, mailField);
                field2MailField.put(listField.getField(), mailField);
            }
        }
        listField2MailField.put(MailListField.FLAG_SEEN, MailField.FLAGS);
        listField2MailField.put(MailListField.FLAG_ANSWERED, MailField.FLAGS);
        listField2MailField.put(MailListField.FLAG_DRAFT, MailField.FLAGS);
        listField2MailField.put(MailListField.FLAG_FLAGGED, MailField.FLAGS);
        listField2MailField.put(MailListField.USER_FLAGS, MailField.FLAGS);

        field2MailField.put(MailListField.FLAG_SEEN.getField(), MailField.FLAGS);
        field2MailField.put(MailListField.FLAG_ANSWERED.getField(), MailField.FLAGS);
        field2MailField.put(MailListField.FLAG_DRAFT.getField(), MailField.FLAGS);
        field2MailField.put(MailListField.FLAG_FLAGGED.getField(), MailField.FLAGS);
        field2MailField.put(MailListField.USER_FLAGS.getField(), MailField.FLAGS);

        LIST_FIELDS_MAP = Maps.immutableEnumMap(listField2MailField);
        FIELDS_MAP = field2MailField;
    }
     ...
     */
    public static final List<MailField> FIELDS_LOW_COST = List.of(ID, FOLDER_ID, ATTACHMENT, CONTENT_TYPE, FROM, TO, CC, BCC, SENDER, REPLY_TO, SUBJECT, SIZE, SENT_DATE, RECEIVED_DATE, FLAGS, THREAD_LEVEL, DISPOSITION_NOTIFICATION_TO, PRIORITY, COLOR_LABEL);
     ...
     */
    public static final List<MailField> FIELDS_WO_BODY = List.of(ID, FOLDER_ID, ATTACHMENT, CONTENT_TYPE, FROM, TO, CC, BCC, SENDER, REPLY_TO, SUBJECT, SIZE, SENT_DATE, RECEIVED_DATE, FLAGS, THREAD_LEVEL, DISPOSITION_NOTIFICATION_TO, PRIORITY, COLOR_LABEL, HEADERS);
