...
public final class TextProcessing {
    ...
    private static class LoggerHolder {
        static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(TextProcessing.class);
    }
    ...
    public static String performLineFolding(String content, int linewrap) {
        ...
        final String[] lines = Strings.splitByLineSeparator(content, false);
        if (lines.length > 0) {
    ...
    private static String readContent(MailPart mailPart, ContentType contentType) throws OXException, IOException {
        ...
        try {
            ...
            LoggerHolder.LOG.warn("Character conversion exception while reading content with charset \"{}\". Using fallback charset \"{}\" instead.", charset, fallback, e);
        ...
        if (mailPart.containsHeader(MessageHeaders.HDR_CONTENT_TYPE)) {
            ...
            if (!CharsetDetector.isValid(cs)) {
                ...
                if (contentType.startsWith("text/")) {
                    ...
                    LoggerHolder.LOG.warn("Illegal or unsupported encoding \"{}\". Using auto-detected encoding: \"{}\"", prev, cs);
                } else {
                    cs = MailProperties.getInstance().getDefaultMimeCharset();
                    LoggerHolder.LOG.warn("Illegal or unsupported encoding \"{}\". Using fallback encoding: \"{}\"", prev, cs);
                }
            }
            charset = cs;
        } else {
