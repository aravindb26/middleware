...
public class CompositionSpaces {
    ...
    public static VCardAndFileName getContactVCard(String contactId, String folderId, Session session) throws OXException {
        Contact contact;
        IDBasedContactsAccess contactsAccess = ServerServiceRegistry.getInstance().getService(IDBasedContactsAccessFactory.class, true).createAccess(session);
        try {
            contact = contactsAccess.getContact(new ContactID(folderId, contactId));
        } finally {
     ...
     * @return The UUID
     * @throws OXException If passed string is invalid
     */
     ...
     *
     * @param id The composition space identifier as an unformatted string; e.g. <code>067e61623b6f4ae2a1712470b63dff00</code>
     * @return The UUID or <code>null</code> if passed string is invalid
     */
    ...
    public static UUID parseAttachmentId(String id) throws OXException {
        try {
            ...
            throw CompositionSpaceErrorCode.INVALID_ATTACHMENT_IDENTIFIER.create(e, id);
        }
    }
    ...
    /**
     * Tries to detect & parse the time zone from given mail's <code>"Date"</code> header.
     *
     * @param msg The mail to parse from
     * @return The time zone or empty
     */
    public static Optional<String> optSentDateTimeZoneId(MailMessage msg) {
        ...
        try {
            ...
            return Optional.of(TimeZone.getTimeZone(zdtWithZoneOffset.getZone()).getID());
        } catch (Exception e) {
            LoggerHolder.LOG.debug("Time zone could not be parsed from text: {}", dateString, e);
            return Optional.empty();
        }
    }

    /**
     * Sanitizes given HTML content according to specified options.
     *
     * @param htmlContent The HTML content
     * @param sanitizeOptions The options
     * @return The sanitized HTML content
     * @throws OXException If sanitizing fails
     */
    public static String sanitizeHtmlContent(String htmlContent, HtmlSanitizeOptions sanitizeOptions) throws OXException {
        HtmlService htmlService = ServerServiceRegistry.getInstance().getService(HtmlService.class);
        if (htmlService == null) {
            throw ServiceExceptionCode.absentService(HtmlService.class);
        }

        return htmlService.sanitize(htmlContent, sanitizeOptions).getContent();
    }
