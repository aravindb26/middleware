...
public interface CompositionSpaceService {
     ...
     * @param deleteAfterTransportOptions The options telling how to handle the draft associated with composition space after resulting mail has been sent
     ...
     */
    MailPath transportCompositionSpace(UUID compositionSpaceId, Optional<StreamedUploadFileIterator> optionalUploadedAttachments, UserSettingMail mailSettings, AJAXRequestData requestData, List<OXException> warnings, DeleteAfterTransportOptions deleteAfterTransportOptions, ClientToken clientToken) throws OXException;
    ...
    /**
     * Adds new attachments to given composition space.
     *
     * @param compositionSpaceId The composition space identifier
     * @param descriptionsAndDatas The tuples of attachment description and attachment's data
     * @param clientToken The clients current token to edit the space
     * @return The added attachments
     * @throws OXException If adding the attachments fails
     */
    default AttachmentResult addAttachmentsToCompositionSpace(UUID compositionSpaceId, List<AttachmentDescriptionAndData> descriptionsAndDatas, ClientToken clientToken) throws OXException {
        ...
        CompositionSpaceInfo compositionSpaceInfo = null;
        List<Attachment> newAttachments = new ArrayList<>(size);
        for (AttachmentDescriptionAndData descriptionAndData : descriptionsAndDatas) {
            AttachmentResult attachmentResult = addAttachmentToCompositionSpace(compositionSpaceId, descriptionAndData.getAttachmentDescription(), descriptionAndData.getData(), clientToken);
            newAttachments.addAll(attachmentResult.getAttachments());
            compositionSpaceInfo = attachmentResult.getCompositionSpaceInfo();
        }
        return new DefaultAttachmentResult(newAttachments, compositionSpaceInfo);
    }
    ...
    /**
     * Deletes the specified attachments from given composition space.
     *
     * @param compositionSpaceId The composition space identifier
     ...
     * @throws OXException If deletion fails
     */
    default AttachmentResult deleteAttachments(UUID compositionSpaceId, List<UUID> attachmentIds, ClientToken clientToken) throws OXException {
        List<UUID> attachIds = attachmentIds == null ? null : attachmentIds.stream().filter(Objects::nonNull).collect(Collectors.toList());
        ...
        CompositionSpaceInfo compositionSpaceInfo = null;
        List<Attachment> deletedAttachments = new ArrayList<>(attachIds.size());
        for (UUID attachmentId : attachIds) {
            AttachmentResult attachmentResult = deleteAttachment(compositionSpaceId, attachmentId, clientToken);
            deletedAttachments.addAll(attachmentResult.getAttachments());
            compositionSpaceInfo = attachmentResult.getCompositionSpaceInfo();
        }
        return new DefaultAttachmentResult(deletedAttachments, compositionSpaceInfo);
    }
