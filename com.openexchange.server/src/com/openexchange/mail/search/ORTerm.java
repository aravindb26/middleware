...
public final class ORTerm extends CatenatingTerm {
    ...
    protected boolean applyCapabilities(CapabilitiesAndOptions capabilities, SearchTerm<?> parentTerm) {
        if (capabilities.isIgnoreUnsupportedTermsIfPossible() == false) {
            return getFirstTerm().applyCapabilities(capabilities, this) && getSecondTerm().applyCapabilities(capabilities, this);
        }

        if (getFirstTerm().applyCapabilities(capabilities, this) == false) {
            if (getSecondTerm().applyCapabilities(capabilities, this) == false) {
                // Not applicable to any of terms
                return false;
            }
            terms[0] = BooleanTerm.FALSE;
        } else if (getSecondTerm().applyCapabilities(capabilities, this) == false) {
            terms[1] = BooleanTerm.FALSE;
        }
        return true;
    }
