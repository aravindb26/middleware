...
public class DefaultMessageGenerator implements MessageGenerator {
    ...
    @Override
    public List<ComposedMailMessage> generateTransportMessagesFor(ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> attachmentIdToContentIdMapping, int moreFiles) throws OXException {
        ...
        return equalMessagesByLocale() ? generateEqualMessagesByLocale(info, shareReference, headers, attachmentIdToContentIdMapping, moreFiles) : generateIndividualMessages(info, shareReference, headers, attachmentIdToContentIdMapping, moreFiles);
    }
     ...
     */
    protected List<ComposedMailMessage> generateEqualMessagesByLocale(ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> headers, Map<String, String> attachmentIdToContentIdMapping, int moreFiles) throws OXException {
        ...
        for (Recipient recipient : recipients) {
            if (recipient.isUser()) {
                ...
                if (null == composedMessage) {
                    composedMessage = generateInternalVersion(recipient, info, shareReference, headers, attachmentIdToContentIdMapping, moreFiles);
                ...
                if (null == composedMessage) {
                    composedMessage = generateExternalVersion(localeForExternalRecipients, recipient, info, shareReference, headers, attachmentIdToContentIdMapping, moreFiles);
     ...
     * @throws OXException If messages cannot be generated
     */
    protected List<ComposedMailMessage> generateIndividualMessages(ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> headers, Map<String, String> attachmentIdToContentIdMapping, int moreFiles) throws OXException {
        Collection<Recipient> recipients = info.getRecipients();
        ...
        for (Recipient recipient : recipients) {
            if (recipient.isUser()) {
                messages.add(generateInternalVersion(recipient, info, shareReference, headers, attachmentIdToContentIdMapping, moreFiles));
            } else {
                if (null == localeForExternalRecipients) {
                    localeForExternalRecipients = determineLocaleForExternalRecipients(info.getComposeContext());
                }
                messages.add(generateExternalVersion(localeForExternalRecipients, recipient, info, shareReference, headers, attachmentIdToContentIdMapping, moreFiles));
            }
        }
        return messages;
    }
    ...
    @Override
    public ComposedMailMessage generateSentMessageFor(ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> attachmentIdToContentIdMapping, int moreFiles) throws OXException {
        ShareComposeLink shareLink = info.getShareLink();
        ...
        ComposedMailMessage sentMessage = generateInternalVersion(info.getRecipients().get(0), info, shareReference, headers, attachmentIdToContentIdMapping, moreFiles);
     ...
     */
    protected ComposedMailMessage generateInternalVersion(Recipient recipient, ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> shareHeaders, Map<String, String> attachmentIdToContentIdMapping, int moreFiles) throws OXException {
        ...
        if (null != regionalSettingsService) {
            ...
            return generateLocaleSpecificVersion(locale, recipient, info, shareReference, shareHeaders, attachmentIdToContentIdMapping, moreFiles, regionalSettings);
        }
        return generateLocaleSpecificVersion(locale, recipient, info, shareReference, shareHeaders, attachmentIdToContentIdMapping, moreFiles, null);
    }
     ...
     * @throws OXException If compose message cannot be returned
     */
    protected ComposedMailMessage generateExternalVersion(Locale localeForExternalRecipients, Recipient recipient, ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> shareHeaders, Map<String, String> attachmentIdToContentIdMapping, int moreFiles) throws OXException {
        // Generate locale-specific version using the configured locale for external recipients
        return generateLocaleSpecificVersion(localeForExternalRecipients, recipient, info, shareReference, shareHeaders, attachmentIdToContentIdMapping, moreFiles, null);
    }
     ...
     */
    protected ComposedMailMessage generateLocaleSpecificVersion(Locale locale, Recipient recipient, ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> shareHeaders, Map<String, String> attachmentIdToContentIdMapping, int moreFiles, RegionalSettings regionalSettings) throws OXException {
        ...
        if (composeContext.isPlainText()) {
            ...
            if (null == plainText) {
                ...
                textBuilder.append(generatePrefix(locale, info, shareReference, false, attachmentIdToContentIdMapping, moreFiles, false, regionalSettings));
                ...
                // Append the prefix that notifies about to access the message's attachment via provided share link
                plainTextBuilder.append(generatePrefix(locale, info, shareReference, loadPrefixFromTemplate(), attachmentIdToContentIdMapping, moreFiles, true, regionalSettings));
            ...
            // Append the prefix that notifies about to access the message's attachment via provided share link
            textBuilder.append(generatePrefix(locale, info, shareReference, loadPrefixFromTemplate(), attachmentIdToContentIdMapping, moreFiles, false, regionalSettings));
     ...
     */
    protected String generatePrefix(Locale locale, ShareComposeMessageInfo info, ShareReference shareReference, boolean fromTemplate, Map<String, String> attachmentIdToContentIdMapping, int moreFiles, boolean isPlainText, RegionalSettings regionalSettings) throws OXException {
        if (isPlainText) {
            HtmlService htmlService = ServerServiceRegistry.getInstance().getService(HtmlService.class);
            return htmlService.html2text(generatePrefixPlain(locale, info, shareReference, regionalSettings), true);
        }
        return fromTemplate ? loadPrefixFromTemplate(locale, info, shareReference, attachmentIdToContentIdMapping, moreFiles, regionalSettings) : generatePrefixPlain(locale, info, shareReference, regionalSettings);
    }
     ...
     */
    protected String loadPrefixFromTemplate(Locale locale, ShareComposeMessageInfo info, ShareReference shareReference, Map<String, String> attachmentIdToContentIdMapping, int moreFiles, RegionalSettings regionalSettings) throws OXException {
        ...
        // Filenames and previews
        {
            ...
            Map<String, String> previewImages = new HashMap<>(attachmentIdToContentIdMapping.size());
            for (Map.Entry<String, String> entry : attachmentIdToContentIdMapping.entrySet()) {
