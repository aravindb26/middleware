...
import com.openexchange.ajax.requesthandler.jobqueue.JobKeys;
...
public final class AllAction extends AbstractMailAction implements MailRequestSha1Calculator, EnqueuableAJAXActionService {
    ...
    private AJAXRequestResult perform0(MailRequest req, MailServletInterface mailInterface, boolean cache) throws OXException {
        try {
            // Read parameters
            ...
            final boolean ignoreDeleted = isIgnoreDeleted(req, false);
            ...
            if (order != null) {
                orderDirection = OrderDirection.getOrderDirection(order);
            ...
            try {
                ...
                if (filterApplied || categoryFilter != null) {
                    ...
                    SearchTerm<?> searchTerm;
                    {
                        ...
                        if (categoryFilter != null && !"none".equals(categoryFilter) && isMailCategoriesEnabled(req.getSession())) {
                            ...
                            if (categoriesService != null) {
                                if ("general".equals(categoryFilter)) {
                                    ...
                                    String flag = categoriesService.getFlagByCategory(req.getSession(), categoryFilter);
                                    if (flag == null) {
                                        throw MailExceptionCode.INVALID_PARAMETER_VALUE.create(categoryFilter);
                                    }

                                    // test if category is a system category
                                    if (categoriesService.isSystemCategory(categoryFilter, req.getSession())) {
     ...
     */
    private static boolean isMailCategoriesEnabled(ServerSession session) throws OXException {
        ServiceLookup services = MailJSONActivator.SERVICES.get();
        CapabilityService capabilityService = services == null ? null : services.getService(CapabilityService.class);
    ...
    public String getSha1For(MailRequest req) throws OXException {
        ...
        return JsonCaches.getSHA1Sum("all", req.checkParameter(Mail.PARAMETER_MAILFOLDER), req.checkParameter(AJAXServlet.PARAMETER_COLUMNS), req.getParameter(AJAXServlet.PARAMETER_SORT), req.getParameter(AJAXServlet.PARAMETER_ORDER),
    ...
    @Override
    public Result isEnqueueable(AJAXRequestData req, ServerSession session) throws OXException {
        String sha1Sum = JobKeys.getSHA1Sum("mail",
                                            "all",
                                            ...
                                            req.getParameter(AJAXServlet.PARAMETER_LIMIT),
        ...
        return EnqueuableAJAXActionService.resultFor(true, new JobKey(session.getUserId(), session.getContextId(), sha1Sum), this);
    }
