...
import com.openexchange.mail.dataobjects.DataProviderMailPart;
...
import com.openexchange.mail.utils.MailPartSizeCache;
...
@RestrictedAction(module = AbstractMailAction.MODULE, type = RestrictedAction.Type.READ, hasCustomRestrictedAccessCheck = true)
public final class GetAttachmentAction extends AbstractMailAction implements ETagAwareAJAXActionService, LastModifiedAwareAJAXActionService {
    ...
    public boolean accessAllowed(AJAXRequestData request, @SuppressWarnings("unused") final ServerSession session, Scope scope) throws OXException {
        if (request.getData() != null) {

            if (scope.has(WRITE_SCOPE) == false) {
                throw RestrictedActionExceptionCodes.INSUFFICIENT_SCOPES.create(WRITE_SCOPE);
            }
        } else if (scope.has(READ_SCOPE) == false) {
            throw RestrictedActionExceptionCodes.INSUFFICIENT_SCOPES.create(READ_SCOPE);
        }
        return true;
    }
    ...
    private JSONObject optJSONObject(MailRequest req) throws OXException {
        ...
        try {
            return JSONServices.parseObject(data.toString());
                ...
                if (imageContentId == null) {
                    ...
                    if (fromStructure) {
                        ...
                        if (MimeStructureFixer.getInstance().isApplicableFor(mail)) {
                            ...
                            if (exactLength) {
                                SizeAndOptionalSink sas = getMailPartSize(requestData.getSession(), folderPath, uid, sequenceId, false, ret);
                                size = sas.size;
                                sink = sas.sink;
                            }
                            if (sink == null) {
                    ...
                    mailPart = optReferencedSharedAttachmentMailPart(mailPart, session);
                    ...
                    if (asJson && mailPart.getContentType().startsWith("message/rfc822")) {
                        ...
                        if (null != nestedMailMessage) {
                            ...
                            nestedMailMessage.setSequenceId(nestedMailMessage.getSequenceId() == null ? sequenceId : new StringBuilder(sequenceId).append('.').append(nestedMailMessage.getSequenceId()).toString());
                    ...
                    if (filter && !saveToDisk && ((Strings.startsWithAny(toLowerCase(mailPart.getContentType().getSubType()), "htm", "xhtm") && fileNameAbsentOrIndicatesHtml(mailPart.getFileName())) || fileNameIndicatesHtml(mailPart.getFileName()))) {
                        ...
                        if (exactLength) {
                            if (null == sink) {
                                SizeAndOptionalSink sas = getMailPartSize(requestData.getSession(), folderPath, uid, sequenceId, false, mailPart);
                                size = sas.size;
                                sink = sas.sink;
                            }
                        }
                        if (sink == null) {
                            if (null == isClosure) {
                                isClosure = new ReconnectingInputStreamClosure(mailPart, folderPath, uid, sequenceId, false, session);
                            }
                        }
                    }
                } else {
                    ...
                    boolean exactLength = calculateExactLength(req);
                    if (exactLength) {
                        SizeAndOptionalSink sas = getMailPartSize(requestData.getSession(), folderPath, uid, imageContentId, true, mailPart);
                        size = sas.size;
                        sink = sas.sink;
                    }
                    if (sink == null) {
                        isClosure = new ReconnectingInputStreamClosure(mailPart, folderPath, uid, imageContentId, true, session);
                    }
                }
    ...
    private SizeAndOptionalSink getMailPartSize(Session session, String folderPath, String mailId, String partId, boolean image, MailPart mailPart) throws OXException {
        ThresholdFileHolder sink = null;
        long size = MailPartSizeCache.getInstance().optSizeFor(session, folderPath, mailId, partId, image);
        if (size < 0) {
            AtomicReference<ThresholdFileHolder> sinkRef = new AtomicReference<ThresholdFileHolder>();
            Callable<Long> sizeLoader = () -> {
                ThresholdFileHolder tfh = new ThresholdFileHolder();
                InputStream in = Streams.getNonEmpty(mailPart.getInputStream());
                tfh.write(null == in ? Streams.EMPTY_INPUT_STREAM : in);
                sinkRef.set(tfh);
                return L(tfh.getLength());
            };
            size = MailPartSizeCache.getInstance().setSizeFor(session, folderPath, mailId, partId, image, sizeLoader);
            sink = sinkRef.get();
        }
        return new SizeAndOptionalSink(size, sink);
    }

    private MailPart optReferencedSharedAttachmentMailPart(MailPart mailPart, ServerSession session) throws OXException {
        ...
        FileItem sharedAttachment = getAttachmentStorage(session).getAttachment(sharedAttachmentRef.getAttachmentId(), sharedAttachmentRef.getFolderId(), session);
        DataProviderMailPart.DataProvider dataProvider = new FileItemDataProvider(sharedAttachment);
        return new DataProviderMailPart(mailPart, dataProvider);
    }
    ...
    private AJAXRequestResult performPUT(MailRequest req, JSONObject jsonFileObject) throws OXException {
        try {
            ...
            // Check for possible shared attachment reference
            mailPart = optReferencedSharedAttachmentMailPart(mailPart, session);
    ...
    private static class FileItemDataProvider implements DataProviderMailPart.DataProvider {

        private final FileItem sharedAttachment;

        FileItemDataProvider(FileItem sharedAttachment) {
            super();
        ...
        @Override
        public long getSize() {
            return sharedAttachment.getSize();
        }

        @Override
        public InputStream getInputStream() throws OXException {
            return sharedAttachment.getData();
        }
    }
    ...
    private static final class ReconnectingInputStreamClosure implements IFileHolder.InputStreamClosure {
        ...
        private final AtomicReference<ThresholdFileHolder> thresholdFileReference;
        ...
        ReconnectingInputStreamClosure(MailPart mailPart, String folderPath, String uid, String id, boolean image, ServerSession session) {
            ...
            this.image = image;
            this.thresholdFileReference = new AtomicReference<ThresholdFileHolder>();
        }

        @Override
        public InputStream newStream() throws OXException, IOException {
            {
                ThresholdFileHolder tfh = thresholdFileReference.get();
                if (null != tfh) {
            ...
            try {
                ...
                // Need to reconnect
                LOG.warn("Lost connection to mail server for user {} in context {}", I(session.getUserId()), I(session.getContextId()), e);
        ...
        private synchronized InputStream getReconnectedStream() throws OXException {
            ...
            try {
                ...
                thresholdFileReference.set(newTfh);
    ...
    private static class SizeAndOptionalSink {

        /** The part's size */
        final long size;

        /** The sink or <code>null</code> */
        final ThresholdFileHolder sink;

        SizeAndOptionalSink(long size, ThresholdFileHolder sink) {
            super();
            this.size = size;
            this.sink = sink;
        }
    } // End of class SizeAndOptionalSink
