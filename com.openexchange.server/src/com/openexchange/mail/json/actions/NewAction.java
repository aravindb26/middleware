...
public final class NewAction extends AbstractMailAction implements EnqueuableAJAXActionService {
    ...
    public EnqueuableAJAXActionService.Result isEnqueueable(AJAXRequestData request, ServerSession session) throws OXException {
        ...
        if (hasUploads(request, session)) {
            ...
            try {
                JSONObject jMail = JSONServices.parseObject(json0);
    ...
    private AJAXRequestResult performWithUploads(MailRequest req, AJAXRequestData request) throws OXException, JSONException {
        ...
        try {
            JSONObject jMail;
            {
                ...
                jMail = JSONServices.parseObject(json0);
            ...
            int accountId;
            try {
                accountId = resolveSender2Account(session, from, true, true);
            } catch (OXException e) {
            ...
            if (isDraft) {
                ...
                MailPath draftPath = mailInterface.saveDraft(composedMail, false, accountId);
                if (draftPath == null) {
                    throw MailExceptionCode.DRAFT_FAILED_UNKNOWN.create();
                }
                msgIdentifier = draftPath.toString();
    ...
    private AJAXRequestResult transportMessage(ServerSession session, int flags, boolean force, InternetAddress from, MailMessage m, AJAXRequestData request) throws OXException, JSONException {
        ...
        final int accountId;
        {
            int accId;
            try {
                accId = resolveSender2Account(session, from, true, !force);
            } catch (OXException e) {
        ...
        if (isOAuthRequest(request)) {
            ...
            } else if (MailAccounts.isSecondaryAccount(accountId, session)) {
                PasswordSource passwordSource = MailProperties.getInstance().getSecondaryPasswordSource(session.getUserId(), session.getContextId());
                switch (passwordSource) {
                    case GLOBAL: {
                        // Just for convenience
                        String masterPassword = MailProperties.getInstance().getSecondaryMasterPassword(session.getUserId(), session.getContextId());
                        if (masterPassword == null) {
