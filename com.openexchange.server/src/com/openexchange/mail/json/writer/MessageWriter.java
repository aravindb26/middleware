...
public final class MessageWriter {
    ...
    static {
        ...
        writers.put(MailListField.SENDER, new MailFieldWriter() {

            @Override
            public void writeField(JSONValue jsonContainer, MailMessage mail, int level, boolean withKey, int accountId, int userId, int contextId, TimeZone optTimeZone) throws OXException {
                try {
                    if (withKey) {
                        jsonContainer.toObject().put(MailJSONField.SENDER.getKey(), getAddressesAsArray(mail.getSender()));
                    } else {
                        jsonContainer.toArray().put(getAddressesAsArray(mail.getSender()));
                    }
                } catch (JSONException e) {
                    throw MailExceptionCode.JSON_ERROR.create(e, e.getMessage());
                }
        ...
        writers.put(MailListField.REPLY_TO, new MailFieldWriter() {

            @Override
            public void writeField(JSONValue jsonContainer, MailMessage mail, int level, boolean withKey, int accountId, int userId, int contextId, TimeZone optTimeZone) throws OXException {
                try {
                    if (withKey) {
                        jsonContainer.toObject().put(MailJSONField.REPLY_TO.getKey(), getAddressesAsArray(mail.getReplyTo()));
                    } else {
                        jsonContainer.toArray().put(getAddressesAsArray(mail.getReplyTo()));
                    }
                } catch (JSONException e) {
                    throw MailExceptionCode.JSON_ERROR.create(e, e.getMessage());
                }
        ...
        writers.put(MailListField.USER_FLAGS, new MailFieldWriter() {
            ...
            public void writeField(JSONValue jsonContainer, MailMessage mail, int level, boolean withKey, int accountId, int userId, int contextId, TimeZone optTimeZone) throws OXException {
                try {
                    ...
                    if (withKey) {
                        if (userFlags != null) {
                            jsonContainer.toObject().put(MailJSONField.USER.getKey(), userFlags.isEmpty() ? JSONArray.EMPTY_ARRAY : new JSONArray(userFlags));
                        }
                    } else {
                        if (userFlags == null) {
                            jsonContainer.toArray().put(JSONObject.NULL);
                        } else {
                            jsonContainer.toArray().put(userFlags.isEmpty() ? JSONArray.EMPTY_ARRAY : new JSONArray(userFlags));
                        }
                    }
                } catch (JSONException e) {
                    throw MailExceptionCode.JSON_ERROR.create(e, e.getMessage());
    ...
    private static final JSONArray EMPTY_JSON_ARR = JSONArray.EMPTY_ARRAY;
