...
public abstract class AbstractMimeProcessing {
    ...
    private static final boolean INCLUDE_TIME_ZONE_IN_TIME_REPLACEMENT = false;
    ...
    protected static String generatePrefixText(String prefixTemplate, LocaleAndTimeZone ltz, MailMessage msg, Session session) {
        ...
        }
        {
            ...
            Optional<TimeZone> optSentDateTimeZone = date == null || !INCLUDE_TIME_ZONE_IN_TIME_REPLACEMENT ? Optional.<TimeZone> empty() : optSentDateTimeZone(msg);
            TimeZone timeZone = optSentDateTimeZone.orElse(ltz.timeZone);
            if (prefix.indexOf("#DATE#") >= 0) {
                try {
                    prefix = PATTERN_DATE.matcher(prefix).replaceAll(date == null ? "" : quoteReplacement(MimeProcessingUtility.getFormattedDate(date, DateFormat.LONG, ltz.locale, timeZone, session)));
                } catch (Exception t) {
                    LOG.warn("", t);
            ...
            if (prefix.indexOf("#TIME#") >= 0) {
                try {
                    String timeReplacement = date == null ? "" : quoteReplacement(MimeProcessingUtility.getFormattedTime(date, DateFormat.SHORT, ltz.locale, timeZone, session));
                    if (optSentDateTimeZone.isPresent()) {
                        StringBuilder sb = new StringBuilder(timeReplacement).append(' ').append(optSentDateTimeZone.get().getDisplayName(false, TimeZone.SHORT, ltz.locale));
                        timeReplacement = sb.toString();
                    }
                    prefix = PATTERN_TIME.matcher(prefix).replaceAll(timeReplacement);
                } catch (Exception t) {
                    LOG.warn("", t);
    ...
    private static Optional<TimeZone> optSentDateTimeZone(MailMessage msg) {
        String dateString = msg.getHeader("Date", null);
        ...
        try {
            ...
            ZonedDateTime zdtWithZoneOffset = ZonedDateTime.parse(dateString, FORMATTER);
            if (zdtWithZoneOffset == null) {
                return Optional.empty();
            }
            return Optional.of(TimeZone.getTimeZone(zdtWithZoneOffset.getZone()));
        } catch (Exception e) {
            LOG.debug("Time zone could not be parsed from text: {}", dateString, e);
            return Optional.empty();
        }
    }
