...
public final class RawJSONMessageHandler implements MailMessageHandler {
    ...
    public boolean handleSender(InternetAddress[] senderAddrs) throws OXException {
        try {
            jsonObject.put(MailJSONField.SENDER.getKey(), MessageWriter.getAddressesAsArray(senderAddrs));
        } catch (JSONException e) {
            throw MailExceptionCode.JSON_ERROR.create(e, e.getMessage());
        }
        return true;
    ...
    public boolean handleReplyTo(InternetAddress[] replyToAddrs) throws OXException {
        try {
            jsonObject.put(MailJSONField.REPLY_TO.getKey(), MessageWriter.getAddressesAsArray(replyToAddrs));
        } catch (JSONException e) {
            throw MailExceptionCode.JSON_ERROR.create(e, e.getMessage());
        }
        return true;
    ...
    private static final Set<HeaderName> COVERED_HEADER_NAMES =
        Set.of(
            ...
            MessageHeaders.SUBJECT, MessageHeaders.TO, MessageHeaders.SENDER);
