...
public final class LoginPerformer {
    ...
    public LoginResult doLogin(LoginRequest request, Map<String, Object> properties, LoginMethodClosure loginMethod) throws OXException {
        ...
        try {
            ...
            Context ctx;
            User user;
            if (authed instanceof ResolvedAuthenticated guestAuthenticated) {
            ...
            if (authed instanceof SessionEnhancement) {
            ...
            // Add brand name to session (if configured)
            {
                ...
                if (null != serverConfigService) {
                    ServerConfig config = serverConfigService.getServerConfig(request.getServerName(), user.getId(), ctx.getContextId());
                    addSessionParam.addEnhancement(session -> session.setParameter(Session.PARAM_BRAND, config.getProductName()));
                }
            }

            // Add segment marker (aka database schema) to session
            {
                AssignmentFactory assignmentFactory = ServerServiceRegistry.getInstance().getService(AssignmentFactory.class);
                if (null != assignmentFactory) {
                    try {
                        String schema = assignmentFactory.get(ctx.getContextId()).getSchema();
                        addSessionParam.addEnhancement(session -> session.setParameter(Session.PARAM_USER_SCHEMA, schema));
                    } catch (Exception e) {
                        LOG.error("Failed to determine the database schema associated with session's context identifier {}", I(ctx.getContextId()), e);
                    }
                }
            }

            // Add enhancement for device user agent parameter if applicable
            optDeviceUserAgentEnhancement(headers).ifPresent(addSessionParam::addEnhancement);
    ...
    /**
     * Optionally extracts the first <code>X-Device-User-Agent</code> header value from the supplied request headers collection, and
     * prepares a suitable session enhancement that stores this value as session parameter in {@link Session#PARAM_DEVICE_USER_AGENT}.
     *
     * @param requestHeaders The request headers to extract the device user agent from
     * @return An optional session enhancement with the device user agent header value
     */
    private static Optional<SessionEnhancement> optDeviceUserAgentEnhancement(Map<String, List<String>> requestHeaders) {
        if (null != requestHeaders) {
            List<String> deviceUserAgentHeaders = requestHeaders.get("X-Device-User-Agent");
            if (null != deviceUserAgentHeaders && !deviceUserAgentHeaders.isEmpty()) {
                String value = deviceUserAgentHeaders.get(0);
                if (Strings.isNotEmpty(value)) {
                    SessionEnhancement sessionEnhancement = (session) -> {
                        session.setParameter(Session.PARAM_DEVICE_USER_AGENT, value);
                    };
                    return Optional.of(sessionEnhancement);
                }
            }
        }
     ...
     */
    public static void triggerLoginHandlers(LoginResult login) {
        ThreadPoolService executor = ThreadPools.getThreadPool();
        if (null == executor) {
            for (Iterator<LoginHandlerService> it = LoginHandlerRegistry.getInstance().getLoginHandlers(); it.hasNext();) {
            ...
            for (Iterator<LoginHandlerService> it = LoginHandlerRegistry.getInstance().getLoginHandlers(); it.hasNext();) {
                LoginHandlerService handler = it.next();
                if (tranzient && (handler instanceof NonTransient)) {
                    // skip
                    continue;
                }
            ...
            if (null != completionService && blocking > 0) {
    ...
    private static void logLoginRequest(final LoginRequest request, final LoginResult result) {
        ...
        LOG.info(sb.toString()); // NOSONARLINT
