...
public class MultifactorChecker {
    ...
    /**
     * Checks if the the given {@link Session} was once validated against a multifactor device, if required.
     * <p>
     * This method throws an appropriated OXException if multifactor authentication is required for this session but was not performed,
     * or the method just returns normally if the authentication was performed once or is not required.
     * </p>
     *
     * @param session The {@link Session} to validate
     * @param request The related {@link HttpServletRequest}
     ...
     */
    public void validateSession(Session session, HttpServletRequest request) throws OXException {
        if (session == null) {
            return;
        }

        MultifactorSessionValidator multifactorSessionValidator = serviceLookup.getOptionalService(MultifactorSessionValidator.class);
        if (multifactorSessionValidator != null) {
            multifactorSessionValidator.validate(session, request);
        }
    }
