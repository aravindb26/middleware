...
public class ConsistencyServiceImpl implements ConsistencyService {
    ...
    public List<String> checkOrRepairConfigDB(boolean repair) throws OXException {
        ...
        OXContextService oxContextService = null;
        if (repair) {
            oxContextService = ServerServiceRegistry.getInstance().getService(OXContextService.class, true);
        }
        ...
        try {
            ...
            Map<String, List<Integer>> schemaMap = new HashMap<>();
            ...
            for (Map.Entry<String, List<Integer>> schemaEntry : schemaMap.entrySet()) {
                String schema = schemaEntry.getKey();
                List<Integer> ctxs = schemaEntry.getValue();

                Integer poolid = schemaPoolMap.get(schema);
                Set<Integer> nonExistentCtxs = new HashSet<Integer>(ctxs);
                ...
                try {
                    ...
                    try {
                        ...
                        if (cause != null && cause.getMessage() != null && cause.getMessage().contains("Unknown database")) {
                            if (repair) {
                                deleteSchemaFromConfigDB(schema, databaseService);
                    ...
                    for (List<Integer> partition : Lists.partition(ctxs, Databases.IN_LIMIT)) {
                        String contextids = Strings.join(partition, ",");
                        // GROUP BY CLAUSE: ensure ONLY_FULL_GROUP_BY compatibility
                        stmt = poolCon.prepareStatement("SELECT cid FROM login2user WHERE cid IN (" + contextids + ") GROUP BY cid");
                        rs = stmt.executeQuery();
                        while (rs.next()) {
                            nonExistentCtxs.remove(I(rs.getInt(1)));
                        }
                        Databases.closeSQLStuff(rs, stmt);
                        stmt = null;
                        rs = null;
                ...
                if (nonExistentCtxs.isEmpty() == false) {
                    LOG.info("Schema {} is broken", schema);
                    for (Integer ctx : nonExistentCtxs) {
                        if (repair) {
                            LOG.info("Deleting inconsistent entry for context {} from configdb", ctx);
                            oxContextService.delete(new com.openexchange.admin.rmi.dataobjects.Context(ctx));
     ...
     */
    private void deleteSchemaFromConfigDB(String schema, DatabaseService databaseService) throws SQLException, OXException {
        Connection configCon = null;
        PreparedStatement stmt = null;
        try {
            configCon = databaseService.getWritable();
            ...
            if (configCon != null) {
                databaseService.backWritable(configCon);
            }
        }
    }
     ...
     * @param args The problem solver to use
            ...
            switch (entity.getType()) {
                ...
                case User:
                    ...
                    snippetsetFuture = submitElseExecute(new AbstractTask<SortedSet<String>>() {
                        ...
                        @Override
                        public SortedSet<String> call() throws OXException {
                            // Files for snippets are always stored in context-related storage
                            return new TreeSet<String>();
                        }
                    });
                    // Preview cache -------------------------------------------------------------------------------------------------------
                    previewsetFuture = submitElseExecute(new AbstractTask<SortedSet<String>>() {
                        ...
                        @Override
                        public SortedSet<String> call() throws OXException {
                            // Files for preview cache are always stored in context-related storage
                            return new TreeSet<String>();
                        }
                    });
                    // vCards --------------------------------------------------------------------------------------------------------------
