 ...
 * @since v7.10.1
 */
public class ConsistencyRMIServiceImpl extends AbstractAuthenticatorRMIService implements ConsistencyRMIService {
    ...
    /**
     * Initializes a new {@link ConsistencyRMIServiceImpl}.
     *
    ...
    @Override
    public List<String> checkOrRepairConfigDB(boolean repair, Credentials auth) throws RemoteException {
    ...
    public Map<ConsistencyEntity, List<String>> listUnassignedFilesInFilestore(int filestoreId) throws RemoteException {
        return handle(service -> {
            ...
            return convertMap(service.listUnassignedFilesInFilestore(filestoreId));
        });
    }
    ...
    @Override
    public void repairFilesInContext(int contextId, String repairPolicy, String repairAction, Credentials auth) throws RemoteException {
        Authenticator authenticator = services.getOptionalService(Authenticator.class);
        authenticate(authenticator, auth);
        ...
        handle(service -> {
            ...
            service.repairFilesInContext(contextId, RepairPolicy.valueOf(Strings.toUpperCase(repairPolicy)), RepairAction.valueOf(Strings.toUpperCase(repairAction)));
            return null;
        });
    }

    @Override
    public void repairFilesInFilestore(int filestoreId, String repairPolicy, String repairAction, Credentials auth) throws RemoteException {
        Authenticator authenticator = services.getOptionalService(Authenticator.class);
        authenticate(authenticator, auth);
        ...
        handle(service -> {
            ...
            service.repairFilesInFilestore(filestoreId, RepairPolicy.valueOf(Strings.toUpperCase(repairPolicy)), RepairAction.valueOf(Strings.toUpperCase(repairAction)));
            return null;
        });
    }

    @Override
    public void repairFilesInDatabase(int databaseId, String repairPolicy, String repairAction, Credentials auth) throws RemoteException {
        Authenticator authenticator = services.getOptionalService(Authenticator.class);
        authenticate(authenticator, auth);
        ...
        handle(service -> {
            ...
            service.repairFilesInDatabase(databaseId, RepairPolicy.valueOf(Strings.toUpperCase(repairPolicy)), RepairAction.valueOf(Strings.toUpperCase(repairAction)));
            return null;
        });
    }

    @Override
    public void repairAllFiles(String repairPolicy, String repairAction, Credentials auth) throws RemoteException {
        Authenticator authenticator = services.getOptionalService(Authenticator.class);
        authenticate(authenticator, auth);

        handle(service -> {
            LOG.info("RMI invocation for: Repair files with repair policy {} and repair action {}", repairPolicy, repairAction);
            service.repairAllFiles(RepairPolicy.valueOf(Strings.toUpperCase(repairPolicy)), RepairAction.valueOf(Strings.toUpperCase(repairAction)));
            return null;
        });
    }
     ...
     */
    private static ConsistencyEntity toConsistencyEntity(Entity entity) {
        return switch (entity.getType()) {
            case Context -> new ConsistencyEntity(entity.getContext().getContextId());
            case User -> new ConsistencyEntity(entity.getContext().getContextId(), entity.getUser().getId());
            default -> throw new IllegalArgumentException("Unknown entity type: " + entity.getType());
        };
    }
     ...
     */
    private static Map<ConsistencyEntity, List<String>> convertMap(Map<Entity, List<String>> entities) {
