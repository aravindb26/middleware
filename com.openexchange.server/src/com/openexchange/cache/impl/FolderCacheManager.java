...
public final class FolderCacheManager {
    ...
    public FolderObject getFolderObject(int objectId, boolean fromCache, Context ctx, Connection readCon) throws OXException {
        ...
        if (fromCache) {
            // Look-up cache
            Object object = folderCache.getFromGroup(cacheKey, Integer.toString(ctx.getContextId()));
    ...
    public FolderObject getFolderObject(int objectId, Context ctx) {
        ...
        Object tmp = folderCache.getFromGroup(getCacheKeyUsing(ctx.getContextId(), objectId, folderCache), Integer.toString(ctx.getContextId()));
    ...
    public List<FolderObject> getTrimedFolderObjects(int[] objectIds, Context ctx) {
        ...
        for (int objectId : objectIds) {
            Object tmp = folderCache.getFromGroup(getCacheKeyUsing(contextId, objectId, folderCache), Integer.toString(ctx.getContextId()));
    ...
    public FolderObject loadFolderObject(final int folderId, final Context ctx, final Connection readCon) throws OXException {
        ...
        if (folderCache.isReplicated()) {
            ...
            lock.lock();
            try {
                final Object tmp = folderCache.getFromGroup(key, Integer.toString(ctx.getContextId()));
                if (tmp instanceof FolderObject) {
                    folderCache.removeFromGroup(key, Integer.toString(ctx.getContextId()));
                    // Dirty hack
        ...
        // Return element
        Object object = folderCache.getFromGroup(key, Integer.toString(ctx.getContextId()));
        if (object instanceof FolderObject) {
            return ((FolderObject) object).clone();
        }
    ...
    private FolderObject putIfAbsentInternal(FolderProvider folderProvider, Context ctx, ElementAttributes elemAttribs, CacheKey cacheKey) throws OXException {
        ...
        lock.lock();
        try {
            Object tmp = folderCache.getFromGroup(key, Integer.toString(ctx.getContextId()));
            if (tmp instanceof FolderObject) {
                // Already in cache
                ...
                if (elemAttribs == null || folderCache.isDistributed()) {
                    ...
                    folderCache.putInGroup(key, Integer.toString(ctx.getContextId()), folderProvider.getFolderObject(), false);
                } else {
                    folderCache.putInGroup(key, Integer.toString(ctx.getContextId()), folderProvider.getFolderObject(), elemAttribs, false);
                }
                if (null != cond) {
                    cond.signalAll();
                }
    ...
    private FolderObject putFolderObject(FolderProvider folderProvider, Context ctx, boolean overwrite, ElementAttributes elemAttribs, boolean returnFolder) throws OXException {
        ...
        lock.lock();
        try {
            Object currentFromCache = folderCache.getFromGroup(key, Integer.toString(ctx.getContextId()));
            ...
            if (elemAttribs == null || folderCache.isDistributed()) {
                // Put with default attributes
                folderCache.putInGroup(key, Integer.toString(ctx.getContextId()), clone, false);
            } else {
                folderCache.putInGroup(key, Integer.toString(ctx.getContextId()), clone, elemAttribs, false);
            }
            if (null != cond) {
                cond.signalAll();
            }
    ...
    public void removeFolderObject(int contextId, int folderId, Connection optConnection) throws OXException {
        ...
        if (folderId > 0) {
            ...
            lock.lock();
            try {
                final Object tmp = folderCache.getFromGroup(cacheKey, Integer.toString(contextId));
                if (!(tmp instanceof Condition)) {
                    folderCache.removeFromGroup(cacheKey, Integer.toString(contextId));
                }
            } finally {
                lock.unlock();
            }
    ...
    public void removeFolderObjects(int contextId, int[] folderIds, Connection optConnection) throws OXException {
        ...
        Map<Integer, List<Serializable>> cacheKeys = new HashMap<Integer, List<Serializable>>(folderIds.length);
        for (int key : folderIds) {
            if (key > 0) {
                List<Serializable> list = cacheKeys.get(Integer.valueOf(contextId));
                if (list == null) {
                    list = new ArrayList<Serializable>();
                    cacheKeys.put(Integer.valueOf(contextId), list);
                }
                list.add(getCacheKeyUsing(contextId, key, folderCache));
            }
        }

        // Remove objects from cache
        for (Map.Entry<Integer, List<Serializable>> entry : cacheKeys.entrySet()) {
            folderCache.removeFromGroup(entry.getValue(), entry.getKey().toString());
        }
