...
public class FilteringGroupService implements GroupService {
    ...
    private Group removeAdminFromGroup(Session session, Group group) {
        HideAdminService hideAdminService = services.getOptionalService(HideAdminService.class);
        if (hideAdminService == null) {
            return group;
        }
        return hideAdminService.removeAdminFromGroupMemberList(session.getContextId(), new Group[] { group })[0];
    }
    ...
    public Group getGroup(Session session, int groupId) throws OXException {
        return removeAdminFromGroup(session, delegate.getGroup(session, groupId));
    }

    @Override
    public Group getGroup(Session session, int groupId, boolean loadMembers) throws OXException {
        return removeAdminFromGroup(session, delegate.getGroup(session, groupId, loadMembers));
    }

    @Override
    public Group[] listGroups(Session session, int[] ids) throws OXException {
        return removeAdminFromGroups(ServerSessionAdapter.valueOf(session).getContext(), delegate.listGroups(session, ids));
    }

    @Override
    public Group[] listModifiedGroups(Session session, Date modifiedSince) throws OXException {
        return removeAdminFromGroups(ServerSessionAdapter.valueOf(session).getContext(), delegate.listModifiedGroups(session, modifiedSince));
    }

    @Override
    public Group[] listDeletedGroups(Session session, Date deletedSince) throws OXException {
        return removeAdminFromGroups(ServerSessionAdapter.valueOf(session).getContext(), delegate.listDeletedGroups(session, deletedSince));
    }

    @Override
    public void update(Session session, Group group, Date lastRead, boolean checkI18nNames) throws OXException {
        delegate.update(session, group, lastRead, checkI18nNames);
    }
}
