...
public abstract class DefaultContactService implements ContactService {
    ...
    public Map<String, SequenceResult> getSequenceNumbers(Session session, List<String> folderIds, String[] ignore) throws OXException {
        Check.argNotNull(session, "session");
        Check.argNotNull(folderIds, "folderIds");
        return doGetSequenceNumbers(session, folderIds, ignore);
    }
    ...
    public Map<String, UpdatesResult<Contact>> getUpdatedContacts(Session session, List<String> folderIds, Date since, ContactField[] fields, String[] ignore) throws OXException {
        Check.argNotNull(session, "session");
        Check.argNotNull(folderIds, "folderId");
        Check.argNotNull(since, "since");
        return doGetUpdatedContacts(session, folderIds, since, fields, ignore);
    }
    ...
    public Contact getContact(final Session session, final String folderId, final String id, final ContactField[] fields) throws OXException {
        ...
        try {
            ...
            contact = searchIterator.hasNext() ? searchIterator.next() : null;
        } finally {
            if (null != searchIterator) {
                searchIterator.close();
            }
        }
        Check.contactNotNull(contact, session.getContextId(), Integer.parseInt(id));
        return contact;
    }
    ...
    public void updateContact(final Session session, final String folderId, final String id, final Contact contact, final Date lastRead)
        ...
        Check.argNotNull(contact, "contact");
        if (contact.containsFolderId(true) && false == Objects.equals(folderId, contact.getFolderId(true))) {
            this.doUpdateAndMoveContact(session, folderId, contact.getFolderId(true), id, contact, lastRead);
        } else {
    ...
    protected abstract Map<String, SequenceResult> doGetSequenceNumbers(Session session, List<String> folderIds, String[] ignore) throws OXException;

    protected abstract Map<String, UpdatesResult<Contact>> doGetUpdatedContacts(Session session, List<String> folderIds, Date since, ContactField[] fields, String[] ignore) throws OXException;
