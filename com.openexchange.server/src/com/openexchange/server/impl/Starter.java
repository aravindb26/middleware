...
import static com.openexchange.logging.Constants.DROP_MDC_MARKER;
...
public class Starter implements Initialization {
    ...
    private final Deque<Initialization> started;
    ...
    private static final void dumpServerInfos() throws OXException {
        ...
        try {
            ...
            message.append("VM Num Processors   : {}{}");
            int availableProcessors = Runtime.getRuntime().availableProcessors();
            args.add(I(availableProcessors));
            args.add(sep);
        } catch (Exception gee) {
            LOG.error("", gee);
        }
        ...
        args.add(ServerServiceRegistry.getServize(VersionService.class, true).getVersion().toString());
        args.add(sep);

        message.append("Server Footprint    : {}{}");
        args.add(I(OXException.getServerId()));
        args.add(sep);

        LOG.info(DROP_MDC_MARKER, message.toString(), args.toArray(new Object[args.size()]));
    }
