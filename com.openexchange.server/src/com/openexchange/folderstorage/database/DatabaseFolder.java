...
public class DatabaseFolder extends AbstractFolder {
    ...
    public DatabaseFolder(final FolderObject folderObject, final boolean cacheable) {
        ...
        logIfADefaultInfostoreFolderBecomesCacheable();
    }
    ...
    protected ServerSession getSession() throws OXException {
    ...
    public UsedForSync getUsedForSync() {
        try {
            ServerSession session = getSession();
            if (null != session) {
                UsedForSync defaultUsedForSync = getDefaultUsedForSync(folderObject, session.getContextId(), session.getUserId());
                if (defaultUsedForSync.isProtected()) {
                    return defaultUsedForSync;
                }
                Optional<Boolean> storedValue = ServerServiceRegistry.getInstance().getService(FolderSubscriptionHelper.class, true).isUsedForSync(
                    Optional.empty(), session.getContextId(), session.getUserId(), folderObject.getObjectID(), folderObject.getModule());
                return UsedForSync.of(b(storedValue.orElse(B(defaultUsedForSync.isUsedForSync()))));
            }
        } catch (OXException e) {
            // Ignore
    ...
    public String[] getSubfolderIDs() {
         ...
         */
        return isSubscribed() ? super.getSubfolderIDs() : Strings.getEmptyStrings();
    }

    private void logIfADefaultInfostoreFolderBecomesCacheable() {
        if (LOG.isTraceEnabled() && isCacheable() && null != folderObject && 
            folderObject.isDefaultFolder() && FolderObject.INFOSTORE == folderObject.getModule() && FolderObject.PUBLIC == folderObject.getType()) {
            Exception e = new Exception("Default folder " + folderObject + " is marked as cacheable.");                                               
