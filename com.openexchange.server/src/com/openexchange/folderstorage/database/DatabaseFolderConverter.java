...
import static com.openexchange.folderstorage.database.getfolder.SystemInfostoreFolder.getDefaultInfoStoreFolderId;
import static com.openexchange.folderstorage.database.getfolder.SystemInfostoreFolder.showPersonalBelowInfoStore;
import static com.openexchange.java.Autoboxing.I;
import static com.openexchange.tools.oxfolder.OXFolderIteratorSQL.getVisibleSubfolders;
...
public final class DatabaseFolderConverter {
    ...
    static {
        ...
        m.put(FolderObject.SYSTEM_LDAP_FOLDER_ID, new FolderConverter() {
            ...
            @Override
            public DatabaseFolder convert(final FolderObject fo, final boolean altNames) throws OXException {
                final DatabaseFolder retval = new LocalizedGlobalAddressBookFolder(fo);
     ...
     * @param separateFederatedShares <code>true</code> to organize federated shares separately, <code>false</code> to integrate them into default folders
     * @param con The connection
     * @return The converted {@link DatabaseFolder} instance
     * @throws OXException If conversion fails
     */
    public static DatabaseFolder convert(final FolderObject fo, final User user, final UserPermissionBits userPermissionBits, final Context ctx, final Session session, final boolean altNames, boolean separateFederatedShares, final Connection con) throws OXException {
        try {
            ...
            if (folderId < FolderObject.MIN_FOLDER_ID) { // Possibly a system folder
                ...
                if (null != folderConverter) {
                    final DatabaseFolder databaseFolder = folderConverter.convert(fo, altNames);
                    return handleDatabaseFolder(databaseFolder, folderId, fo, session, user, userPermissionBits, ctx, altNames, separateFederatedShares, con);
                }
            }
            /*
            ...
            if (fo.isDefaultFolder()) {
                        ...
                        switch (fo.getType()) {
                            case FolderObject.PUBLIC:
                                if (preferDisplayName()) {
                                    ...
                                    cacheable = false; // don't cache folders whose name varies based on "altNames" flag
                                    if (altNames && fo.getCreatedBy() == user.getId()) {
                ...
                if (null != localizableName) {
                    retval = new LocalizedDatabaseFolder(fo, cacheable);
                    retval.setName(localizableName);
                } else {
                    retval = new DatabaseFolder(fo, cacheable);
                }
            } else {
            ...
            return handleDatabaseFolder(retval, folderId, fo, session, user, userPermissionBits, ctx, altNames, separateFederatedShares, con);
    ...
    private static DatabaseFolder handleDatabaseFolder(final DatabaseFolder databaseFolder, final int folderId, final FolderObject fo, final Session session, final User user, final UserPermissionBits userPerm, final Context ctx, boolean altNames, boolean separateFederatedShares, final Connection con) throws OXException, SQLException {
        ...
        if (FolderObject.PRIVATE == fo.getType() && userId != databaseFolder.getCreatedBy()) { // Shared
            ...
            if (visibleSubfolders.isEmpty()) {
                databaseFolder.setSubfolderIDs(Strings.getEmptyStrings());
        ...
        } else if (FolderObject.SYSTEM_USER_INFOSTORE_FOLDER_ID == folderId || FolderObject.SYSTEM_PUBLIC_INFOSTORE_FOLDER_ID == folderId) {
            /*
             * gather visible subfolder ids below userstore / public infostore, including federated sharing folders unless mounted separately
             */
            TIntList visibleSubfolders = getVisibleSubfolders(folderId, userId, user.getGroups(), userPerm.getAccessibleModules(), ctx, con);
            List<String> subfolderIds = new ArrayList<String>(visibleSubfolders.size());
            for (int i = 0; i < visibleSubfolders.size(); i++) {
                int subfolderId = visibleSubfolders.get(i);
                if (FolderObject.SYSTEM_USER_INFOSTORE_FOLDER_ID == folderId && showPersonalBelowInfoStore(session, altNames) && 
                    subfolderId == getDefaultInfoStoreFolderId(session, ctx, con)) {
                    continue; // skip, since user's personal default folder is shown below folder 9 
                }
                subfolderIds.add(Integer.toString(subfolderId));
            }
            if (false == separateFederatedShares) {
                for (SortableId federatedSharingId : FederatedSharingFolders.getFolders(folderId, session, false)) {
                    subfolderIds.add(federatedSharingId.getId());
                }
            }
            /*
             * assign subfolders & enforce no caching on higher layers
             */
            databaseFolder.setSubfolderIDs(subfolderIds.toArray(new String[subfolderIds.size()]));
            databaseFolder.setSubscribedSubfolders(0 < subfolderIds.size());
            databaseFolder.setGlobal(false);
            databaseFolder.setCacheable(false);
        } else {
            /*
