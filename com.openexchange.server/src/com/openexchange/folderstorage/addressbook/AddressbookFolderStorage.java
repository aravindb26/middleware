...
/**
 * {@link AddressbookFolderStorage}
 *
 * @author <a href="mailto:ioannis.chouklis@open-xchange.com">Ioannis Chouklis</a>
 * @since v7.10.5
 */
public class AddressbookFolderStorage implements SubfolderListingFolderStorage {

    /** The contacts folder type */
    private static final FolderType FOLDER_TYPE = new AddressbookFolderType();
    ...
    /**
     * Initializes a new {@link AddressbookFolderStorage}.
     *
     * @param accessFactory The underlying ID-based contacts access factory
     */
    public AddressbookFolderStorage(IDBasedContactsAccessFactory accessFactory) {
        super();
    ...
    public ContentType[] getSupportedContentTypes() {
        return new ContentType[] { AddressDataContentType.getInstance() };
    }
    ...
    public ContentType getDefaultContentType() {
        return AddressDataContentType.getInstance();
    }
    ...
    public String getDefaultFolderID(User user, String treeId, ContentType contentType, Type type, StorageParameters storageParameters) throws OXException {
        if (false == (contentType instanceof AddressDataContentType)) {
    ...
    public Type getTypeByParent(User user, String treeId, String parentId, StorageParameters storageParameters) throws OXException {
        return AddressbookType.getInstance();
    }
    ...
    public SortableId[] getUserSharedFolders(String treeId, ContentType contentType, StorageParameters storageParameters) throws OXException {
        throw new UnsupportedOperationException("AddressbookFolderStorage.getUserSharedFolders()");
    }
    ...
    public void rollback(StorageParameters storageParameters) {
        ...
        if (null != contactsAccess) {
            try {
                ...
                // Ignore
                org.slf4j.LoggerFactory.getLogger(AddressbookFolderStorage.class).warn("Unexpected error during rollback: {}", e.getMessage(), e);
            } finally {
                finish(storageParameters);
            }
        }
    ...
    private SortableId[] getSortableIDs(List<? extends ContactsFolder> folders) {
        ...
        for (int i = 0; i < folders.size(); i++) {
            sortableIds.add(new AddressbookId(folders.get(i).getId(), i, null));
        }
    ...
    private void finish(StorageParameters storageParameters) {
        ...
        if (null != contactsAccess) {
            try {
                ...
                // Ignore
                org.slf4j.LoggerFactory.getLogger(AddressbookFolderStorage.class).warn("Unexpected error during finish: {}", e.getMessage(), e);
            } finally {
