...
import com.openexchange.folderstorage.addressbook.AddressbookStorageFolder;
...
public final class ContactsFolderConverter {
    ...
    public static ParameterizedFolder getStorageFolder(String treeId, ContentType contentType, ContactsFolder contactsFolder, String providerId, int accountId, JSONObject userConfig) {
        ParameterizedFolder folder = new AddressbookStorageFolder(treeId, contentType, (l) -> contactsFolder.getLocalizedName(l));
        ...
        folder.setSubfolderIDs(Strings.getEmptyStrings());
        folder.setSubscribedSubfolders(false);
        folder.setSupportedCapabilities(contactsFolder.getSupportedCapabilities());
        ...
        if ((contactsFolder instanceof GroupwareContactsFolder)) {
    ...
    public static DefaultGroupwareContactsFolder getContactsFolder(Folder folder) {
        ...
        contactsFolder.setNameTranslator((l) -> folder.getLocalizedName(l));
        ...
        contactsFolder.setSupportedCapabilities(folder.getSupportedCapabilities());
        ...
        if (folder instanceof SetterAwareFolder) {
            if (((SetterAwareFolder) folder).containsUsedForSync()) {
    ...
    public static GroupwareFolderType getFolderType(Type type) {
        ...
        if (SystemType.getInstance().equals(type)) {
            return GroupwareFolderType.SYSTEM;
        }
    ...
    private static String getQualifiedAccountID(int accountId) {
        return IDMangler.mangle(ContactsAccount.ID_PREFIX, String.valueOf(accountId));
    }
    ...
    private static <T> T optPropertyValue(Folder folder, FolderField field, Class<T> clazz, T defaultValue) {
        if (null == folder || false == (folder instanceof ParameterizedFolder)) {
            return null;
        }
