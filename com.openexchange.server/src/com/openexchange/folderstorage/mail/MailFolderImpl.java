...
public final class MailFolderImpl extends AbstractFolder implements FolderExtension {
    ...
    private static final String CAPABILITY_MAIL_STRUCTURE = "MAIL_STRUCTURE";
    private static final String CAPABILITY_GROUP_ENTITIES = "GROUP_ENTITIES";
    ...
    private static final Set<MailFolderType> STANDARD_FOLDER_TYPES = EnumSet.of(MailFolderType.INBOX, MailFolderType.TRASH, MailFolderType.DRAFTS, MailFolderType.SENT, MailFolderType.SPAM);
     ...
     * @param translatePrimaryAndSecondaryAccountDefaultFolders Whether to translate names of default folders from the primary account
     * @throws OXException If creation fails
     */
    public MailFolderImpl(MailFolder mailFolder, int accountId, MailConfig mailConfig, StorageParameters params, DefaultFolderFullnameProvider fullnameProvider, MailAccess<?, ?> mailAccess, MailAccount mailAccount, boolean translatePrimaryAndSecondaryAccountDefaultFolders) throws OXException {
        this(mailFolder, accountId, mailConfig, params.getUser(), params.getContextId(), fullnameProvider, mailAccess, mailAccount, translatePrimaryAndSecondaryAccountDefaultFolders);
    }
     ...
     * @param mailAccess The associated {@link MailAccess} instance
     * @param mailAccount The mail account
     * @param translatePrimaryAndSecondaryAccountDefaultFolders Whether to translate names of default folders from the primary/secondary account
     * @throws OXException If creation fails
     */
    public MailFolderImpl(MailFolder mailFolder, int accountId, MailConfig mailConfig, User user, int contextId, DefaultFolderFullnameProvider fullnameProvider, MailAccess<?, ?> mailAccess, MailAccount mailAccount, boolean translatePrimaryAndSecondaryAccountDefaultFolders) throws OXException {
        super();
        ...
        if ("INBOX".equals(fullName)) {
            if (mailAccount.isDefaultOrSecondaryAccount() || isUnifiedMail(mailFolder)) {
                localizedNameProvider = new CommonLocalizedNameProvider(MailStrings.INBOX);
            } else {
                localizedNameProvider = new StaticLocalizedNameProvider(MailStrings.INBOX);
            }
        ...
        // Parse permissions (if any)
        {
            ...
            for (int i = mailPermissions.length; i-- > 0;) {
        ...
        if (mailFolder.isRootFolder()) {
            ...
            MailFolderTypeRetval retval = determineMailFolderType(folderName, mailFolder, mailAccess, mailAccount, fullnameProvider, translatePrimaryAndSecondaryAccountDefaultFolders);
        ...
        if (mailCapabilities.hasMailStructure()) {
            addSupportedCapabilities(CAPABILITY_MAIL_STRUCTURE);
        }
        if (mailCapabilities.hasGroupEntities()) {
            addSupportedCapabilities(CAPABILITY_GROUP_ENTITIES);
        }
        ...
        if (mailFolder.liveAccess()) {
            if ((mailFolder.containsShared() && mailFolder.isShared()) || mailFolder.isTrash() || isUnifiedMail(mailFolder)) {
                // A shared mail folder must not be cacheable
                // Same for trash folder and Unified Mail
                cache = false;
            }
        } else {
     ...
     */
    private static String determineParentFrom(int accountId, String fullName, char separator) {
    ...
    private static LocalizedNameProvider getLocalizedNameProviderForPrimaryStandard(String key, boolean translatePrimaryAccountDefaultFolders, String folderName) {
    ...
    private MailFolderTypeRetval determineMailFolderType(String folderName, MailFolder mailFolder, MailAccess<?, ?> mailAccess, MailAccount mailAccount, DefaultFolderFullnameProvider fullnameProvider, boolean translatePrimaryAndSecondaryAccountDefaultFolders) throws OXException {
        MailFolderType mailFolderType = MailFolderType.NONE;
        LocalizedNameProvider localizedNameProvider = null;
        boolean isPrimaryOrSecondaryAccount = mailAccount.isDefaultOrSecondaryAccount();
        if (mailFolder.containsDefaultFolderType()) {
            ...
            case INBOX:
                ...
                break;
            case TRASH:
                if (isPrimaryOrSecondaryAccount) {
                    localizedNameProvider = getLocalizedNameProviderForPrimaryStandard(MailStrings.TRASH, translatePrimaryAndSecondaryAccountDefaultFolders, folderName);
                }
                mailFolderType = MailFolderType.TRASH;
                break;
            case SENT:
                if (isPrimaryOrSecondaryAccount) {
                    localizedNameProvider = getLocalizedNameProviderForPrimaryStandard(MailStrings.SENT, translatePrimaryAndSecondaryAccountDefaultFolders, folderName);
                }
                mailFolderType = MailFolderType.SENT;
                break;
            case SPAM:
                if (isPrimaryOrSecondaryAccount) {
                    localizedNameProvider = getLocalizedNameProviderForPrimaryStandard(MailStrings.SPAM, translatePrimaryAndSecondaryAccountDefaultFolders, folderName);
                }
                mailFolderType = MailFolderType.SPAM;
                break;
            case DRAFTS:
                if (isPrimaryOrSecondaryAccount) {
                    localizedNameProvider = getLocalizedNameProviderForPrimaryStandard(MailStrings.DRAFTS, translatePrimaryAndSecondaryAccountDefaultFolders, folderName);
                }
            ...
            case CONFIRMED_HAM:
                ...
                break;
            default:
                if (isPrimaryOrSecondaryAccount && translatePrimaryAndSecondaryAccountDefaultFolders && MailFolderStorage.isArchiveFolder(fullName, mailAccess, mailAccount)) {
            ...
            if (null == fullnameProvider) {
                ...
                try {
                    if (fullName.equals(fullnameProvider.getDraftsFolder())) {
                        ...
                        if (isPrimaryOrSecondaryAccount && translatePrimaryAndSecondaryAccountDefaultFolders) {
            ...
            mailFolderType = MailFolderType.NONE; // NOSONARLINT
        }
        return new MailFolderTypeRetval(mailFolderType, localizedNameProvider);
    }

    private static boolean isUnifiedMail(final MailFolder mailFolder) {
    ...
    private static final TIntIntHashMap MAPPING = initMapping();

    private static TIntIntHashMap initMapping() {
        TIntIntHashMap map = new TIntIntHashMap(6);
        map.put(Permission.MAX_PERMISSION, MAX_PERMISSION);
        map.put(MAX_PERMISSION, MAX_PERMISSION);
        map.put(0, 0);
        map.put(2, 1);
        map.put(4, 2);
        map.put(8, 4);
    ...
    public String getLocalizedName(final Locale locale) {
        ...
        return null == localizedNameProvider ? null : localizedNameProvider.getLocalizedName(locale);
    }
