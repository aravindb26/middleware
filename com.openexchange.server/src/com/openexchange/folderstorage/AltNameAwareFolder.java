...
public interface AltNameAwareFolder extends Folder {
    ...
    /**
     * Gets the locale-sensitive name if it is available, otherwise the name is returned as fallback.
     * 
     * @param locale The locale
     * @param altName <code>true</code> to prefer alternative App Suite name; otherwise <code>false</code>
     * @return The locale-sensitive name or the name if not available
     */
    default String getName(Locale locale, boolean altName) {
        String localizedName = getLocalizedName(locale, altName);
