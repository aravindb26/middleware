...
public final class CreatePerformer extends AbstractUserizedFolderPerformer {
    ...
    public String doCreate(final Folder toCreate) throws OXException {
        ...
        if (null == parentId) {
            throw FolderExceptionErrorMessage.MISSING_PARENT_ID.create();
        }
        ...
        try {
             ...
             * Skip check if the folder is a virtual,  not-synced folder for an iCAL feed
             */
            if (false == supportsAutoRename && false == isICalFeed(toCreate)) {
                ...
                if (null != existingFolder) {
                    ...
                    throw FolderExceptionErrorMessage.EQUAL_NAME.create(toCreate.getName(), parent.getName(getLocale()), treeId);
                }
            }
        ...
        if (realStorage.equals(virtualStorage)) {
            ...
            if (Tools.supportsContentType(folderContentType, realStorage)) {
                ...
                if (FolderStorage.PRIVATE_ID.equals(parentId) && CONTENT_TYPE_MAIL.equals(folderContentType.toString())) {
                    final String rootId = MailFolderUtility.prepareFullname(Account.DEFAULT_ID, MailFolder.ROOT_FOLDER_ID);
                    /*
