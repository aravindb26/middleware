    ...
    public IValueHandler getSharedValue() {
        return new ReadOnlyValue() {
            ...
            public void getValue(final Session session, final Context ctx, final User user, final UserConfiguration userConfig, final Setting setting) throws OXException {
                try {
                    if (user.isGuest()) {
                        LOG.debug("Infostore default folder not set since user is a guest");
                        return;
                    }
                    ...
                    if (InfostoreFacades.isInfoStoreAvailable()) {
                        ...
                        if (-1 != folderID) {
                            String value = Integer.toString(folderID);
                            LOG.debug("Infostore default folder set to {}", value);
                            setting.setSingleValue(value);
                        } else {
                            LOG.debug("Infostore default folder not set since not determinable");
                        }
                        return;
                    }
                    ...
                    LOG.error("Infostore default folder could not be applied to user configuration.", any);
                    throw any;
                }
            }
        };
