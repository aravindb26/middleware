...
public class SendAddress implements PreferencesItemService {
    ...
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(SendAddress.class);
        ...
        return new IValueHandler() {
            ...
            public void writeValue(final Session session, final Context ctx, final User user, final Setting setting) throws OXException {
                ...
                if (newAlias.startsWith("{")) {
                    try {
                        JSONObject jsonString = (JSONObject) setting.getSingleValue();
                        newAlias = jsonString.getString("data");
                    } catch (Exception e) {
                        LOG.debug("Couldn't extract alias from JSON data", e);
                    }
                }
