        ...
        return new ReadOnlyValue() {
            ...
            public void getValue(final Session session, final Context ctx, final User user, final UserConfiguration userConfig, final Setting setting) throws OXException {
                try {
                    ...
                    if (primaryOnly) {
                        final Character sep = getSeparator(Account.DEFAULT_ID, session);
                        if (null != sep) {
                            retval.put(Integer.toString(Account.DEFAULT_ID), sep.toString());
                        }
                    } else {
                        ...
                        if (null == mass) {
                            ...
                            if (userConfig.isMultipleMailAccounts()) {
                                accounts = Arrays.asList(mass.getUserMailAccounts(user.getId(), ctx.getContextId()));
                            } else {
                                accounts = Arrays.asList(mass.getUserDefaultAndSecondaryMailAccounts(user.getId(), ctx.getContextId()));
                            }
                            for (final MailAccount mailAccount : accounts) {
                                if (!PROTOCOL_UNIFIED_INBOX.equals(mailAccount.getMailProtocol()) && !mailAccount.isDeactivated()) {
