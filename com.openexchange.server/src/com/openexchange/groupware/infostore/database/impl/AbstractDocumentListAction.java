...
public abstract class AbstractDocumentListAction extends AbstractInfostoreAction {
    ...
    /**
     * Groups given listing of documents by folders and yields a map of folder identifier to document identifiers.
     *
     * @param documents The documents to group
     * @return The grouped documents
     */
    public TLongObjectMap<TIntList> groupByFolder(List<DocumentMetadata> documents) {
        if (documents.isEmpty()) {
            return new TLongObjectHashMap<>(0);
        }

        TLongObjectMap<TIntList> folder2docs = new TLongObjectHashMap<>();
        for (DocumentMetadata document : documents) {
            TIntList docIds = folder2docs.get(document.getFolderId());
            if (docIds == null) {
                docIds = new TIntArrayList();
                folder2docs.put(document.getFolderId(), docIds);
            }
