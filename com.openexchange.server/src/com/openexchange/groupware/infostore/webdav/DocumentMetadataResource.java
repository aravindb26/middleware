...
public class DocumentMetadataResource extends AbstractResource implements OXWebdavResource, OXExceptionConstants {
    ...
    @Override
    public InputStream getBody(long offset, long length) throws WebdavProtocolException {
        ServerSession session = getSession();
        try {
            return database.getDocument(id, InfostoreFacade.CURRENT_VERSION, offset, length, session);
        } catch (WebdavProtocolException e) {
            throw e;
        } catch (Exception e) {
            throw WebdavProtocolException.Code.GENERAL_ERROR.create(getUrl(), HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e);
