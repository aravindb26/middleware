...
public class TasksDelete implements DeleteListener {
    ...
    private void deleteUser(final DeleteEvent event, final Connection con) throws OXException {
        ...
        if (destUser == null) {
            destUser = I(event.getContext().getMailadmin());
        }
        if (destUser.intValue() == 0) {
            // remove all tasks in folders owned by this user
            removeTaskAndFolder(event, con);
        } else {
            assignToUser(event, con, i(destUser));
        }
        // Change createdFrom and modifiedBy attributes of left over tasks.
        changeCFMB(event, con, i(destUser));
    }

    /**
     * Remove all tasks and their folder mappings within folders owned by the deleted user
     *
     * @param event The delete event containing the user deletion
     * @param con The connection to sue
     * @throws OXException
     */
    private void removeTaskAndFolder(DeleteEvent event, Connection con) throws OXException {
        Context ctx = event.getContext();
        int userId = event.getId();
        Map<Integer, List<Integer>> folder2tasks;
        try {
            folder2tasks = foldStor.listTasksInFoldersOwnedBy(ctx, con, userId);
            for (Entry<Integer, List<Integer>> entry : folder2tasks.entrySet()) {
                int folderId = i(entry.getKey());
                List<Integer> tasks = entry.getValue();
                for (int taskId : tasks) {
                    TaskLogic.removeTask(event.getSession(), ctx, con, folderId, taskId, StorageType.ACTIVE, false);
                }
            }
        } catch (OXException e) {
            throw DeleteFailedExceptionCodes.SQL_ERROR.create(e, e.getMessage());
     ...
     * @param event Event.
     * @param con writable database connection.
     * @param destUser The user to assign public task folders to
     * @throws OXException if a problem occurs.
     */
    private void assignToUser(final DeleteEvent event, final Connection con, final int destUser) throws OXException {
        ...
        for (final StorageType type : StorageType.TYPES_AD) {
            ...
            for  (final int taskId : tasks) {
                ...
                if (update.getCreatedBy() == userId) {
                    update.setCreatedBy(tmpUser);
                }
                if (update.getModifiedBy() == userId) {
                    update.setModifiedBy(tmpUser);
                }
