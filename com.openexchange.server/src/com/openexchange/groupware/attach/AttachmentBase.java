...
public interface AttachmentBase extends TransactionAware {
    ...
    /** The value used for the <code>file_id</code> column when there's no file associated with the attachment metadata */
    static final String NO_FILE_ID = "";

    /**
     * Stores a new attachment, associated with the groupware object referenced by the metadata.
     * 
     * @param attachment The attachment metadata to store
     * @param data The attachment's binary data, or <code>null</code> if an external attachment is referenced via {@link AttachmentMetadata#getUri()}
     ...
     * @return The timestamp of modified objects to which this attachment was attached
     */
    default long attachToObject(ServerSession session, AttachmentMetadata attachment, InputStream data) throws OXException {
        return attachToObject(attachment, data, session, session.getContext(), session.getUser(), session.getUserConfiguration());
    }
