...
public class ZipDocumentsAction extends AbstractAttachmentAction {
    ...
    private List<AttachmentInfo> parseAttachmentInfos(AJAXRequestData requestData) throws OXException {
        try {
            ...
            if (null == jBody) {
                ...
                jBody = JSONServices.parse(value);
            }
    ...
    public boolean accessAllowed(AJAXRequestData request, ServerSession session, Scope scope) throws OXException {
        return RestrictedActionUtil.mayReadWithScope(collectContentTypes(request), scope);
    }

    /**
     * Collects all content types from the request
     *
     * @return A Set with all content types from the request
     * @throws OXException if the parameter 'id' is missing
     */
    private Set<ContentType> collectContentTypes(AJAXRequestData request) throws OXException {
        List<AttachmentInfo> attachmentInfos = parseAttachmentInfos(request);
        if (attachmentInfos.isEmpty()) {
            throw AjaxExceptionCodes.MISSING_PARAMETER.create(AJAXServlet.PARAMETER_ID);
        }
        short items = 0;
        ...
        for (AttachmentInfo attachmentInfo : attachmentInfos) {
            if (false == contentTypes.add(getContentType(attachmentInfo.moduleId))) {
                continue;
            }
            items++;
    ...
    private static class AttachmentZipEntryAdder implements ZipEntryAdder {
        ...
        /**
         * Initializes a new {@link AttachmentZipEntryAdder}.
         */
