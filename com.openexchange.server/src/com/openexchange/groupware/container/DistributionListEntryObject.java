...
public class DistributionListEntryObject implements Serializable {
    ...
    /**
     * Initializes a new {@link DistributionListEntryObject}, taking over the properties from another one.
     * 
     * @param member The distribution list member to take over the property values from
     */
    public DistributionListEntryObject(DistributionListEntryObject member) {
        super();
        if (member.containsContactUid()) {
            setContactUid(member.getContactUid());
        }
        ...
        if (member.containsEmailaddress()) {
            try {
                setEmailaddress(member.getEmailaddress(), false);
            } catch (OXException e) {
                // doesn't happen
        ...
        if (member.containsEmailfield()) {
            setEmailfield(member.getEmailfield());
        }
        if (member.containsEntryID()) {
            setEntryID(member.getEntryID());
        }
        ...
        if (member.containsSortName()) {
            setSortName(member.getSortName());
        }
    }
    ...
    // SET METHODS
    public void setEntryID(final String entry_id) {
    ...
    public void setEmailaddress(String emailaddress, boolean verifyAddress) throws OXException {
        if (verifyAddress) {
             ...
             */
            try {
                QuotedInternetAddress.validate(emailaddress);
    ...
    public boolean searchDlistObject(final DistributionListEntryObject dleo) {
        if (dleo.getEntryID() != null) { // this one is not an independent
            if ((getEntryID().equals(dleo.getEntryID())) && (getEmailfield() == dleo.getEmailfield())) {
                return true;
            }
        }
        return false;
