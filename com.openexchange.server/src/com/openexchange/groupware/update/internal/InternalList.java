...
public final class InternalList {
    ...
    private static UpdateTaskV2[] genTaskList() {
        ...
        // Add 'checksum' column to attachment tables; see MW-1235
        ...
        // +++++++++++++++++++++++++++++++++ Version 7.10.6 starts here. +++++++++++++++++++++++++++++++++

        // Sets row type to "dynamic" for "user_mail_account" and "user_transport_account" tables
        list.add(new com.openexchange.groupware.update.tasks.MailAccountTableDynamicRowType());

        // Adds "secondary" column to the "user_mail_account" and "user_transport_account" tables
        list.add(new com.openexchange.groupware.update.tasks.AddSecondaryColumnToMailAccountTableTask());

        // Adds "deactivated" column to the "user_mail_account" and "user_transport_account" tablesailAccountTableTask());
        list.add(new com.openexchange.groupware.update.tasks.AddDeactivatedColumnToMailAccountTableTask());

        // Adds "lastModified" column to "object_use_count" table
        list.add(new com.openexchange.groupware.update.tasks.AddTimestampColumnToUsecountTable());

        // Create "generic_use_count" table
        list.add(new com.openexchange.groupware.update.tasks.objectusagecount.CreateGenericUseCountTableTask());

        // Change default values of unix user columns
        list.add(new com.openexchange.groupware.update.tasks.AddDefaultsForUnixColumnsUpdateTask());
        ...
        // Shrinks length of "folder" and "object" VARCHAR columns from 255 to 191 to be used in UNIQUE KEY
        list.add(new com.openexchange.groupware.update.tasks.objectusagecount.EnsureExistenceOfGenericUseCountTask());
        list.add(new com.openexchange.groupware.update.tasks.objectusagecount.GenericUseCountShrinkFolderAndObjectTask());

        // Extends the UNIQUE KEY of the "generic_use_count" table by "account" column
        list.add(new com.openexchange.groupware.update.tasks.objectusagecount.ExtendGenericUseCountUniqueKeyByAccountTask());

        // Drop unix user related columns
        list.add(new com.openexchange.groupware.update.tasks.DropUnixUserRelatedColumnsUpdateTask());
        list.add(new com.openexchange.groupware.update.tasks.DropUnixUserRelatedColumnsUpdateTaskV2()); // <--- For MWB-2081

        // Aligns certain user fields spread over tables to have at least a common max. size of 191 characters
        list.add(new com.openexchange.groupware.update.tasks.ExtendUserFieldsTask());
        
        // Drops the ldapIds table
        list.add(new com.openexchange.groupware.update.tasks.DropLdapIdsTableTask());

        // (Once again) Extends the "password" column for "user_mail_account" and "user_transport_account" tables
        list.add(new com.openexchange.groupware.update.tasks.MailAccountExtendPasswordTaskV2());

        // Add 'uri' column to prg_attachment table
        list.add(new com.openexchange.groupware.update.tasks.AttachmentAddUriColumnTask());

        // Drops rather needless foreign keys from several settings tables
        list.add(new com.openexchange.groupware.update.tasks.DropForeignKeysFromSettingsTables());

        // Re-crypt anonymous share passwords using new introduced crypto mechanisms
        list.add(new com.openexchange.groupware.update.tasks.RecryptGuestUserPasswords());

        // Extend 'userPassword' column in del_user table
        list.add(new com.openexchange.groupware.update.tasks.ExtendDelUserPasswordColumnTask());

        // Updates the references to users with individual filestores in table filestore2user (config-db)
        list.add(new com.openexchange.groupware.update.tasks.Filestore2UserUpdateReferencesTask());

        // Extends the "login" column for "user_mail_account" and "user_transport_account" tables
        list.add(new com.openexchange.groupware.update.tasks.MailAccountExtendLoginTask());
