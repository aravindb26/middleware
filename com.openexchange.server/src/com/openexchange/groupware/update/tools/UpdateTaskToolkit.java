        ...
        Callable<Void> task = () -> {
            ...
            for (SchemaInfo schema : schemas) {
                ...
                if (throwExceptionOnFailure) {
                    try {
                        ...
                        LoggerHolder.LOG.error("", e1);
                        statusText.set(e1.getPlainLogMessage());
                        throw e1;
                    } catch (Exception e2) {
                        LoggerHolder.LOG.error("", e2);
                        statusText.set(e2.getMessage());
                        throw e2;
                    }
                } else {
                    updateProcess.run();
                    ...
                    if (null != failures && !failures.isEmpty()) {
                        for (TaskInfo taskInfo1 : failures) {
                            Queue<TaskInfo> schemaFailures = totalFailures.get(taskInfo1.getSchema());
                            if (null == schemaFailures) {
                                schemaFailures = new LinkedList<TaskInfo>();
                                totalFailures.put(taskInfo1.getSchema(), schemaFailures);
                            }
                            schemaFailures.offer(taskInfo1);
                        }
                    }
                }
            ...
            if (!totalFailures.isEmpty()) {
                ...
                for (Map.Entry<String, Queue<TaskInfo>> failureEntry : totalFailures.entrySet()) {
                    ...
                    for (TaskInfo taskInfo2 : failureEntry.getValue()) {
                        if (firstTaskInfo) {
                            firstTaskInfo = false;
                        } else {
                            sb.append("\\R");
                        }
                        sb.append(' ').append(taskInfo2.getTaskName()).append(" (schema=").append(taskInfo2.getSchema()).append(')');
                    }
                }
            }
