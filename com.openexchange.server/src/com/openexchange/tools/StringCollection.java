...
public final class StringCollection {
    ...
    public static String prepareForSearch(final String s, final boolean prependWildcard, final boolean appendWildcard, final boolean preparedStatement) {
        ...
        String value = processWildcards(s.trim(), preparedStatement ? null : "'\\" /*Escape every backslash and single-quote character*/);
        if (prependWildcard) {
            final int length = value.length();
            if (length > 0) {
                if (value.charAt(0) != '%') {
    ...
    private static String processWildcards(String s, String charactersToEscape) {
        if (s == null) {
            return s;
        }
        ...
        for (int i = 0; i < length; i++) {
            char ch = s.charAt(i);
            if (('\\' == ch) && (i < length - 1) && WILDCARDS.indexOf(s.charAt(i + 1)) >= 0) {
                // Escaped special character     
                sb = initIfNeeded(sb, i, s);
                sb.append(s.charAt(i + 1));
                i++;
            } else if (charactersToEscape != null && charactersToEscape.indexOf(ch) >= 0) {
                // Escape character
                sb = initIfNeeded(sb, i, s);
                sb.append('\\').append(ch);
            } else if ('%' == ch || '_' == ch) {
    ...
    private static StringBuilder initIfNeeded(StringBuilder sb, int index, String source) {
