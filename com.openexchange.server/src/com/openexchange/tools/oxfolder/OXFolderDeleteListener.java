    ...
    private void handleContextDeletion(Connection writeCon, Context ctx) throws OXException {
        ...
        try {
            ...
            if (cacheService != null) {
                Cache folderCache = cacheService.getCache("OXFolderCache");
                folderCache.invalidateGroup(Integer.toString(ctx.getContextId()));

                Cache globalCache = cacheService.getCache("GlobalFolderCache");
                globalCache.invalidateGroup(Integer.toString(ctx.getContextId()));

                FolderMapManagement.getInstance().dropFor(ctx.getContextId());
            }
