...
public final class OXFolderBatchLoader {
    ...
    public static final TIntObjectMap<List<OCLPermission>> getFolderPermissions(final int[] folderIds, final Context ctx, final Connection readConArg, final String table) throws SQLException, OXException {
        ...
        try {
            TIntObjectMap<List<OCLPermission>> ret = new TIntObjectHashMap<List<OCLPermission>>(folderIds.length);
            for (int i = 0; i < folderIds.length; i+= IN_LIMIT) {
                int offset = 0;
                int rowCount = PAGINATION_LIMIT;
                int[] currentIds = com.openexchange.tools.arrays.Arrays.extract(folderIds, i, IN_LIMIT);
                String sql = getIN("SELECT permission_id,fp,orp,owp,odp,admin_flag,group_flag,`system`,fuid,type,sharedParentFolder FROM #TABLE# WHERE cid=? AND fuid IN (", currentIds.length);
                sql = sql + " ORDER BY fuid, permission_id LIMIT ?,?";
