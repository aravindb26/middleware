...
public final class OXFolderUtility {
    ...
    /**
     * Checks if a deputy permission would be changed or removed when updating permissions.
     *
     * @param fo The folder providing new permissions to apply
     * @param storageObj The storage version providing existent permissions
     * @param context The context
     * @throws OXException If a deputy permission would be changed or deleted when applying permissions
     */
    public static void checkForDeputyPermissionChange(FolderObject fo, FolderObject storageObj, Context context) throws OXException {
        if ("true".equals(LogProperties.get(LogProperties.Name.DEPUTY))) {
            // Allowed...
            return;
        }

        Map<String, Object> meta = storageObj.getMeta();
        if (meta == null) {
        ...
        if (deputyService == null) {
            // No deputy service available
            return;
        }

        // Extract deputy entity identifiers from meta information
        TIntSet deputyEntityIds = null;
        for (Map.Entry<String, Object> metaEntry : meta.entrySet()) {
            ...
            if ((value instanceof Map)) {
                ...
                if ("deputy".equals(deputyMetadata.get("type"))) {
                    String deputyId = metaEntry.getKey();
                    if (deputyService.existsDeputyPermission(deputyId, context.getContextId())) {
                        int entityId = getInt("entity", -1, deputyMetadata);
                        if (entityId >= 0) {
                            if (deputyEntityIds == null) {
                                deputyEntityIds = new TIntHashSet();
                            }
                            deputyEntityIds.add(entityId);
                        }
                    }
                }
            }
        ...
        {
            TIntObjectMap<OCLPermission> modifiedEntityIdMap = getEntityIdMapFor(originalPermissions);
            TIntObjectMap<OCLPermission> updatedEntityIdMap = getEntityIdMapFor(updatedPermissions);
            modifiedEntityIdMap.retainEntries(new TIntObjectProcedure<OCLPermission>() {

                @Override
                public boolean execute(int entityId, OCLPermission permission) {
                    OCLPermission updatedPermission = updatedEntityIdMap.get(entityId);
                    return (updatedPermission != null && !updatedPermission.equalsPermission(permission));
                }
            });
            if (!modifiedEntityIdMap.isEmpty()) {
                for (int modifiedEntityId : modifiedEntityIdMap.keys()) {
                    if (deputyEntityIds.contains(modifiedEntityId)) {
                        throw OXFolderExceptionCode.NO_DEPUTY_PERMISSION_CHANGE.create(I(modifiedEntityId), I(fo.getObjectID()), I(context.getContextId()));
                    }
                }
            }
        }
        ...
        {
            TIntObjectMap<OCLPermission> removedEntityIdMap = getEntityIdMapFor(originalPermissions);
            TIntObjectMap<OCLPermission> updatedEntityIdMap = getEntityIdMapFor(updatedPermissions);
            removedEntityIdMap.retainEntries(new TIntObjectProcedure<OCLPermission>() {

                @Override
                public boolean execute(int entityId, OCLPermission permission) {
                    return (updatedEntityIdMap.containsKey(entityId) == false);
                }
            });
            if (!removedEntityIdMap.isEmpty()) {
                for (int removedEntityId : removedEntityIdMap.keys()) {
                    if (deputyEntityIds.contains(removedEntityId)) {
                        throw OXFolderExceptionCode.NO_DEPUTY_PERMISSION_DELETE.create(I(removedEntityId), I(fo.getObjectID()), I(context.getContextId()));
                    }
                }
            }
        }
    ...
    private static TIntObjectMap<OCLPermission> getEntityIdMapFor(OCLPermission[] permissions) {
        if (permissions == null || permissions.length <= 0) {
            return new TIntObjectHashMap<OCLPermission>(0);
        }

        TIntObjectMap<OCLPermission> entityIds = new TIntObjectHashMap<>(permissions.length);
        for (OCLPermission permission : permissions) {
            entityIds.put(permission.getEntity(), permission);
        }
        return entityIds;
    }
     ...
     * @param ctx The context
     * @param warnings
     * @return <code>true</code> if the folder's permissions were changed along with the check, <code>false</code> otherwise
     * @throws OXException If specified folder's permissions do not pass the checks
     */
    public static boolean checkFolderPermissions(final FolderObject folderObj, final int userId, final Context ctx, final List<OXException> warnings) throws OXException {
        ...
        for (int i = 0; i < permissionsSize; i++) {
            ...
            if (oclPerm.isFolderAdmin()) {
                ...
                if (false == hasInternalAdmin && oclPerm.isFolderAdmin() && false == isGuestEntity(ctx, oclPerm)) {
        ...
        if (!isPrivate && !adminEntities.contains(creator)) {
            ...
            for (int i = 0; !found && i < permissionsSize; i++) {
                ...
                if (oclPerm.getEntity() == creator) {
                    ...
                    adminEntities.add(oclPerm.getEntity());
                    found = true;
                    if (false == hasInternalAdmin && false == isGuestEntity(ctx, oclPerm)) {
                        hasInternalAdmin = true;
                    }
                }
            }
        ...
        if (adminEntities.isEmpty() || false == hasInternalAdmin) {
    ...
    /**
     * Gets a value indicating whether a certain permission entity refers to a guest account or not.
     *
     * @param ctx The context to check the permission entity in
     * @param oclPerm The permission to check
     * @return <code>true</code> if the permission refers to a guest entity, <code>false</code>, otherwise
     */
    private static boolean isGuestEntity(Context ctx, OCLPermission oclPerm) throws OXException {
        if (oclPerm.isGroupPermission()) {
            return GroupStorage.GUEST_GROUP_IDENTIFIER == oclPerm.getEntity();
        }
        return ServerServiceRegistry.getServize(UserService.class).getUser(oclPerm.getEntity(), ctx).isGuest();
    }
    ...
    /**
     * Gets the permissions without folder access or admin access by comparing specified storage-version permissions with update-operation permissions
     *
     * @param newPerms The update-operation permissions
    ...
    public static OCLPermission[] getPermissionsWithoutFolderAccess(final OCLPermission[] newPerms, final OCLPermission[] storagePerms) {
        ...
        for (final OCLPermission storagePerm : storagePerms) {
            ...
            for (int i = 0; i < newPerms.length && !found; i++) {
                if (newPerms[i].getEntity() == storagePerm.getEntity()) {
                     ...
                     */
                    found = (newPerms[i].getFolderPermission() > OCLPermission.NO_PERMISSIONS || newPerms[i].isFolderAdmin());
                }
            }
            if (!found) {
        ...
        for (final OCLPermission newPerm : newPerms) {
            ...
            if (!found && !newPerm.isFolderAdmin() && newPerm.getFolderPermission() <= OCLPermission.NO_PERMISSIONS && !entities.contains(newPerm.getEntity())) {
                /*
    ...
    public static Map<String, Object> deserializeMeta(InputStream inputStream) throws JSONException {
        return JSONServices.parseObject(new AsciiReader(inputStream)).asMap();
    }
