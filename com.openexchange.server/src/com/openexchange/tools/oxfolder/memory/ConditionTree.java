...
public final class ConditionTree {
    ...
    public void insert(Permission p) {
        if (p != null) {
            insert(Collections.singletonList(p));
        }
    }

    /**
     * Inserts specified permissions.
     *
     * @param perms The permissions associated with certain entities
     */
    public void insert(List<Permission> perms) {
        for (Permission p : perms) {
            if (p != null) {
                 ...
                 */
                if (CONDITION_FOLDER_VISIBLE.fulfilled(p)) { // NOSONARLINT
