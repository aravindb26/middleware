 ...
 */
public class WebDAVSessionStore implements WebDAVSessionService {
    ...
    public Session getSession(final LoginRequest loginRequest, HttpServletRequest httpRequest) throws OXException {
        ...
        if (null == sessionAttribute || false == (sessionAttribute instanceof Session)) {
            /*
    ...
    @Override
    public Optional<Session> optSession(LoginRequest request) {
        String key = getKey(request);
        return Optional.ofNullable(sessionIdsPerClient.getIfPresent(key))
                       .map(id -> optSession(id));
    }
    ...
    private static String getKey(LoginRequest loginRequest) {
        int clientHash = (loginRequest.getUserAgent() + loginRequest.getClientIP()).hashCode();
        return Base64.encode(new StringBuilder().append(loginRequest.getLogin())
