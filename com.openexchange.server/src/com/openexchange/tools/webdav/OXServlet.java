...
public abstract class OXServlet extends WebDavServlet {
     ...
     */
    private static final String BASIC_REALM = "OX WebDAV";
    ...
    private static OAuthResourceService getOAuthResourceService() {
    ...
    protected boolean authenticate(final HttpServletRequest req, final HttpServletResponse resp) throws IOException {
        ...
        if (hasMultifactor(session, req, resp)) {  // Check if session marked for multifactor.  Not supported, needs application password
            return false;
        }
    ...
    protected LoginRequest parseLoginRequest(Credentials credentials, HttpServletRequest request) {
        ...
        String clientId = Client.WEBDAV_INFOSTORE.getClientId();
        return new LoginRequestImpl(request, credentials.getLogin(), credentials.getPassword(), getInterface(), clientId, null, userAgent);
    }
    ...
    private Session doOAuth(AuthorizationHeader authHeader, HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
        ...
        } catch (OAuthInvalidTokenException ex) {
    ...
    /**
     * Check if the session has multifactor authentication requirements.
     * Return if true
     *
     * @param session
     * @return <code>true</code>, if multifactor required
     * @throws IOException
     */
    private static boolean hasMultifactor(Session session, final HttpServletRequest req, final HttpServletResponse resp) throws IOException {
        if (Boolean.TRUE.equals(session.getParameter("multifactor"))) {
            removeSession(session.getSessionID());
            removeCookie(req, resp, COOKIE_SESSIONID);
            resp.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Multifactor Required");
            return true;
        }
        return false;
    }
    ...
    protected void addUnauthorizedHeader(final HttpServletResponse resp) {
        if (useHttpAuth()) {
            resp.addHeader("WWW-Authenticate", "Basic realm=\"" + BASIC_REALM + "\", encoding=\"UTF-8\"");
            if (allowOAuthAccess() && getOAuthResourceService() != null) {
    ...
    protected static boolean isTransient(Interface iface) {
        return switch (iface) {
            case CALDAV, CARDDAV, WEBDAV_INFOSTORE, WEBDAV_ICAL, WEBDAV_VCARD, OUTLOOK_UPDATER, DRIVE_UPDATER -> true;
            default -> false;
        };
    }
