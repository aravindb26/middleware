...
public class EventClient {
    ...
    public void create(final FolderObject folder, final FolderObject parentFolder, String[] folderPath) throws OXException {
        ...
        if (null != folder && FolderObject.INFOSTORE == folder.getModule()) {
            Dictionary<String, Object> properties = getEventProperties(folder, parentFolder, folderPath);
            properties.put(CommonEvent.EVENT_KEY, genericEvent);
            triggerEvent(new Event(FileStorageEventConstants.CREATE_FOLDER_TOPIC, properties));
        }
    ...
    public void modify(final FolderObject oldFolder, final FolderObject newFolder, final FolderObject parentFolder, String[] folderPath) throws OXException {
        ...
        if (FolderObject.INFOSTORE == newFolder.getModule()) {
            ...
            if (null != oldFolder.getPermissions()) {
                properties.put(FileStorageEventConstants.OLD_FOLDER_PERMISSIONS, toFileStoragePermissions(oldFolder.getPermissions()));
            }
     ...
     *
     * @param folder The folder
     * @param parentFolder The parent folder, or <code>null</code> if not applicable
     * @param folderPath The full path of the folder down to the root folder, or <code>null</code> if not applicable
     * @throws OXException
     */
    public void delete(final FolderObject folder, final FolderObject parentFolder, String[] folderPath) throws OXException {
        ...
        if (null != parentFolder && FolderObject.INFOSTORE == parentFolder.getModule()) {
            ...
            if (null != folder.getPermissions()) {
                properties.put(FileStorageEventConstants.OLD_FOLDER_PERMISSIONS, toFileStoragePermissions(folder.getPermissions()));
            }
            triggerEvent(new Event(FileStorageEventConstants.DELETE_FOLDER_TOPIC, properties));
        }
    ...
    private static FileStoragePermission toFileStoragePermission(OCLPermission permission) {
        DefaultFileStoragePermission fsPermission = DefaultFileStoragePermission.newInstance();
        fsPermission.setEntity(permission.getEntity());
        fsPermission.setGroup(permission.isGroupPermission());
        fsPermission.setSystem(permission.getSystem());
        fsPermission.setAdmin(permission.isFolderAdmin());
        fsPermission.setAllPermissions(permission.getFolderPermission(), permission.getReadPermission(), permission.getWritePermission(), permission.getDeletePermission());
        return fsPermission;
    }

    private static FileStoragePermission[] toFileStoragePermissions(List<OCLPermission> permissions) {
        if (permissions == null) {
            return null;
        }
        FileStoragePermission[] fsPermissions = new FileStoragePermission[permissions.size()];
        for (int i = 0; i < permissions.size(); i++) {
            fsPermissions[i] = toFileStoragePermission(permissions.get(i));
        }
