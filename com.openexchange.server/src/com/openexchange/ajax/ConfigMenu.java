...
public class ConfigMenu extends SessionServlet {
        ...
        if (setting.isLeaf()) {
            ...
            if (null == multiValue) {
                ...
                if (null == singleValue) {
                    ...
                    try {
                        retval = JSONServices.parseObject(singleValue.toString());
                    } catch (JSONException e) {
                        retval = singleValue;
                    }
                }
     ...
     */
    private static void saveSettingWithSubs(final SettingStorage storage, final Setting setting) throws OXException, JSONException {
        if (setting.isLeaf()) {
            final String value = (String) setting.getSingleValue();
            if (null != value && value.length() > 0) {
                if ('[' == value.charAt(0)) {
                    final JSONArray array = JSONServices.parseArray(value);
                    if (array.length() == 0) {
            ...
            final JSONObject json;
            {
                ...
                if (singleValue instanceof JSONObject) {
                    ...
                    json = JSONServices.parseObject(singleValue.toString());
                }
            }
