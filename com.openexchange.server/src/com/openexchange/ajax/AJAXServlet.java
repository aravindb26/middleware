...
public abstract class AJAXServlet extends HttpServlet implements UploadRegistry {
    ...
    protected void doService(HttpServletRequest req, HttpServletResponse resp, boolean checkRateLimit) throws ServletException, IOException {
        ...
        try {
            ...
            OXException oxe = e instanceof OXRuntimeException ? ((OXRuntimeException) e).getException() : new OXException(e);
            LOG.error("", oxe);
    ...
    /**
     * Gets the charset for HTTP Servlet request's input stream.
     *
    ...
    public static Reader getReaderFor(HttpServletRequest req) throws IOException {
        return getReaderFor(req, getCharsetFor(req));
    }
     ...
     * @throws IOException If an I/O error occurs
     */
    public static Reader getReaderFor(HttpServletRequest req, Charset charset) throws IOException {
        return new BufferedReader(new InputStreamReader(req.getInputStream(), charset), BUF_SIZE);
    }
    ...
    public static JSONValue getBodyAsJsonValue(HttpServletRequest req) throws IOException, JSONException {
        Charset charset = getCharsetFor(req);
        if ("UTF-8".equals(charset.name())) {
            InputStream in = null;
            try {
                in = req.getInputStream();
                return JSONServices.parse(in);
            } finally {
                Streams.close(in);
            }
        }
        ...
        Reader reader = null;
        try {
            reader = new BufferedReader(new InputStreamReader(req.getInputStream(), charset), BUF_SIZE);
     ...
     * @throws IOException If an I/O error occurs
     */
    public static String readFrom(InputStream in, Charset charset) throws IOException {
        if (null == in) {
            return null;
        }
        ...
        Reader reader = null;
        try {
            reader = new BufferedReader(new InputStreamReader(in, charset), BUF_SIZE);
     ...
     */
    protected static boolean isIE(HttpServletRequest req) {
     ...
     */
    protected static boolean isIE7(HttpServletRequest req) {
        String userAgent = req.getHeader("User-Agent");
        return userAgent != null && userAgent.indexOf("MSIE 7") >= 0;
    }
