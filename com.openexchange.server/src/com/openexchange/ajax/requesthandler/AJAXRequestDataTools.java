...
public class AJAXRequestDataTools {
    ...
    public String getModule(final String prefix, final HttpServletRequest req) {
        ...
        module = getModuleFromPath(prefix, pathInfo);
        req.setAttribute("__module", module);
        return module;
    }

    /**
     * Gets the module from the given path
     *
     * @param prefix The prefix
     * @param path The path to get the module from
     * @return The module
     */
    public static String getModuleFromPath(String prefix, String path) {
        String pathInfo = path;
        // Decode x-www-form-urlencoded if necessary
        ...
        // Remove prefix
        String result = null != prefix ? pathInfo.substring(prefix.length()) : pathInfo;
        final int mlen = result.length() - 1;
        if (mlen >= 0 && '/' == result.charAt(mlen)) {
            result = result.substring(0, mlen);
        }
        return result;
    }
