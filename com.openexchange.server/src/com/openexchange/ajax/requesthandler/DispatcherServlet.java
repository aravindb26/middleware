...
public class DispatcherServlet extends SessionServlet {
     ...
     */
    private static final Set<OXExceptionCode> IGNOREES = Set.<OXExceptionCode> of(
     ...
     */
    private static final Set<Category> CAT_IGNOREES = Set.of(Category.CATEGORY_PERMISSION_DENIED, Category.CATEGORY_USER_INPUT);
    ...
    public void handle(HttpServletRequest httpRequest, HttpServletResponse httpResponse, boolean preferStream) throws IOException {
        ...
        */
        ...
        HttpServletResponse httpResp = (httpResponse instanceof StatusKnowing) ? httpResponse : new StatusKnowingHttpServletResponse(httpResponse);
        ...
        try {
            ...
            if (renderResponse(requestData, result, httpRequest, httpResp)) {
                 ...
                 * ... and send response
                 */
                if (ResultType.ENQUEUED == result.getType()) {
                    enqueued = true;
                }
        ...
        } catch (OXRuntimeException oxrte) {
            OXException e = oxrte.getException();
            exc = e;
            Locale locale = getLocaleFrom(session, null);
            if (null != locale) {
                e.setProperty(OXExceptionConstants.PROPERTY_LOCALE, locale.toString());
    ...
    @Override
    protected void handleOXException(OXException oxe, HttpServletRequest req, HttpServletResponse resp) throws IOException {
        OXException e = oxe;
        ...
        if (AjaxExceptionCodes.UNEXPECTED_ERROR.equals(e)) {
            ...
            if (ExceptionUtils.extractFrom(e, ThreadDeath.class) != null) {
                // Thread got killed
                e = AjaxExceptionCodes.REQUEST_ABORTED.create(e);
            }
    ...
    protected void handleError(AJAXRequestResult result, HttpServletResponse httpServletResponse) throws IOException {
        ...
        if (httpStatusCode == HttpServletResponse.SC_ACCEPTED) {
    ...
    private static ServerSession fakeSession() {
