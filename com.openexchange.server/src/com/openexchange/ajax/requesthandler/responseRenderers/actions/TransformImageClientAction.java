...
public class TransformImageClientAction extends TransformImageAction {
    ...
    protected String getCacheKey(@NonNull final ServerSession session, @NonNull final AJAXRequestData request, @NonNull final AJAXRequestResult result, @NonNull final IFileHolder repetitiveFile, @NonNull final TransformImageParameters xformParams) throws OXException {
        if (null != getImageClientIfValid()) {
            final InputStream inputStm = repetitiveFile.getStream();
            if (null != inputStm) {
                try {
                    ...
                    try (final CheckedInputStream checkedInputStm = new CheckedInputStream(inputStm, imageChecksum); final OutputStream nullSink = NullOutputStream.NULL_OUTPUT_STREAM) {
    ...
    @Override
    protected Optional<BasicTransformedImage> performTransformImage(@NonNull final ServerSession session, @NonNull final IFileHolder file, @NonNull final TransformImageParameters xformParams, final String cacheKey, final String fileName)
