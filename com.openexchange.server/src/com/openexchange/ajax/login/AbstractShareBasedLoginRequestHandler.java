    ...
    private final class ShareLoginClosure implements LoginClosure {
        ...
        public LoginResult doLogin(final HttpServletRequest req) throws OXException {
            ...
            try {
                ...
                AuthenticationServiceRegistry registry = ServerServiceRegistry.getInstance().getService(AuthenticationServiceRegistry.class);
                BasicAuthenticationService basicService = registry.optBasicAuthenticationService().orElseThrow(() -> ServiceExceptionCode.absentService(BasicAuthenticationService.class));
                ...
                AuthenticationResult result = basicService.handleLoginInfo(guest.getGuestID(), guest.getContextID());
                if (FAILED.equals(result.getStatus()) || false == result.getAuthenticated().isPresent()) {
                    return null;
                }
                Authenticated authenticated = result.getAuthenticated().get();
                ...
                Session session;
                {
                    ...
                    final SessionEnhancement sessionEnhancement = (authenticated instanceof SessionEnhancement) ? (SessionEnhancement) authenticated : null;
                    {
