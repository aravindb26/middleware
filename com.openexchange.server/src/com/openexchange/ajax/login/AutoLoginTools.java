...
public class AutoLoginTools {
    ...
    public static LoginResult reAuthenticate(LoginResult autoLoginResult, String login, String password, Map<String, Object> properties) throws OXException {
        if (null != autoLoginResult) {
            ...
            AuthenticationResult authResult = authenticationServiceRegistry.doLogin(authRequest, false);
            if (FAILED.equals(authResult.getStatus())) {
                throw authResult.getException().orElseThrow(() -> LoginExceptionCodes.UNKNOWN.create("Authentication failed"));
            }
            Authenticated authenticated = authResult.getAuthenticated().get();
            Context context;
            User user;
            if ((authenticated instanceof ResolvedAuthenticated)) {
                /*
