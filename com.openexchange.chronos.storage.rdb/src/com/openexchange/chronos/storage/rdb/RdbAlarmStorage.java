...
public class RdbAlarmStorage extends RdbStorage implements AlarmStorage {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            ...
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    ...
                    com.openexchange.tools.arrays.Collections.put(alarmsByUserId, I(userId), readAlarm(connection, eventId, resultSet, mappedFields));
                }
            }
        }
        ...
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            ...
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    ...
                    com.openexchange.tools.arrays.Collections.put(alarmsByEventId, eventId, readAlarm(connection, eventId, resultSet, mappedFields));
                }
            }
        }
        ...
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            ...
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    ...
                    com.openexchange.tools.arrays.Collections.put(alarmsByUser, I(resultSet.getInt(2)), readAlarm(connection, eventID, resultSet, mappedFields));
                }
            }
        }
    ...
    private Alarm readAlarm(Connection connection, String eventId, ResultSet resultSet, AlarmField[] fields) throws SQLException, OXException {
        return adjustAfterLoad(connection, eventId, MAPPER.fromResultSet(resultSet, MAPPER.getMappedFields(fields)));
    }
     ...
     */
    private Alarm adjustAfterLoad(Connection connection, String eventId, Alarm alarm) {
        ...
        if (null != attendeeProperties && 0 < attendeeProperties.size()) {
            alarm.setAttendees(decodeAttendees(connection, eventId, attendeeProperties));
     ...
     */
    private List<Attendee> decodeAttendees(Connection connection, String eventId, List<ExtendedProperty> attendeeProperties) {
        ...
        for (ExtendedProperty attendeeProperty : attendeeProperties) {
            ...
            try {
                attendee = entityProcessor.adjustAfterLoad(attendee, connection);
            } catch (OXException e) {
    ...
    private Alarm selectAlarm(Connection con, int cid, int accountId, int alarmId, AlarmField[] fields) throws OXException, SQLException {
        ...
        try (PreparedStatement stmt = con.prepareStatement(stringBuilder.toString())) {
            ...
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                if (resultSet.next()) {
                    String eventId = resultSet.getString(1);
                    return readAlarm(con, eventId, resultSet, fields);
                }
                return null;
            }
        }
