...
public class RdbAttendeeStorage extends RdbStorage implements AttendeeStorage {
    ...
    public Map<String, List<Attendee>> loadAttendees(String[] eventIds) throws OXException {
        return loadAttendees(eventIds, (Boolean) null);
    }
    ...
    public Map<String, List<Attendee>> loadAttendees(String[] eventIds, Boolean internal) throws OXException {
        ...
        try {
            ...
            return selectAttendees(connection, eventIds, null, internal, false, null);
    ...
    @Override
    public Map<String, List<Attendee>> loadAttendees(String[] eventIds, int[] entities) throws OXException {
        Connection connection = null;
        try {
            connection = dbProvider.getReadConnection(context);
            return selectAttendees(connection, eventIds, entities, null, false, null);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    @Override
    public List<Attendee> loadAttendees(String eventId, List<String> folderIds,AttendeeField[] fields) throws OXException {
        Connection connection = null;
        try {
            connection = dbProvider.getReadConnection(context);
            return selectAttendees(connection, eventId, folderIds, fields);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    @Override
    public Map<String, Map<CalendarUserType, Integer>> loadAttendeeCountsPerCUType(String[] eventIds, Boolean internal) throws OXException {
        Connection connection = null;
        try {
            connection = dbProvider.getReadConnection(context);
            return selectAttendeeCountsPerCUType(connection, eventIds, internal, false);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    @Override
    public Set<String> loadEventIdsWithDifferentPartStats(String[] eventIds, ParticipationStatus partStat, Boolean internal, CalendarUserType[] cuTypes, int[] ignoredEntities) throws OXException {
        Connection connection = null;
        try {
            connection = dbProvider.getReadConnection(context);
            return selectEventIdsWithDifferentPartStats(connection, eventIds, partStat, internal, cuTypes, ignoredEntities, false);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    public Map<String, Attendee> loadAttendee(String[] eventIds, Attendee attendee, AttendeeField[] fields) throws OXException {
        ...
        try {
            connection = dbProvider.getReadConnection(context);
            return selectAttendee(connection, eventIds, attendee, fields, false);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    @Override
    public Map<String, Attendee> loadAttendeeTombstone(String[] eventIds, Attendee attendee, AttendeeField[] fields) throws OXException {
        Connection connection = null;
        try {
            connection = dbProvider.getReadConnection(context);
            return selectAttendee(connection, eventIds, attendee, fields, true);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    @Override
    public Map<String, List<Attendee>> loadAttendeeTombstones(String[] eventIds, int[] entities) throws OXException {
        Connection connection = null;
        try {
            connection = dbProvider.getReadConnection(context);
            return selectAttendees(connection, eventIds, entities, null, true, (AttendeeField[]) null);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    public Map<String, List<Attendee>> loadAttendeeTombstones(String[] eventIds) throws OXException {
        ...
        try {
            connection = dbProvider.getReadConnection(context);
            return selectAttendees(connection, eventIds, null, null, true, null);
        } catch (SQLException e) {
            throw asOXException(e);
        } finally {
            dbProvider.releaseReadConnection(context, connection);
    ...
    private int updateAttendees(Connection connection, String eventId, List<Attendee> attendees) throws OXException {
        ...
        for (Attendee attendee : attendees) {
            ...
            String sql = new StringBuilder()
                .append("UPDATE `calendar_attendee` SET ").append(MAPPER.getAssignments(fields, "calendar_attendee."))
    ...
    private int insertAttendees(Connection connection, Map<String, List<Attendee>> attendeesByEventId, boolean tombstones) throws SQLException, OXException {
        ...
        String columnPrefix = tombstones ? "calendar_attendee_tombstone." : "calendar_attendee.";
        StringBuilder stringBuilder = new StringBuilder()
            .append(tombstones ? "REPLACE INTO calendar_attendee_tombstone " : "INSERT INTO `calendar_attendee` ")
            .append("(`cid`,`account`,`event`,").append(MAPPER.getColumns(mappedFields, columnPrefix)).append(") VALUES ")
        ;
    ...
    private Map<String, List<Attendee>> selectAttendees(Connection connection, String[] eventIds, int[] entities, Boolean internal, boolean tombstones, AttendeeField[] fields) throws SQLException, OXException {
        if (null == eventIds || 0 == eventIds.length) {
            return java.util.Collections.emptyMap();
        }
        AttendeeField[] mappedFields = MAPPER.getMappedFields(fields);
        StringBuilder stringBuilder = new StringBuilder()
            .append("SELECT `event`,").append(MAPPER.getColumns(mappedFields, tombstones ? "calendar_attendee_tombstone." : "calendar_attendee."))
        ...
        ;
        if (null != entities) {
            stringBuilder.append(" AND entity").append(Databases.getPlaceholders(entities.length));
        }
        if (null != internal) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            ...
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    ...
                    Collections.put(attendeesByEventId, eventId, readAttendee(connection, eventId, resultSet, mappedFields));
                }
            }
        }
    ...
    private Map<String, Map<CalendarUserType, Integer>> selectAttendeeCountsPerCUType(Connection connection, String[] eventIds, Boolean internal, boolean tombstones) throws SQLException {
        if (null == eventIds || 0 == eventIds.length) {
            return java.util.Collections.emptyMap();
        }
        StringBuilder stringBuilder = new StringBuilder()
            .append("SELECT `event`,`cuType`,COUNT(*) FROM ").append(tombstones ? "`calendar_attendee_tombstone`" : "`calendar_attendee`")
            .append(" WHERE `cid`=? AND `account`=? AND `event`").append(Databases.getPlaceholders(eventIds.length))
        ;
        if (null != internal) {
            stringBuilder.append(" AND `entity`").append(internal.booleanValue() ? ">=0" : "<0");
        }
        stringBuilder.append(" GROUP BY `event`,`cuType`;");
        Map<String, Map<CalendarUserType, Integer>> attendeeCountsByEventIdAndCUType = new HashMap<String, Map<CalendarUserType, Integer>>(eventIds.length);
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            int parameterIndex = 1;
            stmt.setInt(parameterIndex++, context.getContextId());
            stmt.setInt(parameterIndex++, accountId);
            ...
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    Map<CalendarUserType, Integer> attendeeCountsByCUType = attendeeCountsByEventIdAndCUType.computeIfAbsent(resultSet.getString(1), Functions.getNewHashMapFuntion());
                    String cuTypeValue = resultSet.getString(2);
                    CalendarUserType cuType = Strings.isEmpty(cuTypeValue) ? CalendarUserType.INDIVIDUAL : new CalendarUserType(cuTypeValue);
                    attendeeCountsByCUType.put(cuType, I(resultSet.getInt(3)));
                }
            }
        }
        return attendeeCountsByEventIdAndCUType;
    }

    private Set<String> selectEventIdsWithDifferentPartStats(Connection connection, String[] eventIds, ParticipationStatus partStat, Boolean internal, CalendarUserType[] cuTypes, int[] ignoredEntities, boolean tombstones) throws SQLException {
        if (null == eventIds || 0 == eventIds.length) {
            return java.util.Collections.emptySet();
        }
        StringBuilder stringBuilder = new StringBuilder()
            .append("SELECT DISTINCT `event` FROM ").append(tombstones ? "`calendar_attendee_tombstone`" : "`calendar_attendee`")
            .append(" WHERE `cid`=? AND `account`=? AND `event`").append(Databases.getPlaceholders(eventIds.length));
        if (null != internal) {
            stringBuilder.append(" AND `entity`").append(internal.booleanValue() ? ">=0" : "<0");
        }
        if (null != cuTypes) {
            ...
            stringBuilder.append(" AND (`cuType`").append(Databases.getPlaceholders(cuTypes.length));
            if (com.openexchange.tools.arrays.Arrays.contains(cuTypes, CalendarUserType.INDIVIDUAL)) {
                stringBuilder.append(" OR `cuType` IS NULL");
            }
            stringBuilder.append(')');
        }
        if (null != ignoredEntities && 0 < ignoredEntities.length) {
            if (1 == ignoredEntities.length) {
                stringBuilder.append(" AND `entity`<>?");
            } else {
                stringBuilder.append(" AND `entity` NOT ").append(Databases.getPlaceholders(ignoredEntities.length));
            }
        }
        stringBuilder.append(" AND (`partStat`!=?");
        if (false == ParticipationStatus.NEEDS_ACTION.matches(partStat)) {
            stringBuilder.append(" OR `partStat` IS NULL");
        }
        stringBuilder.append(");");
        Set<String> matchingEventIds = new HashSet<String>(eventIds.length);
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            int parameterIndex = 1;
            stmt.setInt(parameterIndex++, context.getContextId());
            stmt.setInt(parameterIndex++, accountId);
            ...
            if (null != cuTypes) {
                for (CalendarUserType cuType : cuTypes) {
                    stmt.setString(parameterIndex++, cuType.getValue());
                }
            }
            if (null != ignoredEntities) {
                for (int ignoredEntity : ignoredEntities) {
                    stmt.setInt(parameterIndex++, ignoredEntity);
                }
            }
            stmt.setString(parameterIndex++, partStat.getValue());
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    matchingEventIds.add(resultSet.getString(1));
                }
            }
        }
    ...
    private Map<String, Attendee> selectAttendee(Connection connection, String[] eventIds, Attendee attendee, AttendeeField[] fields, boolean tombstones) throws SQLException, OXException {
        if (null == eventIds || 0 == eventIds.length) {
            return java.util.Collections.emptyMap();
        }
        AttendeeField[] mappedFields = MAPPER.getMappedFields(fields);
        StringBuilder stringBuilder = new StringBuilder()
            .append("SELECT `event`,").append(MAPPER.getColumns(mappedFields, tombstones ? "calendar_attendee_tombstone." : "calendar_attendee."))
            .append(" FROM ").append(tombstones ? "`calendar_attendee_tombstone`" : "`calendar_attendee`")
            .append(" WHERE `cid`=? AND `account`=? AND ")
            .append(isInternal(attendee) ? "`entity`" : "`uri`").append("=?")
            .append(" AND `event`").append(Databases.getPlaceholders(eventIds.length))
        ;
        ...
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            ...
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    String eventId = resultSet.getString(1);
                    attendeeByEventId.put(eventId, readAttendee(connection, eventId, resultSet, mappedFields));
                }
            }
        }
    ...
    private List<Attendee> selectAttendees(Connection connection, String eventId, List<String> folderIds, AttendeeField[] fields) throws SQLException, OXException {
        if (null == eventId || Collections.isNullOrEmpty(folderIds)) {
            return java.util.Collections.emptyList();
        }
        AttendeeField[] mappedFields = MAPPER.getMappedFields(fields);
        StringBuilder stringBuilder = new StringBuilder()
            .append("SELECT ").append(MAPPER.getColumns(fields, "calendar_attendee."))
            .append(" FROM ").append("`calendar_attendee`")
            .append(" WHERE `cid`=? AND `account`=? AND ")
            .append("`event`").append("=?")
            .append(" AND `folder`").append(Databases.getPlaceholders(folderIds.size()))
        ;
        ...
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            ...
            for (String folder : folderIds) {
                stmt.setString(parameterIndex++, folder);
            }
            try (ResultSet resultSet = logExecuteQuery(stmt)) {
                while (resultSet.next()) {
                    attendees.add(readAttendee(connection, eventId, resultSet, mappedFields));
                }
            }
        }
        return attendees;
    ...
    private Attendee readAttendee(Connection connection, String eventId, ResultSet resultSet, AttendeeField[] fields) throws SQLException, OXException {
        Attendee attendee = MAPPER.fromResultSet(resultSet, fields);
        try {
            return entityProcessor.adjustAfterLoad(attendee, connection);
        } catch (OXException e) {
            if (CalendarExceptionCodes.INVALID_CALENDAR_USER.equals(e) && isInternal(attendee) && Strings.isNotEmpty(attendee.getUri())) {
                ...
                fallback.removeUri();
                try {
                    fallback = entityProcessor.adjustAfterLoad(fallback, connection);
