...
public class EventMapper extends DefaultDbMapper<Event, EventField> {
	...
	protected EnumMap<EventField, DbMapping<? extends Object, Event>> createMappings() {
        ...
        mappings.put(EventField.DTSTAMP, new BigIntMapping<Event>("dtstamp", CalendarStrings.FIELD_DTSTAMP) {

            @Override
            public boolean isSet(Event event) {
                return event.containsDtStamp();
            }

            @Override
            public void set(Event event, Long value) throws OXException {
                event.setDtStamp(null == value ? 0L : l(value));
            }

            @Override
            public Long get(Event event) {
                return L(event.getDtStamp());
            }
