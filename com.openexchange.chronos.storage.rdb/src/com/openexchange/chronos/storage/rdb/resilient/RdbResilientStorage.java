...
public abstract class RdbResilientStorage extends CalendarStorageWarnings {
     ...
     * <p/>
     * If supported in the underlying delegate storage, the severity threshold defining which unsupported data errors can be ignored is
     * applied implicitly.
     *
     * @param services A service lookup reference
     * @param delegate The underlying delegate storage
     ...
     */
    protected RdbResilientStorage(ServiceLookup services, Object delegate, boolean handleTruncations, boolean handleIncorrectStrings, ProblemSeverity unsupportedDataThreshold) {
        super();
        this.services = services;
        this.delegate = delegate;
        this.handleIncorrectStrings = handleIncorrectStrings;
        this.handleTruncations = handleTruncations;
        setUnsupportedDataThreshold(unsupportedDataThreshold);
    }

    /**
     * Gets a combined map of this storage's warnings, merged with the warnings found in the delegate storage.
     * 
     * @param delegate The delegate storage
     * @return The (possibly combined) calendar storage warnings
     */
    @Override
    public Map<String, List<OXException>> getWarnings() {
        Map<String, List<OXException>> warnings = super.getWarnings();
        if ((delegate instanceof CalendarStorageWarnings delegateWarnings)) {
            return CalendarStorageWarnings.combineWarnings(warnings, delegateWarnings.getWarnings());
        }
        return warnings;
    }

    /**
     * Gets and flushes a combined map of this storage's warnings, merged with the warnings found in the delegate storage.
     * 
     * @param delegate The delegate storage
     * @return The (possibly combined) calendar storage warnings
     */
    ...
    public Map<String, List<OXException>> getAndFlushWarnings() {
        Map<String, List<OXException>> warnings = super.getAndFlushWarnings();
        if ((delegate instanceof CalendarStorageWarnings delegateWarnings)) {
            return CalendarStorageWarnings.combineWarnings(warnings, delegateWarnings.getAndFlushWarnings());
        }
        return warnings;
    }
    ...
    public void setUnsupportedDataThreshold(ProblemSeverity severityThreshold) {
        ...
        if ((delegate instanceof CalendarStorageWarnings delegateWarnings)) {
            delegateWarnings.setUnsupportedDataThreshold(severityThreshold);
        }
    }
    ...
    protected <O> boolean handle(String eventId, O object, Throwable failure) {
        if (false == (failure instanceof OXException)) {
            return false;
        }
