...
public class RdbAlarmTriggerStorage extends RdbStorage implements AlarmTriggerStorage {
    ...
    private AlarmTrigger getAlarmTrigger(int id, Connection con) throws OXException {
        try {
            AlarmTriggerField[] mappedFields = MAPPER.getMappedFields();
            StringBuilder stringBuilder = new StringBuilder().append("SELECT `account`,`cid`,").append(MAPPER.getColumns(mappedFields)).append(" FROM `calendar_alarm_trigger` WHERE `cid`=? AND `account`=? AND `alarm`=?");
    ...
    private List<AlarmTrigger> getAlarmTriggers(int user, Date from, Date until, Connection con) throws OXException {
        try {
            AlarmTriggerField[] mappedFields = MAPPER.getMappedFields();
            StringBuilder stringBuilder = new StringBuilder().append("SELECT `account`,`cid`,").append(MAPPER.getColumns(mappedFields)).append(" FROM `calendar_alarm_trigger` WHERE `cid`=? AND `account`=? AND `user`=?");
    ...
    private static void addRelatedDate(Alarm alarm, Event event, AlarmTrigger trigger) {
    ...
    private Set<String> selectHasTriggers(Connection connection, int userId, String[] eventIds) throws SQLException {
        ...
        StringBuilder stringBuilder = new StringBuilder()
            ...
            .append(Databases.getPlaceholders(eventIds.length))
        ;
    ...
    private int deleteTriggers(Connection connection, List<String> ids) throws SQLException {
        ...
        StringBuilder stringBuilder = new StringBuilder().append("DELETE FROM `calendar_alarm_trigger` WHERE `cid`=? AND `account`=? AND `eventId`").append(Databases.getPlaceholders(ids.size()));
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            int parameterIndex = 1;
            stmt.setInt(parameterIndex++, context.getContextId());
            stmt.setInt(parameterIndex++, accountId);
    ...
    private int deleteTriggersById(Connection connection, List<Integer> alarmIds) throws SQLException {
        if (null == alarmIds || alarmIds.isEmpty()) {
            return 0;
        }
        StringBuilder stringBuilder = new StringBuilder().append("DELETE FROM `calendar_alarm_trigger` WHERE `cid`=? AND `account`=? AND `alarm`").append(Databases.getPlaceholders(alarmIds.size()));
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            int parameterIndex = 1;
            stmt.setInt(parameterIndex++, context.getContextId());
            stmt.setInt(parameterIndex++, accountId);
    ...
    private int deleteTriggers(Connection connection, List<String> ids, int userId) throws SQLException {
        ...
        StringBuilder stringBuilder = new StringBuilder().append("DELETE FROM `calendar_alarm_trigger` WHERE `cid`=? AND `account`=? AND `user`=? AND `eventId`").append(Databases.getPlaceholders(ids.size()));
        try (PreparedStatement stmt = connection.prepareStatement(stringBuilder.toString())) {
            int parameterIndex = 1;
            stmt.setInt(parameterIndex++, context.getContextId());
            stmt.setInt(parameterIndex++, accountId);
     ...
     * @throws OXException
     */
    private static PreparedStatement getInsertStatementForBatch(Connection writeCon) throws OXException {
        try {
            AlarmTriggerField[] mappedFields = MAPPER.getMappedFields();
            String sql = new StringBuilder().append("INSERT INTO `calendar_alarm_trigger` (`cid`, `account`, ").append(MAPPER.getColumns(mappedFields)).append(") VALUES (?,?,").append(MAPPER.getParameters(mappedFields)).append(')').toString();
    ...
    private void addBatch(AlarmTrigger trigger, PreparedStatement stmt) throws OXException {
        try {
            ...
            stmt.setInt(parameterIndex++, context.getContextId());
            stmt.setInt(parameterIndex++, accountId);
            MAPPER.setParameters(stmt, parameterIndex, trigger, MAPPER.getMappedFields());
            stmt.addBatch();
        } catch (SQLException e) {
            throw CalendarExceptionCodes.DB_ERROR.create(e, e.getMessage());
        }
