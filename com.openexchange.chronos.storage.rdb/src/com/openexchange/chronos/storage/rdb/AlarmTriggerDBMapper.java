...
import com.openexchange.groupware.tools.mappings.database.UUIDMapping;
...
public class AlarmTriggerDBMapper extends DefaultDbMapper<AlarmTrigger, AlarmTriggerField> {
    ...
    public AlarmTrigger newInstance() {
        return new DefaultAlarmTrigger();
    }
    ...
    protected EnumMap<AlarmTriggerField, ? extends DbMapping<? extends Object, AlarmTrigger>> createMappings() {
        ...
        mappings.put(AlarmTriggerField.CLAIM, new UUIDMapping<AlarmTrigger>("claim", "Claim") {

            @Override
            public void set(AlarmTrigger alarmTrigger, UUID value) {
                alarmTrigger.setClaim(value);
            }

            @Override
            public boolean isSet(AlarmTrigger alarmTrigger) {
                return alarmTrigger.containsClaim();
            }

            @Override
            public UUID get(AlarmTrigger alarmTrigger) {
                return alarmTrigger.getClaim();
            }

            @Override
            public void remove(AlarmTrigger alarmTrigger) {
                alarmTrigger.removeClaim();
            }
        });
        return mappings;
    }
