...
public final class ImapIdlePushManagerService implements PushManagerExtendedService {
    ...
    private boolean isImapIdlePushEnabledFor(int userId, int contextId) throws OXException {
        ...
        return ConfigViews.getDefinedBoolPropertyFrom("com.openexchange.push.imapidle.enabled", false, view);
    }
    ...
    public boolean supportsPermanentListeners() {
        ...
        LeanConfigurationService service = services.getOptionalService(LeanConfigurationService.class);
        return null == service ? defaultValue : service.getBooleanProperty(ImapIdleProperty.supportsPermanentListeners);
    }
        ...
        if (AcquisitionResult.NOT_ACQUIRED == acquisitionResult) {
            ...
            try {
                ...
                try {
                    ...
                    AccessControls.release(lock, acquired);
                }
            } finally {
                if (unlock) {
    ...
    public ImapIdlePushListener injectAnotherListenerFor(Session oldSession) {
        ...
        if (null != sessiondService) {
            ...
            if (ImapIdleClusterLock.Type.LOCAL != clusterLock.getType() && (sessiondService.isCentral() == false)) {
