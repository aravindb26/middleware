...
class ICAPResponseParser {
        ...
        return switch (response.getStatusCode()) {
            case 200 ->
                // Check the encapsulated response and override 'infected' flag if necessary.
                    builder.withInfected(Boolean.valueOf(!checkHttpResponse(response))).build();
            case 204 -> builder.withInfected(Boolean.FALSE).build();
            case 418 -> throw AntiVirusServiceExceptionCodes.REMOTE_SERVER_ERROR.create(response.getStatusLine());
            case 500 -> throw AntiVirusServiceExceptionCodes.REMOTE_INTERNAL_SERVER_ERROR.create(response.getStatusLine());
            default ->
    ...
    private boolean checkHttpResponse(ICAPResponse response) throws OXException {
        return switch (response.getEncapsulatedStatusCode()) {
            case 200 -> true;
            case 403 -> false;
            case 418 -> throw AntiVirusServiceExceptionCodes.REMOTE_SERVER_ERROR.create(response.getStatusLine());
            case 500 -> throw AntiVirusServiceExceptionCodes.REMOTE_INTERNAL_SERVER_ERROR.create(response.getStatusLine());
