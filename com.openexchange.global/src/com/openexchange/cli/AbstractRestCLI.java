 ...
 * @since v7.8.4
 */
public abstract class AbstractRestCLI<R> extends AbstractAdministrativeCLI<R, Builder, RestAuthenticatorClient> {
    ...
    private RestAuthenticatorClient authenticator;
    ...
    public R execute(String[] args) {
        ...
        try {
            ...
            // Check for administrative authentication
            ...
            // Check for API host
            String endpointHost = cmd.hasOption("api-host") ? cmd.getOptionValue("api-host").trim() : ENDPOINT_HOST_DEFAULT;
            ...
            WebTarget endpoint = getEndpoint(endpointHost, cmd);
            if (null == endpoint) {
                return null;
            }
    ...
    protected void optAuthenticate(CommandLine cmd) throws Exception {
        if (useBasicAuth()) {
            administrativeAuth(null, null, cmd, getAuthenticator());
            return;
        }
    ...
    @Override
    protected void administrativeAuth(String login, String password, CommandLine cmd, RestAuthenticatorClient authenticator) throws Exception {
    ...
    @Override
    protected RestAuthenticatorClient getAuthenticator() throws Exception {
        RestAuthenticatorClient authenticator = this.authenticator;
        if (authenticator == null) {
            authenticator = new RestAuthenticatorClient(args);
    ...
    @Override
    protected boolean isAuthEnabled(RestAuthenticatorClient authenticator) throws Exception {
     ...
     *
     * @param endpointHost The end-point host
     * @param cmd The command-line
     * @return The end-point to use
     */
    protected abstract WebTarget getEndpoint(String endpointHost, CommandLine cmd);
}
