...
public class OXException extends Exception implements OXExceptionConstants {
    ...
    private static final int SERVER_ID = CryptoUtil.getSecureRandom().nextInt();
    ...
    private final transient List<ProblematicAttribute> problematics;
    ...
    public OXException(OXException cloneMe) {
        ...
        this.categories = null == cloneMe.categories ? new ArrayList<>(2) : new ArrayList<>(cloneMe.categories);
        ...
        this.problematics = null == cloneMe.problematics ? new LinkedList<>() : new ArrayList<>(cloneMe.problematics);
        this.properties = null == cloneMe.properties ? new HashMap<>(4) : new HashMap<>(cloneMe.properties);
        this.arguments = null == cloneMe.arguments ? new HashMap<>(4) : new HashMap<>(cloneMe.arguments);
        this.lightWeight = cloneMe.lightWeight;
        this.info = cloneMe.info;
    }
    ...
    public Throwable getNonOXExceptionCause() {
        ...
        return cause instanceof OXException oxe ? oxe.getNonOXExceptionCause() : cause;
    }
     ...
     * @throws IllegalArgumentException If <code>prefix</code> argument is <code>null</code>
     */
    ...
    /**
     * Sets optional additional information.
     *
     ...
     * Sets optional additional information.
     * <p>
     * Allows specifying a message pattern according to slf4j specification; e.g.
     * <pre>
     *   e.setInfo("Cannot find user {}", session.getUserId());
     * </pre>
     *
     * @param pattern The information pattern string
     * @param arguments The arguments to format
     */
    public void setInfo(String pattern, Object ... arguments) {
        this.info = MessageFormatter.format(pattern, arguments);
    }
    ...
    public Category getCategory() {
        List<Category> cats = getCategories();
        return cats.isEmpty() ? CATEGORY_ERROR : cats.get(0);
    }
    ...
    public List<Category> getCategories() {
        List<Category> cats = new ArrayList<>(this.categories);
        int size = cats.size();
        switch (size) {
            case 0:
    ...
    private static List<ComparableCategory> toComparables(List<Category> categories) {
        List<ComparableCategory> ret = new ArrayList<>(categories.size());
