...
public class MessagingComparator implements Comparator<MessagingMessage> {
    ...
    private static void checkField(final MessagingField field) throws OXException {
    ...
    private static final MailDateFormat DATE_FORMAT = new MailDateFormat(); // NOSONARLINT - The parse() method of `MailDateFormat` is thread-safe!

    private static Object getValue(final MessagingHeader h) {
        if (DATE.equalsIgnoreCase(h.getName())) {
            // The parse() method of `MailDateFormat` is thread-safe!
            try {
    ...
    private Object transform(final Object o) {
        if (INT_FIELDS.contains(field) && o instanceof String) {
            try {
