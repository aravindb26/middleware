...
import org.glassfish.grizzly.http.server.RequestFormPostSizeExceededException;
...
import com.openexchange.http.grizzly.util.SemaphoreReadWriteLock;
...
public class OSGiServletHandler extends ServletHandler implements OSGiHandler {
    ...
    private final SemaphoreReadWriteLock readWriteLock = new SemaphoreReadWriteLock();
    private final AtomicBoolean active = new AtomicBoolean(true);
    private final Lock readLock = readWriteLock.getReadLock();
    private final Lock writeLock = readWriteLock.getWriteLock();
    ...
    public void customizeErrorPage(Response response, String message, int errorCode, Throwable t) {
        if (ExceptionUtils.isEitherOf(t, RequestFormPostSizeExceededException.class)) {
            super.customizeErrorPage(response, HttpStatus.REQUEST_URI_TOO_LONG_414.getReasonPhrase(), 414, t);
        } else {
            super.customizeErrorPage(response, message, errorCode, t);
        }
    }
