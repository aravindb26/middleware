...
public final class StartUpTracker implements ServiceTrackerCustomizer<SignalStartedService, SignalStartedService> {
        ...
        synchronized (grizzlyConfig) {
            ...
            if (isOk) {
                try {
                    ...
                    LOGGER.error(" ---=== /!\\ ===--- Grizzly network listeners could not be started! ---=== /!\\ ===--- {}{}Process started twice?", sep, sep, e);
                } catch (Exception e) {
    ...
    public void removedService(final ServiceReference<SignalStartedService> reference, final SignalStartedService service) {
        synchronized (grizzlyConfig) {
            ...
            if (grizzlyConfig.isShutdownFast()) {
                ...
                OSGiCleanMapper.markGracePeriodExpired();
            } else {
                ...
                if (awaitShutDownSeconds > 0) {
                    ...
                    // Await shut-down
                    boolean shuDownCompleted = false;
                    try {
                        LOGGER.info("Awaiting orderly shut-down for {} seconds...", Integer.valueOf(awaitShutDownSeconds));
                        shutDownTask.get(awaitShutDownSeconds, TimeUnit.SECONDS);
                        shuDownCompleted = true;
                        ...
                        if (shuDownCompleted == false) {
                            OSGiCleanMapper.markGracePeriodExpired();
                        }
                    }
                } else {
                    doShutDown(Optional.empty());
                }
            }
    ...
    private Future<Void> initiateShutDown(final AtomicBoolean doQuit) {
        FutureTask<Void> shutDownTask = new FutureTask<Void>(() -> doShutDown(Optional.of(doQuit)), null);
        new Thread(shutDownTask, "Grizzly Shut-Down Performer").start();
        return shutDownTask;
    }

    void doShutDown(Optional<AtomicBoolean> optionalDoQuitFlag) {
        LOGGER.info("Performing orderly shut-down...");

        List<Lock> acquiredLocks = new LinkedList<Lock>();
        try {
            // Mark all end-points as inactive
            List<Lock> removalLocks = new LinkedList<Lock>();
            for (HttpHandler httpHandler : OSGiCleanMapper.getAllHttpHandlers()) {
                if (httpHandler instanceof OSGiServletHandler) {
                    ...
                    servletHandler.setActive(false);
                    removalLocks.add(servletHandler.getRemovalLock());
                }
            }

            // Acquire removal locks to ensure every processing thread is through
            if (optionalDoQuitFlag.isPresent()) {
                AtomicBoolean doQuit = optionalDoQuitFlag.get();
                // Interrupt aware...
                Thread current = Thread.currentThread();
                for (Lock removalLock : removalLocks) {
