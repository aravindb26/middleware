...
public class OXHttpServerFilter extends HttpServerFilter {
     ...
     */
    private final HttpRequestGuard httpRequestGuard;
    ...
    public OXHttpServerFilter(GrizzlyConfig grizzlyConfig, final ServerFilterConfiguration config, final DelayedExecutor delayedExecutor) {
        ...
        this.httpRequestGuard = HttpRequestGuard.guardFor(grizzlyConfig.getMaxNumberOfConcurrentRequests());
    ...
    public NextAction handleRead(final FilterChainContext ctx)
        ...
        try {
            ...
            if (shuttingDown.get()) { // if we're in the shutting down phase - serve shutdown page and exit
            ...
            } else if (httpRequestGuard.mayNotHandle(handlerRequest)) { // max. number of concurrent requests reached
                handlerResponse.getResponse().getProcessingState().setError(true);
                HtmlHelper.setErrorAndSendErrorPage(
                        handlerRequest, handlerResponse,
                        config.getDefaultErrorPageGenerator(),
                ...
                HtmlHelper.setErrorAndSendErrorPage(
                        ...
                        "The request payload size exceeds the maximum post size limitation", null);
            } else if (!checkMaxQueryStringSize(request, config)) {
                handlerResponse.getResponse().getProcessingState().setError(true);
                HtmlHelper.setErrorAndSendErrorPage(
                        handlerRequest, handlerResponse,
                        config.getDefaultErrorPageGenerator(),
                        414, HttpStatus.REQUEST_URI_TOO_LONG_414.getReasonPhrase(),
                        "The request query string size exceeds the maximum query string size limitation", null);
            } else {
    ...
    private boolean checkMaxPostSize(final long requestContentLength) {
        ...
        long maxPostSize = getConfiguration().getMaxPostSize();
        return maxPostSize < 0 || maxPostSize >= requestContentLength;
    }

    /**
     * @param request The HTTP request to check
     * @param config The configuration to use
     * @return <tt>true</tt> if max. query string size is not exceeded; otherwise <code>false</code> if it is exceeded
     */
    private boolean checkMaxQueryStringSize(HttpRequestPacket request, ServerFilterConfiguration config) {
        long maxQueryStringSize = config.getMaxFormPostSize();
        return (maxQueryStringSize <= 0 || request.getQueryStringDC().getLength() <= maxQueryStringSize);
    }
