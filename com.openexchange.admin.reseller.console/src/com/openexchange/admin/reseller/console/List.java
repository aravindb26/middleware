...
public class List extends ResellerAbstraction {
    ...
    private void sysoutOutput(java.util.List<ResellerAdmin> admns) throws InvalidDataException {
        final ArrayList<ArrayList<String>> data = new ArrayList<ArrayList<String>>();
        for (final ResellerAdmin admin : admns) {
            data.add(makeStandardData(admin, false));
        }
    ...
    private void precsvinfos(java.util.List<ResellerAdmin> adminlist) throws InvalidDataException {
     ...
     */
    private ArrayList<String> makeDataForCsv(ResellerAdmin admin) {
        final ArrayList<String> admin_data = makeStandardData(admin, true);

        return admin_data;
    }

    private ArrayList<String> makeStandardData(ResellerAdmin admin, boolean csv) {
        ...
        if (csv) {
            Set<String> capabilities = admin.getCapabilities();
            if (null != capabilities) {
                admin_data.add(ResellerAbstraction.getCollectionAsString(capabilities)); // restrictions
            } else {
                admin_data.add("");
            }
            Map<String, String> configuration = admin.getConfiguration();
            if (null != configuration) {
                admin_data.add(ResellerAbstraction.getMapAsString(configuration)); // restrictions
            } else {
                admin_data.add("");
            }
        }
