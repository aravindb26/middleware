...
public abstract class ResellerAbstraction extends ObjectNamingAbstraction {
    ...
    protected static final Optional<String> dynamicAttrDesc = Optional.of("Allows to configure various reseller attributes");
    protected static final Optional<String> dynamicRemoveAttrDesc = Optional.of("Allows to remove various reseller attributes");
    ...
    protected void setConfigToAdd(final AdminParser parser) {
        this.configToAdd = setLongOpt(parser, OPT_CONFIGURATION_TO_ADD, "Add/Change reseller specific configuration, e. g. '--config/com.openexchange.oauth.google=false --config/com.openexchange.oauth.microsoft.graph=true'", false, false);
    }

    protected void setConfigToRemove(final AdminParser parser) {
        this.configToRemove = setLongOpt(parser, OPT_CONFIGURATION_TO_REMOVE, "Remove reseller specific configuration, e. g. '--remove-config/com.openexchange.oauth.google --remove-config/com.openexchange.oauth.microsot.graph'", false, false);
    }
    ...
    protected void parseAndSetAdminId(final AdminParser parser, final ResellerAdmin adm) {
        ...
        this.adminid = Integer.valueOf(optionValue);
        adm.setId(adminid);
    }
    ...
    protected void parseAndSetParentId(final AdminParser parser, final ResellerAdmin adm) {
        ...
        if (null != optionValue) {
            adm.setParentId(Integer.valueOf(optionValue));
        }
    }
    ...
    public static Restriction getRestrictionFromString(final String opt) throws InvalidDataException {
        ...
        String[] keyval = Strings.splitBy(opt, '=', true);
        if (keyval.length > 2) {
    ...
    private Set<String> parseAndSetCapabilities(CLIOption cliOption, AdminParser parser) {
        ...
        // Split
        String[] arr = Strings.splitByComma(s);
    ...
    /**
     * This method takes a collection of objects and format them to one comma-separated string
     *
     * @param objects
     * @return
     */
    public static <O> String getCollectionAsString(Collection<O> objects) {
    ...
    /**
     * This method takes a map and formats it to one comma-separated string
     *
     * @param map
     * @return
     */
