...
public class Bug37002Test extends Abstrac2UserAJAXSession {
    ...
    public void testForLostConfirmStatus() throws OXException, IOException, JSONException {
        ...
        assertEquals(1, participants.length, "Number of task participants should be one.");
        UserParticipant participant = participants[0];
        assertEquals(Task.ACCEPT, participant.getConfirm(), "Confirmation status got lost.");
        assertEquals("Will do the stuff.", participant.getConfirmMessage(), "Confirm message got lost.");
    }
}
