...
public class Bug38782Test extends AbstractAJAXSession {
    ...
    public void testGetAllTasks() throws Exception {
        ...
        if (!response.hasError()) {
            ...
            if (response.hasError()) {
                assertTrue(AjaxExceptionCodes.UNEXPECTED_ERROR.create().similarTo(response.getException()), "This is the expected NullPointerException from the bug report.");
            }
            task.setLastModified(response.getTimestamp());
        } else {
            assertTrue(TaskExceptionCode.MISSING_RECURRENCE_VALUE.create().similarTo(response.getException()), "Expected exception for recurring tasks getting start and end date removed.");
        }
    }
}
