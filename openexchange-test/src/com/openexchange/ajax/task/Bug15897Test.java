...
public class Bug15897Test extends AbstractTaskTest {
    ...
    private void checkConfirmation(String confirmMessage, int confirmStatus) throws Throwable {
        ...
        assertEquals(1, participants.length, "Number of participants does not match.");
        UserParticipant participant = participants[0];
        assertEquals(participantId, participant.getIdentifier(), "Participant does not match.");
        assertEquals(confirmMessage, participant.getConfirmMessage(), "Confirmation message does not match.");
        assertEquals(confirmStatus, participant.getConfirm(), "Confirmation status does not match.");
    }
}
