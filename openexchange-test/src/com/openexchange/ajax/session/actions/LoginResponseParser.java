...
public class LoginResponseParser extends AbstractAJAXParser<LoginResponse> {
        ...
        if (isFailOnError()) {
            ...
            while (iter.hasNext()) {
                ...
                if (Tools.JSESSIONID_COOKIE.equals(element.getName())) {
                    ...
                    assertTrue(dotPos > 0, "jvmRoute is missing.");
                    jvmRoute = jsessionId.substring(dotPos + 1);
                    continue;
                }
            }
            assertTrue(oxCookieFound, "Secret cookie is missing.");
            assertTrue(jsessionIdCookieFound, "JSESSIONID cookie is missing.");
        }
        return body;
    }
    ...
    protected LoginResponse createResponse(final Response response) throws JSONException {
        ...
        if (isFailOnError()) {
            ...
            assertTrue(json.has(LoginServlet.PARAMETER_SESSION), "Session ID is missing.");
            assertFalse(json.has(LoginFields.RANDOM_PARAM), "Random should be missing.");
        }
        return retval;
    }
}
