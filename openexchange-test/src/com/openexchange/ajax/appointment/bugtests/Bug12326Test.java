    ...
    public void testAppointmentException() throws Throwable {
        ...
        try {
            ...
            final Appointment occurence;
            {
                ...
                assertEquals(recurrence_position, occurence.getRecurrencePosition(), "Occurence must have a recurrence position.");
            }
            // Create exception
            ...
            // Check exception in get response
            {
                ...
                // Check exception
                assertEquals(Appointment.NO_RECURRENCE, exception.getRecurrenceType(), "Exception is still a series.");
                assertEquals(occurence.getRecurrencePosition(), exception.getRecurrencePosition(), "Exception must have a recurrence position.");
                assertEquals(series.getObjectID(), exception.getRecurrenceID(), "Exception is missing reference to series.");
            }
            // Check exception in list response
            {
                ...
                assertEquals(null, data[2], "Exception is still a series.");
                assertEquals(Integer.valueOf(occurence.getRecurrencePosition()), data[3], "Exception must have a recurrence position.");
                assertEquals(Integer.valueOf(series.getObjectID()), data[4], "Exception is missing reference to series.");
                series.setLastModified(response.getTimestamp());
            }
        } finally {
