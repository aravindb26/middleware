...
public abstract class AbstractNegativeAssertion extends AbstractAssertion {
    ...
    private void checkForError(OXException expectedError) {
        ...
        assertTrue(manager.hasLastException(), state() + " Expecting exception, did not get one");
        try {
            OXException lastException = (OXException) manager.getLastException();
            assertTrue(expectedError.similarTo(lastException), state() + " Expected error: " + expectedError + ", actual error: " + lastException.getErrorCode());
