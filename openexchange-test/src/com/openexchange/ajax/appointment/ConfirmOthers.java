...
public class ConfirmOthers extends AbstractAJAXSession {
    ...
    public void setUp(TestInfo testInfo) throws Exception {
        ...
        Participant external = new ExternalUserParticipant("test@example.com");
    ...
    public void testConfirmOthersAllowed() throws Exception {
        ...
        for (UserParticipant user : loadedAppointment.getUsers()) {
            if (user.getIdentifier() == userIdA) {
                ...
                assertEquals("yap!", user.getConfirmMessage(), "Wrong confirm message.");
            }
        }
    }
    ...
    public void testConfirmOthersNotAllowed() throws Exception {
        ...
        for (UserParticipant user : loadedAppointment.getUsers()) {
            if (user.getIdentifier() == userIdA) {
                assertEquals(Appointment.NONE, user.getConfirm(), "Wrong confirm status.");
                assertEquals(null, user.getConfirmMessage(), "Wrong confirm message.");
            }
        }
    }
    ...
    public void testConfirmExternal() throws Exception {
        clientC.execute(new ConfirmRequest(clientC.getValues().getPrivateAppointmentFolder(), appointment.getObjectID(), Appointment.TENTATIVE, "maybe", "test@example.com", appointment.getLastModified(), false));
        ...
        for (ConfirmableParticipant p : loadedAppointment.getConfirmations()) {
            if (p.getEmailAddress().equals("test@example.com")) {
                assertEquals(Appointment.TENTATIVE, p.getConfirm(), "Wrong confirm status.");
                assertEquals("maybe", p.getMessage(), "Wrong confirm message.");
            }
        }
    }
}
