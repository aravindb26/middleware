...
public class LocalizedMessagesTest extends ShareTest {
    ...
    public void testSharedFolder() throws Exception {
        ...
        assertTrue(null != redeemResponse.getMessage() && redeemResponse.getMessage().contains("freigegeben"), "Wrong localization. Expected 'freigegeben' but was: '" + redeemResponse.getMessage() + "'");
        redeemResponse = guestClient.execute(new RedeemRequest(token, "fr_FR", true));
        assertTrue(null != redeemResponse.getMessage() && redeemResponse.getMessage().contains("partag\u00e9"), "Wrong localization. Expected 'partag\u00e9' but was: '" + redeemResponse.getMessage() + "'");
        redeemResponse = guestClient.execute(new RedeemRequest(token, "en_US", true));
        assertTrue(null != redeemResponse.getMessage() && redeemResponse.getMessage().contains("shared"), "Wrong localization. Expected 'shared' but was: '" + redeemResponse.getMessage() + "'");
