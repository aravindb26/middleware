...
public class AJAXClient {
    ...
    /**
     * 
     * Initializes a new {@link AJAXClient}.
     *
     * @param user The user
     * @param customizer Defined a callback to customize the used HTTP client within the {@link AJAXSession}
     */
    public AJAXClient(TestUser user, Consumer<RequestConfig.Builder> customizer) throws OXException, IOException, JSONException {
        super();
        ...
        session = null == customizer ? new AJAXSession(createdBy) : new AJAXSession(createdBy, AJAXSession.newHttpClientBuilder(createdBy, customizer).build());
        session.setId(execute(new LoginRequest(user.getLogin(), user.getPassword(), LoginTools.generateAuthId(), AJAXClient.class.getName(), VERSION)).getSessionId());
    }
