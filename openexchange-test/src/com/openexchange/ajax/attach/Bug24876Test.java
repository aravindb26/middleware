...
public final class Bug24876Test extends AbstractTestEnvironment {
    ...
    public void testList() throws Exception {
        ...
        assertEquals(1, response.getArray().length, "attachment listing did not return the only created attachment");
        long toTest = 0;
        for (Object[] values : response) {
            toTest = ((Long) values[response.getColumnPos(AttachmentField.CREATION_DATE)]).longValue();
        }
        assertEquals(timestamp, toTest, "time stamp for attachments is wrong in list request");
    }
}
