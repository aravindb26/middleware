...
public abstract class AbstractObjectCountTest extends Abstrac2UserAJAXSession {
    ...
    /**
     * Gets a value indicating whether a certain folder indicates support for the <code>count_total</code> capability.
     * 
     * @param client The underlying ajax client
     ...
     */
    protected static boolean supportsCountTotal(AJAXClient client, int folderId) throws Exception {
        String capabilityName = Strings.asciiLowerCase(com.openexchange.file.storage.FileStorageCapability.COUNT_TOTAL.name());
        int[] columns = com.openexchange.tools.arrays.Arrays.addUniquely(DEFAULT_COLUMNS, FolderObject.CAPABILITIES);
        Folder folder = getFolder(client, folderId, columns);
        Set<String> supportedCapabilities = folder.getSupportedCapabilities();
        return null != supportedCapabilities && supportedCapabilities.contains(capabilityName);
    }

    /**
     * Gets the <code>count_total</code> value for a specific folder, after checking that the <code>count_total</code> capability is properly indicated.
     * 
     * @param client The underlying ajax client
     * @param folderId The identifier of the folder to check
     * @return The total count value
     */
    protected static int getCountTotal(AJAXClient client, int folderId) throws Exception {
        Assumptions.assumeTrue(supportsCountTotal(client, folderId));
        return getFolder(client, folderId, DEFAULT_COLUMNS).getTotal();
    }
