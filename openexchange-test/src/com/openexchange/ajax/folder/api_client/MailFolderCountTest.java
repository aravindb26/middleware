...
public class MailFolderCountTest extends AbstractConfigAwareAPIClientSession {
    ...
    public void setUp(TestInfo testInfo) throws Exception {
        ...
        FolderUpdateResponse createFolder = folderApi.createFolderBuilder().withFolderId(FOLDER).withNewFolderBody(body).withTree("1").execute();
        Assertions.assertNull(createFolder.getErrorDesc(), createFolder.getError());
        Assertions.assertNotNull(createFolder.getData());
    ...
    public void testFolderCount() throws Exception {
        FolderResponse resp = folderApi.getFolder(folderId, "1", null, null, null);
        FolderData folder = checkResponse(resp);
        Assertions.assertEquals(2, folder.getUnread().intValue());

        CONFIG.put("com.openexchange.imap.ignoreDeleted", Boolean.TRUE.toString());
        super.setUpConfiguration();
        resp = folderApi.getFolder(folderId, "1", null, null, null);
        folder = checkResponse(resp);
        Assertions.assertEquals(1, folder.getUnread().intValue());
    }
