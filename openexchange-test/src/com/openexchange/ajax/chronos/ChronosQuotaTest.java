...
public class ChronosQuotaTest extends AbstractChronosTest {
    ...
    public void testExeededQuota() throws Exception {
        ...
        MatcherAssert.assertThat("Can't assign response data to the known output", data, instanceOf(Map.class));

        // Suppress since we already checked ..
        @SuppressWarnings("unchecked") Map<String, Object> info = (Map<String, Object>) data;

        // Check quota info
        MatcherAssert.assertThat("Account identifier does not match", info.get("account_id"), is("0"));
        MatcherAssert.assertThat("Account name does not match", info.get("account_name"), is("Internal Calendar"));
        MatcherAssert.assertThat("Account quota should be null and therefore not be part of the response.", info.get("countquota"), is(nullValue()));
        /*
         * Can't check something like
         * MatcherAssert.assertThat("Account use does not match", info.get("countuse"), is(Integer.valueOf(0)));
        ...
        MatcherAssert.assertThat("No response!", resultResponse, is(not(nullValue())));
        MatcherAssert.assertThat("Event could be saved out of quota limit", resultResponse.getError(), is(not(nullValue())));
        String expectedCode = "QUOTA-0003";
        MatcherAssert.assertThat("Code does not match. Expected '" + expectedCode + "'.", resultResponse.getCode(), is(expectedCode));
    }
