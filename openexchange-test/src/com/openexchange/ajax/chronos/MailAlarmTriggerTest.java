...
public class MailAlarmTriggerTest extends AbstractAlarmTriggerTest {
    ...
    private TimeZone getUserTimeZone() throws ApiException {
        UserResponse userResponse = userApi.getUser((String) null);
        Assertions.assertNull(userResponse.getErrorDesc(), userResponse.getError());
        Assertions.assertNotNull(userResponse.getData());
    ...
    private void checkMail(String summary, Calendar time, List<Long> expectedSentDates, int mails) throws ApiException {
        ...
        Assertions.assertNull(mailResponse.getError());
        Assertions.assertNotNull(mailResponse.getData());
        int found = 0;

        long delta = TimeUnit.SECONDS.toMillis(5);
        for (List<Object> mail : mailResponse.getData()) {
            String subject = mail.get(2).toString();
            if (subject != null && subject.contains(summary)) {
                if (mails == 0) {
                    Assertions.fail("Mail found even though no mail should be sent out.");
                }
                long sentDate = ((Double) mail.get(1)).longValue();
                ...
                element.setId(mail.get(0).toString());
                ...
                Assertions.assertTrue(matchAnySentDate, "Wrong sent date. Expected a maximal difference of " + delta + " but was " + closest);
            }
        }
        if (mails == 0) {
            return;
        }
        Assertions.assertEquals(mails, found, "Found a wrong amount of notification mails.");
    }
