...
public class TimezoneAlarmTriggerTest extends AbstractUserTimezoneAlarmTriggerTest {
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void testTrigger(TimeZone tz, TestInfo testInfo) throws Exception {
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void testTriggerWithPositiveDuration(TimeZone tz, TestInfo testInfo) throws Exception {
        prepareTest(tz, testInfo);
        int currentTriggers = getAlarmTriggers().size();
        Calendar start = DateTimeUtil.getUTCCalendar();
        start.add(Calendar.DAY_OF_MONTH, 1);
