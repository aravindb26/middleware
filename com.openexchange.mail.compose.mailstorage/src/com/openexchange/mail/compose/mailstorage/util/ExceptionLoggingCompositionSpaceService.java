...
public class ExceptionLoggingCompositionSpaceService implements CompositionSpaceService {
    ...
    public MailPath transportCompositionSpace(UUID compositionSpaceId, Optional<StreamedUploadFileIterator> optionalUploadedAttachments, UserSettingMail mailSettings, AJAXRequestData requestData, List<OXException> warnings, DeleteAfterTransportOptions deleteAfterTransportOptions, ClientToken clientToken) throws OXException {
        try {
            return service.transportCompositionSpace(compositionSpaceId, optionalUploadedAttachments, mailSettings, requestData, warnings, deleteAfterTransportOptions, clientToken);
        } catch (OXException e) {
    ...
    @Override
    public AttachmentResult deleteAttachments(UUID compositionSpaceId, List<UUID> attachmentIds, ClientToken clientToken) throws OXException {
        try {
            return service.deleteAttachments(compositionSpaceId, attachmentIds, clientToken);
        } catch (OXException e) {
            LOG.debug("Exception occurred while calling deleteAttachments() with composition space identifier {}", getUnformattedStringObjectFor(compositionSpaceId), e);
            throw e;
        }
    }
