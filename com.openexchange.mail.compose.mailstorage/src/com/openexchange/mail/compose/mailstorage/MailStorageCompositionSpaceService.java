...
public class MailStorageCompositionSpaceService implements CompositionSpaceService {
    ...
    @Override
    public MailPath transportCompositionSpace(UUID compositionSpaceId, Optional<StreamedUploadFileIterator> optionalUploadedAttachments, UserSettingMail mailSettings, AJAXRequestData request, List<OXException> warnings, DeleteAfterTransportOptions deleteAfterTransportOptions, final ClientToken clientToken) throws OXException {
        ...
        try {
            ...
            if (deleteAfterTransportOptions.isDeleteAfterTransport()) {
                try {
                    ...
                    if (null != editFor && false == deleteAfterTransportOptions.isDeleteDraftAfterTransport() && false == MailProperties.getInstance().isDeleteDraftOnTransport(session.getUserId(), session.getContextId())) {
     ...
     * @param warnings
     */
    private static void updateReferencedMessages(Meta meta, Session session, MailServletInterface mailInterface, List<OXException> warnings) {
    ...
    public CompositionSpace openCompositionSpace(OpenCompositionSpaceParameters parameters) throws OXException {
        ...
        try {
            ...
            if (false == referencesOpenCompositionSpace && null != editFor && null != mailInterface) {
                LogProperties.put(LogProperties.Name.MAIL_COMPOSE_LEAVE_SHARED_FOLDER_REFS, "true");
                try {
                    mailInterface.deleteMessages(editFor.getFolderArgument(), new String[] { editFor.getMailID() }, true);
                } catch (Exception e) {
                    LOG.warn("Failed to delete edited draft mail '{}'", editFor, e);
                } finally {
                    LogProperties.remove(LogProperties.Name.MAIL_COMPOSE_LEAVE_SHARED_FOLDER_REFS);
    ...
    public AttachmentResult addAttachmentToCompositionSpace(UUID compositionSpaceId, AttachmentDescription attachmentDesc, InputStream data, ClientToken clientToken) throws OXException {
        try {
            ...
            LOG.debug("Adding attachment with disposition {} to composition space: {}", attachmentDesc.getContentDisposition(), getUUIDForLogging(compositionSpaceId));
            ...
            try {
                ...
                Lock operationLock = lookUpResult.getAssociation().getOperationLock();
                boolean locked = acquiredOperationLock(operationLock);
                try {
                    if (!locked) {
                        throw CompositionSpaceErrorCode.CONCURRENT_OPERATION.create(UUIDs.getUnformattedString(compositionSpaceId));
                    }
                ...
                throw CompositionSpaceErrorCode.OPERATION_ABORTED.create(e, UUIDs.getUnformattedString(compositionSpaceId));
            } finally {
        ...
        try {
            ...
            try {
                for (AttachmentDescriptionAndData descriptionAndData : descriptionsAndDatas) {
                    ...
                    attachmentDesc.setCompositionSpaceId(compositionSpaceId);
                    newAttachments.add(spoolUploadFile(attachmentDesc, descriptionAndData.getData()));
                }
                ...
                try {
                    ...
                    long totalSize = determineSize(descriptionsAndDatas);
                    checkStorageQuota(lookUpResult.getAssociation(), totalSize, ContentDisposition.ATTACHMENT);
                    checkMaxMailSize(lookUpResult.getAssociation(), totalSize);

                    List<Attachment> newAttachs = newAttachments;
                    MailStorageCallable<NewAttachmentsInfo> addAttachmentsCallable = new MailStorageCallable<NewAttachmentsInfo>() {

                        @Override
                        public MailStorageResult<NewAttachmentsInfo> call(LookUpResult lookUpResult, IMailStorage mailStorage, Session session, ClientToken clientToken) throws OXException, MissingDraftException {
                            return mailStorage.addAttachments(lookUpResult.getAssociation(), newAttachs, clientToken, session);
                        }
                    };
                    NewAttachmentsInfo result = MailStorageExclusiveOperation.performOperation(lookUpResult, this, addAttachmentsCallable, clientToken);
                    newAttachments = result.getNewAttachments();

                    AttachmentResult retval = attachmentResultFor(newAttachments, getCurrentCompositionSpaceInfo(compositionSpaceId, result.getLastModified()));
    ...
    private static long determineSize(List<AttachmentDescriptionAndData> descriptionsAndDatas) {
        long total = 0L;
        for (AttachmentDescriptionAndData descriptionAndData : descriptionsAndDatas) {
            long size = descriptionAndData.getAttachmentDescription().getSize();
            if (size < 0) {
                // Unknown
                return -1L;
            }
    ...
    public List<CompositionSpace> getCompositionSpaces(MessageField[] fields) throws OXException {
        return doGetCompositionSpaces(fields, true);
    }

    /**
     * Queries available composition spaces from storage.
     *
     * @param fields The fields to pre-fill
     * @param returnSpaces Whether to return composition spaces at all
     * @return The queried composition spaces; unless <code>returnSpaces</code> is set to <code>false</code>
     * @throws OXException If operation fails
     */
    List<CompositionSpace> doGetCompositionSpaces(MessageField[] fields, boolean returnSpaces) throws OXException {
        ...
        if (!associations.isEmpty()) {
            existingDraftPaths = new LinkedHashMap<>(existingDraftPaths);

            for (CompositionSpaceToDraftAssociation association : associations) {
                MailPath draftPath = association.getDraftPath();
                if (!existingDraftPaths.containsKey(draftPath)) {
                    UUID compositionSpaceId = association.getCompositionSpaceId();

                    // Look-up by draft path
                    MailStorageResult<Optional<MailStorageId>> lookUpResult = mailStorage.lookUp(draftPath, session);
                    Optional<MailStorageId> optionalMailStorageId = lookUpResult.getResult();
                    if (optionalMailStorageId.isPresent()) {
                        if (!compositionSpaceId.equals(optionalMailStorageId.get().getCompositionSpaceId())) {
                            // Look-up by composition space identifier since UUIDs mismatch
                            lookUpResult = mailStorage.lookUp(compositionSpaceId, session);
                            optionalMailStorageId = lookUpResult.getResult();
                        }
                    } else {
                        // Look-up by composition space identifier
                        lookUpResult = mailStorage.lookUp(compositionSpaceId, session);
                        optionalMailStorageId = lookUpResult.getResult();
                    }
        ...
        if (existingDraftPaths.isEmpty()) {
            ...
            LOG.debug("Verified no open composition spaces exist. Dropping cache entries & signaling no available composition spaces.");
    ...
    @Override
    public AttachmentResult deleteAttachments(UUID compositionSpaceId, List<UUID> attachmentIdsToDelete, ClientToken clientToken) throws OXException {
        List<UUID> attachmentIds = attachmentIdsToDelete == null ? null : attachmentIdsToDelete.stream().filter(Objects::nonNull).collect(Collectors.toList());
        if (attachmentIds == null || attachmentIds.isEmpty()) {
            CompositionSpace compositionSpace = getCompositionSpace(compositionSpaceId);
            return AttachmentResults.attachmentResultFor(compositionSpace);
        }

        if (attachmentIds.isEmpty()) {
            throw CompositionSpaceErrorCode.ERROR.create("Attachment identifiers must not be empty");
        }
        ...
        try {
            ...
            try {
                ...
                MailStorageCallable<MessageInfo> deleteAttachmentsCallable = new MailStorageCallable<MessageInfo>() {
                    ...
                    @Override
                    public MailStorageResult<MessageInfo> call(LookUpResult lookUpResult, IMailStorage mailStorage, Session session, ClientToken clientToken) throws OXException, MissingDraftException {
                        return mailStorage.deleteAttachments(lookUpResult.getAssociation(), attachmentIds, clientToken, session);
                    }
                };
                MessageInfo messageInfo = MailStorageExclusiveOperation.performOperation(lookUpResult, this, deleteAttachmentsCallable, clientToken);
                return attachmentResultFor(getCurrentCompositionSpaceInfo(compositionSpaceId, messageInfo.getLastModified()));
    ...
    private List<Attachment> spoolUploadFiles(UUID compositionSpaceId, StreamedUploadFileIterator uploadedAttachments, ContentDisposition contentDisposition) throws OXException {
        ...
        try {
            ...
            newAttachments = null; // Avoid premature closing
            return retval;
        } finally {
            if (newAttachments != null) {
    ...
    private Attachment spoolNextUploadFile(UUID compositionSpaceId, @Nullable UUID attachmentId, StreamedUploadFileIterator uploadedAttachments, ContentDisposition contentDisposition) throws OXException {
        if (!uploadedAttachments.hasNext()) {
            // No further upload file available
            return null;
        }
        ...
        try {
            return spoolUploadFile(attachmentDesc, uploadFile.getStream());
        } catch (IOException e) {
    ...
    private Attachment spoolUploadFile(AttachmentDescription attachmentDesc, InputStream data) throws OXException {
        ...
        try {
            ...
            Attachment newAttachment = attachment.build();
    ...
    private static boolean acquiredOperationLock(Lock operationLock) throws OXException, InterruptedException {
        long operationWaitTime = MailStorageCompositionSpaceConfig.getInstance().getOperationWaitTime();
        if (operationWaitTime <= 0) {
            // Wait forever
            operationLock.lockInterruptibly();
            return true;
        }

        // Wait until operation wait time is elapsed
        return operationLock.tryLock(operationWaitTime, TimeUnit.MILLISECONDS);
    }
