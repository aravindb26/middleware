...
public final class ListAction extends AbstractFolderAction {
    ...
    protected AJAXRequestResult doPerform(final AJAXRequestData request, final ServerSession session) throws OXException {
        ...
        if (length <= 0) {
            ...
            return new AJAXRequestResult(JSONArray.EMPTY_ARRAY, dNull).addWarnings(subfoldersResponse.getWarnings());
        }
        ...
        if (filterDuplicateNames) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                final String name = userizedFolder.getName(locale);
    ...
    public boolean accessAllowed(AJAXRequestData request, ServerSession session, Scope scope) throws OXException {
        ...
        boolean all;
        {
            String parameter = request.getParameter(AJAXServlet.PARAMETER_ALL);
            all = "1".equals(parameter) || Boolean.parseBoolean(parameter);
        }

        FolderService folderService = ServiceRegistry.getInstance().getService(FolderService.class, true);
        FolderResponse<UserizedFolder[]> subfoldersResponse =
            folderService.getSubfolders(
                treeId,
                parentId,
                all,
        ...
        if (filterDuplicateNames) {
            for (int i = 0; i < length; i++) {
                UserizedFolder userizedFolder = subfolders[i];
                if (!RestrictedActionUtil.mayReadWithScope(userizedFolder.getContentType(), scope)) {
                    return false;
                }
            }
        } else {
