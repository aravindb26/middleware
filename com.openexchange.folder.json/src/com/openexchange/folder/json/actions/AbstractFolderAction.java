...
public abstract class AbstractFolderAction implements AJAXActionService {
    ...
    protected static Boolean isSuppressUnifiedMail(final AJAXRequestData request) {
        ...
        return Boolean.valueOf((ses != null && isUsmEas(ses.getClient())) || RestrictedActionUtil.isRequestRestricted(request));
    }
     ...
     * @return The default allowed modules, or an empty list if not applicable (no restricted scope set in session)
     * @throws OXException
     */
    protected static List<ContentType> getDefaultAllowedModules(final AJAXRequestData request) throws OXException {
        Scope passedScope = RestrictedActionUtil.obtainPassedScopes(request.getSession(), request);
        if (null != passedScope) {
            return new ArrayList<ContentType>(getReadableContentTypesForRequest(passedScope));
        }
    ...
    protected static List<ContentType> collectAllowedContentTypes(final AJAXRequestData request) throws OXException {
        ...
        Scope passedScopes = RestrictedActionUtil.obtainPassedScopes(request.getSession(), request);
        Set<ContentType> oAuthWhitelist = null != passedScopes ? getReadableContentTypesForRequest(passedScopes) : null;
        /*
            ...
            if (null != oAuthWhitelist) {
                for (ContentType matchingContentType : matchingContentTypes) {
                    if (false == oAuthWhitelist.contains(matchingContentType)) {
                        throw RestrictedActionExceptionCodes.INSUFFICIENT_SCOPES.create(RestrictedActionUtil.readScopeForContentType(matchingContentType));
                    }
                }
            }
    ...
    protected static ContentType parseAndCheckContentTypeParameter(final String parameterName, final AJAXRequestData request) throws OXException {
        ...
        if (RestrictedActionUtil.isRequestRestricted(request)) {
            oAuthWhitelist = getReadableContentTypesForRequest(RestrictedActionUtil.obtainPassedScopes(request.getSession(), request));
        }
        if (oAuthWhitelist != null && !containsContentTypeName(oAuthWhitelist, contentType)) {
            throw RestrictedActionExceptionCodes.INSUFFICIENT_SCOPES.create(RestrictedActionUtil.readScopeForContentType(contentType));
        }
    ...
    /**
     * Gets all content types whose folders are readable for the specified scope
     *
     * @param scope The scope
     * @return A set of content types
     */
    protected static Set<ContentType> getReadableContentTypesForRequest(Scope scope) {
        Set<ContentType> contentTypes = new HashSet<>();
        contentTypes.add(SystemContentType.getInstance());
        for (String s : scope.get()) {
            Set<ContentType> contentType = RestrictedActionUtil.contentTypesForReadScope(s);
