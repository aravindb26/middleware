...
import static com.openexchange.chronos.itip.json.actions.Utils.convertToResult;
...
/**
 * {@link LegacySchedulingAction} - For legacy actions
 *
 * @author <a href="mailto:daniel.becker@open-xchange.com">Daniel Becker</a>
 * @since v7.10.6
 */
public class LegacySchedulingAction extends AbstractSchedulingAction {

    private static final Logger LOGGER = LoggerFactory.getLogger(LegacySchedulingAction.class);

    private final List<SchedulingMethod> methods;

    /**
     * Initializes a new {@link LegacySchedulingAction}.
     * 
     * @param method The scheduling method to handle
     * @param services The service lookup
     */
    public LegacySchedulingAction(ServiceLookup services, SchedulingMethod... method) {
        super(services);
    ...
    AJAXRequestResult process(AJAXRequestData requestData, CalendarSession calendarSession, IncomingSchedulingMessage message) throws OXException {
        if (canPerform(message)) {
            return perform(requestData, message, calendarSession, Utils.getTimeZone(requestData, calendarSession.getSession()));
        }
        return null;
    }
    ...
    public AJAXRequestResult perform(AJAXRequestData request, IncomingSchedulingMessage message, CalendarSession session, TimeZone tz) throws OXException {
         ...
         */
        services.getServiceSafe(MessageStatusService.class).setMessageStatus(session.getSession(), message, MessageStatus.APPLIED);
        /*
        ...
        try {
            return convertToResult(services, session, tz, updatedEvents);
     ...
     * @return The user attendee with the new participant status to reply with or <code>null</code> to not trigger a participant status update
     */
