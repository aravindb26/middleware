...
public class SocketLoggerCLT extends AbstractRmiCLI<Void> {
    ...
    protected Void invoke(Options options, CommandLine cmd, String optRmiHostName) throws Exception {
        try {
            ...
            return register == null ? listLoggers(cmd, service) : manageLogger(cmd, service);
     ...
     * @throws RemoteException if an error is occurred
     */
    private Void manageLogger(CommandLine cmd, SocketLoggerRMIService service) throws RemoteException {
        String adminUser = cmd.getOptionValue('A');
        String adminPassword = cmd.getOptionValue('P');

        if (register.booleanValue()) {
            service.registerLoggerFor(loggerName, new Credentials(adminUser, adminPassword));
        } else {
            service.unregisterLoggerFor(loggerName, new Credentials(adminUser, adminPassword));
        }
        return null;
    }
}
