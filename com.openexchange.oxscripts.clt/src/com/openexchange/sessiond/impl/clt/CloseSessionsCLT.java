...
public final class CloseSessionsCLT extends AbstractRmiCLI<Void> {
    ...
    protected Void invoke(Options options, CommandLine cmd, String optRmiHostName) throws Exception {
        ...
        if (userId > 0) {
            if (global) {
                rmiService.clearUserSessionsGlobally(userId, contextId, new Credentials(adminUser, adminPassword));
                System.out.println("Globally cleared sessions for user " + userId + " in context " + contextId);
            } else {
                rmiService.clearUserSessions(userId, contextId, new Credentials(adminUser, adminPassword));
                System.out.println("Locally cleared sessions for user " + userId + " in context " + contextId);
            }
            return null;
        }
        if (global) {
            rmiService.clearContextSessionsGlobally(contextId, new Credentials(adminUser, adminPassword));
            System.out.println("Globally cleared sessions for context " + contextId);
        } else {
            rmiService.clearContextSessions(contextId, new Credentials(adminUser, adminPassword));
