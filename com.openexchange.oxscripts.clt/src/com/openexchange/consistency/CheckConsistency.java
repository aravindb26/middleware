...
public class CheckConsistency extends AbstractRmiCLI<Void> {
    ...
    protected Void invoke(Options options, CommandLine cmd, String optRmiHostName) throws Exception {
        ...
        switch (action) {
            case check_configdb:
                ...
                printList(rmiService.checkOrRepairConfigDB(false, new Credentials(adminUser, adminPassword)));
            ...
            case repair_configdb:
                ...
                printList(rmiService.checkOrRepairConfigDB(true, new Credentials(adminUser, adminPassword)));
                break;
            case list_missing:
            ...
            case list_unassigned:
                ...
                break;
            case repair:
                repair(rmiService, new Credentials(adminUser, adminPassword));
                break;
            default:
                // Should never happen
     ...
     * @throws RemoteException if a remote error is occurred
     */
    private void repair(ConsistencyRMIService rmiService, Credentials auth) throws RemoteException {
        switch (source) {
            case all:
                rmiService.repairAllFiles(policy.name(), policyAction.name(), auth);
                break;
            case context:
                rmiService.repairFilesInContext(sourceId, policy.name(), policyAction.name(), auth);
                break;
            case database:
                rmiService.repairFilesInDatabase(sourceId, policy.name(), policyAction.name(), auth);
                break;
            case filestore:
                rmiService.repairFilesInFilestore(sourceId, policy.name(), policyAction.name(), auth);
                break;
            default:
                System.err.println("Invalid source '" + source + "'");
                printHelp();
