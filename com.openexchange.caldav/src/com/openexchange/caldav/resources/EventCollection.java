...
public class EventCollection extends CalDAVFolderCollection<Event> {
    ...
    protected static final EventField[] BASIC_FIELDS = new EventField[] {
        ...
        EventField.CLASSIFICATION, EventField.START_DATE, EventField.END_DATE, EventField.RECURRENCE_RULE, EventField.DTSTAMP,
            ...
            try {
                syncToken = new CalendarAccessOperation<String>(factory) {
                    ...
                    protected String perform(IDBasedCalendarAccess access) throws OXException {
                        SequenceResult result = access.getSequenceNumbers(Collections.singletonList(folderID)).get(folderID);
                        return new SyncToken(result.getTimestamp(), Long.toString(result.getTotalCount())).toString();
                    }
    ...
    protected SyncStatus<WebdavResource> getSyncStatus(SyncToken syncToken, int limit) throws OXException {
        ...
        /*
         * encode total number of events in folder as 'additional' info into sync-token to recognize stale deletions
         */
        String additional = Long.toString(updates.getTotalCount());
        /*
        ...
        if (updates.isTruncated()) {
            ...
            nextSyncToken = new SyncToken(updates.getTimestamp(), additional, Flag.TRUNCATED);
        } else {
            nextSyncToken = new SyncToken(updates.getTimestamp(), additional);
        }
        /*
