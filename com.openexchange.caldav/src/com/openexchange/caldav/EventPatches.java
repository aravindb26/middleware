...
public class EventPatches {
    ...
    public static ICalParameters applyIgnoredProperties(EventResource resource, ICalParameters parameters) {
        if (null != parameters) {
            ...
            if (false == DAVUserAgent.EM_CLIENT.equals(userAgent)) {
                /*
                 * forcibly ignore X-MICROSOFT-CDO-ALLDAYEVENT, X-MICROSOFT-CDO-INTENDEDSTATUS and X-MICROSOFT-CDO-BUSYSTATUS for clients other than em client
                 */
                parameters.set(ICalParameters.IGNORED_PROPERTIES, new String[] { "X-MICROSOFT-CDO-ALLDAYEVENT", "X-MICROSOFT-CDO-BUSYSTATUS", "X-MICROSOFT-CDO-INTENDEDSTATUS" });
            }
            /*
    ...
    public static final class Incoming {
                    ...
                    if (null != originalOccurrence) {
                        ...
                        if (null != originalAlarm) {
                            ...
                            EventUpdate eventUpdate = DefaultEventUpdate.builder()
                                ...
                                .ignoredEventFields(EventField.TIMESTAMP, EventField.LAST_MODIFIED, EventField.SEQUENCE, EventField.RECURRENCE_RULE, EventField.RECURRENCE_ID, EventField.CREATED, EventField.EXTENDED_PROPERTIES, EventField.DTSTAMP)
                                .ignoredEventFields(AttendeeField.TIMESTAMP, AttendeeField.SEQUENCE)
                                .considerUnset(true)
                                .ignoreDefaults(true)
                            .build();
        ...
        /**
         * Removes all <code>X-LIC-ERROR</code> properties that may have been inserted by Mozilla/Lightning from the imported event, as well as its alarm components.
         * 
         * @param resource The event resource
         * @param importedEvent The event being imported
         */
        private static void removeMozillaParserErrors(EventResource resource, Event importedEvent) {
            if (DAVUserAgent.THUNDERBIRD_LIGHTNING.equals(resource.getUserAgent())) {
                removeExtendedProperties(importedEvent.getExtendedProperties(), Lightning.X_LIC_ERROR.getId());
                if (null != importedEvent.getAlarms()) {
                    for (Alarm alarm : importedEvent.getAlarms()) {
                        removeExtendedProperties(alarm.getExtendedProperties(), Lightning.X_LIC_ERROR.getId());
                    }
                }
            }
        }
        ...
        private static void applyCalendarserverAccess(EventResource eventResource, CalDAVImport caldavImport) {
            if (DAVUserAgent.IOS.equals(eventResource.getUserAgent()) || DAVUserAgent.MAC_CALENDAR.equals(eventResource.getUserAgent())) {
                ...
                if (null == property || false == (property.getValue() instanceof String) || Strings.isEmpty((String) property.getValue())) {
                    return;
                }
                        ...
                        for (RecurrenceId exceptionDate : originalSeriesMaster.getChangeExceptionDates()) {
                            ...
                            if (null != matchingExceptionDate && now.after(exceptionDate.getValue())) {
                                ...
                                if (null != attendee && ParticipationStatus.DECLINED.matches(attendee.getPartStat())) {
                                     ...
                                     */
                                    LOG.info("Restoring previously declined occurrence {} for indicated delete exception at {}", originalChangeException, matchingExceptionDate);
        ...
        public CalDAVImport applyAll(EventResource resource, CalDAVImport caldavImport) {
            ...
            if (null != importedEvent) {
                ...
                removeMozillaParserErrors(resource, importedEvent);
            ...
            if (null != importedChangeExceptions && 0 < importedChangeExceptions.size()) {
                ...
                for (Event importedChangeException : importedChangeExceptions) {
                    ...
                    removeMozillaParserErrors(resource, importedChangeException);
    ...
    public static final class Outgoing {
        ...
        public static CalendarExport applyExport(EventResource eventResource, CalendarExport calendarExport, List<Event> exportedEvents) throws OXException {
            ...
            if (DAVUserAgent.THUNDERBIRD_LIGHTNING.equals(eventResource.getUserAgent()) && (eventResource.getEvent() instanceof PhantomMaster)) {
                // Add fake master
