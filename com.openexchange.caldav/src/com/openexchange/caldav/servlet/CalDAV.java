...
public class CalDAV extends DAVServlet {
    ...
    protected boolean checkPermission(HttpServletRequest request, WebdavMethod method, ServerSession session) {
        ...
        try {
            Boolean b = performer.getFactory().requireService(ConfigViewFactory.class).getView(session.getUserId(), session.getContextId()).property("com.openexchange.caldav.enabled", Boolean.class).get();
            enabled = b != null && b(b);
        } catch (OXException e) {
