...
public class GoogleContactsSubscribeService extends AbstractOAuthSubscribeService {
    ...
    public void ping(Session session, OAuthAccount account) throws OXException {
        try {
            getPeopleService(session, account).people().connections().list(RESSOURCE).setPageSize(I(1)).setPersonFields(PersonFields.NAMES.getName()).execute();
        } catch (IOException e) {
            LOG.error("", e);
            throw SubscriptionErrorMessage.IO_ERROR.create(e, e.getMessage());
        }
     ...
     */
    private PeopleService createPeopleService(Session session, OAuthAccount oauthAccount) throws OXException, IOException {
        // @formatter:off
    ...
    protected void fetchContacts(PeopleService googlePeopleService, String nextPageToken, Subscription subscription, Consumer<ListConnectionsResponse> responseHandler) throws IOException {
        ...
        while (pageToken != null) {
            ...
            ListConnectionsResponse response = googlePeopleService.people().connections().list(RESSOURCE).setPageSize(I(CHUNK_SIZE)).setPageToken(pageToken).setPersonFields(PERSON_FIELDS).execute();
