...
public class UpdateTest extends FileActionTest {
    ...
    public void testNoUpload() throws JSONException, OXException {
        ...
        request(Optional.ofNullable(file)).param("timestamp", "1337").body(new JSONObject("{id: '23', folder_id: '12', title: 'nice title'}"));

        fileAccess().expectCall("saveFileMetadata", file, L(1337l), Arrays.asList(File.Field.FOLDER_ID, File.Field.TITLE, File.Field.ID), Boolean.FALSE, Boolean.FALSE);
    ...
    public void testMissingId() throws JSONException {
        DefaultFile file = new DefaultFile();
        request(Optional.ofNullable(file)).param("timestamp", "1337").body(new JSONObject("{folder_id: '12', title: 'nice title'}"));
        try {
            action.handle(request);
            fail("Expected Exception due to missing parameters");
        } catch (OXException x) {
