...
public class DefaultRegistrationPerformer implements RegistrationPerformer {
    ...
    private String getLoginFor(RegistrationContext registrationContext, DoveAdmClient doveAdmClient) throws OXException {
        ...
        String login;
        {
            Session optionalSession = registrationContext.getSession();
            if (optionalSession == null) {
                login = MailConfig.getMailLogin(defaultMailAccount, user.getLoginInfo(), userId, contextId);
            } else {
                login = MailConfig.getLogin(defaultMailAccount, user.getLoginInfo(), userId, contextId, true, Optional.of(optionalSession));
            }
        }
    ...
    private RegistrationResult initateRegistration(Session session) {
        ...
        try {
            ...
            // Get IMAP store
            final IMAPStore imapStore = IMAPAccess.getIMAPFolderStorageFrom(mailAccess).getImapStore();
