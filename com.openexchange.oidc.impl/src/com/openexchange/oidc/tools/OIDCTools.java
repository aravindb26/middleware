...
public class OIDCTools {
    ...
    public static final String IDTOKEN = Session.PARAM_OAUTH_ID_TOKEN;
    ...
    public static OAuthTokens convertNimbusTokens(Tokens tokens) {
        ...
        if (tokens instanceof OIDCTokens oidcTokens) {
            return new OAuthTokens(tokens.getAccessToken().getValue(), expiryDate, refreshTokenValue, oidcTokens.getIDTokenString());
        }
    ...
    /**
     * Remembers an OIDC to OX session id mapping within the given state management instance.
     *
     * @param stateManagement The state management instance to use
     * @param session The session to remember the id association for
     * @param idToken The ID token to extract the OIDC session id from
     * @return <code>true</code> if the session ids could be extracted and remembered successfully, <code>false</code>, otherwise
     */
    public static boolean rememberSessionIds(StateManagement stateManagement, Session session, String idToken) {
        /*
         * extract sid from id token
         */
        String sid = optOidcSessionId(idToken);
        if (Strings.isEmpty(sid)) {
            LOG.warn("Unable to extract OIDC session from \"{}\", unable to remember session id.", idToken);
            return false;
        }
        /*
         * derive ttl from refresh token if possible
         */
        long ttl = -1l;
        try {
            // @formatter:off
            ttl = l(Services.getServiceSafe(SessionOAuthTokenService.class).getFromSession(session)
                                                                           .map(tokens -> L(tokens.getExpiresInMillis()))
                                                                           .orElse(L(-1l)));
            // @formatter:on
        } catch (OXException e) {
            LOG.warn("Unexepcted error getting access token to derive session mapping TTL, falling back to defaults.", e);
        }
        try {
            stateManagement.rememberSession(sid, session.getSessionID(), ttl, TimeUnit.MILLISECONDS);
            LOG.trace("Successfully remembered oidc to ox session mapping for \"{}\" -> \"{}\" with TTL {}ms.", sid, session.getSessionID(), L(ttl));
            return true;
        } catch (OXException e) {
            LOG.warn("Unexepcted error remembering oidc to ox session mapping for \"{}\": {}", session.getSessionID(), e.getMessage(), e);
        }
        return false;
    }

    /**
     * Removes an OIDC to OX session id mapping within the given state management instance.
     *
     * @param stateManagement The state management instance to use
     * @param idToken The ID token to extract the OIDC session id from
     * @return <code>true</code> if the session ids could be extracted and removed successfully, <code>false</code>, otherwise
     */
    public static boolean forgetSessionIds(StateManagement stateManagement, String idToken) {
        /*
         * extract sid from id token
         */
        String sid = optOidcSessionId(idToken);
        if (Strings.isEmpty(sid)) {
            LOG.warn("Unable to extract OIDC session from \"{}\", unable to remove session id association.", idToken);
            return false;
        }
        String sessionId;
        try {
            sessionId = stateManagement.getAndRemoveOXSessionId(sid);
        } catch (OXException e) {
            LOG.warn("Unexepcted error removing oidc to ox session mapping for \"{}\": {}", sid, e.getMessage(), e);
            return false;
        }
        if (null != sessionId) {
            LOG.trace("Successfully removed oidc to ox session mapping for \"{}\".", sid);
            return true;
        }
        LOG.trace("No previous oidc to ox session mapping for \"{}\" found.", sid);
        return false;
    }

    /**
     * Optionally parses and extracts the <code>sid</code> claim, i.e. the OPs session identifier, from the supplied id token string.
     *
     * @param idToken The id token to extract the OIDC session identifier from
     * @return The OIDC session identifier, or <code>null</code> if none could be extracted
     */
    public static String optOidcSessionId(String idToken) {
        if (Strings.isNotEmpty(idToken)) {
            try {
                JWTClaimsSet claimsSet = JWTParser.parse(idToken).getJWTClaimsSet();
                return null != claimsSet ? claimsSet.getStringClaim("sid") : null;
            } catch (Exception e) {
                LOG.debug("Error getting \"sid\" from id token {}", idToken, e);
            }
        }
        return null;
    }
