...
public abstract class AbstractOIDCBackend implements OIDCBackend {
    ...
    public OIDCExceptionHandler getExceptionHandler() {
        return new OIDCCoreExceptionHandler(this.getBackendConfig(), Services.getOptionalService(SegmentedUpdateService.class));
    }
    ...
    public IDTokenClaimsSet validateIdToken(JWT idToken, String nonce) throws OXException {
        ...
        try {
            ...
            // @formatter:off
            IDTokenValidator idTokenValidator = new IDTokenValidator(new Issuer(config.getOpIssuer()),
                                                                     new ClientID(config.getClientID()),
                                                                     expectedJWSAlg,
                                                                     new URL(config.getOpJwkSetEndpoint()),
                                                                     resourceRetriever);
            // @formatter:on
    ...
    @Override
    public LogoutTokenClaimsSet validateLogoutToken(JWT logoutToken) throws OXException {
        LOG.trace("IDTokenClaimsSet validateLogoutToken(JWT logoutToken: {})", logoutToken.getParsedString());
        JWSAlgorithm expectedJWSAlg = this.getJWSAlgorithm();
        try {
            ResourceRetriever resourceRetriever = new HttpClientResourceRetriever(() -> {
                HttpClientService httpClientService = Services.getOptionalService(HttpClientService.class);
            ...
            LogoutTokenValidator logoutTokenValidator = new LogoutTokenValidator(new Issuer(this.getBackendConfig().getOpIssuer()),
                                                                                 new ClientID(this.getBackendConfig().getClientID()),
                                                                                 expectedJWSAlg,
                                                                                 new URL(this.getBackendConfig().getOpJwkSetEndpoint()),
                                                                                 resourceRetriever);
            ...
            throw OIDCExceptionCode.LOGOUTTOKEN_VALIDATON_FAILED.create(e, "");
        } catch (MalformedURLException e) {
            throw OIDCExceptionCode.LOGOUTTOKEN_VALIDATON_FAILED.create(e, "Unable to parse JWKSet URL");
        }
    }
    ...
    public LoginRequest getLoginRequest(HttpServletRequest request, int userID, int contextID, LoginConfiguration loginConfiguration) throws OXException {
        Optional<String> optLogin = buildLoginFromRequestAttributes(request);
        String login = optLogin.isPresent() ? optLogin.get() : buildLoginFromLoginInfo(userID, contextID);
    ...
    /**
     * Tries to build login from given request's attributes; provided that formerly set in 'c.o.oidc.impl.OIDCLoginRequestHandler.OIDCHandler.loginUser()'.
     *
     * @param request The HTTP request
     * @return The login or empty
     */
    private Optional<String> buildLoginFromRequestAttributes(HttpServletRequest request) {
        String contextInfo = (String) request.getAttribute(OIDCTools.CONTEXT_LOGIN_INFO);
        if (Strings.isEmpty(contextInfo)) {
            return Optional.empty();
        }

        String userName = (String) request.getAttribute(OIDCTools.USER_LOGIN_INFO);
        if (Strings.isEmpty(userName)) {
            return Optional.empty();
        }

        return Optional.of(new StringBuilder(userName).append('@').append(contextInfo).toString());
    }

    /**
     * Builds the login from stored login information of given user/context.
     *
     * @param userID The user identifier
     * @param contextID The context identifier
     * @return The login build from login information
     * @throws OXException If user/context information cannot be obtained
     */
    private String buildLoginFromLoginInfo(int userID, int contextID) throws OXException {
        ...
        // Grab login info
        String ctxLoginInfo = null;
        String[] ctxLoginInfos = context.getLoginInfo();
        for (int j = 0; ctxLoginInfo == null && j < ctxLoginInfos.length; j++) {
            String li = ctxLoginInfos[j];
            if (Strings.parsePositiveInt(li) >= 0) {
                // Context identifier. Skip...
            } else {
                ctxLoginInfo = li;
            }
        }
        return user.getLoginInfo() + '@' + (ctxLoginInfo == null ? Integer.toString(contextID) : ctxLoginInfo);
    }
    ...
    @Override
    public AuthenticationInfo resolveAuthenticationResponse(com.openexchange.authentication.AuthenticationRequest authenticationRequest, OIDCTokenResponse tokenResponse) throws OXException {
        return resolveAuthenticationResponse(tokenResponse);
    }

    private AuthenticationInfo resolveAuthenticationResponse(OIDCTokenResponse tokenResponse) throws OXException {
        AuthenticationInfo resultInfo = resolveLoginInfo(extractLoginInfo(tokenResponse.getOIDCTokens().getIDToken()));
        resultInfo.setProperty(AUTH_RESPONSE, tokenResponse.toJSONObject().toJSONString());
        return resultInfo;
    }

    @Override
    public AuthenticationInfo resolveLoginInfo(Authenticated loginInfo) throws OXException {
        return loadUserFromServer(loginInfo.getContextInfo(), loginInfo.getUserInfo());
    }

    @Override
    public Authenticated extractLoginInfo(JWT idToken) throws OXException {
        if (null == idToken) {
        ...
        try {
            ...
            LOG.debug("Extracted the following context claim for \"{}\": {}", config.getContextLookupClaim(), contextClaim);
            ...
            LOG.debug("Extracted the following user claim for \"{}\": {}", config.getUserLookupClaim(), userClaim);
        ...
        LOG.debug("Successfully extracted the following names for context/user look-up: context-name={} | user-name={}", contextName, userName);
        return new DefaultAuthenticated(contextName, userName);
    }
    ...
    /**
     * Helper method to update a session.
     *
     * @param session The session that should be updated.
     * @throws OXException In case something went wrong.
     */
    private void storeSession(Session session) throws OXException {
