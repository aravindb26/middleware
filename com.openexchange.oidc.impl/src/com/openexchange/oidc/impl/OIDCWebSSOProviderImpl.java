...
public class OIDCWebSSOProviderImpl implements OIDCWebSSOProvider {
                ...
                try {
                    ...
                    if (null != sessionOAuthTokenService) {
                        ...
                        try {
                            ...
                            if (result.isSuccess()) {
                                /*
                                 * if successfully refreshed, keep remembering oidc -> ox session id mapping in state
                                 */
                                if (SuccessReason.REFRESHED.equals(result.getSuccessReason()) && (config.isFrontchannelLogoutEnabled() || config.isBackchannelLogoutEnabled())) {
    ...
    private String buildLoginRequest(State state, Nonce nonce, HttpServletRequest request) throws OXException {
        ...
        if (targetLinkUri != null) {
            if (!targetLinkUri.equals(OIDCTools.getUIWebPath(backend.getLoginConfiguration(), backend.getBackendConfig()))) {
                throw OIDCExceptionCode.INVALD_TARGET_LINK_URI.create("Given target_link_uri does not match the specifications defined in com.openexchange.oidc.uiWebPath.");
            }
        }
    ...
    public String getLogoutRedirectRequest(HttpServletRequest request, HttpServletResponse response) throws OXException {
        ...
        if (this.backend.getBackendConfig().isSSOLogout()) {
            ...
            /*
             * if regular logout is performed, prevent redundant front-/backchannel logout by removing mapped oidc session beforehand
             */
            if (backend.getBackendConfig().isBackchannelLogoutEnabled() || backend.getBackendConfig().isFrontchannelLogoutEnabled()) {
                LOG.trace("Clearing oidc to ox session mapping to prevent redundant front-/backchannel logout.");
                OIDCTools.forgetSessionIds(stateManagement, (String) session.getParameter(OIDCTools.IDTOKEN));
            }
        } else {
    ...
    @Override
    public void backchannelLogout(HttpServletRequest request, HttpServletResponse response) throws OXException {
        LOG.trace("backchannelLogout(HttpServletRequest request: {}, HttpServletResponse response)", request.getRequestURI());
        if (false == backend.getBackendConfig().isBackchannelLogoutEnabled()) {
            throw OIDCExceptionCode.INVALID_LOGOUT_REQUEST.create("Backchannel logout disabled by configuration");
        }
        /*
         * parse oidc session from logout token
         */
        String logoutTokenParameter = request.getParameter("logout_token");
        if (Strings.isEmpty(logoutTokenParameter)) {
            throw OIDCExceptionCode.INVALID_LOGOUT_REQUEST.create("No logout_token request parameter found");
        }
        JWT jwt;
        try {
            jwt = JWTParser.parse(logoutTokenParameter);
        } catch (java.text.ParseException e) {
            LOG.warn("Error parsing {}: {}", logoutTokenParameter, e.getMessage(), e);
            throw OIDCExceptionCode.LOGOUTTOKEN_VALIDATON_FAILED.create(e, e.getMessage());
        }
        LogoutTokenClaimsSet logoutToken;
        try {
            logoutToken = backend.validateLogoutToken(jwt);
        } catch (OXException e) {
            LOG.warn("Error validating {}: {}", logoutTokenParameter, e.getMessage(), e);
            throw OIDCExceptionCode.LOGOUTTOKEN_VALIDATON_FAILED.create(e, e.getMessage());
        }
        String sid = null != logoutToken.getSessionID() ? logoutToken.getSessionID().getValue() : null;
        if (Strings.isEmpty(sid)) {
            LOG.warn("No sid in logout token {}, unable to process backchannel logout", logoutTokenParameter);
            throw OIDCExceptionCode.INVALID_LOGOUT_REQUEST.create("No sid in logout_token");
        }
        LOG.trace("Successfully parsed sid \"{}\" from logout token", sid);
        /*
         * lookup matching ox session & logout
         */
        String sessionId = stateManagement.getAndRemoveOXSessionId(sid);
        if (null == sessionId) {
            LOG.info("No session ID associated with OIDC session \"{}\" from backchannel logout found.", sid);
            return;
        }
        LOG.trace("Retrieved mapped session \"{}\" for parsed OID session \"{}\"", sessionId, sid);
        boolean removed;
        try {
            removed = services.getServiceSafe(SessiondService.class).removeSession(sessionId);
        } catch (OXException e) {
            LOG.error("Unexpected error removing session by ID \"{}\" upon backchannel logout.", sessionId, e);
            throw e;
        }
        if (removed) {
            LOG.info("Successfully removed session with ID \"{}\" (OIDC session \"{}\") from all nodes via backchannel logout.", sessionId, sid);
        } else {
            LOG.info("No additional sessions with ID \"{}\" (OIDC session \"{}\") removed from nodes.", sessionId, sid);
        }
    }

    @Override
    public void frontchannelLogout(HttpServletRequest request, HttpServletResponse response) throws OXException {
        LOG.trace("frontchannelLogout(HttpServletRequest request: {}, HttpServletResponse response)", request.getRequestURI());
        if (false == backend.getBackendConfig().isFrontchannelLogoutEnabled()) {
            throw OIDCExceptionCode.INVALID_LOGOUT_REQUEST.create("Frontchannel logout disabled by configuration");
        }
        /*
         * validate request issuer
         */
        if (false == validateThirdPartyRequest(request)) {
            LOG.warn("Issuer \"{}\" of front channel logout request could not be validated.", request.getParameter("iss"));
            throw OIDCExceptionCode.INVALID_LOGOUT_REQUEST.create("Issuer of frontchannel logout request could not be validated");
        }
        /*
         * parse oidc session from request
         */
        String sid = request.getParameter("sid");
        if (Strings.isEmpty(sid)) {
            LOG.warn("No sid in request {}, unable to process frontchannel logout", request.getRequestURI());
            throw OIDCExceptionCode.INVALID_LOGOUT_REQUEST.create("No sid in frontchannel logout request");
        }
        LOG.trace("Successfully got sid \"{}\" from logout request", sid);
        /*
         * lookup matching ox session & logout
         */
        String sessionId = stateManagement.getAndRemoveOXSessionId(sid);
        if (null == sessionId) {
            LOG.info("No session ID associated with OIDC session \"{}\" from frontchannel logout found.", sid);
            return;
        }
        LOG.trace("Found mapped session \"{}\" for parsed OID session \"{}\"", sessionId, sid);
        Session session = LoginPerformer.getInstance().lookupSession(sessionId);
        if (null == session) {
            LOG.info("Session \"{}\" (OIDC session \"{}\") not found.", sessionId, sid);
            return;
        }
        backend.logoutCurrentUser(session, request, response);
        LOG.info("Successfully logged out session \"{}\" (OIDC session \"{}\") via frontchannel logout.", sessionId, sid);
    }
