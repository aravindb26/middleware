...
public class OIDCLoginRequestHandler implements LoginRequestHandler {
     ...
     * @param A reference to the state management service
     */
    public OIDCLoginRequestHandler(OIDCBackend backend, StateManagement stateManagement) {
        super();
        this.handler = new OIDCHandler(backend, stateManagement);
    }
    ...
    public static class OIDCHandler {
         ...
         */
        public OIDCHandler(OIDCBackend backend, StateManagement stateManagement) {
                ...
                try {
                    ...
                    if (session != null) {
                        ...
                        if (backend.getBackendConfig().isBackchannelLogoutEnabled() || backend.getBackendConfig().isFrontchannelLogoutEnabled()) {
                            OIDCTools.rememberSessionIds(stateManagement, session, reservation.getState().get(OIDCTools.IDTOKEN));
                        }
        ...
        void performLogin(HttpServletRequest request, HttpServletResponse response, boolean respondWithJson, LoginRequestContext requestContext) throws IOException, OXException, JSONException {
            ...
            /*
             * extract and remember oidc session to ox session mapping as needed
             */
            if (backend.getBackendConfig().isBackchannelLogoutEnabled() || backend.getBackendConfig().isFrontchannelLogoutEnabled()) {
                OIDCTools.rememberSessionIds(stateManagement, session, idToken);
            }
        ...
        private LoginResult loginUser(HttpServletRequest request, final Context context, final User user, final Map<String, String> state) throws OXException {
            LOG.trace("loginUser(HttpServletRequest request: {}, final Context context: {}, final User user: {}, final Map<String, String> state.size: {})", request.getRequestURI(), I(context.getContextId()), I(user.getId()), I(state.size()));
            {
                ...
                String userName = state.get(OIDCTools.USER_LOGIN_INFO);
                if (Strings.isNotEmpty(userName) && Strings.isNotEmpty(contextInfo)) {
                    request.setAttribute(OIDCTools.CONTEXT_LOGIN_INFO, contextInfo);
                    request.setAttribute(OIDCTools.USER_LOGIN_INFO, userName);
                }
            }
