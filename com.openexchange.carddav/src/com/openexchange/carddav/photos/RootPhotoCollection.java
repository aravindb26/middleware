...
public class RootPhotoCollection extends DAVRootCollection {
    ...
    public DAVResource getChild(String name) throws WebdavProtocolException {
        ...
        try {
            contactsAccess = factory.requireService(IDBasedContactsAccessFactory.class).createAccess(factory.getSession());
            contactsAccess.set(ContactsParameters.PARAMETER_FIELDS, IMAGE_FIELDS);
            Contact loadedContact = contactsAccess.getContact(ContactIDUtil.createContactID(contact.getFolderId(true), contact.getId(true)));
