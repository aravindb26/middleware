...
import static com.openexchange.carddav.Tools.decodeCollectionName;
import static com.openexchange.carddav.Tools.encodeFolderId;
import static com.openexchange.carddav.Tools.filterByCapability;
import static com.openexchange.carddav.Tools.filterNotUsedForSync;
import static com.openexchange.carddav.Tools.getAggregatedCollectionId;
...
public class RootCollection extends DAVRootCollection {
    ...
    public List<WebdavResource> getChildren() throws WebdavProtocolException {
        List<WebdavResource> children = new LinkedList<>();
        if (isUseAggregatedCollection()) {
            /*
        ...
        if (isUseFolderCollections()) {
             ...
             */
            try {
                for (UserizedFolder folder : getFolderCollectionsFolders()) {
    ...
    public DAVCollection getChild(String name) throws WebdavProtocolException {
        try {
            ...
            if (isUseFolderCollections()) {
                String decodedName = decodeCollectionName(name);
                for (UserizedFolder folder : getFolderCollectionsFolders()) {
                    if (decodedName.equals(folder.getID())) {
                        LOG.debug("{}: found child collection by name '{}'", getUrl(), decodedName);
    ...
    private CardDAVCollection createFolderCollection(UserizedFolder folder) throws WebdavProtocolException {
        try {
            return factory.mixin(new CardDAVCollection(factory, constructPathForChildResource(encodeFolderId(folder.getID())), folder));
        } catch (OXException e) {
            throw protocolException(getUrl(), e);
        }
    }
    ...
    private List<UserizedFolder> getFolderCollectionsFolders() throws OXException {
        List<UserizedFolder> folders = factory.getState().getAllFolders();
        List<UserizedFolder> consideredFolders = new ArrayList<>(folders.size());
        for (UserizedFolder folder : folders) {
            if (considerForFolderCollections(folder)) {
                consideredFolders.add(folder);
            }
        }
        return consideredFolders;
    }

    private List<UserizedFolder> getAggregatedFolders() throws OXException {
        /*
         * get either a specific, all, or a special reduced set of folders for aggregated collection
         */
        AggregatedCollectionMode aggregatedCollectionMode = getAggregatedCollectionMode();
        if (AggregatedCollectionMode.DEFAULT_ONLY.equals(aggregatedCollectionMode)) {
            return Collections.singletonList(factory.getState().getDefaultFolder());
        }
        List<UserizedFolder> folders;
        if (AggregatedCollectionMode.REDUCED.equals(aggregatedCollectionMode) || AggregatedCollectionMode.REDUCED_SYNCED.equals(aggregatedCollectionMode)) {
            folders = factory.getState().getReducedFolders();
        } else {
            folders = factory.getState().getAllFolders();
        }
        /*
         * consider only "sync aware" folders
         */
        folders = filterByCapability(folders, ContactsAccessCapability.SYNC);
        /*
         * additionally strip out non-synchronized folders if applicable
         */
        if (AggregatedCollectionMode.ALL_SYNCED.equals(aggregatedCollectionMode) || AggregatedCollectionMode.REDUCED_SYNCED.equals(aggregatedCollectionMode) || null == aggregatedCollectionMode) {
            folders = filterNotUsedForSync(folders);
        }
        return folders;
    }
    ...
    private static boolean considerForFolderCollections(UserizedFolder folder) {
        if (null == folder) {
            return false;
        }
        EnumSet<ContactsAccessCapability> supportedCapabilities = getSupportedCapabilities(folder);
        if (false == supportedCapabilities.contains(ContactsAccessCapability.SYNC) && false == supportedCapabilities.contains(ContactsAccessCapability.CTAG)) {
            return false; // neither support for CTag, nor for sync-collection
        }
        if (supportedCapabilities.contains(ContactsAccessCapability.SUBSCRIBE) && false == folder.isSubscribed()) {
            return false; // currently not subscribed
        }
        return null == folder.getUsedForSync() || folder.getUsedForSync().isUsedForSync(); // nor selected for sync
    }
