...
import com.openexchange.contact.vcard.impl.httpclient.properties.VCardPhotoHttpProperties;
...
public class PhotoMapping extends AbstractMapping {
    ...
    private static Photo exportPhoto(Contact contact, VCardParameters parameters, List<OXException> warnings) {
        ...
        if (null != contactImage) {
            ...
            Optional<TransformedImage> transformedImage = Optional.empty();
            try {
                try {
                ...
                if (transformedImage.isEmpty()) {
                    return new Photo(contactImage, imageType);
                }
                try {
                    Photo photo = new Photo(transformedImage.get().getImageData(), imageType);
                    photo.addParameter(X_ABCROP_RECTANGLE, getABCropRectangle(transformedImage.get()));
                    return photo;
                } catch (OXException e) {
                    addConversionWarning(warnings, e, "PHOTO", e.getMessage());
                }
            } finally {
                transformedImage.ifPresent(image -> Streams.close(image));
            }
        }
        return null;
    }
    ...
    /**
     * Scales a contact image to fit into the supplied target dimension if necessary
     *
     ...
     * @return The optional scaled image data
     */
    private static Optional<TransformedImage> scaleImageIfNeeded(byte[] imageBytes, String formatName, VCardParameters parameters, List<OXException> warnings) throws OXException {
        ...
        try {
            ...
            return imageService.transfom(imageBytes, getSource(parameters))
                               .scale((int) targetDimension.getWidth(), (int) targetDimension.getHeight(), ScaleType.CONTAIN, true)
                               .optFullTransformedImage(formatName);
            // @formatter:on
        } catch (IOException e) {
    ...
    private static final Set<String> ALLOWED_PROTOCOLS = Set.of("http", "https", "ftp", "ftps");
    private static final HostList DENIED_HOSTS = HostList.valueOf("localhost", "127.0.0.1", LOCAL_HOST_ADDRESS, LOCAL_HOST_NAME);
    ...
    private static HttpClient getHttpClient() throws OXException {
        HttpClientService httpClientService = VCardServiceLookup.getOptionalService(HttpClientService.class);
        if (httpClientService == null) {
            throw ServiceExceptionCode.absentService(HttpClientService.class);
        }
        return httpClientService.getHttpClient(VCardPhotoHttpProperties.getHttpClientId());
    }
    ...
    private static LoadedImage doLoadImageFromURL(String urlString, String origUrlString, List<OXException> warnings) throws IOException, OXException {
        ...
        boolean error = true;
        try {
            if (PROTOCOLS_HTTP.contains(Strings.asciiLowerCase(url.getProtocol()))) {
                ...
                try {
                    ...
                    try {
                        ...
                        mimeType = contentTypeHdr == null ? null : contentTypeHdr.getValue();
