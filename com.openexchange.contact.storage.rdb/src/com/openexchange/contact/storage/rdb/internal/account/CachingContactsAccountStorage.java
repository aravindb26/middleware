...
public class CachingContactsAccountStorage implements ContactsAccountStorage {
     ...
     */
    private static ContactsAccount optClonedAccount(Object cachedAccount) throws OXException {
        return (cachedAccount instanceof ContactsAccount) ? clone((ContactsAccount) cachedAccount) : null;
    }
     ...
     * @return The cloned account identifiers {@link List} or <code>null</code> if the cached accounts is <code>null</code>
     */
    private static int[] optClonedAccountIds(Object cachedAccountIds) {
        return (cachedAccountIds instanceof int[]) ? clone((int[]) cachedAccountIds) : null;
    }
     ...
     * @throws OXException if an error is occurred
     */
    private static ContactsAccount clone(ContactsAccount account) throws OXException {
        JSONObject internalConfig = null == account.getInternalConfiguration() ? null : new JSONObject(account.getInternalConfiguration());
        JSONObject userConfig = null == account.getUserConfiguration() ? null : new JSONObject(account.getUserConfiguration());
